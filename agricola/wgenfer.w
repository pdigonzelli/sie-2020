...
FI-desde-fecha RADIO-SET-1 FI-hasta-fecha FI-feriado 
&Scoped-Define DISPLAYED-OBJECTS fi-empresa nombre-empresa Fi-sector ~
nombre-sector FI-desde-fecha RADIO-SET-1 FI-hasta-fecha FI-feriado 
...
DEFINE FRAME fMain
     ...
     Fi-sector AT ROW 3.57 COL 20 COLON-ALIGNED WIDGET-ID 50
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Genera planillas feriados"
         HEIGHT             = 8.71
         WIDTH              = 98.6
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.2
...
ON END-ERROR OF wWin /* Genera planillas feriados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Genera planillas feriados */
DO:
...
DO:
  ...
  CASE integer(radio-set-1:SCREEN-VALUE):
      WHEN 1 THEN
          RUN p_generar_planfer.p (INPUT v_empresa, INPUT v_sector,
                                  INPUT v_desde_fecha, INPUT v_hasta_fecha,
                                  INPUT v_feriado).
      WHEN 2 THEN
          RUN p_borrar_planfer.p (INPUT v_empresa, INPUT v_sector,
                                  INPUT v_feriado).
  END CASE.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-empresa nombre-empresa Fi-sector nombre-sector FI-desde-fecha 
          RADIO-SET-1 FI-hasta-fecha FI-feriado 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE Btn_OK fi-empresa Btn_Cancel Fi-sector FI-desde-fecha RADIO-SET-1 
         FI-hasta-fecha FI-feriado 
      WITH FRAME fMain IN WINDOW wWin.
