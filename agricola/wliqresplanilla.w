...
&Scoped-Define ENABLED-OBJECTS Btn_OK Btn_Cancel fi-empresa Fi-sector ~
FI-sucursal RADIO-SET-1 FI-desde-fecha FI-hasta-fecha T-excel FI-impresora ~
B-cambiar 
&Scoped-Define DISPLAYED-OBJECTS nombre-empresa fi-empresa nombre-sector ~
Fi-sector FI-sucursal nombre-sucursal RADIO-SET-1 FI-desde-fecha ~
FI-hasta-fecha T-excel FI-impresora 
...
DEFINE FRAME fMain
     ...
     fi-empresa AT ROW 2.52 COL 18.6 COLON-ALIGNED WIDGET-ID 44
     nombre-sector AT ROW 3.62 COL 30 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     Fi-sector AT ROW 3.86 COL 19 COLON-ALIGNED WIDGET-ID 8
     ...
     T-excel AT ROW 9.81 COL 83 WIDGET-ID 46
     FI-impresora AT ROW 12.67 COL 19 COLON-ALIGNED WIDGET-ID 34
     B-cambiar AT ROW 12.67 COL 82 WIDGET-ID 32
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 102.4 BY 13.48 WIDGET-ID 100.
...
DO:
  ...
  define var v_titulo as character format "x(60)".
      ...
      v_filtro = "liq_items_control_tareas.id_empresa = " + fi-empresa:screen-value    +
           ...
           " and liq_items_control_tareas.id_tarea <> 0".
           
      IF INTEGER(fi-sucursal:SCREEN-VALUE IN FRAME {&FRAME-NAME}) <> 0 THEN
      DO:
          v_filtro = v_filtro + " and liq_items_control_tareas.id_sucursal = " + fi-sucursal:screen-value in frame {&frame-name}.   
      END.

     v_filtro = v_filtro + " and liq_items_control_tareas.id_tipo_planilla = " + radio-set-1:screen-value in frame {&frame-name}.   
...
RB-MEMO-FILE = SESSION:TEMP-DIRECTORY + STRING(RANDOM(1,1000000)) + '.TXT'.
    ...
    RUN  aderb\_prntrb2(
         (if index(propath,"supervisor.pl") > 0 then vlc_dir_objetos else vlc_dir_fuentes) +  "agricola~\agricola.prl", /* RB-REPORT-LIBRARY */
          "liq_resumen_planilla_legajo", /* RB-REPORT-NAME */
          "",                             /* RB-DB-CONNECTION */
          ...
          "",                              /* RB-OUTPUT-FILE */
           ...
           0,                              /* RB-END-PAGE - zero */
          no,                              /* RB-TEST-PATTERN */
          "Resumen x Tipo de Planillas",         /* RB-WINDOW-TITLE */
...
------------------------------------------------------------------------------*/
  DISPLAY nombre-empresa fi-empresa nombre-sector Fi-sector FI-sucursal 
          nombre-sucursal RADIO-SET-1 FI-desde-fecha FI-hasta-fecha T-excel 
          FI-impresora 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE Btn_OK Btn_Cancel fi-empresa Fi-sector FI-sucursal RADIO-SET-1 
         FI-desde-fecha FI-hasta-fecha T-excel FI-impresora B-cambiar 
      WITH FRAME fMain IN WINDOW wWin.
