...
FI-desde-fecha Fi-tipo-liq T-centros FI-hasta-fecha FI-carpeta 
&Scoped-Define DISPLAYED-OBJECTS fi-empresa nombre-empresa Fi-sector ~
nombre-sector FI-desde-fecha Fi-tipo-liq T-centros FI-hasta-fecha ~
FI-carpeta 
...
DEFINE VARIABLE FI-carpeta AS CHARACTER FORMAT "X(256)":U INITIAL "~\~\tucuman3~\agricola~\" 
     LABEL "Carpeta" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE T-centros AS LOGICAL INITIAL no 
     LABEL "Nuevos CC" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME fMain
     ...
     T-centros AT ROW 6 COL 83 WIDGET-ID 48
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Agricola - Liquidacion Personal SA San Miguel - RHPRO"
         HEIGHT             = 8.91
         WIDTH              = 104.8
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.2
...
ON END-ERROR OF wWin /* Agricola - Liquidacion Personal SA San Miguel - RHPRO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Agricola - Liquidacion Personal SA San Miguel - RHPRO */
DO:
...
DO:
  ...
  define VAR v_empresa like liq_control_tareas.id_empresa.
    ...
    DEFINE VAR  v_tipo AS CHARACTER.
    ...
    v_carpeta = fi-carpeta:SCREEN-VALUE.
    v_tipo = fi-tipo-liq:SCREEN-VALUE.
 ...
 for each liq_items_control_tareas where 
  ...
  case integer(fi-sector:SCREEN-VALUE IN FRAME {&FRAME-NAME}):
   WHEN 6 THEN
       IF t-centros:SCREEN-VALUE = "no" THEN
              run liq_genera_rhpro02-c.p (input v_empresa,
                                    input v_sector,
                                    input v_fecha_desde,
                                    input v_fecha_hasta,
                                    INPUT v_carpeta,
                                    INPUT v_tipo).
         ELSE
             run liq_genera_rhpro02-c1.p (input integer(fi-empresa:screen-value),
   ...
   OTHERWISE
       IF t-centros:SCREEN-VALUE = "no" THEN
               run liq_genera_rhpro01-c.p (input integer(fi-empresa:screen-value),
                                              input integer(fi-sector:screen-value),
                                              input date(fi-desde-fecha:screen-value),
                                              input date(fi-hasta-fecha:screen-value),
                                              INPUT fi-carpeta:SCREEN-VALUE,
                                              INPUT fi-tipo-liq:SCREEN-VALUE).
          ELSE
              run liq_genera_rhpro01-c1.p (input integer(fi-empresa:screen-value),
                                             input integer(fi-sector:screen-value),
                                             input date(fi-desde-fecha:screen-value),
                                             input date(fi-hasta-fecha:screen-value),
                                             INPUT fi-carpeta:SCREEN-VALUE,
...
------------------------------------------------------------------------------*/
  DISPLAY fi-empresa nombre-empresa Fi-sector nombre-sector FI-desde-fecha 
          Fi-tipo-liq T-centros FI-hasta-fecha FI-carpeta 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE Btn_OK fi-empresa Btn_Cancel Fi-sector FI-desde-fecha Fi-tipo-liq 
         T-centros FI-hasta-fecha FI-carpeta 
      WITH FRAME fMain IN WINDOW wWin.
