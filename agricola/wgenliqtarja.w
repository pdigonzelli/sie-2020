...
B-actualizar FI-desde-fecha FI-hasta-fecha FI-carpeta 
&Scoped-Define DISPLAYED-OBJECTS fi-empresa nombre-empresa Fi-sector ~
nombre-sector FI-desde-fecha FI-hasta-fecha FI-carpeta 
...
DEFINE VARIABLE FI-desde-fecha AS DATE FORMAT "99/99/99":U INITIAL ? 
     LABEL "Desde Fecha" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE FI-hasta-fecha AS DATE FORMAT "99/99/99":U INITIAL ? 
     LABEL "Hasta Fecha" 
     VIEW-AS FILL-IN 
     SIZE 18 BY 1 NO-UNDO.
...
DO:
  run p_actualiza_archivo_personal.p 
     (input integer(fi-empresa:screen-value in frame {&frame-name}),
...
DO:
  ...
  case integer(fi-sector:SCREEN-VALUE IN FRAME {&FRAME-NAME}):
   WHEN 2 THEN
     run genera_liqtarja02-c.p (input integer(fi-empresa:screen-value),
   ...
   OTHERWISE 
     run genera_liqtarja01-c.p (input integer(fi-empresa:screen-value),
                              input integer(fi-sector:screen-value),
                              input date(fi-desde-fecha:screen-value),
                              input date(fi-hasta-fecha:screen-value),
                              INPUT fi-carpeta:SCREEN-VALUE  ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Btn_OK fi-empresa Btn_Cancel Fi-sector B-actualizar FI-desde-fecha 
         FI-hasta-fecha FI-carpeta 
      WITH FRAME fMain IN WINDOW wWin.
