...
DO:
    ...
    DO:
        MESSAGE "Debe ingresar una empresa para el reporte" VIEW-AS ALERT-BOX WARNING.
        RETURN.
    END.
    IF INTEGER(fi-sector:SCREEN-VALUE IN FRAME {&FRAME-NAME}) = 0 THEN
    DO:
        MESSAGE "Debe ingresar un sector para el reporte" VIEW-AS ALERT-BOX WARNING.
        RETURN.
    END.
      ...
      DO:
          v_filtro = v_filtro + " and liq_items_control_tareas.legajo = " + fi-legajo:screen-value in frame {&frame-name}.   
      END.
END.

CASE INTEGER(radio-set-1:SCREEN-VALUE IN FRAME {&FRAME-NAME}):
    WHEN 1 THEN v_nombre = "liq_control_destajo".
    WHEN 2 THEN v_nombre = "liq_control_destajo_lote".
END CASE.

IF t-excel:SCREEN-VALUE = "yes" THEN
DO:
    run p_liq_excel_destajo.p (INPUT INTEGER(fi-empresa:SCREEN-VALUE),
