...
T-todos Fi-sector FI-desde-fecha RADIO-SET-1 FI-hasta-fecha T-fincas ~
T-tareas T-archivo 
&Scoped-Define DISPLAYED-OBJECTS FI-sucursal nombre-sucursal fi-empresa ~
nombre-empresa T-todos Fi-sector nombre-sector FI-desde-fecha RADIO-SET-1 ~
FI-hasta-fecha FI-id_origen nombre-finca FI-id_proveedor T-fincas T-tareas ~
FI-tarea nombre-tarea FI-salida T-archivo 
...
if t-todos:screen-value = 'yes' Then
      v_filtro = "liq_items_control_tareas.id_empresa > 101 " +
...
if fi-sucursal:screen-value <> ""  Then
   do:
      v_filtro = v_filtro + " and liq_items_control_tareas.id_sucursal = " + fi-sucursal:screen-value.
   end.

if t-fincas:screen-value = 'no' Then
   do:
      v_tipo = 1.
      v_filtro = v_filtro + " and liq_items_control_tareas.id_proveedor = " + fi-id_proveedor:screen-value +
                " and liq_items_control_tareas.id_origen = " + fi-id_origen:screen-value. 
   end.

if t-tareas:screen-value = 'no' Then
   do:
      v_filtro = v_filtro + " and liq_items_control_tareas.id_tarea = " + fi-tarea:screen-value.
   end.
   ...
   when "1" Then
     ...
     when 5 or when 7 Then
      RUN  aderb\_prntrb2(
       ...
       "liq_res_finca_3eros_total", /* RB-REPORT-NAME */
       "",                             /* RB-DB-CONNECTION */
     ...
     when 6 Then
      RUN  aderb\_prntrb2(
         (if index(propath,"supervisor.pl") > 0 then vlc_dir_objetos else vlc_dir_fuentes) +  "agricola~\agricola.prl", /* RB-REPORT-LIBRARY */
       "liq_res_finca_3eros_total_cos", /* RB-REPORT-NAME */
       "",                             /* RB-DB-CONNECTION */
       "O",                             /* RB-INCLUDE-RECORDS */
       v_filtro,                              /* RB-FILTER */
       RB-MEMO-FILE,                              /* RB-MEMO-FILE */
   ...
   when "2" Then
     ...
     when 5 or when 7 Then
        if t-archivo:screen-value = 'no' Then
            ...
            Else
              run liq_genera_resumen_3eros.p (input integer(fi-sector:screen-value),
     ...
     when 6 Then
        if t-archivo:screen-value = 'no' Then
                RUN  aderb\_prntrb2(
                    (if index(propath,"supervisor.pl") > 0 then vlc_dir_objetos else vlc_dir_fuentes) +  "agricola~\agricola.prl", /* RB-REPORT-LIBRARY */
                  "liq_res_3eros_cos", /* RB-REPORT-NAME */
                  "",                             /* RB-DB-CONNECTION */
                  "O",                             /* RB-INCLUDE-RECORDS */
                  v_filtro,                              /* RB-FILTER */
                  RB-MEMO-FILE,                              /* RB-MEMO-FILE */
              ...
              Else
                  run liq_genera_resumen_3eros_cos.p (input integer(fi-sector:screen-value),
                                                  input date(fi-desde-fecha:screen-value),
                                                  input date(fi-hasta-fecha:screen-value),
                                                  input fi-salida:screen-value). 
     ...
     DO:
      if t-todos:screen-value = 'yes' Then
        v_filtro = "liq_items_control_tareas.id_empresa > 101 " +
           ...
           " and liq_items_control_tareas.fecha <= date('" + fi-hasta-fecha:screen-value  + "')" +
           " and liq_items_control_tareas.id_tarea <> 0
             AND liq_items_control_tareas.id_proveedor = 0 
             and liq_items_control_tareas.id_origen = 0 
             and liq_items_control_tareas.id_lote = 0".  
      Else
      v_filtro = "liq_items_control_tareas.id_empresa = " + fi-empresa:screen-value    +
           " and liq_items_control_tareas.id_sector = " + fi-sector:screen-value    +
           " and liq_items_control_tareas.fecha >= date('" + fi-desde-fecha:screen-value  + "')" + 
      ...
      RUN  aderb\_prntrb2(
         (if index(propath,"supervisor.pl") > 0 then vlc_dir_objetos else vlc_dir_fuentes) +  "agricola~\agricola.prl", /* RB-REPORT-LIBRARY */
       "liq_res_3eros_sin_finca", /* RB-REPORT-NAME */
       "",                             /* RB-DB-CONNECTION */
       "O",                             /* RB-INCLUDE-RECORDS */
       v_filtro,                              /* RB-FILTER */
       RB-MEMO-FILE,                              /* RB-MEMO-FILE */
...
------------------------------------------------------------------------------*/
  DISPLAY FI-sucursal nombre-sucursal fi-empresa nombre-empresa T-todos 
          Fi-sector nombre-sector FI-desde-fecha RADIO-SET-1 FI-hasta-fecha 
