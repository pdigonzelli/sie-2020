...
for each items_control_tareas WHERE 
    ...
    if control_tareas.id_empresa = 1 Then v_empresa = 1. /* Propias */
      ...
      find first personal_finca where personal_finca.id_empresa_cosechera =
       ...
       find first rb_resumen_agricola use-index grupos_tareas where 
                  rb_resumen_agricola.id_reporte = p_reporte and
                  rb_resumen_agricola.id_empresa = v_empresa and
                  rb_resumen_agricola.id_grupo_tarea = tareas.id_grupo_tarea and
                  rb_resumen_agricola.id_proveedor = control_tareas.id_proveedor and
                  rb_resumen_agricola.id_origen = control_tareas.id_origen 
                  no-error.
       if not available rb_resumen_agricola Then
          do:
             ...
             assign 
                  ...
                  rb_resumen_agricola.id_grupo_tarea = tareas.id_grupo_tarea 
                  rb_resumen_agricola.id_proveedor = control_tareas.id_proveedor 
                  rb_resumen_agricola.id_origen = control_tareas.id_origen
                  rb_resumen_agricola.descrip_origenes = origenes.descripcion
                  rb_resumen_agricola.descrip_tareas = grupos_tareas.descripcion
                  rb_resumen_agricola.id_zona = zonas_agricola.id_zona
    ...
    if control_tareas.id_empresa = 1 Then v_empresa = 1. /* Propias */
      ...
      find first personal_finca where personal_finca.id_empresa_cosechera =
       ...
       if not available rb_resumen_agricola Then
          do:
             find first origenes of control_tareas no-lock.
             find first zonas_agricolas of origenes no-lock.
             create rb_resumen_agricola.
             assign 
                  rb_resumen_agricola.id_reporte = p_reporte 
                  rb_resumen_agricola.id_empresa = v_empresa
