...
for each liq_tarjas no-lock where id_empresa = v_empresa and 
      liq_tarjas.id_sector = v_sector and
      liq_tarjas.fecha >= v_fecha_desde and liq_tarjas.fecha <= v_fecha_hasta 
      ,each liq_items_tarjas of liq_tarjas no-lock,
       ...
       by liq_items_tarjas.id_empresa by legajo by liq_items_tarjas.fecha:
...
for each liq_tarjas no-lock where id_empresa = v_empresa and 
      ...
      ,each liq_items_tarjas of liq_tarjas where
       ...
       first liq_legajos where liq_legajos.id_empresa_liq =
           liq_items_tarjas.id_empresa and liq_legajos.legajo = liq_items_tarjas.legajo AND
      (IF v-tipo <> "" THEN liq_legajos.tipo_liquidacion = v-tipo ELSE TRUE)  NO-LOCK,
       first liq_tareas where liq_tareas.id_tarea = liq_items_tarjas.id_tarea and 
       (liq_tareas.id_sector <> 0) no-lock 
       break by liq_items_tarjas.id_empresa by legajo by liq_items_tarjas.fecha:
              ...
              if liq_items_tarjas.cant_jornal <> 0 Then
                 v_horas = v_horas + (liq_items_tarjas.cant_jornal * 8).
