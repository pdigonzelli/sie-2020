...
if v-sucursal = 0 Then
 ...
 if v-finca = YES  THEN
  ...
  for each liq_items_control_tareas where 
       ...
       liq_items_control_tarea.legajo = v-legajo AND
     ...
     {liq-calcula-rhpro-cosecha.i}
         ...
         FIND FIRST rb_items_tarja WHERE
             ...
             rb_items_tarja.id_lote = liq_items_control_tareas.id_lote  NO-ERROR.
...
Else
 do:
  for each liq_items_control_tareas where 
      ...
      if liq_items_control_tareas.id_tarea = 0 and liq_items_control_tareas.cantidad = 0 Then next.
