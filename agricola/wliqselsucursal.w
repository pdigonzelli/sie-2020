...
&Scoped-Define ENABLED-OBJECTS RECT-19 Btn_OK B-cancel FI-sucursal 
&Scoped-Define DISPLAYED-OBJECTS FI-sucursal nombre-sucursal 
...
DEFINE FRAME fMain
     Btn_OK AT ROW 1.48 COL 62 WIDGET-ID 4
     B-cancel AT ROW 2.67 COL 62 WIDGET-ID 2
     FI-sucursal AT ROW 3.38 COL 11 COLON-ALIGNED WIDGET-ID 8
     nombre-sucursal AT ROW 3.38 COL 22 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     "Selecci¢n" VIEW-AS TEXT
          SIZE 54 BY .95 AT ROW 1 COL 1 WIDGET-ID 16
          BGCOLOR 1 FGCOLOR 15 
     RECT-19 AT ROW 1 COL 1 WIDGET-ID 14
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 80 BY 7.67 WIDGET-ID 100.
...
ON END-ERROR OF wWin /* Tarja Personal */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Tarja Personal */
DO:
...
v_sucursal = integer(fi-sucursal:screen-value in frame {&FRAME-NAME}).
 ...
 /*FIND FIRST par_permisos WHERE nombre_programa = "wsuc-" + STRING(v_sucursal) + ".w" NO-LOCK NO-ERROR.
 IF AVAILABLE par_permisos THEN
 DO:
     IF LOOKUP(USERID("userdb"),par_permisos.puede_ejecutar) = 0 THEN
     DO:
         MESSAGE "Usuario no habilitado para la zona" VIEW-AS ALERT-BOX WARNING.
         RETURN.
     END.
 END.*/
  
   run wliqcontroltareas.w (input integer(fi-sucursal:screen-value in frame {&FRAME-NAME})).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY FI-sucursal nombre-sucursal 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE RECT-19 Btn_OK B-cancel FI-sucursal 
      WITH FRAME fMain IN WINDOW wWin.
