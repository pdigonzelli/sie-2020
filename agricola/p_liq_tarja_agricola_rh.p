...
if v-finca = YES  THEN
 ...
 for each liq_items_tarjas where 
     ...
     FIND FIRST rb_items_tarja WHERE
         ...
         rb_items_tarja.id_lote = liq_items_tarjas.id_lote AND
         rb_items_tarja.id_unidad_liquidacion = liq_items_tarjas.id_unidad_liquidacion NO-ERROR.
    ...
    END.
        ...
        ASSIGN rb_items_tarja.cant_jornal = liq_items_tarjas.cant_jornal_norm
            ...
            rb_items_tarja.cant_hs_complementarias = liq_items_tarjas.hs_adicionales
            rb_items_tarja.codigo_hs_complementarias = liq_items_tarjas.id_codigo_abacus_adicional
            rb_items_tarja.id_unidad_liquidacion = liq_items_tarjas.id_unidad_liquidacion 
        ...
        IF rb_items_tarja.id_unidad_liquidacion <> 0 THEN
        DO:
            FIND FIRST unidades_liquidacion OF rb_items_tarja NO-LOCK NO-ERROR.
            IF AVAILABLE unidades_liquidacion THEN
                rb_items_tarja.nombre_unidad = unidades_liquidacion.descripcion.
        END.
   /*  display rb_items_tarja.legajo v_codigo v_codigo-1 v_codigo-2.
    ...
    if v_codigo <> 9999 and v_codigo <> 0 Then
       ...
       if not available t-personal Then
          ...
          t-personal.cantidad = t-personal.cantidad + liq_items_tarjas.cant_jornal_norm.
       end.
    if v_codigo-1 <> 9999 and v_codigo-1 <> 0 Then
     do: 
       ...
       if not available t-personal Then
          do:
             create t-personal.
             assign t-personal.id_empresa = liq_items_tarjas.id_empresa 
                    t-personal.legajo = liq_items_tarjas.legajo
                    t-personal.nombre = liq_items_tarjas.nombre
                    t-personal.dni_cuil = liq_items_tarjas.dni_cuil
