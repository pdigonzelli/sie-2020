...
v_archivo = "z:\temp\samilate.txt".

output to value(v_archivo).



PUT "ANALISIS SAMI LATE".
PUT SKIP.
PUT "Periodo ".
PUT  v_periodo.
PUT SKIP.
...
for each inv_lotes_plantacion WHERE inv_lotes_plantacion.anio = v_periodo AND
    inv_lotes_plantacion.id_proveedor = 1 AND
    (inv_lotes_plantacion.id_variedad = 207 OR
     inv_lotes_plantacion.id_variedad = 208 OR
     inv_lotes_plantacion.id_variedad = 209) NO-LOCK,
