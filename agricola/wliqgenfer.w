...
&Scoped-Define ENABLED-OBJECTS Btn_OK fi-empresa Btn_Cancel FI-desde-fecha ~
FI-hasta-fecha RADIO-SET-1 FI-feriado 
&Scoped-Define DISPLAYED-OBJECTS nombre-empresa fi-empresa FI-desde-fecha ~
FI-hasta-fecha RADIO-SET-1 FI-feriado 
...
DEFINE VARIABLE FI-feriado AS DATE FORMAT "99/99/99":U 
     LABEL "Feriado" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME fMain
     ...
     FI-desde-fecha AT ROW 4.57 COL 19 COLON-ALIGNED WIDGET-ID 6
     FI-hasta-fecha AT ROW 5.76 COL 19 COLON-ALIGNED WIDGET-ID 10
     RADIO-SET-1 AT ROW 6.95 COL 59 NO-LABEL WIDGET-ID 22
     FI-feriado AT ROW 7.91 COL 20 COLON-ALIGNED WIDGET-ID 52
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 97.4 BY 8.71 WIDGET-ID 100.
...
ON END-ERROR OF wWin /* Genera Feriados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Genera Feriados */
DO:
...
DO:
  ...
  v_feriado = DATE(fi-feriado:SCREEN-VALUE IN FRAME {&FRAME-NAME}).


  CASE integer(radio-set-1:SCREEN-VALUE):
      WHEN 1 THEN
          RUN p_tarja_planfer.p (INPUT v_empresa, INPUT v_sector,
                                  INPUT v_desde_fecha, INPUT v_hasta_fecha,
                                  INPUT v_feriado).
      WHEN 2 THEN
          /*RUN p_borrartarja_planfer.p (INPUT v_empresa, INPUT v_sector,
                                  INPUT v_feriado).*/
...
------------------------------------------------------------------------------*/
  DISPLAY nombre-empresa fi-empresa FI-desde-fecha FI-hasta-fecha RADIO-SET-1 
          FI-feriado 
      WITH FRAME fMain IN WINDOW wWin.
  ENABLE Btn_OK fi-empresa Btn_Cancel FI-desde-fecha FI-hasta-fecha RADIO-SET-1 
         FI-feriado 
      WITH FRAME fMain IN WINDOW wWin.
