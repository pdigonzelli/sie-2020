...
if v-empresa = 0 Then
 ...
 for each liq_items_tarjas where 
      ...
      find first rb_tarja where rb_tarja.id_reporte = v_nro_reporte and
        ...
        if not available rb_tarja Then
           do:
              ...
              rb_tarja.id_cargo = liq_legajos.id_ccargo_liq.
              rb_tarja.id_centro_abacus = liq_legajos.id_ccostos_liq.

              find first liq_ccostosliq where liq_ccostosliq.id_centro_costo_liq = rb_tarja.id_centro_abacus no-lock
                   no-error.    
                   if available liq_ccostosliq Then
                      rb_tarja.nombre_centro_abacus = liq_ccostosliq.descripcion.   
           end. 



          v_cargo = liq_legajos.id_ccargo_liq.
          v_centro = liq_legajos.id_ccostos_liq.
    ...
    END.
        ...
        ASSIGN rb_items_tarja.cant_horas = rb_items_tarja.cant_horas + liq_items_tarjas.cant_horas
            rb_items_tarja.cant_hs_norm = rb_items_tarja.cant_hs_norm + liq_items_tarjas.cant_hs_norm
            ...
            rb_items_tarja.cantidad = rb_items_tarja.cantidad + liq_items_tarjas.cantidad
            rb_items_tarja.codigo_cantidad = liq_items_tarjas.id_codigo_abacus_cantidad
        ...
        IF liq_items_tarjas.hs_acond_finca <> 0 THEN
            ASSIGN rb_items_tarja.cant_haf = rb_items_tarja.cant_haf + liq_items_tarjas.hs_acond_finca
                   rb_items_tarja.cod_haf = 1645.

        IF liq_items_tarjas.hs_plus_tareas_automatico <> 0 THEN
            ASSIGN rb_items_tarja.cant_hpta = rb_items_tarja.cant_hpta + liq_items_tarjas.hs_plus_tareas_automatico
                   rb_items_tarja.cod_hpta = 1430.

        IF liq_items_tarjas.hs_plus_tareas_trabajadas <> 0 THEN
            ASSIGN rb_items_tarja.cant_hptt = rb_items_tarja.cant_hptt + liq_items_tarjas.hs_plus_tareas_trabajadas
                   rb_items_tarja.cod_hptt = 1430.

        IF liq_items_tarjas.hs_adicionales_tareas_trabajadas <> 0 THEN
            ASSIGN rb_items_tarja.cant_hatt = rb_items_tarja.cant_hatt + liq_items_tarjas.hs_adicionales_tareas_trabajadas
                   rb_items_tarja.cod_hatt = 1430.
        /****************/
    ...
    DO:
        ...
        find first liq_conceptos where liq_conceptos.id_concepto = liq_tareas.id_codigo_abacus NO-LOCK NO-ERROR.
        IF AVAILABLE liq_conceptos THEN
            rb_items_tarja.nombre_unidad = ENTRY(1,liq_conceptos.descripcion,"-").
    END.
...
for each rb_tarja WHERE rb_tarja.id_reporte = v_nro_reporte AND
     rb_tarja.id_empresa = v-empresa  break by rb_tarja.legajo:
     if last-of(rb_tarja.legajo) Then
     do:
        j = 0. 
        v_total = 0.
        FOR EACH rb_items_tarja WHERE 
            ...
            rb_items_tarja.legajo = rb_tarja.legajo AND
            rb_items_tarja.codigo_horas <> 0 NO-LOCK BREAK BY rb_items_tarja.codigo_horas:
            v_total = v_total + rb_items_tarja.cant_hs_norm.
            ...
            DO:
               ...
               find first liq_conceptos where 
                   liq_conceptos.id_concepto = rb_items_tarja.codigo_horas
                     no-lock no-error.
                if available liq_conceptos Then     
        ...
        FOR EACH rb_items_tarja WHERE 
            ...
            DO:
               ...
               find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.codigo_cantidad
                     no-lock no-error.
                if available liq_conceptos Then     
                    rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
        ...
        FOR EACH rb_items_tarja WHERE 
            ...
            DO:
               ...
               find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.codigo_diferencial
                     no-lock no-error.
                if available liq_conceptos Then     
                    rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
        ...
        FOR EACH rb_items_tarja WHERE 
            ...
            rb_items_tarja.id_empresa = v-empresa AND
            rb_items_tarja.legajo = rb_tarja.legajo AND
            rb_items_tarja.cod_haf <> 0 NO-LOCK BREAK BY cod_haf :
            v_total = v_total + rb_items_tarja.cant_haf.
            IF LAST-OF(rb_items_tarja.cod_haf) THEN
            DO:
               j = j + 1.
               ASSIGN rb_tarja.id_concepto[j] = rb_items_tarja.cod_haf. 
               find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.cod_haf
                     no-lock no-error.
                if available liq_conceptos Then     
                    rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
        ...
        FOR EACH rb_items_tarja WHERE 
            ...
            rb_items_tarja.id_empresa = v-empresa AND
            rb_items_tarja.legajo = rb_tarja.legajo AND
            rb_items_tarja.cod_hpta <> 0 NO-LOCK BREAK BY cod_hpta :
            v_total = v_total + rb_items_tarja.cant_hpta.
            IF LAST-OF(rb_items_tarja.cod_hpta) THEN
            DO:
               j = j + 1.
               ASSIGN rb_tarja.id_concepto[j] = rb_items_tarja.cod_hpta. 
               find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.cod_hpta
                     no-lock no-error.
                if available liq_conceptos Then     
                    rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
      ...
      FOR EACH rb_items_tarja WHERE 
          ...
          rb_items_tarja.id_empresa = v-empresa AND
          rb_items_tarja.legajo = rb_tarja.legajo AND
          rb_items_tarja.cod_hptt <> 0 NO-LOCK BREAK BY cod_hptt :
          v_total = v_total + rb_items_tarja.cant_hptt.
          IF LAST-OF(rb_items_tarja.cod_hptt) THEN
          DO:
             j = j + 1.
             ASSIGN rb_tarja.id_concepto[j] = rb_items_tarja.cod_hptt. 
             find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.cod_hptt
                   no-lock no-error.
              if available liq_conceptos Then     
                  rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
    ...
    FOR EACH rb_items_tarja WHERE 
        ...
        rb_items_tarja.id_empresa = v-empresa AND
        rb_items_tarja.legajo = rb_tarja.legajo AND
        rb_items_tarja.cod_hatt <> 0 NO-LOCK BREAK BY cod_hatt :
        v_total = v_total + rb_items_tarja.cant_hatt.
        IF LAST-OF(rb_items_tarja.cod_hatt) THEN
        DO:
           j = j + 1.
           ASSIGN rb_tarja.id_concepto[j] = rb_items_tarja.cod_hatt. 
           find first liq_conceptos where liq_conceptos.id_concepto = rb_items_tarja.cod_hatt
                 no-lock no-error.
            if available liq_conceptos Then     
                rb_tarja.nombre_concepto[j] = liq_conceptos.descripcion.
...
PROCEDURE imprime-reporte:
    ...
    define var v_nombre_empresa as character.
      ...
      v_filtro = "rb_tarja.id_reporte = " + string(v_nro_reporte) + " and " +
          "rb_tarja.id_empresa = " + string(v-empresa).
