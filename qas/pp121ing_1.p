...
DEFINE VAR CANT1 AS DECIMAL NO-UNDO.
...
MESSAGE 'POR IR A ORDPRO'.

FOR EACH CABECERA-TEMPORAL.
/*    RUN CREAPOSICIONES. */
    MESSAGE 'ENTRA ORDPRO'.
    RUN ORDPROD.P (
...
PROCEDURE CREATABLACALIBRES.
    ...
    FOR EACH TCALIBRE.
        MESSAGE TCALIBRE.PCALIBRESTD TCALIBRE.PCANTIDAD VIEW-AS ALERT-BOX.
    ...
    FOR EACH TCALIBRE.
        ...
        ASSIGN CALIBRE-TEMPORAL.PCALIBRE = TCALIBRE.PCALIBRESTD
               CALIBRE-TEMPORAL.PPORCENTAJE = ROUND(INTEGER(TCALIBRE.PCANTIDAD) / TOTALC, 2).
        TOTALD = TOTALD + CALIBRE-TEMPORAL.PPORCENTAJE.
        MESSAGE TOTALD .
    END.
    FIND LAST TCALIBRE.
    CALIBRE-TEMPORAL.PPORCENTAJE = CALIBRE-TEMPORAL.PPORCENTAJE + 1 - TOTALD.
    MESSAGE 'ACA' .
END PROCEDURE.

DEFINE VAR IUNIDAD AS INTEGER NO-UNDO.
DEFINE VAR ITOT1 AS INTEGER NO-UNDO.
DEFINE VAR ITOT2 AS INTEGER NO-UNDO.


PROCEDURE CREATABLATEMPORAL.
    ISERIE = 0.
    
    MESSAGE 'TABLA TEMPORAL: '.
    FOR EACH POSICION.
    
          MESSAGE 'POSICION' CABECERA.SUNIDAD.
          
          
        
          CASE SUBSTRING(CABECERA.SUNIDAD,2,1):
            WHEN 'A' THEN IUNIDAD = 100 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'B' THEN IUNIDAD = 110 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'C' THEN IUNIDAD = 120 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'D' THEN IUNIDAD = 130 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'E' THEN IUNIDAD = 140 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'F' THEN IUNIDAD = 150 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'G' THEN IUNIDAD = 160 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'H' THEN IUNIDAD = 170 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            WHEN 'I' THEN IUNIDAD = 180 + INTEGER(SUBSTRING(CABECERA.SUNIDAD,3,1)).
            OTHERWISE IUNIDAD = INTEGER(SUBSTRING(CABECERA.SUNIDAD,2)).
          END.        
        ISERIE = ISERIE + 1.


          MESSAGE 'POSICION' CABECERA.SUNIDAD IUNIDAD.
        ...
        FOR EACH CALIBRE-TEMPORAL.
            ...
            DO:
                ...
                {assignposicion_2.i}
            END.
            IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , POSICION.PCANTIDADORDEN ) <> 0 THEN
                cant1 = DECIMAL(POSICION.PCANTIDADORDEN).
            ELSE
            DO:
                IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
                    cant1 = DECIMAL (REPLACE(POSICION.PCANTIDADORDEN , ',' , '.')  ).
                ELSE
                    cant1 = DECIMAL (REPLACE(POSICION.PCANTIDADORDEN , '.' , ',') ).
            END.

            POSICION-TEMPORAL.PCANTIDAD = INTEGER(( CANT1 / IUNIDAD) * CALIBRE-TEMPORAL.PPORCENTAJE ).
            ITOT1 = ITOT1 + POSICION-TEMPORAL.PCANTIDAD.
            MESSAGE 'ITOT1' ITOT1 'CANTIDAD' CANT1 IUNIDAD POSICION-TEMPORAL.PCANTIDAD.
        END.
        FIND LAST POSICION-TEMPORAL NO-ERROR.
        POSICION-TEMPORAL.PCANTIDAD = POSICION-TEMPORAL.PCANTIDAD   + INTEGER(CANT1) / IUNIDAD - ITOT1.
    END.
    MESSAGE 'TABLA TEMPORAL'. 
END PROCEDURE.
