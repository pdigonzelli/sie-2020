...
DEFINE VAR CENTREGA AS CHARACTER NO-UNDO.
...
FOR FIRST RETORNO.
    ...
    ELSE
    DO:
        FOR EACH T-ITEMS BY T-ITEMS.T-ROWID.
            FIND items_stock WHERE ROWID(ITEMS_STOCK) = T-ITEMS.T-ROWID NO-LOCK NO-ERROR.
        ...
        END.
        CSTATUS = CSTATUS + RETORNO.MENSAJE + CHR(10).
    END. 
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR LAST T-ITEMS BY T-ITEMS.T-ROWID.
        FIND items_stock WHERE ROWID(ITEMS_STOCK) = T-ITEMS.T-ROWID NO-LOCK NO-ERROR.
        FIND movsucu OF items_stock NO-LOCK NO-ERROR.
        IF AVAILABLE movsucu THEN 
    ...
    FOR EACH T-ITEMS NO-LOCK BY T-ITEMS.T-ROWID.

        IPOSICION = IPOSICION + 10.
        
        FIND items_stock WHERE ROWID(ITEMS_STOCK) = T-ITEMS.T-ROWID NO-LOCK NO-ERROR.
        
        FIND movsucu OF items_stock NO-LOCK.
