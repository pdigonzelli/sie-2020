...
&Scoped-Define ENABLED-OBJECTS aceptar salir RECT-1 RECT-2 
...
DEFINE VARIABLE h_bc_items_orden_entrega AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
     aceptar AT ROW 18.86 COL 123
     salir AT ROW 18.86 COL 135
     RECT-1 AT ROW 18.62 COL 120
     RECT-2 AT ROW 2.19 COL 3
     SPACE(2.19) SKIP(2.42)
...
DO:
  ...
  run devuelve-rowid-items-oe ( output rresult).
  r=rresult.  
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_b_new_orden_entrega ( 3.14 , 8.00 ) NO-ERROR.
       RUN set-size IN h_b_new_orden_entrega ( 6.71 , 124.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_items_orden_entrega.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_items_orden_entrega ).
       RUN set-position IN h_bc_items_orden_entrega ( 10.52 , 7.00 ) NO-ERROR.
       RUN set-size IN h_bc_items_orden_entrega ( 6.71 , 127.00 ) NO-ERROR.

       /* Links to csmartbrowser h_bc_items_orden_entrega. */
       RUN add-link IN adm-broker-hdl ( h_b_new_orden_entrega , 'Record':U , h_bc_items_orden_entrega ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_new_orden_entrega ,
             aceptar:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_items_orden_entrega ,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE devuelve-rowid-items-oe D-Dialog 
PROCEDURE devuelve-rowid-items-oe :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
run get-rowid1 in h_bc_items_orden_entrega (output p_r).
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ENABLE aceptar salir RECT-1 RECT-2 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
