...
define input parameter p3 as integer.
...
&Scoped-Define ENABLED-OBJECTS RECT-3 FILL-IN-1 BUTTON-1 BUTTON-2 
...
DEFINE VARIABLE h_b_posiciones_filtro_inventaria AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "SELECCIONE LA POSICION QUE DESEA INVENTARIAR" 
     VIEW-AS FILL-IN 
     SIZE 106 BY .95
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 1.76 COL 1 COLON-ALIGNED NO-LABEL
     BUTTON-1 AT ROW 11.76 COL 74
     BUTTON-2 AT ROW 11.76 COL 91
     RECT-3 AT ROW 11.1 COL 71
...
DO:
  ...
  run get-rowid1 in h_b_posiciones_filtro_inventaria (output r). 
  find posiciones where rowid(posiciones) = r no-lock no-error.
  if available posiciones then
   do:
        run w_carga_etiquetas_inventario.w (input posiciones.id_empresa, 
                                            input posiciones.id_sucursal, 
                                            input posiciones.id_locacion,
                                            input posiciones.id_posicion).
        
   end.
   else message "Existe un error en la seleccion de la Posicion" view-as alert-box.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_posiciones_filtro_inventariar.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_posiciones_filtro_inventaria ).
       RUN set-position IN h_b_posiciones_filtro_inventaria ( 3.48 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 73.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_posiciones_filtro_inventaria ,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dame-parametros W-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
define output parameter p11 as integer.
define output parameter p22 as integer.
define output parameter p33 as integer.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-3 FILL-IN-1 BUTTON-1 BUTTON-2 
      WITH FRAME F-Main IN WINDOW W-Win.
