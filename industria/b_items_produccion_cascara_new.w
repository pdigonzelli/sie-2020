...
&Scoped-define INTERNAL-TABLES items_produccion_cascara

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table items_produccion_cascara.Fecha ~
items_produccion_cascara.id_bolsa items_produccion_cascara.Humedad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ~
items_produccion_cascara.Fecha items_produccion_cascara.id_bolsa ~
items_produccion_cascara.Humedad 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}Fecha ~{&FP2}Fecha ~{&FP3}~
 ~{&FP1}id_bolsa ~{&FP2}id_bolsa ~{&FP3}~
 ~{&FP1}Humedad ~{&FP2}Humedad ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table items_produccion_cascara
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table items_produccion_cascara
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH items_produccion_cascara OF produccion_cascara WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table items_produccion_cascara
&Scoped-define FIRST-TABLE-IN-QUERY-br_table items_produccion_cascara
...
<FOREIGN-KEYS>
id_sucursal||y|general.items_produccion_cascara.id_sucursal
id_turno||y|general.items_produccion_cascara.id_turno
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
DEFINE QUERY br_table FOR 
      items_produccion_cascara SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      items_produccion_cascara.Fecha
      items_produccion_cascara.id_bolsa
      items_produccion_cascara.Humedad
  ENABLE
      items_produccion_cascara.Fecha
      items_produccion_cascara.id_bolsa
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 56 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.items_produccion_cascara OF general.produccion_cascara"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.items_produccion_cascara.Fecha
"items_produccion_cascara.Fecha" ? ? "date" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > general.items_produccion_cascara.id_bolsa
"items_produccion_cascara.id_bolsa" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > general.items_produccion_cascara.Humedad
"items_produccion_cascara.Humedad" ? ? "integer" ? ? ? ? ? ? yes ?
...
define sub-menu Ordena 
  menu-item items_produccion_cascara-Fecha
label 'Fecha( items_produccion_cascara ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=items_produccion_cascara.Fecha').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by items_produccion_cascara.Fecha
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item items_produccion_cascara-id_bolsa
label 'id_bolsa( items_produccion_cascara ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=items_produccion_cascara.id_bolsa').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by items_produccion_cascara.id_bolsa
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item items_produccion_cascara-Humedad
label 'Humedad( items_produccion_cascara ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=items_produccion_cascara.Humedad').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by items_produccion_cascara.Humedad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by items_produccion_cascara.Fecha
...
if available produccion_cascara then
    do:
        assign items_produccion_cascara.id_empresa = produccion_cascara.id_empresa
               items_produccion_cascara.id_sucursal = produccion_cascara.id_sucursal
               items_produccion_cascara.id_produccion = produccion_cascara.id_produccion
               items_produccion_cascara.id_articulo = produccion_cascara.id_articulo
               items_produccion_cascara.item_produccion_cascara = items_produccion_cascara.id_bolsa.   
    end.
else message "Hubo un error en la grabacion" view-as alert-box.
...
case p:
  WHEN 'items_produccion_cascara.Fecha' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  Fecha = date(valor)  no-lock no-error .
  WHEN 'items_produccion_cascara.id_bolsa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_bolsa = integer(valor)  no-lock no-error .
  WHEN 'items_produccion_cascara.Humedad' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  Humedad = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "items_produccion_cascara" "id_sucursal"}
  {src/adm/template/sndkycas.i "id_turno" "items_produccion_cascara" "id_turno"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "items_produccion_cascara"}
