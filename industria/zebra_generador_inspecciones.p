...
for each tambores_lote where id_lote = 109 and id_tambor = 71 no-lock.
            output to c:\lpt1. 
            put control "^XA".
            put control "^LH0,0".
            put control "^PQ" 2 "^FS".
            ...
            if tambores_lote.id_articulo = 52 then
                do:
                    put control "^FO30,220^A0N,40,40^FD" "PRODUCT: CONCENTRATED LEMON JUICE" "^FS".
                end.
            if tambores_lote.id_articulo = 53 then
                do:
            ...
            put control "^FO100,330^A0N,45,60^FD" tambores_lote.fecha "^FS".
            put control "^FO441,280^GB370,720,3^FS".
            put control "^FO446,290^A0N,34,25^FD" "Batch N." "^FS".
            put control "^FO560,330^A0N,45,60^FD" string(tambores_lote.id_lote,">>99") + "/" + substr(string(tambores_lote.fecha),7,2) "^FS".
            
          find last inspecciones_lote where inspecciones_lote.id_lote = 109 no-lock no-error.
