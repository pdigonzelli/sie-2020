...
DEFINE VARIABLE vSmtpServer AS CHARACTER NO-UNDO 
                               FORMAT "X(50)"
                               LABEL "Smtp Server" 
                               INITIAL "namail.bedford.progress.com".

DEFINE VARIABLE vTo         AS CHARACTER NO-UNDO
                               FORMAT "X(50)"
                               LABEL "To" 
                               INITIAL "unknown@progress.com".
    ...
    IF vlength > 0 THEN
        DO:
            ...
            CASE vState:
                ...
                WHEN 4 THEN 
                    IF v = 250 THEN
                        RUN newState(5, "RCPT TO: " + "unknown@bedford.progress.com" + "~r~n").
                    ELSE 
                        vState = -1.
                WHEN 5 THEN
                    IF v = 250 THEN
                        RUN newState(6, "DATA ~r~n").
                    ELSE
                        vState = -1.
                WHEN 6 THEN
                    IF v = 354 THEN
                        RUN newState(7, "From: " + vFrom + "~r~n" + "To: " + vTo + " ~r~n" + "Subject: " + vSubject + " ~r~n~r~n" + vBody + "~r~n" + ".~r~n").
                    ELSE
                        vState = -1.
                WHEN 7 THEN 
                    IF v = 250 THEN
                        RUN newState(8,"QUIT~r~n").
                    ELSE 
                        vState = -1.
            END CASE.
        END.
        ...
        IF vState < 0 THEN 
            MESSAGE vState SKIP str SKIP "Email has been aborted".
END PROCEDURE.
