...
/* AUTOR: DANIEL REYNA                      */
/********************************************/
define vari vtipo like tipos_movimientos.id_tipo_movimiento format ">>>9".
...
define vari vstock0 as logical format "Si/No" initial false.
...
/*    productos_terminados.id_articulo = 520 and */
 ...
 if vtipo = 0 then assign vtipod  = 1 vtipoh = 999999999.
  ...
  */            if vlote <> 0 then bstock.id_lote = vlote and
       ...
       if not verror then do:
        ...
        assign 
               ...
               auxi.desde        = bstock.tambor_desde
               auxi.hasta        = bstock.tambor_hasta
...
break by id_articulo.
   ...
   if not verror then do:
    ...
    if first-of(auxi.id_articulo) then
        display
            string(auxi.id_articulo) + " - " + 
            upper(productos_terminados.descripcion) + " -----> Suc: " +
            vnomsuco format "x(100)"
            with frame ftitulo side-label no-labels scrollable.
    vcontrol = 0.    
    ...
    display auxi.id_articulo
            ...
            vcontrol  column-label "Ctrol" (total)
...
for each tambores_industria where
             ...
             /* tambores_industria.id_tambor    >= vdesde and
                ...
                if id_locacion_ubicacion = 4 then
                vcontrol = vcontrol + 1.
