...
&Scoped-Define ENABLED-FIELDS RowObject.Fecha RowObject.observaciones 
&Scoped-define ENABLED-TABLES RowObject
&Scoped-define FIRST-ENABLED-TABLE RowObject
&Scoped-Define DISPLAYED-FIELDS RowObject.Fecha RowObject.cantidad_bolsas ~
RowObject.id_produccion RowObject.observaciones 
...
DEFINE FRAME F-Main
     RowObject.Fecha AT ROW 2.1 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 13.2 BY 1
     RowObject.cantidad_bolsas AT ROW 3.1 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9 BY 1
     RowObject.id_produccion AT ROW 3.14 COL 54 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 11.8 BY 1
     RowObject.observaciones AT ROW 5.14 COL 16 COLON-ALIGNED
          LABEL "Observaciones"
          VIEW-AS FILL-IN 
          SIZE 49.6 BY 1
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN repositionObject IN h_dsucursales ( 2.19 , 32.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */
       ...
       RUN constructObject (
             ...
             OUTPUT h_dynselect ).
       RUN repositionObject IN h_dynselect ( 1.00 , 18.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  vcQry = "FOR EACH productos_terminados WHERE id_articulo = 54 OR id_articulo = 954 OR id_articulo = 38".
...
------------------------------------------------------------------------------*/
  ...
  vcQry = "FOR EACH productos_terminados WHERE id_articulo = 54 OR id_articulo = 954 OR id_articulo = 69 OR id_articulo = 55 OR id_articulo = 38".
  DYNAMIC-FUNCTION('setQueryWhere' IN h_dProductosTerminados, vcQry).
  DYNAMIC-FUNCTION('openQuery' IN h_dProductosTerminados).
  
  vhData = DYNAMIC-FUNCTION('getDataSource').
  ASSIGN rowObject.id_produccion:SCREEN-VALUE IN FRAME F-Main = DYNAMIC-FUNCTION('getNextNumeroProduccion' IN vhData).
