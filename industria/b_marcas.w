...
&Scoped-define INTERNAL-TABLES marcas_prod

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table marcas_prod.id_marca ~
marcas_prod.descripcion marcas_prod.Descripcion_ingles 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH marcas_prod WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table marcas_prod
&Scoped-define FIRST-TABLE-IN-QUERY-br_table marcas_prod
...
<FOREIGN-KEYS>
id_marca||y|comercial.marcas.id_marca
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      marcas_prod.id_marca FORMAT "->>9":U WIDTH 8.2
      marcas_prod.descripcion FORMAT "X(30)":U WIDTH 38.2
      marcas_prod.Descripcion_ingles COLUMN-LABEL "Descripcion ingles" FORMAT "X(30)":U
            WIDTH 39.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 98 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.marcas_prod"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.marcas_prod.id_marca
"id_marca" ? ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > general.marcas_prod.descripcion
"descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "38.2" yes no no "U" "" ""
     _FldNameList[3]   > general.marcas_prod.Descripcion_ingles
"Descripcion_ingles" "Descripcion ingles" ? "character" ? ? ? ? ? ? no ? no no "39.2" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item marcas_prod-id_marca
label 'id_marca( marcas_prod ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=marcas_prod.id_marca').
                  run set-attribute-list('titulo=Marca').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by marcas_prod.id_marca
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item marcas_prod-descripcion
label 'descripcion( marcas_prod ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=marcas_prod.descripcion').
                  run set-attribute-list('titulo=Descripción').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by marcas_prod.descripcion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item marcas_prod-Descripcion_ingles
label 'Descripcion_ingles( marcas_prod ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=marcas_prod.Descripcion_ingles').
                  run set-attribute-list('titulo=Descripcion en ingles').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by marcas_prod.Descripcion_ingles
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by marcas_prod.id_marca
...
case p:
  WHEN 'marcas_prod.id_marca' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_marca = integer(valor)  no-lock no-error .
  WHEN 'marcas_prod.descripcion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  descripcion begins valor  no-lock no-error .
  WHEN 'marcas_prod.Descripcion_ingles' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  Descripcion_ingles begins valor  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_marca" "marcas" "id_marca"}
