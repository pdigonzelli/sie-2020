...
&Scoped-define INTERNAL-TABLES tipostambor

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table tipostambor.id_tipotambor ~
tipostambor.descripcion tipostambor.abreviatura 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tipostambor WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tipostambor
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tipostambor
...
<FOREIGN-KEYS>
id_tipotambor||y|general.tipostambor.id_tipotambor
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tipostambor.id_tipotambor FORMAT ">9":U
      tipostambor.descripcion FORMAT "X(30)":U
      tipostambor.abreviatura FORMAT "X(12)":U WIDTH 16.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 79 BY 12.14
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.tipostambor"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.tipostambor.id_tipotambor
"id_tipotambor" ? ">9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = general.tipostambor.descripcion
     _FldNameList[3]   > general.tipostambor.abreviatura
"abreviatura" ? ? "character" ? ? ? ? ? ? no ? no no "16.4" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item tipostambor-id_tipotambor
label 'id_tipotambor( tipostambor ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tipostambor.id_tipotambor').
                  run set-attribute-list('titulo=Tipo tambor').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tipostambor.id_tipotambor
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item tipostambor-descripcion
label 'descripcion( tipostambor ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tipostambor.descripcion').
                  run set-attribute-list('titulo=Tipos de Tambor').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tipostambor.descripcion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item tipostambor-abreviatura
label 'abreviatura( tipostambor ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tipostambor.abreviatura').
                  run set-attribute-list('titulo=Abreviatura').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tipostambor.abreviatura
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by tipostambor.id_tipotambor
...
case p:
  WHEN 'tipostambor.id_tipotambor' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_tipotambor = integer(valor)  no-lock no-error .
  WHEN 'tipostambor.descripcion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  descripcion begins valor  no-lock no-error .
  WHEN 'tipostambor.abreviatura' then  
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_tipotambor" "tipostambor" "id_tipotambor"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tipostambor"}
