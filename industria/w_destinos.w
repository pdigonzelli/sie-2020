...
DEFINE VARIABLE h_b_destinos AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_destinos AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     RECT-2 AT Y 0 X 125
     RECT-4 AT ROW 10.76 COL 5
     "Nota: Al cargar un destino tener" VIEW-AS TEXT
          SIZE 33 BY .95 AT ROW 11.24 COL 86
     "en cuenta lo siguiente:" VIEW-AS TEXT
          SIZE 33 BY .95 AT ROW 12.19 COL 86
     "Cargar Suc 91 si no es un Frigorifico" VIEW-AS TEXT
          SIZE 35 BY .95 AT ROW 13.14 COL 86
     "En caso que si, cargar la sucursal" VIEW-AS TEXT
          SIZE 33 BY .95 AT ROW 14.1 COL 86
     "correspondiente" VIEW-AS TEXT
          SIZE 33 BY .95 AT ROW 15.05 COL 86
...
ON END-ERROR OF W-Win /* Destinos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Destinos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_destinos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_destinos ).
       RUN set-position IN h_b_destinos ( 3.62 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b_destinos ( 6.71 , 117.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_destinos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_destinos ).
       RUN set-position IN h_v_destinos ( 11.24 , 20.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.76 , 64.00 ) */

       /* Links to csmartbrowser h_b_destinos. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_destinos ).

       /* Links to SmartViewer h_v_destinos. */
       RUN add-link IN adm-broker-hdl ( h_b_destinos , 'Record':U , h_v_destinos ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_destinos ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_destinos ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_destinos ,
             h_b_destinos , 'AFTER':U ).
    END. /* Page 0 */
