...
define var v_sucursal as integer initial 95.

define temp-table tt_stock
    ...
    field sem_des as integer 
    field sem_has as integer 
    field anio_sem as integer 
    ...
    index id_tipotambor id_tipotambor id_articulo id_lote ascending.
...
&Scoped-define INTERNAL-TABLES tt_stock

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table id_lote anio tambores kilos kilos_400 envase articulo calidad orden_f contrato cliente sem_des sem_h anio_sem   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH tt_stock WHERE ~{&KEY-PHRASE} NO-LOCK     ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tt_stock
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tt_stock
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      id_lote    format ">>>9" column-label "Lote"
     anio       format "x(2)" column-label "Año"
     tambores   format ">>>9" column-label "Tambores"
     kilos      format ">>>>>9" column-label "Kilos"
     kilos_400  format ">>>>>9" column-label "Conv 400"
     envase     format "x(12)" column-label "Envase"
     articulo   format "x(12)" column-label "Articulo"
     calidad    format "x(12)" column-label "Calidad"
     orden_f    format ">>9" column-label "OF"
     contrato   format "x(8)" column-label "Contrato"
     cliente    format "x(30)" column-label "Cliente"
     sem_des    format ">9" column-label "Sem"
     sem_h      format ">9" column-label "ana"
     anio_sem   format ">>>9" column-label "Año"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 151 BY 19.52
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt_stock WHERE ~{&KEY-PHRASE} NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cargar_datos B-table-Win 
...
define var p_suc as integer initial 96.
...
run dame_sucursal in h_con (output p_suc).


for each tambores_industria where id_sucursal_ubicacion = p_suc
    ...
    if last-of(tambores_industria.id_lote) then                               
        do:
            ...
            create tt_stock.
            assign tt_stock.id_lote     = tambores_industria.id_lote 
                   tt_stock.id_tipotambor = tambores_industria.id_tipotambor
                   tt_stock.anio        = STRING(tambores_industria.anio)
                   tt_stock.envase      = envases_prod.abreviatura
                   tt_stock.articulo    = productos_terminados.abreviatura
                   tt_stock.id_articulo = productos_terminados.id_articulo.
                   if available calidades then
                    do:
                        assign tt_stock.calidad     = calidades.abreviatura
                               tt_stock.id_calidad  = calidades.id_calidad.
                    end.                  
                   
                   if available contratos then
                    do:
                         assign tt_stock.orden_f     = contratos.orden_fabricacion 
                                tt_stock.contrato    = contratos.id_contrato
                                tt_stock.cliente     = clientes.nombre
                                tt_stock.sem_des     = items_contrato.semana_entrega
                                tt_stock.sem_has     = items_contrato.semana_entrega_hasta
                                tt_stock.anio_sem    = items_contrato.anio_semana.
                    end.
            assign tt_stock.tambores    = (accum count by tambores_industria.id_lote tambores_industria.id_lote)
                   tt_stock.kilos       = (accum total by tambores_industria.id_lote tambores_industria.kilos_tambor).
                   if available r_productos_calidad then
                   assign tt_stock.kilos_400   = (accum total by tambores_industria.id_lote tambores_industria.kilos_tambor) * 
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tt_stock"}
