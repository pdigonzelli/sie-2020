...
DEFINE VARIABLE h_b_prioridades AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_prioridades AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* ABM de Prioridades */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ABM de Prioridades */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'b_prioridades.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_prioridades ).
       RUN set-position IN h_b_prioridades ( 3.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_prioridades ( 6.71 , 83.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'v_prioridades.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_prioridades ).
       RUN set-position IN h_v_prioridades ( 11.00 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 48.00 ) */

       /* Links to csmartbrowser h_b_prioridades. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_prioridades ).

       /* Links to SmartViewer h_v_prioridades. */
       RUN add-link IN adm-broker-hdl ( h_b_prioridades , 'Record':U , h_v_prioridades ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_prioridades ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_prioridades ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_prioridades ,
             h_b_prioridades , 'AFTER':U ).
    END. /* Page 0 */
