...
lotes_aceite.id_lote productos_terminados.descripcion lotes_aceite.fecha ~
lotes_aceite.control_calidad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_aceite WHERE ~{&KEY-PHRASE} NO-LOCK, ~
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_aceite,comercial.sucursales OF general.lotes_aceite,general.productos_terminados OF general.lotes_aceite"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = comercial.sucursales.nombre
     _FldNameList[2]   > general.lotes_aceite.id_lote
"general.lotes_aceite.id_lote" ? ">>>>9" "integer" ? ? ? ? ? ? no ?
...
label 'fecha( lotes_aceite ) '
  ...
  menu-item lotes_aceite-control_calidad
label 'control_calidad( lotes_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=lotes_aceite.control_calidad').
                  run set-attribute-list('titulo=Estado Lote').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by lotes_aceite.control_calidad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by sucursales.nombre
...
case p:
  ...
  WHEN 'lotes_aceite.control_calidad' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  control_calidad no-lock no-error .
end.
END PROCEDURE.
