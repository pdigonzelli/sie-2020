...
lotes_jugo.id_lote lotes_jugo.anio productos_terminados.descripcion ~
lotes_jugo.Fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_jugo WHERE ~{&KEY-PHRASE} ~
      AND lotes_jugo.Fecha >= date("01/07/2002") ~
 AND lotes_jugo.id_tipotambor = 11 NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      lotes_jugo.anio FORMAT ">>>9":U
      productos_terminados.descripcion COLUMN-LABEL "Producto" FORMAT "x(25)":U
      lotes_jugo.Fecha FORMAT "99/99/99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85 BY 5.62
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "general.lotes_jugo.Fecha >= date(""01/07/2002"")
 AND general.lotes_jugo.id_tipotambor = 11"
...
"lotes_jugo.id_lote" "Lote" ">>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = general.lotes_jugo.anio
...
"productos_terminados.descripcion" "Producto" "x(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.lotes_jugo.Fecha
"lotes_jugo.Fecha" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
