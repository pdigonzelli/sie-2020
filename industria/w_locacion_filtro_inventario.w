...
DEFINE VARIABLE h_b_locacion_filtro AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "SELECCIONE LA LOCACION QUE DESEA INVENTARIAR" 
     VIEW-AS FILL-IN 
     SIZE 106 BY .95
...
DO:
  ...
  run get-rowid1 in h_b_locacion_filtro (output r).
  find locaciones where rowid(locaciones) = r no-lock no-error.
  if available locaciones then
   do:
        run w_posicion_filtro_inventariar.w (input locaciones.id_empresa, 
                                   input locaciones.id_sucursal, 
                                   input locaciones.id_locacion).
        
   end.
   else message "Existe un error en la seleccion de la Locación" view-as alert-box.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_locacion_filtro.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_locacion_filtro ).
       RUN set-position IN h_b_locacion_filtro ( 3.29 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 106.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_locacion_filtro ,
