...
&Scoped-define FIELDS-IN-QUERY-br_table remitos.id_sucursal remitos.nro ~
remitos.fecha remitos.estado remitos.c_usuario remitos.c_hora 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      remitos.id_sucursal COLUMN-LABEL "Suc" FORMAT ">>9":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 89 BY 2.62
         FONT 0.
...
"remitos.id_sucursal" "Suc" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.remitos.nro
"remitos.nro" "Nro" ">>>>>>>>9" "integer" ? ? ? ? ? ? no ? no no "12.4" yes no no "U" "" ""
     _FldNameList[3]   > general.remitos.fecha
"remitos.fecha" ? ? "date" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
     _FldNameList[4]   > general.remitos.estado
"remitos.estado" ? ? "logical" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[5]   > general.remitos.c_usuario
"remitos.c_usuario" ? ? "character" ? ? ? ? ? ? no ? no no "16" yes no no "U" "" ""
     _FldNameList[6]   > general.remitos.c_hora
...
DO:
  ...
  {src/adm/template/brschnge.i}
      ...
      RUN grabo-rowid-remito IN hcon (INPUT ROWID(remitos)).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE paso-rowid B-table-Win 
PROCEDURE paso-rowid :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE OUTPUT PARAMETER r_remito AS ROWID.

r_remito = ROWID({&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}).
