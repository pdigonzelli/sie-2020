...
&Scoped-Define ENABLED-FIELDS lotes_jugo.control_calidad ~
lotes_jugo.quimico_control_calidad 
&Scoped-define ENABLED-TABLES lotes_jugo
&Scoped-define FIRST-ENABLED-TABLE lotes_jugo
&Scoped-Define DISPLAYED-FIELDS lotes_jugo.control_calidad ~
lotes_jugo.quimico_control_calidad 
&Scoped-define DISPLAYED-TABLES lotes_jugo
&Scoped-define FIRST-DISPLAYED-TABLE lotes_jugo
&Scoped-Define DISPLAYED-OBJECTS fi-quimicos-nombre 
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_calidad,id_envase,id_sucursal"':U).
/**************************
</EXECUTING-CODE> */
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE VARIABLE fi-quimicos-nombre AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 40 BY .95
     FGCOLOR 1  NO-UNDO.
...
DEFINE FRAME F-Main
     lotes_jugo.control_calidad AT ROW 1 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16.6 BY 1
     fi-quimicos-nombre AT ROW 1.95 COL 23 COLON-ALIGNED NO-LABEL
     lotes_jugo.quimico_control_calidad AT ROW 2 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8 BY 1
...
/* SETTINGS FOR FILL-IN fi-quimicos-nombre IN FRAME F-Main
   NO-ENABLE                                                            */
...
general.lotes_jugo.quimico_control_calidad;wc_quimicos.w;quimicos.nombre;id_quimico;
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME lotes_jugo.control_calidad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.control_calidad V-table-Win
ON LEAVE OF lotes_jugo.control_calidad IN FRAME F-Main /* Estado Lote */
DO:
...
&Scoped-define SELF-NAME fi-quimicos-nombre
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-quimicos-nombre V-table-Win
ON LEAVE OF fi-quimicos-nombre IN FRAME F-Main
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME lotes_jugo.quimico_control_calidad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.quimico_control_calidad V-table-Win
ON GO OF lotes_jugo.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.quimico_control_calidad V-table-Win
ON LEAVE OF lotes_jugo.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.quimico_control_calidad V-table-Win
ON MOUSE-SELECT-DBLCLICK OF lotes_jugo.quimico_control_calidad IN FRAME F-Main /* Quimico */
do: 
define var r as rowid no-undo.
run wc_quimicos.w(output r).
find quimicos where rowid(quimicos) = r no-lock no-error.
if available quimicos then 
general.lotes_jugo.quimico_control_calidad:screen-value = string(quimicos.id_quimico).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.quimico_control_calidad V-table-Win
ON U1 OF lotes_jugo.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
IF USERID("userdb") = "y_rvelez" THEN DO:
  MESSAGE "Ud. No esta autorizado para realizar esta operacion" VIEW-AS ALERT-BOX INFO BUTTONS OK.
  RETURN "ADM-ERROR".
END.

if lotes_jugo.control_calidad:screen-value in frame F-Main = "Aprobado" and 
   integer(lotes_jugo.quimico_control_calidad:screen-value in frame F-Main) <> 0 then
   do:
      assign lotes_jugo.estado_lote = 3.
      if lotes_jugo.id_articulo > 900 then
            do:
                v_articulo = lotes_jugo.id_articulo - 900.
                assign lotes_jugo.id_articulo = v_articulo.
                for each tambores_industria of lotes_jugo.
                    assign tambores_industria.id_articulo = v_articulo.
                end.
            end.

   end.

if lotes_jugo.control_calidad:screen-value in frame F-Main = "Desaprobado" and 
   integer(lotes_jugo.quimico_control_calidad:screen-value in frame F-Main) <> 0 then
   do:
        message "Esta seguro que desea desaprobar el lote con la consecuencia de cambiar a materia prima?"
                view-as alert-box QUESTION BUTTONS YES-NO-CANCEL TITLE "" UPDATE choice AS LOGICAL.
        if choice then
            do:
                if lotes_jugo.id_articulo < 900 then v_articulo = lotes_jugo.id_articulo + 900.
                else 
                  do:  
                    v_articulo = lotes_jugo.id_articulo.
                    message "Esta desaprobando un lote que ya esta desaprobado" view-as alert-box.
                  end.
                  
                assign lotes_jugo.estado_lote = 4
                       lotes_jugo.id_articulo = v_articulo.
...
find first quimicos where quimicos.id_quimico = integer(lotes_jugo.quimico_control_calidad:screen-value in frame F-Main)  no-lock no-error .
if available quimicos then 
fi-quimicos-nombre:screen-value in frame F-Main = string(quimicos.nombre).
else
fi-quimicos-nombre:screen-value in frame F-Main = ''.
...
define var lista_relacion as character no-undo initial "quimico_control_calidad".
