...
DEFINE TEMP-TABLE tt-oe 
    ...
    FIELD kgs_tambores           AS decimal  COLUMN-LABEL "Kgs Brutos"               DECIMALS 4.
...
DEFINE FRAME fMain
     ...
     "  Exporta Productos Calidad Envase" VIEW-AS TEXT
...
FOR EACH r_productos_calidad_envase NO-LOCK
                                    BY id_articulo
                                    BY id_calidad 
                                    BY id_envase
                                    .
    FIND FIRST productos_terminados OF r_productos_calidad_envase
                                    NO-LOCK NO-ERROR.
    FIND FIRST calidades OF r_productos_calidad_envase
                                    NO-LOCK NO-ERROR.
    FIND FIRST envases_prod OF r_productos_calidad_envase
                                    NO-LOCK NO-ERROR.

    IF r_productos_calidad_envase.id_articulo > 0 AND
       r_productos_calidad_envase.id_calidad > 0 AND
       r_productos_calidad_envase.id_envase > 0 THEN DO:

        CREATE tt-oe.
        ASSIGN tt-oe.id_articulo    = r_productos_calidad_envase.id_articulo
               tt-oe.articulo       = IF AVAILABLE productos_terminados THEN productos_terminados.descripcion
                                                                        ELSE "NONE"
               tt-oe.id_calidad     = r_productos_calidad_envase.id_calidad
               tt-oe.calidad        = IF AVAILABLE calidades THEN calidades.descripcion
                                                             ELSE "NONE"
               tt-oe.id_envase      = r_productos_calidad_envase.id_envase
               tt-oe.envase         = IF AVAILABLE envases_prod THEN envases_prod.descripcion
                                                                ELSE "NONE"
               tt-oe.kgs_tambor     = r_productos_calidad_envase.kilos.
    END.
