...
DEFINE VARIABLE h_b_consulta_tambores_produccion AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Produccion de Tambores de Jugo"
         HEIGHT             = 22.38
         WIDTH              = 152.4
         MAX-HEIGHT         = 22.76
         MAX-WIDTH          = 157
...
ON END-ERROR OF W-Win /* Consulta Produccion de Tambores de Jugo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Consulta Produccion de Tambores de Jugo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 79.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 102.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_consulta_tambores_produccion_jugo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_consulta_tambores_produccion ).
       RUN set-position IN h_b_consulta_tambores_produccion ( 3.76 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.33 , 148.00 ) */

       /* Links to csmartbrowser h_b_consulta_tambores_produccion. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_consulta_tambores_produccion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             h_cus-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_consulta_tambores_produccion ,
             h_cus-misc , 'AFTER':U ).
    END. /* Page 0 */
