...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.id_contrato ~
rowObject.item rowObject.cantidad rowObject.Cumplidos rowObject.Articulo ~
rowObject.precio_venta rowObject.Moneda rowObject.CondVenta ~
rowObject.Calidad rowObject.Envase rowObject.semana_entrega ~
rowObject.semana_entrega_hasta rowObject.anio_semana_entrega rowObject.Tipo ~
rowObject.OrdenFabricacion 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      id_contrato FORMAT "X(12)":U WIDTH 8
      item FORMAT ">>>,>>9":U WIDTH 6
      cantidad FORMAT "->>>,>>>,>>9.99":U
      Cumplidos FORMAT ">>>,>>>,>>9":U
      Articulo FORMAT "x(20)":U
      precio_venta FORMAT "->>>,>>>,>>>,>>9.999":U
      Moneda FORMAT "x(15)":U
      CondVenta FORMAT "x(50)":U
      Calidad FORMAT "x(20)":U WIDTH 15
      Envase FORMAT "x(20)":U
      semana_entrega FORMAT ">>9":U
      semana_entrega_hasta FORMAT ">>9":U
      anio_semana_entrega FORMAT "9999":U
      Tipo FORMAT "x(15)":U
      OrdenFabricacion FORMAT "x(5)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 66 BY 4.05 EXPANDABLE.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.id_contrato
"id_contrato" ? ? "character" ? ? ? ? ? ? no "?" no no "8" yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.item
"item" ? ? "integer" ? ? ? ? ? ? no "?" no no "6" yes no no "U" "" ""
     _FldNameList[3]   = _<SDO>.rowObject.cantidad
     _FldNameList[4]   = _<SDO>.rowObject.Cumplidos
     _FldNameList[5]   > _<SDO>.rowObject.Articulo
"Articulo" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[6]   = _<SDO>.rowObject.precio_venta
     _FldNameList[7]   > _<SDO>.rowObject.Moneda
"Moneda" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[8]   > _<SDO>.rowObject.CondVenta
"CondVenta" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[9]   > _<SDO>.rowObject.Calidad
"Calidad" ? ? "character" ? ? ? ? ? ? no "?" no no "15" yes no no "U" "" ""
     _FldNameList[10]   > _<SDO>.rowObject.Envase
"Envase" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[11]   = _<SDO>.rowObject.semana_entrega
     _FldNameList[12]   = _<SDO>.rowObject.semana_entrega_hasta
     _FldNameList[13]   = _<SDO>.rowObject.anio_semana_entrega
     _FldNameList[14]   = _<SDO>.rowObject.Tipo
     _FldNameList[15]   = _<SDO>.rowObject.OrdenFabricacion
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND ((hCont:FILE-NAME MATCHES "*wEspecCont.w") OR  
                              (hCont:FILE-NAME MATCHES "*wResultAnal.w"))
    THEN DO:
    iBgColor = IF rowObject.rowNum MODULO 2 = 0 THEN 19 /*11 light blue*/ ELSE ? /*default*/.
    
    ASSIGN rowObject.id_contrato:BGCOLOR IN BROWSE br_table          = ibgcolor
           rowObject.ITEM:BGCOLOR IN BROWSE br_table                 = ibgcolor
           rowObject.cantidad:BGCOLOR IN BROWSE br_table             = ibgcolor
           ...
           rowObject.semana_entrega_hasta:BGCOLOR IN BROWSE br_table = ibgcolor
           rowObject.anio_semana_entrega:BGCOLOR IN BROWSE br_table  = ibgcolor
           rowObject.tipo:BGCOLOR IN BROWSE br_table                 = ibgcolor
           rowObject.ordenfabricacion:BGCOLOR IN BROWSE br_table     = ibgcolor
           .
