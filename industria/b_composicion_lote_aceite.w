...
composicion_lote_aceite.Numeracion_hasta composicion_lote_aceite.tara 
...
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ...
 ~{&FP1}cantidad_tambores ~{&FP2}cantidad_tambores ~{&FP3}~
 ~{&FP1}Numeracion_desde ~{&FP2}Numeracion_desde ~{&FP3}~
 ~{&FP1}Numeracion_hasta ~{&FP2}Numeracion_hasta ~{&FP3}~
...
<FOREIGN-KEYS>
id_sucursal||y|general.composicion_lote_aceite.id_sucursal
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      composicion_lote_aceite.fecha FORMAT "99/99/9999"
      composicion_lote_aceite.cantidad_tambores
      composicion_lote_aceite.Numeracion_desde
      composicion_lote_aceite.Numeracion_hasta
      composicion_lote_aceite.tara FORMAT ">>9.99"
  ENABLE
...
"composicion_lote_aceite.fecha" ? "99/99/9999" "date" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > general.composicion_lote_aceite.cantidad_tambores
"composicion_lote_aceite.cantidad_tambores" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > general.composicion_lote_aceite.Numeracion_desde
"composicion_lote_aceite.Numeracion_desde" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[4]   > general.composicion_lote_aceite.Numeracion_hasta
"composicion_lote_aceite.Numeracion_hasta" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[5]   > general.composicion_lote_aceite.tara
"composicion_lote_aceite.tara" ? ">>9.99" "decimal" ? ? ? ? ? ? yes ?
...
else message "Se ha producido un error en la grabación".
...
message "Desde tambor " desde " hasta 2 " hasta view-as alert-box.
if available lotes_aceite then
do:
    do i = desde to hasta:
        ...
        assign tambores_industria.id_empresa = lotes_aceite.id_empresa
         ...
         assign tambores_industria.tara      = tara
               ...
               find r_productos_calidad where r_productos_calidad.id_articulo = lotes_aceite.id_articulo
                                        no-lock no-error.
               if available r_productos_calidad then assign tambores_industria.id_calidad = 
                                                            r_productos_calidad.id_calidad.
               else 
                do:
                    message "Se ha creado un lote con un articulo no-comun, por favor avisar a sistemas"
                            view-as alert-box.
                    assign tambores_industria.id_calidad = 602.
                end.

    end.
end.
else message "Hubo un error en la grabación de los tambores".
...
find composicion_lote_aceite where rowid(composicion_lote_aceite) = r.
if available composicion_lote_aceite then
do:
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "composicion_lote_aceite" "id_sucursal"}
