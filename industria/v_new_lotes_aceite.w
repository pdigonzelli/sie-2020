...
DEFINE FRAME F-Main
     ...
     lotes_aceite.Peso_neto AT ROW 4.81 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 17 BY 1
     lotes_aceite.tanque AT ROW 4.81 COL 41 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 4.8 BY 1
...
ON GO OF lotes_aceite.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.id_articulo V-table-Win
ON LEAVE OF lotes_aceite.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.id_articulo V-table-Win
ON MOUSE-SELECT-DBLCLICK OF lotes_aceite.id_articulo IN FRAME F-Main /* Artículo */
do: 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.id_articulo V-table-Win
ON U1 OF lotes_aceite.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&Scoped-define SELF-NAME lotes_aceite.tanque
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.tanque V-table-Win
ON LEAVE OF lotes_aceite.tanque IN FRAME F-Main /* Tanque */
DO:
{custom/support/validacion.i}
END.
...
FIND FIRST lotes_ind WHERE ROWID(lotes_ind) = r NO-LOCK NO-ERROR.
IF AVAILABLE lotes_ind THEN DO:
    FIND LAST b_lote USE-INDEX lotes_aceite OF lotes_ind NO-ERROR.
    ...
    IF AVAILABLE b_lote THEN DO:
        FIND FIRST b_tam OF b_lote 
                         WHERE b_tam.id_sucursal_ubicacion = lotes_ind.id_sucursal
                         AND   b_tam.id_locacion_ubicacion = 4 NO-LOCK NO-ERROR.
    
        IF AVAILABLE b_tam THEN DO:
            desde = b_tam.id_tambor.
        END.
        FIND LAST b_tam OF b_lote 
                        WHERE b_tam.id_sucursal_ubicacion = lotes_ind.id_sucursal
                        AND   b_tam.id_locacion_ubicacion = 4 NO-LOCK NO-ERROR.
        ...
        RUN y_gstkcre.p (input b_lote.id_empresa,
                         input b_lote.id_sucursal,
                         input b_lote.id_tipotambor,
                         input b_lote.nromov,
                         INPUT desde,
                         INPUT hasta,
                         input 14) "lotes_aceite".
...
FIND FIRST lotes_ind WHERE ROWID(lotes_ind) = r NO-LOCK NO-ERROR.
IF AVAILABLE lotes_ind THEN DO:
    FIND LAST lotes_aceite USE-INDEX lotes_aceite OF lotes_ind NO-ERROR.
...
FOR EACH sobrante_lotes_aceite WHERE sobrante_lotes_aceite.id_empresa       = del_empresa_lote 
                                 AND sobrante_lotes_aceite.id_sucursal      = del_sucursal_lote 
                                 AND sobrante_lotes_aceite.id_tipotambor    = del_tipotambor_lote 
                                 AND sobrante_lotes_aceite.nromov           = del_nromov_lote.
    ...
    /***** ACA BORRO LOS TAMBORES SOBRANTE DE LOTE, POR ENDE LO CARGO EN EL MOVIMINTOS DE STOCK  ****/
    ...
    IF desde > 0 THEN DO:
        ...
        END.
    END.
    FOR EACH tambores_industria WHERE tambores_industria.id_empresa     = del_empresa 
                                  AND tambores_industria.id_sucursal    = del_sucursal 
                                  AND tambores_industria.id_tipotambor  = del_tipotambor 
                                  AND tambores_industria.nromov         = del_nromov.
...
/******* BORRO LOS TAMBORES DEL LOTE PROPIAMENTE DICHO  **********************************/
...
for each tambores_industria WHERE tambores_industria.id_empresa_destino = del_empresa_lote and
    ...
    IF FIRST-OF(tambores_industria.nromov) THEN DO:
        desde = tambores_industria.id_tambor
    END.
...
IF AVAILABLE lotes_aceite THEN DO:      
    ...
    IF lotes_aceite.activo THEN DO:
        ...
        FOR EACH sobrante_lotes_aceite WHERE sobrante_lotes_aceite.id_empresa       = lotes_aceite.id_empresa
            ...
            FOR EACH tambores_industria WHERE tambores_industria.id_empresa = del_empresa 
                                          AND tambores_industria.id_sucursal = del_sucursal 
                                          AND tambores_industria.id_tipotambor = del_tipotambor 
                                          AND tambores_industria.nromov = del_nromov NO-LOCK.
    ...
    END.
    ELSE DO:
        MESSAGE "No se puede borrar un lote que no esta activo" view-as alert-box.
        RETURN "ADM-ERROR".
    END.
END.        
IF existe THEN DO:
...
IF AVAILABLE lotes_aceite THEN DO:
    ...
    IF NOT AVAILABLE tambores_industria THEN DO:
        MESSAGE "No puede crear un nuevo proceso porque el Lote " lotes_aceite.id_lote
                " con Articulo " lotes_aceite.id_articulo " no tiene los tambores creados."
                VIEW-AS ALERT-BOX.
        /*RUN adm-cancel-record.*/
        RETURN "error".        
    END.
END.
 */ 
...
------------------------------------------------------------------------------*/
  case nombre:
    ...
    when "fecha" then
      do: 
        if DATE(lotes_aceite.fecha:screen-value in frame F-Main) <> TODAY AND
           DATE(lotes_aceite.fecha:screen-value in frame F-Main) <> TODAY - 1 then 
            do:
                mensaje = "No puede cargar una fecha de produccion anterior a " + STRING(TODAY - 1).
                return false.
            end.
