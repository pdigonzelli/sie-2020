...
&Scoped-define INTERNAL-TABLES subd_vtas r_items_venta_pack_list ~
items_packing_list orden_entrega
...
subd_vtas.nro_comp subd_vtas.fecha_comp orden_entrega.id_orden_entrega ~
orden_entrega.semana_entrega items_packing_list.nro_orden_embarque ~
items_packing_list.nro_lote items_packing_list.id_packing_list ~
items_packing_list.c_fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH subd_vtas WHERE ~{&KEY-PHRASE} ~
      AND subd_vtas.impreso = TRUE ~
 AND subd_vtas.id_punto_venta = 81 NO-LOCK, ~
      FIRST r_items_venta_pack_list OF subd_vtas OUTER-JOIN NO-LOCK, ~
      FIRST items_packing_list OF r_items_venta_pack_list OUTER-JOIN NO-LOCK, ~
      FIRST orden_entrega WHERE orden_entrega.id_orden_entrega = items_packing_list.nro_orden_embarque OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table subd_vtas r_items_venta_pack_list ~
items_packing_list orden_entrega
&Scoped-define FIRST-TABLE-IN-QUERY-br_table subd_vtas
&Scoped-define SECOND-TABLE-IN-QUERY-br_table r_items_venta_pack_list
&Scoped-define THIRD-TABLE-IN-QUERY-br_table items_packing_list
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table orden_entrega
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      subd_vtas.nro_comp FORMAT ">>>>>>>9":U WIDTH 16.8
      subd_vtas.fecha_comp FORMAT "99/99/99":U WIDTH 15.2
      orden_entrega.id_orden_entrega FORMAT ">>>>>9":U
      orden_entrega.semana_entrega FORMAT ">>9":U WIDTH 24.8
      items_packing_list.nro_orden_embarque COLUMN-LABEL "OE PackList" FORMAT "->,>>>,>>9":U
      items_packing_list.nro_lote COLUMN-LABEL "Lote PackList" FORMAT "x(8)":U
      items_packing_list.id_packing_list FORMAT "->,>>>,>>9":U
      items_packing_list.c_fecha COLUMN-LABEL "Fecha PL" FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 139 BY 17.62
         FONT 0 ROW-HEIGHT-CHARS .71.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ventas.subd_vtas,ventas.r_items_venta_pack_list OF ventas.subd_vtas,ventas.items_packing_list OF ventas.r_items_venta_pack_list,general.orden_entrega WHERE ventas.items_packing_list ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST OUTER, FIRST OUTER, FIRST OUTER"
     _Where[1]         = "ventas.subd_vtas.impreso = TRUE
 AND ventas.subd_vtas.id_punto_venta = 81"
     _JoinCode[4]      = "general.orden_entrega.id_orden_entrega = ventas.items_packing_list.nro_orden_embarque"
     _FldNameList[1]   > ventas.subd_vtas.id_punto_venta
"subd_vtas.id_punto_venta" ? ? "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[2]   > ventas.subd_vtas.nro_comp
"subd_vtas.nro_comp" ? ? "integer" ? ? ? ? ? ? no ? no no "16.8" yes no no "U" "" ""
     _FldNameList[3]   > ventas.subd_vtas.fecha_comp
"subd_vtas.fecha_comp" ? ? "date" ? ? ? ? ? ? no ? no no "15.2" yes no no "U" "" ""
     _FldNameList[4]   = general.orden_entrega.id_orden_entrega
     _FldNameList[5]   > general.orden_entrega.semana_entrega
"orden_entrega.semana_entrega" ? ? "integer" ? ? ? ? ? ? no ? no no "24.8" yes no no "U" "" ""
     _FldNameList[6]   > ventas.items_packing_list.nro_orden_embarque
"items_packing_list.nro_orden_embarque" "OE PackList" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > ventas.items_packing_list.nro_lote
"items_packing_list.nro_lote" "Lote PackList" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[8]   = ventas.items_packing_list.id_packing_list
     _FldNameList[9]   > ventas.items_packing_list.c_fecha
"items_packing_list.c_fecha" "Fecha PL" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
label 'id_orden_entrega( orden_entrega ) '
  ...
  menu-item orden_entrega-semana_entrega
label 'semana_entrega( orden_entrega ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=orden_entrega.semana_entrega').
                  run set-attribute-list('titulo=Semana de entrega').
                  run set-attribute-list('busca= no').
                  &SCOPE SORTBY-PHRASE  by orden_entrega.semana_entrega
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by subd_vtas.id_punto_venta
