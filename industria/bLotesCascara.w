...
rowObject.id_lote rowObject.anio rowObject.Producto rowObject.Cantidad ~
rowObject.control_calidad rowObject.usuario_control_calidad 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Sucursal FORMAT "x(20)":U
      id_lote COLUMN-LABEL "Lote" FORMAT "->,>>>,>>9":U
      anio FORMAT ">>>9":U
      Producto FORMAT "x(25)":U
      Cantidad FORMAT ">,>>9":U
      control_calidad FORMAT "Aprobado/Desaprobado":U
      usuario_control_calidad FORMAT "X(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"id_lote" "Lote" ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
...
"anio" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     ...
     _FldNameList[6]   = _<SDO>.rowObject.control_calidad
     _FldNameList[7]   = _<SDO>.rowObject.usuario_control_calidad
...
DO:
  ...
  IF VALID-HANDLE(vhSource) THEN DO:
    IF DYNAMIC-FUNCTION('getLoteDespachado' IN vhSource) THEN DO:
      rowObject.sucursal:BGCOLOR IN BROWSE br_table = 19.
      rowObject.id_lote:BGCOLOR IN BROWSE br_table = 19.
      rowObject.anio:BGCOLOR IN BROWSE br_table = 19.
      rowObject.cantidad:BGCOLOR IN BROWSE br_table = 19.
      rowObject.CONTROL_calidad:BGCOLOR IN BROWSE br_table = 19.
      rowObject.usuario_control_calidad:BGCOLOR IN BROWSE br_table = 19.
    END.
  END.
  ELSE 
    MESSAGE "Handle Invalido" VIEW-AS ALERT-BOX TITLE "bLotesCascara.ROW-DISPLAY()".
