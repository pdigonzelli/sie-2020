...
DEFINE VARIABLE h_b_instruc_pago AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_instruc_pago AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Payment Instruction */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Payment Instruction */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_instruc_pago.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_instruc_pago ).
       RUN set-position IN h_b_instruc_pago ( 4.10 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_instruc_pago ( 6.71 , 136.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_instruc_pago.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_instruc_pago ).
       RUN set-position IN h_v_instruc_pago ( 11.48 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.43 , 101.00 ) */

       /* Links to csmartbrowser h_b_instruc_pago. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_instruc_pago ).

       /* Links to SmartViewer h_v_instruc_pago. */
       RUN add-link IN adm-broker-hdl ( h_b_instruc_pago , 'Record':U , h_v_instruc_pago ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_instruc_pago ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_instruc_pago ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_instruc_pago ,
             h_b_instruc_pago , 'AFTER':U ).
    END. /* Page 0 */
