...
DEFINE FRAME F-Main
     ...
     lotes_ind.anio AT ROW 1.95 COL 34 COLON-ALIGNED FORMAT "9999"
          VIEW-AS FILL-IN 
          SIZE 7.6 BY 1
...
ON LEAVE OF lotes_ind.id_lote IN FRAME F-Main /* Lote */
DO:
{custom/support/validacion.i}
END.
...
------------------------------------------------------------------------------*/
  case nombre:
    when "anio" then
        if integer(valor) < 1000 then 
        do:
            mensaje = "Debe cargar un año valido".
            return false.
         end.
    end case.
