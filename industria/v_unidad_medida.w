...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES unidad_medida
&Scoped-define FIRST-EXTERNAL-TABLE unidad_medida


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR unidad_medida.
/* Standard List Definitions                                            */
&Scoped-Define ENABLED-FIELDS unidad_medida.id_um unidad_medida.id_medicion ~
unidad_medida.descripcion unidad_medida.abreviatura unidad_medida.decimales 
&Scoped-define FIELD-PAIRS~
 ~{&FP1}id_um ~{&FP2}id_um ~{&FP3}~
 ~{&FP1}id_medicion ~{&FP2}id_medicion ~{&FP3}~
 ~{&FP1}descripcion ~{&FP2}descripcion ~{&FP3}~
 ~{&FP1}abreviatura ~{&FP2}abreviatura ~{&FP3}~
 ~{&FP1}decimales ~{&FP2}decimales ~{&FP3}
&Scoped-define ENABLED-TABLES unidad_medida
&Scoped-define FIRST-ENABLED-TABLE unidad_medida
&Scoped-Define DISPLAYED-FIELDS unidad_medida.id_um ~
...
<FOREIGN-KEYS>
abreviatura|y|y|comercial.unidad_medida.abreviatura
id_um|y|y|comercial.unidad_medida.id_um
...
RUN set-attribute-list (
    'Keys-Accepted = "abreviatura,id_um",
     Keys-Supplied = "abreviatura,id_um,id_medicion"':U).
/**************************
</EXECUTING-CODE> */
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     unidad_medida.id_um AT ROW 1 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 4.8 BY 1
     unidad_medida.id_medicion AT ROW 2 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 3.4 BY 1
     unidad_medida.descripcion AT ROW 3 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 22 BY 1
     unidad_medida.abreviatura AT ROW 4 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 6.4 BY 1
     unidad_medida.decimales AT ROW 5 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 6.4 BY 1
...
/* Settings for THIS-PROCEDURE
   Type: SmartViewer
   External Tables: comercial.unidad_medida
...
&Scoped-define SELF-NAME unidad_medida.abreviatura
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL unidad_medida.abreviatura V-table-Win
ON LEAVE OF unidad_medida.abreviatura IN FRAME F-Main /* Abreviatura */
DO:
...
&Scoped-define SELF-NAME unidad_medida.decimales
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL unidad_medida.decimales V-table-Win
ON LEAVE OF unidad_medida.decimales IN FRAME F-Main /* Decimales */
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME unidad_medida.descripcion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL unidad_medida.descripcion V-table-Win
ON LEAVE OF unidad_medida.descripcion IN FRAME F-Main /* Descripción */
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME unidad_medida.id_medicion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL unidad_medida.id_medicion V-table-Win
ON LEAVE OF unidad_medida.id_medicion IN FRAME F-Main /* Tipo medición */
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME unidad_medida.id_um
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL unidad_medida.id_um V-table-Win
ON LEAVE OF unidad_medida.id_um IN FRAME F-Main /* Tipo UM */
DO:
{custom/support/validacion.i}
END.
  ...
  CASE RETURN-VALUE:
    WHEN 'abreviatura':U THEN
       {src/adm/template/find-tbl.i
           &TABLE = unidad_medida
           &WHERE = "WHERE unidad_medida.abreviatura eq key-value"
       }
    WHEN 'id_um':U THEN
       {src/adm/template/find-tbl.i
           &TABLE = unidad_medida
           &WHERE = "WHERE unidad_medida.id_um eq INTEGER(key-value)"
       }
  END CASE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "unidad_medida"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "unidad_medida"}
