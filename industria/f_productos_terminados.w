...
&Scoped-Define ENABLED-OBJECTS RECT-15 RECT-16 RECT-2 
...
DEFINE VARIABLE h_b_productos_terminados AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_productos_terminados AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_productos_terminados.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_productos_terminados ).
       RUN set-position IN h_b_productos_terminados ( 3.57 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_productos_terminados ( 6.71 , 155.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_productos_terminados.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_productos_terminados ).
       RUN set-position IN h_v_productos_terminados ( 10.81 , 29.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.29 , 108.60 ) */

       /* Links to csmartbrowser h_b_productos_terminados. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_productos_terminados ).

       /* Links to SmartViewer h_v_productos_terminados. */
       RUN add-link IN adm-broker-hdl ( h_b_productos_terminados , 'Record':U , h_v_productos_terminados ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_productos_terminados ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_productos_terminados ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_productos_terminados ,
             h_b_productos_terminados , 'AFTER':U ).
    END. /* Page 0 */
...
lista_reportes = "Listado Productos".
...
if lista_reportes = "" then
    ...
    do:
        ...
        case cresult:
            when "Listado Productos" then 
                do:
                    /************************************************************************************************************/
                    /********************ENVIO DE DATOS AL REPORT BUILDER********************************************************/
                    /************************************************************************************************************/
                    
                    v_filtro = "productos_terminados.id_tipo_articulo = 2".
                    
                          RUN  aderb\_prntrb2(
                           "..\industria\reports.prl", /* RB-REPORT-LIBRARY */
                           "productos_terminados",                    /* RB-REPORT-NAME */
                           "",                             /* RB-DB-CONNECTION */
                           ...
                           "",                              /* RB-OUTPUT-FILE */
                            ...
                            0,                              /* RB-END-PAGE - zero */
                           no,                              /* RB-TEST-PATTERN */
                           "Reporte de Productos Termiandos",         /* RB-WINDOW-TITLE */
