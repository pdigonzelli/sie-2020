...
produccion_jugo.id_produccion produccion_jugo.id_articulo ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH produccion_jugo WHERE ~{&KEY-PHRASE} ~
      AND ( if produccion_jugo.id_articulo = 53 then produccion_jugo.id_articulo <> articulo else true) ~
 AND produccion_jugo.Fecha <= fecha ~
 AND produccion_jugo.Fecha > 01/01/2002 ~
...
productos_terminados sucursales tambores_industria
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getCantidadTambores B-table-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      produccion_jugo.id_produccion FORMAT ">>>>>9":U
      produccion_jugo.id_articulo FORMAT ">>>>>9":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.produccion_jugo,general.productos_terminados OF general.produccion_jugo,comercial.sucursales OF general.produccion_jugo,industria.tambores_industria OF general.produccion_jugo"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,, FIRST"
     _Where[1]         = "( if produccion_jugo.id_articulo = 53 then general.produccion_jugo.id_articulo <> articulo else true)
 AND general.produccion_jugo.Fecha <= fecha
 AND general.produccion_jugo.Fecha > 01/01/2002
...
"general.produccion_jugo.id_produccion" ? ">>>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.produccion_jugo.id_articulo
"general.produccion_jugo.id_articulo" ? ">>>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
"general.productos_terminados.descripcion" ? "x(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = general.produccion_jugo.Fecha
...
run get-articulo in hcontainer (output articulo, output fecha, output sucursal).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getCantidadTambores B-table-Win 
FUNCTION getCantidadTambores RETURNS INTEGER
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER prod_jugo FOR produccion_jugo.

  RUN devuelve-rowid (OUTPUT r).

  FIND FIRST prod_jugo WHERE ROWID(prod_jugo) = r NO-LOCK NO-ERROR.
  IF AVAILABLE prod_jugo THEN DO:
    FOR EACH tambores_industria WHERE prod_jugo.nromov = tambores_industria.nromov
