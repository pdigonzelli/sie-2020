...
define var rol1 as char.
define var cat as integer.
...
&Scoped-define INTERNAL-TABLES envasadores_fabrica

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table envasadores_fabrica.id_legajo ~
envasadores_fabrica.rol envasadores_fabrica.id_categoria 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ~
envasadores_fabrica.id_legajo 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}id_legajo ~{&FP2}id_legajo ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table envasadores_fabrica
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table envasadores_fabrica
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH envasadores_fabrica OF composicion_lote WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table envasadores_fabrica
&Scoped-define FIRST-TABLE-IN-QUERY-br_table envasadores_fabrica
...
<FOREIGN-KEYS>
id_sucursal||y|general.envasadores_fabrica.id_sucursal
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
general.envasadores_fabrica.id_legajo;wc_legajo.w;legajo.nomb-12
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      envasadores_fabrica.id_legajo COLUMN-LABEL "Legajo"
      envasadores_fabrica.rol COLUMN-LABEL "Rol"
      envasadores_fabrica.id_categoria COLUMN-LABEL "Categoria"
  ENABLE
      envasadores_fabrica.id_legajo
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 67 BY 4.76
         BGCOLOR 15 FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.envasadores_fabrica OF general.composicion_lote"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.envasadores_fabrica.id_legajo
"envasadores_fabrica.id_legajo" "Legajo" ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > general.envasadores_fabrica.rol
"envasadores_fabrica.rol" "Rol" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > general.envasadores_fabrica.id_categoria
"envasadores_fabrica.id_categoria" "Categoria" ? "integer" ? ? ? ? ? ? no ?
...
&Scoped-define SELF-NAME envasadores_fabrica.id_legajo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL envasadores_fabrica.id_legajo br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF envasadores_fabrica.id_legajo IN BROWSE br_table /* Legajo */
DO:
...
message "ahora" view-as alert-box.*/
run automatico.
/******** EMPIEZA MENU ************/
define sub-menu Ordena 
  menu-item envasadores_fabrica-id_legajo
label 'id_legajo( envasadores_fabrica ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=envasadores_fabrica.id_legajo').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by envasadores_fabrica.id_legajo
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item envasadores_fabrica-rol
label 'rol( envasadores_fabrica ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=envasadores_fabrica.rol').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by envasadores_fabrica.rol
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item envasadores_fabrica-id_categoria
label 'id_categoria( envasadores_fabrica ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=envasadores_fabrica.id_categoria').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by envasadores_fabrica.id_categoria
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by envasadores_fabrica.id_legajo
...
envasadores_fabrica.id_legajo:label-bgcolor in browse {&BROWSE-NAME} = 7.
envasadores_fabrica.id_legajo:label-fgcolor in browse {&BROWSE-NAME} = 15.
on MOUSE-SELECT-DBLCLICK,F10 of browse {&BROWSE-NAME} do:
   ...
   if hcolumn:name = 'id_legajo' then do: 
...
define var r as rowid.
run set-attribute-list('alta-automatica=no').
/*message "teoricamente es no" view-as alert-box.*/
...
run dame-rowid-composicion in hcontainer (output r).

  /*  find composicion_lote where composicion_lote.id_empresa = lotes_jugo.id_empresa and
                                composicion_lote.id_sucursal = lotes_jugo.id_sucursal and
                                composicion_lote.id_lote = lotes_jugo.id_lote and
                                composicion_lote.fecha = today no-lock no-error.*/
find composicion_lote where rowid(composicion_lote) = r no-lock no-error.    
    if available composicion_lote then
     do:
        /*message "post create browser" composicion_lote.fecha view-as alert-box.*/
        assign envasadores_fabrica.id_empresa = composicion_lote.id_empresa
               envasadores_fabrica.id_sucursal = composicion_lote.id_sucursal
               envasadores_fabrica.id_lote = composicion_lote.id_lote
               envasadores_fabrica.id_categoria = cat
               envasadores_fabrica.rol = rol1
               envasadores_fabrica.fecha = composicion_lote.fecha
               envasadores_fabrica.c_usuario = userid("userdb")
               envasadores_fabrica.c_fecha   = today
               envasadores_fabrica.c_hora    = string(time,"HH:MM:SS").
    
                /*message composicion_lote view-as alert-box.*/
    end.
   else
    do:
        message "Primero debe ingresar los tambores que se envasaron en el dia! " view-as alert-box.
        return "ADM-ERROR".
    end.
...
case p:
  WHEN 'envasadores_fabrica.id_legajo' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_legajo = integer(valor)  no-lock no-error .
  WHEN 'envasadores_fabrica.rol' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  rol begins valor  no-lock no-error .
  WHEN 'envasadores_fabrica.id_categoria' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_categoria = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "envasadores_fabrica" "id_sucursal"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "envasadores_fabrica"}
