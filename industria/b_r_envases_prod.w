...
&Scoped-define INTERNAL-TABLES r_envases_prod productos_terminados ~
envases_prod
...
&Scoped-define FIELDS-IN-QUERY-br_table productos_terminados.descripcion ~
envases_prod.descripcion r_envases_prod.Kilos r_envases_prod.kilos_nominal 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_envases_prod WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF r_envases_prod NO-LOCK, ~
      EACH envases_prod OF r_envases_prod NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table r_envases_prod productos_terminados ~
envases_prod
&Scoped-define FIRST-TABLE-IN-QUERY-br_table r_envases_prod
...
<FOREIGN-KEYS>
id_envase|y|y|general.r_envases_prod.id_envase
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      productos_terminados.descripcion COLUMN-LABEL "Articulos"
      envases_prod.descripcion COLUMN-LABEL "Envases"
      r_envases_prod.Kilos
      r_envases_prod.kilos_nominal
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_envases_prod,general.productos_terminados OF general.r_envases_prod,general.envases_prod OF general.r_envases_prod"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.productos_terminados.descripcion
"productos_terminados.descripcion" "Articulos" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   > general.envases_prod.descripcion
"envases_prod.descripcion" "Envases" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   = general.r_envases_prod.Kilos
     _FldNameList[4]   = general.r_envases_prod.kilos_nominal
...
label 'descripcion( envases_prod ) '
  ...
  menu-item r_envases_prod-Kilos
label 'Kilos( r_envases_prod ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_envases_prod.Kilos').
                  run set-attribute-list('titulo=Kilos').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_envases_prod.Kilos
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item r_envases_prod-kilos_nominal
label 'kilos_nominal( r_envases_prod ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_envases_prod.kilos_nominal').
                  run set-attribute-list('titulo=Kilos nom').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_envases_prod.kilos_nominal
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by productos_terminados.descripcion
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'id_envase':U THEN DO:
       &Scope KEY-PHRASE r_envases_prod.id_envase eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* id_envase */
    OTHERWISE DO:
...
case p:
  WHEN 'r_envases_prod.Kilos' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  Kilos = decimal(valor)  no-lock no-error .
  WHEN 'r_envases_prod.kilos_nominal' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  kilos_nominal = decimal(valor)  no-lock no-error .
end.
END PROCEDURE.
