...
DEFINE VARIABLE cStreamSilos AS CHARACTER  NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getLinea wWin 
FUNCTION getLinea RETURNS INTEGER
  ( /* parameter-definitions */ )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getTurno wWin 
FUNCTION getTurno RETURNS INTEGER
  ( /* parameter-definitions */ )  FORWARD.
...
DEFINE VARIABLE h_ftouchmoliendapg1 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ftouchmoliendapg2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF wWin /* Registro Molienda */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON F11 OF wWin /* Registro Molienda */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Registro Molienda */
DO:
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_ftouchmoliendapg1 ).
       RUN repositionObject IN h_ftouchmoliendapg1 ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in AB:  ( 33.33 , 202.20 ) */

    END. /* Page 0 */

    WHEN 1 THEN DO:
       RUN constructObject (
             INPUT  'ftouchmoliendapg2.w':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'LogicalObjectNamePhysicalObjectNameDynamicObjectnoRunAttributeHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_ftouchmoliendapg2 ).
       RUN repositionObject IN h_ftouchmoliendapg2 ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in AB:  ( 33.33 , 203.60 ) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE exitApp wWin 
PROCEDURE exitApp :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  /* 
  wWin:MIN-BUTTON = FALSE.
  wWin:MAX-BUTTON = FALSE.
  */
  def var li_hwnd as int no-undo.
  RUN splashdemo.p.

  wWin:WIDTH-PIXELS = 1022.
  wWin:HEIGHT-PIXELS = 766.
  wWin:X = 0.
  wWin:Y = 0.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE maximizeWindow wWin 
...
------------------------------------------------------------------------------*/
  ...
  IF piPageNum = 1 THEN
    RUN loadSilos IN h_fTouchMoliendaPg2 (cStreamSilos).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE switchPage wWin 
...
------------------------------------------------------------------------------*/
  ...
  IF pcArgs <> "" THEN DO: 
    ASSIGN dFecha = DATE(ENTRY(1, pcArgs))
           iLinea = INTEGER(ENTRY(2, pcArgs))
           iTurno = INTEGER(ENTRY(3, pcArgs))
           .
    cStreamSilos = DYNAMIC-FUNCTION('getMoliendaSilos' IN hLib, dFecha, iLinea, iTurno).
  END. 
  RUN selectPage (piPage).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getLinea wWin 
FUNCTION getLinea RETURNS INTEGER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getTurno wWin 
FUNCTION getTurno RETURNS INTEGER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
