...
DEFINE VARIABLE h_b_r_productos_calidades_envase AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_r_productos_calidades_envase AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relacion entre Productos Calidades Envases"
         HEIGHT             = 17.14
         WIDTH              = 144
         MAX-HEIGHT         = 19.86
         MAX-WIDTH          = 156.6
...
ON END-ERROR OF W-Win /* Relacion entre Productos Calidades Envases */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Relacion entre Productos Calidades Envases */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 21.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 55.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 78.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_r_productos_calidades_envases.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_r_productos_calidades_envase ).
       RUN set-position IN h_b_r_productos_calidades_envase ( 3.86 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 141.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_r_productos_calidades_envases.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_r_productos_calidades_envase ).
       RUN set-position IN h_v_r_productos_calidades_envase ( 11.48 , 36.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 64.00 ) */

       /* Links to csmartbrowser h_b_r_productos_calidades_envase. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_r_productos_calidades_envase ).

       /* Links to SmartViewer h_v_r_productos_calidades_envase. */
       RUN add-link IN adm-broker-hdl ( h_b_r_productos_calidades_envase , 'Record':U , h_v_r_productos_calidades_envase ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_r_productos_calidades_envase ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_r_productos_calidades_envase ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_r_productos_calidades_envase ,
             h_b_r_productos_calidades_envase , 'AFTER':U ).
    END. /* Page 0 */
