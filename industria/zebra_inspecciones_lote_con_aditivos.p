...
do:
  ...
  /*zebra gabriel*/
  /*cZebraPrinter = "\\192.168.1.58\zebra105".*/
...
/*****************GENERO LA ETIQUETA NUMERO CERO***********************************************************/
            ...
            put control "^FO025,50^A0N,40,40^FD" "PRODUCED BY S.A. SAN MIGUEL A.G.I.C.I. y F." "^FS".
            put control "^FO25,96^GB785,0,3^FS".
            if lotes_jugo.id_sucursal = 96 then
            do:  
                put control "^FO30,106^A0N,28,23^FD" "Country of Origin: ARGENTINA" "^FS".            
                put control "^FO30,136^A0N,28,23^FD" "Industrial Plant N. 1 - Lavalle 4001-Tel: (0381) 4512600 - Fax: (0381) 4512612" "^FS".                 
                put control "^FO25,165^GB785,0,3^FS".
                ...
                if available r_productos_calidad and r_productos_calidad.rnpa <> "" then
                  do:
                    put control "^FO30,170^A0N,36,23^FD" "CP(4000) S.M. de Tucuman - Argentina.  RNE:23000573 RNPA:" + r_productos_calidad.rnpa + "^FS".
                  end.
                else 
                  do:
                    ...
                    CASE choice:
                        WHEN TRUE THEN /* Yes */
                          DO:
                            put control "^FO30,170^A0N,36,23^FD" "(4000) S.M. de Tucuman - Argentina.  RNE:23000573 RNPA:23026750" "^FS".        
                          END.
                        WHEN FALSE THEN /* No */
                          DO:
                            return no-apply.                      
            ...
            do:
                ...
                put control "^FO30,106^A0N,28,23^FD" "Country of Origin: ARGENTINA" "^FS".            
                put control "^FO30,136^A0N,28,23^FD" "Industrial Plant N. 2 - Ruta Provincial 38 -Tel: (0381) 4512650" "^FS".            
                put control "^FO25,165^GB785,0,3^FS".
                find r_productos_calidad where r_productos_calidad.id_calidad = lotes_jugo.id_calidad
                                           and r_productos_calidad.id_articulo = lotes_jugo.id_articulo no-error.
                ...
                if available r_productos_calidad and r_productos_calidad.rnpa <> "" then
                  do:
                    put control "^FO30,170^A0N,36,23^FD" "CP(4132) Padilla - Famailla - Argentina.  RNE:23001973 RNPA:" + r_productos_calidad.rnpa + "^FS".
                  end.
                else 
                  do:
                    MESSAGE "Esta por imprimir una etiqueta que no tiene asociado un RNPA definido. Desea imprimir
                    ...
                    CASE choice95:
                        WHEN TRUE THEN /* Yes */
                          DO:
                            put control "^FO30,170^A0N,36,23^FD" "CP(4132) Padilla - Famailla - Argentina.  RNE:23001973 RNPA:23026750" "^FS".        
                          END.
                        WHEN FALSE THEN /* No */
                          DO:
                            return no-apply.                      
            ...
            if lotes_jugo.id_articulo = 52 then
                do:
                  put control "^FO30,220^A0N,40,30^FD" "PRODUCT: FROZEN CONCENTRATED CLOUDY LEMON JUICE" "^FS".
                end.
            if lotes_jugo.id_articulo = 53 then
                do:
                    put control "^FO30,220^A0N,40,30^FD" "PRODUCT: FROZEN CONCENTRATED CLEAR LEMON JUICE" "^FS".
                end.
            put control "^FO25,265^GB785,0,3^FS".
            put control "^FO25,280^GB355,720,3^FS".
            put control "^FO30,290^A0N,34,25^FD" "Fill Date" "^FS".
            put control "^FO100,330^A0N,45,60^FD" lotes_jugo.fecha "^FS".
            put control "^FO441,280^GB370,720,3^FS".
            put control "^FO446,290^A0N,34,25^FD" "Tracedbility data." "^FS".
            put control "^FO560,330^A0N,45,60^FD" string(lotes_jugo.id_lote,">>99") + "/" + substr(string(lotes_jugo.anio),3,2) "^FS".
            
          find last inspecciones_lote of lotes_jugo WHERE inspecciones_lote.final = TRUE no-lock no-error.
          if available inspecciones_lote then
           do:                                     
            ...
            put control "^FO30,394^A0N,34,25^FD" "Brix Uncorrected" "^FS".
            ...
            put control "^FO30,522^A0N,34,25^FD" "Acidity w/w" "^FS".
            ...
            put control "^FO30,650^A0N,34,25^FD" "Brix Corrected" "^FS".
            ...
            put control "^FO30,770^A0N,35,25^FD" "Acidity (GPL)" "^FS".            
            put control "^FO184,810^A0N,45,45^FD" string(inspecciones_lote.acidez_w_v,"999.99") "^FS".
            put control "^FO30,860^A0N,20,25^FD" "as anhydrous citric acid (ACA)" "^FS".
            ...
            if lotes_jugo.id_articulo = 52 then  do:   /*turbio*/
              put control "^FO30,898^A0N,34,25^FD" "Pulp %" "^FS".
              put control "^FO0150,895^A0N,38,45^FD" string(inspecciones_lote.porcentaje_pulpa,">9.99") + "%" "^FS".
              
              put control "^FO25,925^GB355,0,3^FS".
              /*put control "^FO030,935^A0N,25,25^FD" "Additives: Not Applicable" "^FS".*/
              put control "^FO030,929^A0N,18,25^FD" "Additives: Sodium Benzoate and " "^FS".
              put control "^FO130,945^A0N,18,25^FD" "Sodium Metabisulphite" "^FS".
              put control "^FO25,960^GB355,0,3^FS".
              put control "^FO030,968^A0N,25,25^FD" "Product Code: Not Applicable" "^FS".
            end.
            ELSE DO: /*claro*/           
              /*put control "^FO030,898^A0N,34,25^FD" "Additives: Not Applicable" "^FS".*/
              put control "^FO030,898^A0N,20,25^FD" "Additives: Sodium Benzoate and " "^FS".
              put control "^FO130,920^A0N,20,25^FD" "Sodium Metabisulphite" "^FS".
              put control "^FO25,940^GB355,0,3^FS".
              put control "^FO030,960^A0N,34,25^FD" "Product Code: Not Applicable" "^FS".
            END.    
            ...
            put control "^FO358,1010^A0N,30,20^FD" "STORAGE" "^FS".
            put control "^FO35,1042^A0N,30,20^FD" "KEEP THIS PRODUCT FROZEN, AT -18 / -20 C. IF THESE TEMPERATURES ARE NOT KEPT," "^FS".
            put control "^FO142,1080^A0N,30,20^FD" "S.A. SAN MIGUEL BE COMES NOT RESPONSIBLE FOR THIS PRODUCT" "^FS".
            ...
            */
            ...
            do:  
                ...
                */
                ...
                if available r_productos_calidad and r_productos_calidad.rnpa <> "" then
                  do:
                    put control "^FO30,170^A0N,36,23^FD" "(4000) S.M. de Tucuman - Argentina.  RNE:23000573 RNPA:" + r_productos_calidad.rnpa + "^FS".
                  end.
                else 
                  do:
                    MESSAGE "Esta por imprimir una etiqueta que no tiene asociado un RNPA definido. Desea imprimir
            ...
            if lotes_jugo.id_articulo = 52 then
                do:
                   put control "^FO30,220^A0N,40,40^FD" "PRODUCT: FROZEN CONCENTRATED LEMON JUICE" "^FS".
