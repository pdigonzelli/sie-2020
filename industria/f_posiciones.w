...
&Scoped-Define ENABLED-OBJECTS RECT-18 RECT-15 RECT-2 
...
DEFINE VARIABLE h_b_posiciones AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_posiciones AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 33.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 67.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 90.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_posiciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_posiciones ).
       RUN set-position IN h_b_posiciones ( 3.95 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 140.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_posiciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_posiciones ).
       RUN set-position IN h_v_posiciones ( 11.86 , 37.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 72.00 ) */

       /* Links to csmartbrowser h_b_posiciones. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_posiciones ).

       /* Links to SmartViewer h_v_posiciones. */
       RUN add-link IN adm-broker-hdl ( h_b_posiciones , 'Record':U , h_v_posiciones ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_posiciones ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_posiciones ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_posiciones ,
             h_b_posiciones , 'AFTER':U ).
    END. /* Page 0 */
