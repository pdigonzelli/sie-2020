...
&Scoped-define INTERNAL-TABLES posiciones sucursales locaciones

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table posiciones.id_empresa ~
sucursales.nombre posiciones.id_locacion locaciones.descripcion ~
posiciones.id_posicion posiciones.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH posiciones WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH sucursales OF posiciones NO-LOCK, ~
      EACH locaciones OF posiciones NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table posiciones sucursales locaciones
&Scoped-define FIRST-TABLE-IN-QUERY-br_table posiciones
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      posiciones.id_empresa COLUMN-LABEL "Emp."
      sucursales.nombre COLUMN-LABEL "Sucursal"
      posiciones.id_locacion COLUMN-LABEL "Loc." FORMAT ">,>>9"
      locaciones.descripcion COLUMN-LABEL "Locación"
      posiciones.id_posicion COLUMN-LABEL "Pos." FORMAT ">,>>9"
      posiciones.descripcion COLUMN-LABEL "Posición"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.posiciones,comercial.sucursales OF general.posiciones,industria.locaciones OF general.posiciones"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.posiciones.id_empresa
"general.posiciones.id_empresa" "Emp." ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > comercial.sucursales.nombre
"comercial.sucursales.nombre" "Sucursal" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > general.posiciones.id_locacion
"general.posiciones.id_locacion" "Loc." ">,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[4]   > general.locaciones.descripcion
"general.locaciones.descripcion" "Locación" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > general.posiciones.id_posicion
"general.posiciones.id_posicion" "Pos." ">,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[6]   > general.posiciones.descripcion
"general.posiciones.descripcion" "Posición" ? "character" ? ? ? ? ? ? no ?
...
define sub-menu Ordena 
  menu-item posiciones-id_empresa
label 'id_empresa( posiciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=posiciones.id_empresa').
                  run set-attribute-list('titulo=Empresa').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by posiciones.id_empresa
...
label 'nombre( sucursales ) '
  ...
  menu-item posiciones-id_locacion
label 'id_locacion( posiciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=posiciones.id_locacion').
                  run set-attribute-list('titulo=Locacion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by posiciones.id_locacion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
label 'descripcion( locaciones ) '
  ...
  menu-item posiciones-id_posicion
label 'id_posicion( posiciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=posiciones.id_posicion').
                  run set-attribute-list('titulo=Posicion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by posiciones.id_posicion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  WHEN 'posiciones.id_empresa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_empresa = integer(valor)  no-lock no-error .
  WHEN 'posiciones.id_locacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_locacion = integer(valor)  no-lock no-error .
  WHEN 'posiciones.id_posicion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_posicion = integer(valor)  no-lock no-error .
  WHEN 'posiciones.descripcion' then  
