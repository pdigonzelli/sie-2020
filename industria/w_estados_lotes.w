...
DEFINE VARIABLE h_b_estados_lotes AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_estados_lotes AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Estados Lotes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Estados Lotes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_estados_lotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_estados_lotes ).
       RUN set-position IN h_b_estados_lotes ( 3.86 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_estados_lotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_estados_lotes ).
       RUN set-position IN h_v_estados_lotes ( 11.24 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.00 , 52.00 ) */

       /* Links to csmartbrowser h_b_estados_lotes. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_estados_lotes ).

       /* Links to SmartViewer h_v_estados_lotes. */
       RUN add-link IN adm-broker-hdl ( h_b_estados_lotes , 'Record':U , h_v_estados_lotes ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_estados_lotes ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_estados_lotes ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_estados_lotes ,
             h_b_estados_lotes , 'AFTER':U ).
    END. /* Page 0 */
