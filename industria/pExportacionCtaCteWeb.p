...
DEFINE VARIABLE iArticulo AS INTEGER    NO-UNDO INITIAL 1.
DEFINE VARIABLE iCampana AS INTEGER    NO-UNDO INITIAL 2003.
...
FOR EACH ITEM_viajes WHERE ITEM_viajes.id_articulo = iArticulo,
       FIRST viajes WHERE viajes.id_viaje   = ITEM_viajes.id_viaje AND
                          viajes.id_campana  >= iCampana AND
     ...
     /* Venta de Gestion          */
         RUN ..\cons\getFobMO.p (BUFFER ITEM_viajes,
                         INPUT TODAY,
                         OUTPUT vfob,
                         OUTPUT vpall,
                         OUTPUT vbult).
        ...
        ASSIGN
              ...
              IF LAST-OF(viajes.id_cliente) THEN DO:
                  ...
                  FOR EACH cobranzas_me WHERE cobranzas_me.id_cliente = viajes.id_cliente AND
                                              cobranzas_me.id_campana >= iCampana          AND
...
&IF DEFINED(EXCLUDE-getDatosCobranzas) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getDatosCobranzas Procedure 
PROCEDURE getDatosCobranzas :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE OUTPUT PARAMETER pfijacob   AS DECIMAL    NO-UNDO.
  DEFINE OUTPUT PARAMETER pacargacob AS DECIMAL    NO-UNDO.
  DEFINE OUTPUT PARAMETER pdoccob    AS DECIMAL    NO-UNDO.
  DEFINE OUTPUT PARAMETER parribocob AS DECIMAL    NO-UNDO.
  DEFINE OUTPUT PARAMETER psaldocob  AS DECIMAL    NO-UNDO.

/* Condiciones y formas de Pago de Cobranzas Imputadas */ 
  FOR EACH r_cobranzas_viajes WHERE r_cobranzas_viajes.id_viaje = viajes.id_viaje AND
                                    r_cobranzas_viajes.fecha <= TODAY AND
                                    r_cobranzas_viajes.id_articulo = ITEM_viajes.id_articulo AND
                                    r_cobranzas_viajes.id_variedad = ITEM_viajes.id_variedad NO-LOCK:

        CASE r_cobranzas_viajes.id_condicion:
           WHEN 1 THEN DO:  /* Suma Fija */
              /*
                IF AVAILABLE b_itemscondaux THEN DO:
                    FOR EACH cronograma_cond_pagos 
                           WHERE cronograma_cond_pagos.id_cond_pago_cliente = b_itemscondaux.id_cond_pago_cliente AND
                                 cronograma_cond_pagos.ITEM                 = b_itemscondaux.ITEM NO-LOCK:
    
                           IF pfecha >= cronograma_cond_pagos_clientes.fecha THEN
                              vtotalsumafija = vtotalsumafija + cronograma_cond_pagos_clientes.importe.
                    END.
                END.*/

                pfijacob = pfijacob + r_cobranzas_viajes.importe_pago.

           END.
           WHEN 2 THEN DO: /* Contro Orden de Pedido */
           END.
           WHEN 3 THEN  /* Contra la Carga */
                pacargacob = pacargacob + r_cobranzas_viajes.importe_pago.
           
           WHEN 4 THEN DO: /* Contra envio de Documentos */
               IF TODAY >= viajes.fecha_arribo + 7 THEN
                 pdoccob = pdoccob + r_cobranzas_viajes.importe_pago.
           END.
           WHEN 5 THEN DO: /* Al arribo */
               IF TODAY >= viajes.fecha_arribo THEN
                  parribocob = parribocob + r_cobranzas_viajes.importe_pago.
               
           END.
           WHEN 6 THEN  /* Saldo*/
                psaldocob = psaldocob + r_cobranzas_viajes.importe_pago.
