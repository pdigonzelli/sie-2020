...
&Scoped-define INTERNAL-TABLES muestras contactos_muestras ~
...
productos_terminados.descripcion_ingles contactos_muestras.nombre ~
muestras.directo_cliente 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH muestras WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH contactos_muestras WHERE contactos_muestras.id_contacto = muestras.id_destinatario NO-LOCK, ~
      EACH productos_terminados OF muestras NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH muestras WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH contactos_muestras WHERE contactos_muestras.id_contacto = muestras.id_destinatario NO-LOCK, ~
      EACH productos_terminados OF muestras NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table muestras contactos_muestras ~
productos_terminados
&Scoped-define FIRST-TABLE-IN-QUERY-br_table muestras
&Scoped-define SECOND-TABLE-IN-QUERY-br_table contactos_muestras
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      muestras.id_muestra COLUMN-LABEL "Cod" FORMAT ">>>9":U
      muestras.anio_muestra FORMAT "99":U WIDTH 5.6
      muestras.fecha COLUMN-LABEL "Pedido" FORMAT "99/99/99":U
      productos_terminados.descripcion_ingles COLUMN-LABEL "Producto" FORMAT "X(27)":U
      contactos_muestras.nombre COLUMN-LABEL "Enviar a" FORMAT "X(30)":U
      muestras.directo_cliente FORMAT "SI/NO":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.muestras,industria.contactos_muestras WHERE general.muestras ...,general.productos_terminados OF general.muestras"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,"
     _JoinCode[2]      = "general.contactos_muestras.id_contacto = general.muestras.id_destinatario"
     _FldNameList[1]   > general.muestras.id_muestra
"muestras.id_muestra" "Cod" ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.muestras.anio_muestra
"muestras.anio_muestra" ? "99" "integer" ? ? ? ? ? ? no ? no no "5.6" yes no no "U" "" ""
     _FldNameList[3]   > general.muestras.fecha
"muestras.fecha" "Pedido" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.descripcion_ingles
"productos_terminados.descripcion_ingles" "Producto" "X(27)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.contactos_muestras.nombre
"contactos_muestras.nombre" "Enviar a" "X(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > general.muestras.directo_cliente
"muestras.directo_cliente" ? "SI/NO" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
label 'fecha( muestras ) '
  ...
  menu-item productos_terminados-descripcion_ingles
label 'descripcion_ingles( productos_terminados ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=productos_terminados.descripcion_ingles').
                  run set-attribute-list('titulo=Descripción en inglés').
                  run set-attribute-list('busca= no').
                  &SCOPE SORTBY-PHRASE  by productos_terminados.descripcion_ingles
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
