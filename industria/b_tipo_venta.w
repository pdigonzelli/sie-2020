...
&Scoped-define INTERNAL-TABLES tipo_venta

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table tipo_venta.id_tipo_venta ~
tipo_venta.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tipo_venta WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tipo_venta
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tipo_venta
...
<FOREIGN-KEYS>
id_tipo_venta||y|ventas.tipo_venta.id_tipo_venta
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      tipo_venta.id_tipo_venta FORMAT ">>9":U WIDTH 15.2
      tipo_venta.descripcion FORMAT "X(20)":U WIDTH 43.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ventas.tipo_venta"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > ventas.tipo_venta.id_tipo_venta
"id_tipo_venta" ? ? "integer" ? ? ? ? ? ? no ? no no "15.2" yes no no "U" "" ""
     _FldNameList[2]   > ventas.tipo_venta.descripcion
"descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "43.2" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item tipo_venta-id_tipo_venta
label 'id_tipo_venta( tipo_venta ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tipo_venta.id_tipo_venta').
                  run set-attribute-list('titulo=Cod. Venta').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tipo_venta.id_tipo_venta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item tipo_venta-descripcion
label 'descripcion( tipo_venta ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tipo_venta.descripcion').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tipo_venta.descripcion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by tipo_venta.id_tipo_venta
...
case p:
  WHEN 'tipo_venta.id_tipo_venta' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_tipo_venta = integer(valor)  no-lock no-error .
  WHEN 'tipo_venta.descripcion' then  
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_tipo_venta" "tipo_venta" "id_tipo_venta"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tipo_venta"}
