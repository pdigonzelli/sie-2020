...
&Scoped-Define ENABLED-OBJECTS BUTTON-17 RECT-2 
...
DEFINE VARIABLE h_b_lotes_orden_fabricacion_todo AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_lotes_jugo_items_oe AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Asignacion de OE a los Lotes de Jugo"
         HEIGHT             = 13.52
         WIDTH              = 148.4
         MAX-HEIGHT         = 19.38
         MAX-WIDTH          = 148.4
...
ON END-ERROR OF W-Win /* Asignacion de OE a los Lotes de Jugo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Asignacion de OE a los Lotes de Jugo */
DO:
...
run get-rowid1 in h_b_lotes_orden_fabricacion_todo (output r).
find lotes_jugo where rowid(lotes_jugo) = r no-lock no-error.
if available lotes_jugo then
    do:
        FIND FIRST items_orden_entrega of lotes_jugo no-lock no-error.
        ...
        ELSE MESSAGE "No encontro la parte " lotes_jugo.ITEM_oe " de la OE " lotes_jugo.id_orden_entrega
                VIEW-AS ALERT-BOX.
    end.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_cus-update-only ( 1.24 , 49.00 ) NO-ERROR.
       RUN set-size IN h_cus-update-only ( 1.91 , 20.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 68.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 91.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_orden_fabricacion_todos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_orden_fabricacion_todo ).
       RUN set-position IN h_b_lotes_orden_fabricacion_todo ( 3.86 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b_lotes_orden_fabricacion_todo ( 6.71 , 142.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_lotes_jugo_items_oe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_lotes_jugo_items_oe ).
       RUN set-position IN h_v_lotes_jugo_items_oe ( 11.24 , 34.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 64.00 ) */

       /* Links to csmartbrowser h_b_lotes_orden_fabricacion_todo. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_lotes_orden_fabricacion_todo ).

       /* Links to SmartViewer h_v_lotes_jugo_items_oe. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_orden_fabricacion_todo , 'Record':U , h_v_lotes_jugo_items_oe ).
       RUN add-link IN adm-broker-hdl ( h_cus-update-only , 'TableIO':U , h_v_lotes_jugo_items_oe ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_orden_fabricacion_todo ,
             BUTTON-17:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_lotes_jugo_items_oe ,
             h_b_lotes_orden_fabricacion_todo , 'AFTER':U ).
    END. /* Page 0 */
