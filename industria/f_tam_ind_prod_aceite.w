...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 cantidad BUTTON-2 RECT-15 RECT-17 ~
RECT-2 
&Scoped-Define DISPLAYED-OBJECTS cantidad 
...
DEFINE VARIABLE h_b_tam_ind_prod_aceite AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_tam_ind_prod_aceite AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1.48 COL 99
     cantidad AT ROW 14.1 COL 107 COLON-ALIGNED
     BUTTON-2 AT ROW 14.1 COL 118
     RECT-15 AT ROW 1 COL 1
     RECT-17 AT ROW 13.38 COL 4
     RECT-2 AT Y 2 X 75
...
run get-rowid1 in h_b_tam_ind_prod_aceite (output r).
run zebra_produccion_aceite.p (input r).
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.33 , 16.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.33 , 50.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.33 , 73.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tam_ind_prod_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tam_ind_prod_aceite ).
       RUN set-position IN h_b_tam_ind_prod_aceite ( 3.95 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b_tam_ind_prod_aceite ( 9.29 , 126.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_tam_ind_prod_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_tam_ind_prod_aceite ).
       RUN set-position IN h_v_tam_ind_prod_aceite ( 13.86 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.10 , 69.00 ) */

       /* Links to csmartbrowser h_b_tam_ind_prod_aceite. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tam_ind_prod_aceite ).

       /* Links to SmartViewer h_v_tam_ind_prod_aceite. */
       RUN add-link IN adm-broker-hdl ( h_b_tam_ind_prod_aceite , 'Record':U , h_v_tam_ind_prod_aceite ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_tam_ind_prod_aceite ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tam_ind_prod_aceite ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_tam_ind_prod_aceite ,
             h_b_tam_ind_prod_aceite , 'AFTER':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE creacion-tambores F-Frame-Win 
PROCEDURE creacion-tambores :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
tambores = integer(cantidad:screen-value in frame F-Main).
run get-rowid1 in h_b_tam_ind_prod_aceite (output r).
find tam where rowid(tam) = r no-error.
if available tam then
    do:
        do ii = 1 to tambores:
            ...
            buffer-copy tam except id_tambor 
                                   id_etiqueta 
                                   nromov 
                                   c_usuario 
                                   c_fecha 
                                   c_hora to tambores_industria.
            assign tambores_industria.id_tambor     = tam.id_tambor + ii
            ...
            RUN y_gstkcre.p (input tam.id_empresa,
                             input tam.id_sucursal,
                             input tam.id_tipotambor,
                             input v_nromov,
                             INPUT tam.id_tambor + ii,
                             INPUT tam.id_tambor + ii,
    ...
    do:
        message "No se pueden crear los tambores porque no esta debidamente seleccionado el origen" view-as alert-box.
        
    end.

run dispatch in h_b_tam_ind_prod_aceite ('open-query':U).
...
------------------------------------------------------------------------------*/
  DISPLAY cantidad 
      WITH FRAME F-Main.
  ENABLE BUTTON-1 cantidad BUTTON-2 RECT-15 RECT-17 RECT-2 
      WITH FRAME F-Main.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-sucursal F-Frame-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE OUTPUT PARAMETER p_suc AS INTEGER.
DEFINE VAR hcon AS HANDLE.

RUN get-container (OUTPUT hcon).
RUN get-suc-ventana IN hcon (OUTPUT p_suc).
END PROCEDURE.
