...
DEFINE VARIABLE h_b_productos_terceros AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_productos_terceros AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     LABEL "Impreción de Etiquetas" 
     SIZE 30 BY 1.43.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1.48 COL 121
     RECT-15 AT ROW 1 COL 2
     RECT-16 AT ROW 10.52 COL 4
     RECT-2 AT Y 0 X 190
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Impreción de Etiquetas */
DO:
  define var r as rowid.
  run get-rowid1 in h_b_productos_terceros (output r).
  run zebra_productos_terceros.p (input r).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 39.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 73.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 96.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_productos_terceros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_productos_terceros ).
       RUN set-position IN h_b_productos_terceros ( 3.62 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b_productos_terceros ( 6.71 , 150.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_productos_terceros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_productos_terceros ).
       RUN set-position IN h_v_productos_terceros ( 10.76 , 8.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.29 , 140.00 ) */

       /* Links to csmartbrowser h_b_productos_terceros. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_productos_terceros ).

       /* Links to SmartViewer h_v_productos_terceros. */
       RUN add-link IN adm-broker-hdl ( h_b_productos_terceros , 'Record':U , h_v_productos_terceros ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_productos_terceros ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_productos_terceros ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_productos_terceros ,
             h_b_productos_terceros , 'AFTER':U ).
    END. /* Page 0 */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
define output parameter r-del as rowid.
run get-rowid1 in h_b_productos_terceros (output r-del).
END PROCEDURE.
...
lista_reportes = "Todos los Prod.Terceros,Recepcion de Productos de Terceros,Prod. Terceros x Proveedor,Exportacion Excell".
...
if lista_reportes = "" then
    ...
    do:
        ...
        if available productos_terceros then do:
            v_query = " productos_terceros.id_lote = " + string(productos_terceros.id_lote) +
                      " and productos_terceros.id_proveedor = " + string(productos_terceros.id_proveedor)  +
                      " and productos_terceros.fecha = date('" + string(productos_terceros.fecha) + "')" + 
                      " and productos_terceros.id_sucursal = " + string(productos_terceros.id_sucursal) +
                      " and productos_terceros.id_articulo = " + string(productos_terceros.id_articulo).
        end.
    end.
case cresult:
    when "Todos los Prod.Terceros" then do:
        run p_reportes.p (input "productos_terceros",
                          input "Reporte de todos los Productos de Terceros ingresados",
                          input "",
                          input "").
    end.
    when "Recepcion de Productos de Terceros" then do:
        run p_reportes.p (input "recepcion_prod_terceros",
                          input "Recepcion de Productos de Terceros",
                          input v_query,
                          input "").
    end.
    when "Prod. Terceros x Proveedor" then do:
        run wc_proveedores.w (output r_prov).
        find proveedores where rowid(proveedores) = r_prov no-lock no-error.
        if available proveedores then do:
        
            run p_reportes.p (input "productos_terceros",
                              input "Reporte de todos los Productos de Terceros de " + proveedores.nombre,
                              input " productos_terceros.id_proveedor = " + string(proveedores.id_proveedor),
                              input "").
        end.
    end.
