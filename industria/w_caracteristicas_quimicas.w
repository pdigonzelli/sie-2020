...
DEFINE VARIABLE h_b_caracteristicas_quimicas AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_caracteristicas_quimicas AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Caracteristicas Quimicas"
         HEIGHT             = 15.52
         WIDTH              = 135.6
         MAX-HEIGHT         = 19.14
         MAX-WIDTH          = 135.6
...
ON END-ERROR OF W-Win /* Caracteristicas Quimicas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Caracteristicas Quimicas */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 28.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 62.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 85.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_caracteristicas_quimicas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_caracteristicas_quimicas ).
       RUN set-position IN h_b_caracteristicas_quimicas ( 3.86 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_caracteristicas_quimicas ( 6.71 , 134.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_caracteristicas_quimicas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_caracteristicas_quimicas ).
       RUN set-position IN h_v_caracteristicas_quimicas ( 11.71 , 24.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.91 , 78.00 ) */

       /* Links to csmartbrowser h_b_caracteristicas_quimicas. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_caracteristicas_quimicas ).

       /* Links to SmartViewer h_v_caracteristicas_quimicas. */
       RUN add-link IN adm-broker-hdl ( h_b_caracteristicas_quimicas , 'Record':U , h_v_caracteristicas_quimicas ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_caracteristicas_quimicas ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_caracteristicas_quimicas ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_caracteristicas_quimicas ,
             h_b_caracteristicas_quimicas , 'AFTER':U ).
    END. /* Page 0 */
