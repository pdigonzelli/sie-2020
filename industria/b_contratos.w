...
&Scoped-define INTERNAL-TABLES contratos clientes tipos_contratos

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table contratos.anio ~
contratos.id_contrato contratos.orden_fabricacion ~
tipos_contratos.descripcion contratos.id_cliente clientes.razon_social ~
contratos.fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH contratos WHERE ~{&KEY-PHRASE} ~
      AND contratos.id_tipo_contrato < 100 ~
 AND ( if (userid("userdb") = "y_martinl") then contratos.id_tipo_contrato = 4 else true) NO-LOCK, ~
      EACH clientes OF contratos OUTER-JOIN NO-LOCK, ~
      EACH tipos_contratos OF contratos NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH contratos WHERE ~{&KEY-PHRASE} ~
      AND contratos.id_tipo_contrato < 100 ~
 AND ( if (userid("userdb") = "y_martinl") then contratos.id_tipo_contrato = 4 else true) NO-LOCK, ~
      EACH clientes OF contratos OUTER-JOIN NO-LOCK, ~
      EACH tipos_contratos OF contratos NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table contratos clientes tipos_contratos
&Scoped-define FIRST-TABLE-IN-QUERY-br_table contratos
&Scoped-define SECOND-TABLE-IN-QUERY-br_table clientes
&Scoped-define THIRD-TABLE-IN-QUERY-br_table tipos_contratos
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      contratos.anio FORMAT "9999":U
      contratos.id_contrato FORMAT "X(12)":U
      contratos.orden_fabricacion FORMAT ">>,>>>,>>9":U
      tipos_contratos.descripcion FORMAT "X(8)":U
      contratos.id_cliente FORMAT "->,>>>,>>9":U
      clientes.razon_social FORMAT "x(40)":U
      contratos.fecha FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.contratos,general.clientes OF general.contratos,industria.tipos_contratos OF general.contratos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", OUTER,"
     _OrdList          = "general.contratos.orden_fabricacion|yes"
     _Where[1]         = "general.contratos.id_tipo_contrato < 100
 AND ( if (userid(""userdb"") = ""y_martinl"") then general.contratos.id_tipo_contrato = 4 else true)"
     _FldNameList[1]   = general.contratos.anio
     _FldNameList[2]   = general.contratos.id_contrato
     _FldNameList[3]   = general.contratos.orden_fabricacion
     _FldNameList[4]   = general.tipos_contratos.descripcion
     _FldNameList[5]   = general.contratos.id_cliente
     _FldNameList[6]   > general.clientes.razon_social
"clientes.razon_social" ? "x(40)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   = general.contratos.fecha
...
define sub-menu Ordena 
  menu-item contratos-anio
label 'anio( contratos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=contratos.anio').
                  run set-attribute-list('titulo=Año').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by contratos.anio
...
label 'descripcion( tipos_contratos ) '
  ...
  menu-item contratos-id_cliente
label 'id_cliente( contratos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=contratos.id_cliente').
                  run set-attribute-list('titulo=Cliente').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by contratos.id_cliente
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item clientes-razon_social
label 'razon_social( clientes ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=clientes.razon_social').
                  run set-attribute-list('titulo=Razón social').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by clientes.razon_social
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item contratos-fecha
label 'fecha( contratos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=contratos.fecha').
                  run set-attribute-list('titulo=Fecha').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by contratos.fecha
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by contratos.anio
