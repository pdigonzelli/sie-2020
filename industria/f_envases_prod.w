...
&Scoped-Define ENABLED-OBJECTS FILL-IN-1 RECT-15 RECT-16 RECT-2 
...
DEFINE VARIABLE h_b_envases_prod AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_envases_prod AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "LOS ENVASES DE INDUSTRIA TIENEN QUE SER ENTRE 500 Y 600" 
     VIEW-AS FILL-IN 
     SIZE 70 BY 1
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 11.24 COL 23 COLON-ALIGNED NO-LABEL
     RECT-15 AT ROW 1 COL 1
     RECT-16 AT ROW 12.67 COL 3
     RECT-2 AT Y 2 X 115
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.33 , 24.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.33 , 58.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.33 , 81.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_envases_prod.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_envases_prod ).
       RUN set-position IN h_b_envases_prod ( 4.24 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b_envases_prod ( 6.71 , 129.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_envases_prod.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_envases_prod ).
       RUN set-position IN h_v_envases_prod ( 13.38 , 24.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.00 , 88.80 ) */

       /* Links to csmartbrowser h_b_envases_prod. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_envases_prod ).

       /* Links to SmartViewer h_v_envases_prod. */
       RUN add-link IN adm-broker-hdl ( h_b_envases_prod , 'Record':U , h_v_envases_prod ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_envases_prod ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_envases_prod ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_envases_prod ,
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-1 
      WITH FRAME F-Main.
  ENABLE FILL-IN-1 RECT-15 RECT-16 RECT-2 
      WITH FRAME F-Main.
