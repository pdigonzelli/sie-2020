...
&Scoped-define FIELDS-IN-QUERY-br_table tambores_industria.id_empresa ~
tambores_industria.id_etiqueta tambores_industria.id_tambor ~
tambores_industria.id_empresa_ubicacion ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tambores_industria WHERE ~{&KEY-PHRASE} NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      tambores_industria.id_sucursal_ubicacion
      tambores_industria.id_locacion_ubicacion
      tambores_industria.id_posicion_ubicacion FORMAT ">,>>9"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[2]   > general.tambores_industria.id_etiqueta
"id_etiqueta" ? ">,>>>,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[3]   > general.tambores_industria.id_tambor
"id_tambor" ? ">>>,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[4]   = general.tambores_industria.id_empresa_ubicacion
     ...
     _FldNameList[7]   > general.tambores_industria.id_posicion_ubicacion
"id_posicion_ubicacion" ? ">,>>9" "integer" ? ? ? ? ? ? no ?
...
define sub-menu Ordena 
  menu-item tambores_industria-id_empresa
label 'id_empresa( tambores_industria ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tambores_industria.id_empresa').
                  run set-attribute-list('titulo=Empresa').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tambores_industria.id_empresa
...
case p:
  WHEN 'tambores_industria.id_empresa' then  
  ...
  WHEN 'tambores_industria.id_empresa_ubicacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_empresa_ubicacion = integer(valor)  no-lock no-error .
  ...
  WHEN 'tambores_industria.id_posicion_ubicacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_posicion_ubicacion = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
