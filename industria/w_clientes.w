...
DEFINE VARIABLE h_b_clientes AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_clientes AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* clientes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* clientes */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 37.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 71.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 94.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_clientes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_clientes ).
       RUN set-position IN h_b_clientes ( 3.86 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_clientes ( 6.71 , 152.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_clientes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_clientes ).
       RUN set-position IN h_v_clientes ( 11.48 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.76 , 124.00 ) */

       /* Links to csmartbrowser h_b_clientes. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_clientes ).

       /* Links to SmartViewer h_v_clientes. */
       RUN add-link IN adm-broker-hdl ( h_b_clientes , 'Record':U , h_v_clientes ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_clientes ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_clientes ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_clientes ,
             h_b_clientes , 'AFTER':U ).
    END. /* Page 0 */
...
lista_reportes = "clientes Mercado Externo".
...
ELSE DO:
        ...
        case cresult:
            when "clientes Mercado Externo" then 
                do:
                    /************************************************************************************************************/
                    /********************ENVIO DE DATOS AL REPORT BUILDER********************************************************/
                    /************************************************************************************************************/
                    
                    v_filtro = "clientes.mercado = 1".
                    
                          RUN  aderb\_prntrb2(
                           "..\industria\reports.prl", /* RB-REPORT-LIBRARY */
                           "clientes_mercado_externo",                    /* RB-REPORT-NAME */
                           "",                             /* RB-DB-CONNECTION */
                           ...
                           "",                              /* RB-OUTPUT-FILE */
                            ...
                            0,                              /* RB-END-PAGE - zero */
                           no,                              /* RB-TEST-PATTERN */
                           "Reporte de clientes Mercado Externo",         /* RB-WINDOW-TITLE */
