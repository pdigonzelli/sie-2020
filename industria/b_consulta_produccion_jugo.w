...
&Scoped-define INTERNAL-TABLES produccion_jugo sucursales envases_prod ~
...
produccion_jugo.id_produccion productos_terminados.descripcion ~
produccion_jugo.Fecha produccion_jugo.litros produccion_jugo.cantidad ~
envases_prod.descripcion produccion_jugo.c_fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH produccion_jugo WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH sucursales OF produccion_jugo NO-LOCK, ~
      EACH envases_prod OF produccion_jugo NO-LOCK, ~
      EACH productos_terminados OF produccion_jugo NO-LOCK ~
    BY produccion_jugo.id_produccion DESCENDING.
&Scoped-define TABLES-IN-QUERY-br_table produccion_jugo sucursales ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      sucursales.nombre
      produccion_jugo.id_produccion FORMAT ">>,>>9"
      productos_terminados.descripcion
      produccion_jugo.Fecha
      produccion_jugo.litros FORMAT ">>>,>>9"
      produccion_jugo.cantidad FORMAT ">>>9"
      envases_prod.descripcion
      produccion_jugo.c_fecha COLUMN-LABEL "C_Fecha"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 147 BY 19.43
         BGCOLOR 17 FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.produccion_jugo,comercial.sucursales OF general.produccion_jugo,general.envases_prod OF general.produccion_jugo,general.productos_terminados OF general.produccion_jugo"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ",,,"
     _OrdList          = "general.produccion_jugo.id_produccion|no"
     _FldNameList[1]   = comercial.sucursales.nombre
     _FldNameList[2]   > general.produccion_jugo.id_produccion
"general.produccion_jugo.id_produccion" ? ">>,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[3]   = general.productos_terminados.descripcion
     _FldNameList[4]   = general.produccion_jugo.Fecha
     _FldNameList[5]   > general.produccion_jugo.litros
"general.produccion_jugo.litros" ? ">>>,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[6]   > general.produccion_jugo.cantidad
"general.produccion_jugo.cantidad" ? ">>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[7]   = general.envases_prod.descripcion
     _FldNameList[8]   > general.produccion_jugo.c_fecha
"general.produccion_jugo.c_fecha" "C_Fecha" ? "date" ? ? ? ? ? ? no ?
...
label 'Fecha( produccion_jugo ) '
  ...
  menu-item produccion_jugo-litros
label 'litros( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.litros').
                  run set-attribute-list('titulo=Litros').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.litros
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  ...
  WHEN 'produccion_jugo.litros' then  
