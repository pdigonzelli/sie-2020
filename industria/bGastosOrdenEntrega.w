...
"rowObject.id_gasto" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.Gasto
"rowObject.Gasto" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND (hCont:FILE-NAME MATCHES "*wOEIndustria.w") 
    THEN DO:
