...
&Scoped-define INTERNAL-TABLES r_productos_calidad productos_terminados ~
calidades
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_productos_calidad WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF r_productos_calidad NO-LOCK, ~
      EACH calidades OF r_productos_calidad NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table r_productos_calidad ~
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_productos_calidad,general.productos_terminados OF general.r_productos_calidad,general.calidades OF general.r_productos_calidad"
     ...
     _FldNameList[6]   = general.r_productos_calidad.coeficiente
...
label 'rnpa( r_productos_calidad ) '
  ...
  menu-item r_productos_calidad-coeficiente
label 'coeficiente( r_productos_calidad ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_productos_calidad.coeficiente').
                  run set-attribute-list('titulo=Coeficiente').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_productos_calidad.coeficiente
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by r_productos_calidad.id_articulo
...
case p:
  ...
  WHEN 'r_productos_calidad.coeficiente' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  coeficiente = decimal(valor)  no-lock no-error .
end.
END PROCEDURE.
