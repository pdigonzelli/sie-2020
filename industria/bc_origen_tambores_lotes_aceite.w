...
DEFINE VAR v_id_sucursal_ubicacion AS INTEGER.
...
tambores_industria.id_tambor tambores_industria.Fecha_cierre ~
tambores_industria.citral tambores_industria.kilos_tambor ~
tambores_industria.Fecha tambores_industria.id_sucursal_ubicacion ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH tambores_industria WHERE ~{&KEY-PHRASE} ~
 ...
 AND if v_sucursal = 0 then true else tambores_industria.id_sucursal = v_sucursal ~
AND (if v_art = 0 then true else tambores_industria.id_articulo = v_art) ~
AND (if v_lote = 0 then true else tambores_industria.id_lote = v_lote) ~
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else tambores_industria.id_tambor >= v_tambor_desde) ~
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else tambores_industria.id_tambor <= v_tambor_hasta) NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS sucursal art lote tambor_desde tambor_hasta ~
BUTTON-13 br_table 
&Scoped-Define DISPLAYED-OBJECTS sucursal art lote tambor_desde ~
tambor_hasta 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      tambores_industria.id_lote FORMAT ">>>>9":U WIDTH 7.8
      tambores_industria.Anio FORMAT ">>>9":U
      tambores_industria.id_tambor FORMAT ">>>>9":U WIDTH 10
      tambores_industria.Fecha_cierre FORMAT "99/99/99":U WIDTH 13.2
      tambores_industria.citral FORMAT ">>9.99":U WIDTH 9.2
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 139 BY 15.14
         FONT 0.
...
DEFINE FRAME F-Main
     sucursal AT ROW 1 COL 10 COLON-ALIGNED
     art AT ROW 1 COL 30 COLON-ALIGNED
     lote AT ROW 1 COL 50 COLON-ALIGNED
     tambor_desde AT ROW 1 COL 80 COLON-ALIGNED
     tambor_hasta AT ROW 1 COL 99 COLON-ALIGNED
     BUTTON-13 AT ROW 1 COL 119
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "general.tambores_industria.id_tipotambor = 6
 ...
 AND if v_sucursal = 0 then true else general.tambores_industria.id_sucursal = v_sucursal
AND (if v_art = 0 then true else general.tambores_industria.id_articulo = v_art)
AND (if v_lote = 0 then true else general.tambores_industria.id_lote = v_lote)
...
"general.tambores_industria.id_articulo" "Art" ">>9" "integer" ? ? ? ? ? ? no ? no no "4.4" yes no no "U" "" ""
...
"general.tambores_industria.id_lote" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no "7.8" yes no no "U" "" ""
     _FldNameList[5]   > general.tambores_industria.Anio
"general.tambores_industria.Anio" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > general.tambores_industria.id_tambor
"general.tambores_industria.id_tambor" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" ""
     _FldNameList[7]   > general.tambores_industria.Fecha_cierre
"general.tambores_industria.Fecha_cierre" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
     _FldNameList[8]   > general.tambores_industria.citral
"general.tambores_industria.citral" ? ? "decimal" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
...
"general.tambores_industria.kilos_tambor" ? ">>>9.99" "decimal" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[10]   > general.tambores_industria.Fecha
"general.tambores_industria.Fecha" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[11]   = general.tambores_industria.id_sucursal_ubicacion
     _FldNameList[12]   = general.tambores_industria.nromov
...
run get-articulos in hcontainer (output articulo, output fecha, OUTPUT v_id_sucursal_ubicacion).
