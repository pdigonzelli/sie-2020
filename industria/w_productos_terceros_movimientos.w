...
DEFINE VARIABLE h_b_prod_terceros AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_seleccion_locaciones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_prod_terceros AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "PRODUCTOS DE TERCEROS" 
     VIEW-AS FILL-IN 
     SIZE 127 BY .95
     BGCOLOR 2 FGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE FILL-IN-4 AS CHARACTER FORMAT "X(256)":U INITIAL "TAMBORES DE PRODUCTOS DE TERCEROS" 
     VIEW-AS FILL-IN 
     SIZE 127 BY .95
...
DEFINE FRAME F-Main
     ...
     FILL-IN-5 AT ROW 17.43 COL 4 NO-LABEL
     BUTTON-1 AT ROW 18.86 COL 92
     BUTTON-2 AT ROW 20.52 COL 92
...
ON END-ERROR OF W-Win /* PRODUCTOS DE TERCEROS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PRODUCTOS DE TERCEROS */
DO:
...
DO:
  ...
  run get-rowid1 in h_b_prod_terceros (output r).
  find productos_terceros where rowid(productos_terceros) = r no-lock no-error.
  if available productos_terceros then
    do:
        for each tambores_industria of productos_terceros.
            {i_mov_manuales.i}
        end.      
    end.                                              

run dispatch in h_b_tambores_prod_terceros ('open-query':U).
...
DO:
  ...
  run get-rowid1 in h_b_tambores_prod_terceros (output r).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_prod_terceros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_prod_terceros ).
       RUN set-position IN h_b_prod_terceros ( 2.43 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 122.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_prod_terceros.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_prod_terceros ).
       RUN set-position IN h_b_tambores_prod_terceros ( 10.52 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 127.00 ) */
       ...
       RUN set-position IN h_b_seleccion_locaciones ( 18.62 , 5.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 22.19 , 100.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_tambores_prod_terceros. */
       RUN add-link IN adm-broker-hdl ( h_b_prod_terceros , 'Record':U , h_b_tambores_prod_terceros ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_prod_terceros ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_prod_terceros ,
