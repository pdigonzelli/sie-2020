...
&Scoped-define INTERNAL-TABLES posiciones locaciones sucursales
...
posiciones.id_sucursal sucursales.nombre posiciones.id_locacion ~
locaciones.descripcion posiciones.id_posicion 
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH posiciones WHERE ~{&KEY-PHRASE} NO-LOCK, ~
    ...
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table posiciones locaciones sucursales
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      posiciones.id_empresa
      posiciones.id_sucursal FORMAT ">>9"
      sucursales.nombre
      posiciones.id_locacion FORMAT ">>9"
      locaciones.descripcion
      posiciones.id_posicion FORMAT ">>9"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 140 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.posiciones,industria.locaciones OF general.posiciones,comercial.sucursales OF general.posiciones"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = general.posiciones.id_empresa
     _FldNameList[2]   > general.posiciones.id_sucursal
"general.posiciones.id_sucursal" ? ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[3]   = comercial.sucursales.nombre
     _FldNameList[4]   > general.posiciones.id_locacion
"general.posiciones.id_locacion" ? ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[5]   = general.locaciones.descripcion
     _FldNameList[6]   > general.posiciones.id_posicion
"general.posiciones.id_posicion" ? ">>9" "integer" ? ? ? ? ? ? no ?
...
label 'id_empresa( posiciones ) '
  ...
  menu-item posiciones-id_sucursal
label 'id_sucursal( posiciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=posiciones.id_sucursal').
                  run set-attribute-list('titulo=Sucursal').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by posiciones.id_sucursal
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  ...
  WHEN 'posiciones.id_sucursal' then  
