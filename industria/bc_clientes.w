...
&Scoped-define FIELDS-IN-QUERY-br_table clientes.id_cliente clientes.nombre ~
clientes.razon_social clientes.domicilio 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH clientes WHERE ~{&KEY-PHRASE} NO-LOCK ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      clientes.id_cliente COLUMN-LABEL "Cod.Cli."
      clientes.nombre
      clientes.razon_social FORMAT "x(30)"
      clientes.domicilio
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.clientes"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.clientes.id_cliente
"id_cliente" "Cod.Cli." ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   = general.clientes.nombre
     _FldNameList[3]   > general.clientes.razon_social
"razon_social" ? "x(30)" "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   = general.clientes.domicilio
...
define sub-menu Ordena 
  menu-item clientes-id_cliente
label 'id_cliente( clientes ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=clientes.id_cliente').
                  run set-attribute-list('titulo=Cód.Cliente').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by clientes.id_cliente
...
label 'razon_social( clientes ) '
  ...
  menu-item clientes-razon_social-1
label 'razon_social-1( clientes ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=clientes.razon_social-1').
                  run set-attribute-list('titulo=Razón social').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by clientes.razon_social-1
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item clientes-domicilio
label 'domicilio( clientes ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=clientes.domicilio').
                  run set-attribute-list('titulo=Domicilio').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by clientes.domicilio
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by clientes.id_cliente
