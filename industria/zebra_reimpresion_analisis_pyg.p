...
DEFINE INPUT PARAMETER p_lote_jugo AS INTEGER.
DEFINE INPUT PARAMETER p_anio AS INTEGER.
DEFINE INPUT PARAMETER p_tambor_lote_jugo AS INTEGER.
DEFINE INPUT PARAMETER p_articulo AS INTEGER.
DEFINE INPUT PARAMETER p_sucursal_impresion AS INTEGER.
...
FIND FIRST lotes_jugo WHERE lotes_jugo.id_sucursal      = p_sucursal
                        AND lotes_jugo.id_lote          = p_lote_jugo
                        AND lotes_jugo.anio             = p_anio
                        AND lotes_jugo.id_articulo      = p_articulo
                        NO-LOCK no-error.
IF available lotes_jugo THEN DO:
    ...
    FOR each tambores_industria OF lotes_jugo
                                WHERE tambores_industria.id_tambor = p_tambor_lote_jugo
                                NO-LOCK.
/**********************************************************************************************************/
/*****************GENERO LA ETIQUETA NUMERO CERO***********************************************************/
        IF tambores_industria.id_tambor = 1 THEN DO:
            IF p_sucursal_impresion = 96 THEN output to c:\lpt1. 
            ELSE output to \\192.168.2.11\zebra.
