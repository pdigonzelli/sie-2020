...
DEFINE VARIABLE h_b_clausulas_gastos AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_clausulas_gastos AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relacion Clausulas - Gastos"
         HEIGHT             = 16.19
         WIDTH              = 89.6
         MAX-HEIGHT         = 19.14
         MAX-WIDTH          = 107
...
ON END-ERROR OF W-Win /* Relacion Clausulas - Gastos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Relacion Clausulas - Gastos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_clausulas_gastos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_clausulas_gastos ).
       RUN set-position IN h_b_clausulas_gastos ( 3.62 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 81.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_clausulas_gastos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_clausulas_gastos ).
       RUN set-position IN h_v_clausulas_gastos ( 11.48 , 8.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 67.00 ) */

       /* Links to csmartbrowser h_b_clausulas_gastos. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_clausulas_gastos ).

       /* Links to SmartViewer h_v_clausulas_gastos. */
       RUN add-link IN adm-broker-hdl ( h_b_clausulas_gastos , 'Record':U , h_v_clausulas_gastos ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_clausulas_gastos ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_clausulas_gastos ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_clausulas_gastos ,
             h_b_clausulas_gastos , 'AFTER':U ).
    END. /* Page 0 */
