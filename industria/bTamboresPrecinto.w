...
rowObject.id_tambor rowObject.nro_precinto rowObject.TipoPrecinto ~
rowObject.Articulo rowObject.Calidad rowObject.Envase ~
rowObject.SucursalLote rowObject.nromov 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table rowObject.nro_precinto 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      rowObject.nro_precinto FORMAT "X(50)":U
      rowObject.TipoPrecinto FORMAT "x(10)":U
...
"rowObject.Anio" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[3]   = _<SDO>.rowObject.id_tambor
     _FldNameList[4]   > _<SDO>.rowObject.nro_precinto
"rowObject.nro_precinto" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = _<SDO>.rowObject.TipoPrecinto
     _FldNameList[6]   = _<SDO>.rowObject.Articulo
     _FldNameList[7]   = _<SDO>.rowObject.Calidad
     _FldNameList[8]   = _<SDO>.rowObject.Envase
     _FldNameList[9]   = _<SDO>.rowObject.SucursalLote
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND (hCont:FILE-NAME MATCHES "*wControlPrecinto.w") THEN DO:
    ...
    ASSIGN rowObject.id_lote:BGCOLOR IN BROWSE br_table               = ibgcolor
           ...
           rowObject.nro_precinto:BGCOLOR IN BROWSE br_table          = ibgcolor
           ...
           rowObject.tipoprecinto:BGCOLOR IN BROWSE br_table          = ibgcolor.
  END.  
END.
