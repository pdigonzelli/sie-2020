...
produccion_jugo.anio produccion_jugo.id_turno produccion_jugo.Fecha ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH produccion_jugo WHERE ~{&KEY-PHRASE} ~
      AND produccion_jugo.id_tipotambor = 11 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
<FOREIGN-KEYS>
id_tipotambor|y|y|general.produccion_jugo.id_tipotambor
...
RUN set-attribute-list (
    'Keys-Accepted = "id_tipotambor",
     Keys-Supplied = "id_tipotambor,id_articulo"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      produccion_jugo.id_produccion COLUMN-LABEL "Prod." FORMAT ">>>>9":U
            WIDTH 8.2
      produccion_jugo.anio FORMAT ">>>9":U WIDTH 7.2
      produccion_jugo.id_turno FORMAT ">>>9":U
      produccion_jugo.Fecha FORMAT "99/99/99":U WIDTH 12.8
      produccion_jugo.desde_bolsa FORMAT ">>>>>9":U
      produccion_jugo.hasta_bolsa FORMAT ">>>>>9":U WIDTH 10.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 88 BY 6.71
         FONT 0.
...
"produccion_jugo.id_produccion" "Prod." ">>>>9" "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > general.produccion_jugo.anio
"produccion_jugo.anio" ? ? "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[3]   = general.produccion_jugo.id_turno
     _FldNameList[4]   > general.produccion_jugo.Fecha
"produccion_jugo.Fecha" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no "12.8" yes no no "U" "" ""
     _FldNameList[5]   = general.produccion_jugo.desde_bolsa
     _FldNameList[6]   > general.produccion_jugo.hasta_bolsa
"produccion_jugo.hasta_bolsa" ? ? "integer" ? ? ? ? ? ? no ? no no "10.4" yes no no "U" "" ""
...
label 'id_produccion( produccion_jugo ) '
  ...
  menu-item produccion_jugo-anio
label 'anio( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.anio').
                  run set-attribute-list('titulo=Año').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.anio
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item produccion_jugo-id_turno
label 'id_turno( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.id_turno').
                  run set-attribute-list('titulo=CodTurno').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.id_turno
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
label 'Fecha( produccion_jugo ) '
  ...
  menu-item produccion_jugo-desde_bolsa
label 'desde_bolsa( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.desde_bolsa').
                  run set-attribute-list('titulo=Desde bolsa').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.desde_bolsa
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item produccion_jugo-hasta_bolsa
label 'hasta_bolsa( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.hasta_bolsa').
                  run set-attribute-list('titulo=hasta').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.hasta_bolsa
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by produccion_jugo.id_produccion
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'id_tipotambor':U THEN DO:
       &Scope KEY-PHRASE produccion_jugo.id_tipotambor eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* id_tipotambor */
    OTHERWISE DO:
...
case p:
  ...
  WHEN 'produccion_jugo.anio' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  anio = integer(valor)  no-lock no-error .
  WHEN 'produccion_jugo.id_turno' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_turno = integer(valor)  no-lock no-error .
  WHEN 'produccion_jugo.Fecha' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  Fecha = date(valor)  no-lock no-error .
  WHEN 'produccion_jugo.desde_bolsa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  desde_bolsa = integer(valor)  no-lock no-error .
  WHEN 'produccion_jugo.hasta_bolsa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  hasta_bolsa = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
