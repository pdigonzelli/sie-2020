...
DEFINE VARIABLE h_bc_contratos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bc_items_contratos AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     IMAGE-UP FILE "c:/dlc83b/gui/adeicon/check.bmp":U
     LABEL "Button 1" 
     SIZE 23 BY 1.43.

DEFINE BUTTON BUTTON-2 
     IMAGE-UP FILE "c:/dlc83b/gui/adeicon/cross.bmp":U
     LABEL "Button 2" 
     SIZE 23 BY 1.43.
...
DEFINE FRAME F-Main
     ...
     FILL-IN-2 AT ROW 9.57 COL 2 COLON-ALIGNED NO-LABEL
     BUTTON-1 AT ROW 16.24 COL 88
     BUTTON-2 AT ROW 16.24 COL 116
...
DO:
  ...
  RUN get-rowid1 IN h_bc_envios_items (OUTPUT rresult).
  r = rresult.  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_contratos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_contratos ).
       RUN set-position IN h_bc_contratos ( 2.43 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.43 , 139.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_items_contratos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_items_contratos ).
       RUN set-position IN h_bc_items_contratos ( 11.00 , 8.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 60.00 ) */

       /* Links to csmartbrowser h_bc_items_contratos. */
       RUN add-link IN adm-broker-hdl ( h_bc_contratos , 'Record':U , h_bc_items_contratos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_contratos ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_items_contratos ,
