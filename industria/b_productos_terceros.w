...
&Scoped-define INTERNAL-TABLES productos_terceros productos_terminados ~
...
productos_terceros.fecha proveedores.nombre ~
productos_terminados.descripcion productos_terceros.id_lote ~
productos_terceros.anio_lote productos_terceros.estado ~
productos_terceros.kilos2 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH productos_terceros WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF productos_terceros NO-LOCK, ~
      EACH proveedores OF productos_terceros NO-LOCK, ~
      EACH sucursales OF productos_terceros NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH productos_terceros WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF productos_terceros NO-LOCK, ~
      EACH proveedores OF productos_terceros NO-LOCK, ~
      EACH sucursales OF productos_terceros NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table productos_terceros ~
productos_terminados proveedores sucursales
...
&Scoped-define THIRD-TABLE-IN-QUERY-br_table proveedores
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.productos_terceros,general.productos_terminados OF general.productos_terceros,comercial.proveedores OF general.productos_terceros,comercial.sucursales OF general.productos_terceros"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > comercial.sucursales.nombre
"sucursales.nombre" "Sucursal" "x(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.productos_terceros.fecha
"productos_terceros.fecha" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > comercial.proveedores.nombre
"proveedores.nombre" "Proveedor" "x(25)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.descripcion
"productos_terminados.descripcion" "Producto" "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.productos_terceros.id_lote
"productos_terceros.id_lote" ? ">>>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   = general.productos_terceros.anio_lote
     _FldNameList[7]   = general.productos_terceros.estado
     _FldNameList[8]   = general.productos_terceros.kilos2
...
label 'id_lote( productos_terceros ) '
  ...
  menu-item productos_terceros-cantidad
label 'cantidad( productos_terceros ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=productos_terceros.cantidad').
                  run set-attribute-list('titulo=Cantidad').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by productos_terceros.cantidad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by sucursales.nombre
...
case p:
  ...
  WHEN 'productos_terceros.cantidad' then  
