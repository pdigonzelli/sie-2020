...
DEFINE VARIABLE h_b_tipostambor AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_tipostambor AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tipostambor.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tipostambor ).
       RUN set-position IN h_b_tipostambor ( 3.95 , 9.00 ) NO-ERROR.
       RUN set-size IN h_b_tipostambor ( 6.71 , 66.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_tipostambor.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_tipostambor ).
       RUN set-position IN h_v_tipostambor ( 11.95 , 18.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.81 , 55.00 ) */

       /* Links to csmartbrowser h_b_tipostambor. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tipostambor ).

       /* Links to SmartViewer h_v_tipostambor. */
       RUN add-link IN adm-broker-hdl ( h_b_tipostambor , 'Record':U , h_v_tipostambor ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_tipostambor ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tipostambor ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_tipostambor ,
             h_b_tipostambor , 'AFTER':U ).
    END. /* Page 0 */
