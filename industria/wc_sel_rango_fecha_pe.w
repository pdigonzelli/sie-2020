...
&Scoped-Define ENABLED-OBJECTS fecha_desde fecha_hasta BUTTON-1 salir 
&Scoped-Define DISPLAYED-OBJECTS fecha_desde fecha_hasta 
...
DEFINE FRAME F-Main
     fecha_desde AT ROW 1.48 COL 23 COLON-ALIGNED
     fecha_hasta AT ROW 2.67 COL 23 COLON-ALIGNED
...
ON END-ERROR OF W-Win /* Eleccion Fecha */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Eleccion Fecha */
DO:
...
DO:
  DEFINE VAR v_fecha_desde AS CHAR.
  DEFINE VAR v_fecha_hasta AS CHAR.
  DEFINE VAR f_desde AS CHAR.
  DEFINE VAR f_hasta AS CHAR.
  define var v_filtro as char.
  define var RB-MEMO-FILE as char.
  
  v_fecha_desde    = fecha_desde:screen-value in frame F-Main.
  f_desde  = SUBSTRING(v_fecha_desde,4,2) + "/" + SUBSTRING(v_fecha_desde,1,2) + "/" + SUBSTRING(v_fecha_desde,7,2).
  v_fecha_hasta    = fecha_hasta:screen-value in frame F-Main.
  f_hasta  = SUBSTRING(v_fecha_hasta,4,2) + "/" + SUBSTRING(v_fecha_hasta,1,2) + "/" + SUBSTRING(v_fecha_hasta,7,2).
  
  IF f_desde = f_hasta AND f_desde = string(TODAY) THEN
  DO:
      v_filtro = "".
  END.
  ELSE
  DO:
      v_filtro = "permisos_embarque.fecha_envio_tuc >= " + STRING(f_desde) +
             " and permisos_embarque.fecha_envio_tuc <= " + STRING(f_hasta).
  ...
  "permiso_embarque",                    /* RB-REPORT-NAME */
  "",                             /* RB-DB-CONNECTION */
  ...
  0,                              /* RB-END-PAGE - zero */
  no,                              /* RB-TEST-PATTERN */
  "Reporte de Permisos de Embarque",         /* RB-WINDOW-TITLE */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE fecha_desde fecha_hasta BUTTON-1 salir 
      WITH FRAME F-Main IN WINDOW W-Win.
...
fecha_desde:screen-value in frame F-Main = STRING(TODAY)    .
fecha_hasta:screen-value in frame F-Main = STRING(TODAY)    .
