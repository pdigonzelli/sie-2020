...
&Scoped-define INTERNAL-TABLES r_muestras_protocolos protocolos ~
tambores_industria productos_terminados
...
protocolos.anio productos_terminados.descripcion_ingles ~
tambores_industria.id_lote tambores_industria.Anio ~
tambores_industria.id_orden_entrega tambores_industria.item_oe 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH r_muestras_protocolos OF items_muestras WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH protocolos WHERE protocolos.id_protocolo = r_muestras_protocolos.id_protocolo ~
  AND protocolos.anio = r_muestras_protocolos.anio ~
  AND protocolos.id_articulo = r_muestras_protocolos.id_articulo NO-LOCK, ~
  ...
  AND tambores_industria.nromov = protocolos.nromov OUTER-JOIN NO-LOCK, ~
    ...
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_muestras_protocolos OF items_muestras WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH protocolos WHERE protocolos.id_protocolo = r_muestras_protocolos.id_protocolo ~
  AND protocolos.anio = r_muestras_protocolos.anio ~
  AND protocolos.id_articulo = r_muestras_protocolos.id_articulo NO-LOCK, ~
      FIRST tambores_industria WHERE tambores_industria.id_empresa = protocolos.id_empresa ~
  ...
  AND tambores_industria.nromov = protocolos.nromov OUTER-JOIN NO-LOCK, ~
      EACH productos_terminados OF protocolos NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table r_muestras_protocolos protocolos ~
tambores_industria productos_terminados
&Scoped-define FIRST-TABLE-IN-QUERY-br_table r_muestras_protocolos
&Scoped-define SECOND-TABLE-IN-QUERY-br_table protocolos
&Scoped-define THIRD-TABLE-IN-QUERY-br_table tambores_industria
...
<FOREIGN-KEYS>
id_muestra||y|general.r_muestras_protocolos.id_muestra
id_articulo||y|general.r_muestras_protocolos.id_articulo
id_protocolo||y|general.r_muestras_protocolos.id_protocolo
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_muestra,id_articulo,id_protocolo"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      protocolos.id_protocolo COLUMN-LABEL "Protocolo" FORMAT ">>>>>9":U
            WIDTH 12.2
      protocolos.anio FORMAT "99":U WIDTH 6.2
      productos_terminados.descripcion_ingles COLUMN-LABEL "Producto" FORMAT "X(28)":U
      tambores_industria.id_lote FORMAT ">>>9":U WIDTH 9.2
      tambores_industria.Anio FORMAT "9999":U WIDTH 5.2
      tambores_industria.id_orden_entrega FORMAT ">>>>9":U WIDTH 9.2
      tambores_industria.item_oe COLUMN-LABEL "Parte" FORMAT ">>9":U
            WIDTH 10.8
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 99 BY 6.71
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_muestras_protocolos OF general.items_muestras,industria.protocolos WHERE general.r_muestras_protocolos ...,industria.tambores_industria WHERE general.protocolos ...,general.productos_terminados OF general.protocolos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",, FIRST OUTER,,"
     _JoinCode[2]      = "general.protocolos.id_protocolo = general.r_muestras_protocolos.id_protocolo
  AND general.protocolos.anio = general.r_muestras_protocolos.anio
  AND general.protocolos.id_articulo = general.r_muestras_protocolos.id_articulo"
     _JoinCode[3]      = "general.tambores_industria.id_empresa = general.protocolos.id_empresa
...
"protocolos.id_protocolo" "Protocolo" ">>>>>9" "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[2]   > general.protocolos.anio
"protocolos.anio" ? "99" "integer" ? ? ? ? ? ? no ? no no "6.2" yes no no "U" "" ""
     _FldNameList[3]   > general.productos_terminados.descripcion_ingles
"productos_terminados.descripcion_ingles" "Producto" "X(28)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.tambores_industria.id_lote
"tambores_industria.id_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[5]   > general.tambores_industria.Anio
"tambores_industria.Anio" ? "9999" "integer" ? ? ? ? ? ? no ? no no "5.2" yes no no "U" "" ""
     _FldNameList[6]   > general.tambores_industria.id_orden_entrega
"tambores_industria.id_orden_entrega" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[7]   > general.tambores_industria.item_oe
"tambores_industria.item_oe" "Parte" ">>9" "integer" ? ? ? ? ? ? no ? no no "10.8" yes no no "U" "" ""
...
label 'Anio( tambores_industria ) '
  ...
  menu-item tambores_industria-id_orden_entrega
label 'id_orden_entrega( tambores_industria ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tambores_industria.id_orden_entrega').
                  run set-attribute-list('titulo=OE').
                  run set-attribute-list('busca= no').
                  &SCOPE SORTBY-PHRASE  by tambores_industria.id_orden_entrega
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item tambores_industria-item_oe
label 'item_oe( tambores_industria ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tambores_industria.item_oe').
                  run set-attribute-list('titulo=Parte OE').
                  run set-attribute-list('busca= no').
                  &SCOPE SORTBY-PHRASE  by tambores_industria.item_oe
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by protocolos.id_protocolo
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_muestra" "r_muestras_protocolos" "id_muestra"}
  {src/adm/template/sndkycas.i "id_articulo" "r_muestras_protocolos" "id_articulo"}
  {src/adm/template/sndkycas.i "id_protocolo" "r_muestras_protocolos" "id_protocolo"}
