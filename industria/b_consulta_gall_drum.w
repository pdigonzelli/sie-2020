...
define temp-table tt_lotes
    field id_sucursal as integer
    field sucursal as char
    field id_lote as integer
    field anio_lote as integer
    field gal_drum as decimal
    field fecha as date
    field brix_corr as decimal
    index lote id_lote descending.
...
&Scoped-define INTERNAL-TABLES tt_lotes

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table tt_lotes.fecha tt_lotes.sucursal tt_lotes.id_lote tt_lotes.anio_lote tt_lotes.gal_drum tt_lotes.brix_corr   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH tt_lotes WHERE ~{&KEY-PHRASE} NO-LOCK     ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH tt_lotes WHERE ~{&KEY-PHRASE} NO-LOCK     ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tt_lotes
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tt_lotes
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      tt_lotes.fecha column-label "Fecha"
      tt_lotes.sucursal format "x(30)" column-label "Planta"
      tt_lotes.id_lote  format ">>>9" column-label  "Lote"
      tt_lotes.anio_lote    format ">>99" column-label ""
      tt_lotes.gal_drum format ">>9.99" column-label "Gall/Drum"
      tt_lotes.brix_corr format ">>9.99" column-label "Brix/Corr"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 77 BY 15.95.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt_lotes WHERE ~{&KEY-PHRASE} NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE calcular_datos B-table-Win 
PROCEDURE calcular_datos :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
for each lotes_jugo where year(lotes_jugo.fecha) >= (year(today) - 1) by fecha desc.
    find sucursales of lotes_jugo no-lock no-error.
    ...
    if available inspecciones_lote then
        do:
          ...
          if available brix then 
            do:
               gall = (lotes_jugo.peso_neto / brix.pe) / 3.785.
            end.
             
          create tt_lotes.
          assign tt_lotes.id_sucursal   = lotes_jugo.id_sucursal
                 tt_lotes.fecha         = lotes_jugo.fecha
                 tt_lotes.sucursal      = sucursales.abreviatura
                 tt_lotes.id_lote       = lotes_jugo.id_lote
                 tt_lotes.anio_lote     = lotes_jugo.anio
                 tt_lotes.gal_drum      = round(gall,2)
                 tt_lotes.brix_corr     = Inspecciones_lote.Bx_correg.
        end.
end.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tt_lotes"}
