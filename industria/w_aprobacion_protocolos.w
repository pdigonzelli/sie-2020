...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-25 
...
DEFINE VARIABLE h_b_aprobacion_protocolos AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_aprobacion_protocolos AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Aprobacion de Protocolos"
         HEIGHT             = 14.1
         WIDTH              = 105.4
         MAX-HEIGHT         = 19.14
         MAX-WIDTH          = 107
...
ON END-ERROR OF W-Win /* Aprobacion de Protocolos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Aprobacion de Protocolos */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_aprobacion_protocolos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_aprobacion_protocolos ).
       RUN set-position IN h_b_aprobacion_protocolos ( 3.86 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b_aprobacion_protocolos ( 6.71 , 98.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_aprobacion_protocolos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_aprobacion_protocolos ).
       RUN set-position IN h_v_aprobacion_protocolos ( 11.95 , 40.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 19.40 ) */

       /* Links to csmartbrowser h_b_aprobacion_protocolos. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_aprobacion_protocolos ).

       /* Links to SmartViewer h_v_aprobacion_protocolos. */
       RUN add-link IN adm-broker-hdl ( h_b_aprobacion_protocolos , 'Record':U , h_v_aprobacion_protocolos ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_aprobacion_protocolos ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_aprobacion_protocolos ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_aprobacion_protocolos ,
             h_b_aprobacion_protocolos , 'AFTER':U ).
    END. /* Page 0 */
