...
&Scoped-define INTERNAL-TABLES protocolos
...
protocolos.anio protocolos.contramarca protocolos.aprobado ~
protocolos.quimico protocolos.fecha_aprobacion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH protocolos WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table protocolos
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_protocolo,id_tipotambor"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      protocolos.id_protocolo COLUMN-LABEL "Protocolo" FORMAT "->>>>>9":U
            WIDTH 12.2
      protocolos.anio FORMAT ">>>9":U WIDTH 8.2
      protocolos.contramarca FORMAT "X(15)":U WIDTH 16.2
      protocolos.aprobado COLUMN-LABEL "Aprob" FORMAT "si/no":U
            WIDTH 8.2
      protocolos.quimico FORMAT "x(12)":U
      protocolos.fecha_aprobacion FORMAT "99/99/99":U WIDTH 25.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 98 BY 6.71
         BGCOLOR 15 FGCOLOR 0 FONT 0 ROW-HEIGHT-CHARS .62.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.protocolos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.protocolos.id_protocolo
"id_protocolo" "Protocolo" ? "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[2]   > general.protocolos.anio
"anio" ? ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[3]   > general.protocolos.contramarca
"contramarca" ? "X(15)" "character" ? ? ? ? ? ? no ? no no "16.2" yes no no "U" "" ""
     _FldNameList[4]   > general.protocolos.aprobado
"aprobado" "Aprob" ? "logical" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[5]   = general.protocolos.quimico
     _FldNameList[6]   > general.protocolos.fecha_aprobacion
"fecha_aprobacion" ? ? "date" ? ? ? ? ? ? no ? no no "25.4" yes no no "U" "" ""
...
label 'aprobado( protocolos ) '
  ...
  menu-item protocolos-quimico
label 'quimico( protocolos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=protocolos.quimico').
                  run set-attribute-list('titulo=Quimico').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by protocolos.quimico
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  ...
  WHEN 'protocolos.contramarca' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  contramarca begins valor  no-lock no-error .
  WHEN 'protocolos.aprobado' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  aprobado no-lock no-error .
  WHEN 'protocolos.quimico' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  quimico begins valor  no-lock no-error .
