...
&Scoped-define INTERNAL-TABLES lotes_aceite sucursales productos_terminados ~
tambores_industria
...
lotes_aceite.id_lote lotes_aceite.fecha productos_terminados.descripcion ~
lotes_aceite.citral 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_aceite WHERE ~{&KEY-PHRASE} ~
      AND lotes_aceite.id_articulo = v_articulo or ~
(lotes_aceite.id_articulo > (v_articulo * 10) and ~
 lotes_aceite.id_articulo < ((v_articulo + 1) * 10)) NO-LOCK, ~
      EACH sucursales OF lotes_aceite NO-LOCK, ~
      EACH productos_terminados OF lotes_aceite NO-LOCK, ~
      FIRST tambores_industria OF lotes_aceite ~
      WHERE tambores_industria.id_locacion_ubicacion = 4 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table lotes_aceite sucursales ~
productos_terminados tambores_industria
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      lotes_aceite.id_lote FORMAT ">>>9":U WIDTH 7.4
      lotes_aceite.fecha FORMAT "99/99/9999":U
      productos_terminados.descripcion COLUMN-LABEL "Producto" FORMAT "x(30)":U
      lotes_aceite.citral FORMAT ">9.99":U WIDTH 8.6
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 128 BY 13.81
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_aceite,comercial.sucursales OF general.lotes_aceite,general.productos_terminados OF general.lotes_aceite,industria.tambores_industria OF general.lotes_aceite"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,, FIRST"
     _OrdList          = "general.lotes_aceite.fecha|no,industria.lotes_aceite.id_lote|no"
     _Where[1]         = "general.lotes_aceite.id_articulo = v_articulo or
(lotes_aceite.id_articulo > (v_articulo * 10) and
 lotes_aceite.id_articulo < ((v_articulo + 1) * 10))"
...
"comercial.sucursales.nombre" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.lotes_aceite.id_lote
"general.lotes_aceite.id_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "7.4" yes no no "U" "" ""
     _FldNameList[3]   > general.lotes_aceite.fecha
"general.lotes_aceite.fecha" ? "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
"general.lotes_aceite.citral" ? ">9.99" "decimal" ? ? ? ? ? ? no ? no no "8.6" yes no no "U" "" ""
...
case p:
  WHEN 'lotes_aceite.id_lote' then  
