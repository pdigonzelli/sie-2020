...
&Scoped-define INTERNAL-TABLES items_muestras envases_muestras ~
estados_muestra
...
&Scoped-define FIELDS-IN-QUERY-br_table items_muestras.item_muestra ~
envases_muestras.descripcion items_muestras.cantidad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH items_muestras WHERE items_muestras.id_muestra = muestras.id_muestra ~
  AND items_muestras.anio_muestra = muestras.anio_muestra NO-LOCK, ~
      EACH envases_muestras OF items_muestras NO-LOCK, ~
      EACH estados_muestra WHERE estados_muestra.id_estado_muestra = items_muestras.id_estado_lote_enviado NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH items_muestras WHERE items_muestras.id_muestra = muestras.id_muestra ~
  AND items_muestras.anio_muestra = muestras.anio_muestra NO-LOCK, ~
      EACH envases_muestras OF items_muestras NO-LOCK, ~
      EACH estados_muestra WHERE estados_muestra.id_estado_muestra = items_muestras.id_estado_lote_enviado NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table items_muestras envases_muestras ~
estados_muestra
&Scoped-define FIRST-TABLE-IN-QUERY-br_table items_muestras
&Scoped-define SECOND-TABLE-IN-QUERY-br_table envases_muestras
&Scoped-define THIRD-TABLE-IN-QUERY-br_table estados_muestra
...
id_muestra||y|general.items_muestras.id_muestra
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_envase,id_muestra"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      items_muestras.item_muestra FORMAT ">>9":U WIDTH 7.2
      envases_muestras.descripcion COLUMN-LABEL "Envase" FORMAT "X(20)":U
      items_muestras.cantidad FORMAT ">>9":U WIDTH 11.6
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.items_muestras WHERE general.muestras <external> ...,industria.envases_muestras OF general.items_muestras,industria.estados_muestra WHERE general.items_muestras ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "general.items_muestras.id_muestra = general.muestras.id_muestra
  AND general.items_muestras.anio_muestra = general.muestras.anio_muestra"
     _JoinCode[3]      = "general.estados_muestra.id_estado_muestra = general.items_muestras.id_estado_lote_enviado"
     _FldNameList[1]   > general.items_muestras.item_muestra
"items_muestras.item_muestra" ? ? "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[2]   > general.envases_muestras.descripcion
"envases_muestras.descripcion" "Envase" "X(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.items_muestras.cantidad
"items_muestras.cantidad" ? ? "integer" ? ? ? ? ? ? no ? no no "11.6" yes no no "U" "" ""
...
case p:
  WHEN 'items_muestras.item_muestra' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  item_muestra = integer(valor)  no-lock no-error .
  WHEN 'items_muestras.caracteristicas' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  caracteristicas begins valor  no-lock no-error .
  WHEN 'items_muestras.cantidad' then  
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_muestra" "items_muestras" "id_muestra"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "estados_muestra"}
