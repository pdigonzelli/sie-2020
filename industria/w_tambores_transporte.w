...
DEFINE VARIABLE h_b_tambores_transporte AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_transporte.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_transporte ).
       RUN set-position IN h_b_tambores_transporte ( 4.81 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_tambores_transporte ( 6.71 , 152.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_tambores_transporte. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tambores_transporte ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             h_cus-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_transporte ,
             h_cus-misc , 'AFTER':U ).
    END. /* Page 0 */
