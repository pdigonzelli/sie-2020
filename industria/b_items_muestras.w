...
envases_muestras.abreviatura items_muestras.cantidad ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH items_muestras WHERE items_muestras.id_muestra = muestras.id_muestra ~
  AND items_muestras.anio_muestra = muestras.anio_muestra NO-LOCK, ~
      EACH envases_muestras OF items_muestras OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      items_muestras.item_muestra FORMAT ">>9":U WIDTH 8.2
      envases_muestras.abreviatura COLUMN-LABEL "Envase" FORMAT "X(12)":U
            WIDTH 16.2
      items_muestras.cantidad FORMAT ">>9":U WIDTH 12.2
      items_muestras.fecha_enviado_tuc COLUMN-LABEL "F.EnvioTuc" FORMAT "99/99/99":U
            WIDTH 14.2
      items_muestras.fecha_enviado_bue COLUMN-LABEL "F.EnvioBue" FORMAT "99/99/99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 76 BY 5.24
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.items_muestras WHERE general.muestras <external> ...,industria.envases_muestras OF general.items_muestras"
...
"items_muestras.item_muestra" ? ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > general.envases_muestras.abreviatura
"envases_muestras.abreviatura" "Envase" ? "character" ? ? ? ? ? ? no ? no no "16.2" yes no no "U" "" ""
     _FldNameList[3]   > general.items_muestras.cantidad
"items_muestras.cantidad" ? ? "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[4]   > general.items_muestras.fecha_enviado_tuc
"items_muestras.fecha_enviado_tuc" "F.EnvioTuc" ? "date" ? ? ? ? ? ? no ? no no "14.2" yes no no "U" "" ""
     _FldNameList[5]   > general.items_muestras.fecha_enviado_bue
"items_muestras.fecha_enviado_bue" "F.EnvioBue" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
