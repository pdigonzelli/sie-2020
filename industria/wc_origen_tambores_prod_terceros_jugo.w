...
DEFINE VARIABLE h_bc_origen_tambores_prod_tercer AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN get-rowid1 in h_bc_origen_tambores_prod_tercer (output vr).
  ...
  CASE choice:
    WHEN TRUE THEN /* Yes */ DO:
        FIND FIRST productos_terceros where rowid(productos_terceros) = vr no-lock no-error.
        IF AVAILABLE productos_terceros THEN DO:
            DO i = 1 TO v_tambores:
                FIND FIRST tambores_industria where tambores_industria.id_empresa    = productos_terceros.id_empresa
                ...
                END.
                ELSE  MESSAGE "Se ha producido un erro con la eleccion del origen. Avisar a sistemas"
                        VIEW-AS ALERT-BOX.  
                
                APPLY "window-close" TO h_bc_origen_tambores_prod_tercer. 
            END.
        END.
    END.      
  END CASE.
...
RUN get-rowid1 IN h_bc_origen_tambores_prod_tercer (output p_r).
FIND FIRST productos_terceros WHERE ROWID(productos_terceros) = p_r NO-LOCK NO-ERROR.
IF AVAILABLE productos_terceros THEN DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_origen_tambores_prod_terceros_jugo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_origen_tambores_prod_tercer ).
       RUN set-position IN h_bc_origen_tambores_prod_tercer ( 2.43 , 7.00 ) NO-ERROR.
       RUN set-size IN h_bc_origen_tambores_prod_tercer ( 15.14 , 140.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_origen_tambores_prod_tercer ,
