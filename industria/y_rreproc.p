/********************************************/
/* REPORTE DE MOVIMIENTOS DE STOCK  X TIPO  */
/* AUTOR: DANIEL REYNA                      */
/********************************************/
  ...
  */                       
   if not verror then do:
    ...
    run auditoria       (input auxi.id_empresa,
                         ...
                         output vcontrol,
                         output verror).
    find tipostambor where tipostambor.id_tipotambor = auxi.id_tipotambor
    no-lock no-error.
/*    find productos_terminados where
        productos_terminados.id_articulo = auxi.articulo
        no-lock no-error. */
            
    accum  auxi.cant_tmb (total by auxi.id_articulo).
       
    display auxi.id_articulo
            productos_terminados.descripcion format "x(29)"
            auxi.id_tipotambor column-label "T." format "9"
            tipostambor.descripcion format "x(20)"
            auxi.id_lote
            auxi.anio
            auxi.cant_tmb column-label "Cant.Tamb."  
