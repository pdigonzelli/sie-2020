...
&Scoped-Define ENABLED-OBJECTS RECT-15 RECT-2 RECT-16 
...
DEFINE VARIABLE h_b_locaciones AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_locaciones AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.33 , 32.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.33 , 66.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.33 , 89.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_locaciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_locaciones ).
       RUN set-position IN h_b_locaciones ( 4.52 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 136.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_locaciones.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_locaciones ).
       RUN set-position IN h_v_locaciones ( 11.95 , 41.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 64.00 ) */

       /* Links to csmartbrowser h_b_locaciones. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_locaciones ).

       /* Links to SmartViewer h_v_locaciones. */
       RUN add-link IN adm-broker-hdl ( h_b_locaciones , 'Record':U , h_v_locaciones ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_locaciones ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_locaciones ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_locaciones ,
             h_b_locaciones , 'AFTER':U ).
    END. /* Page 0 */
