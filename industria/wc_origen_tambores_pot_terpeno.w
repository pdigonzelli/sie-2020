...
DEFINE VARIABLE h_bc_origen_tambores_pot_terpeno AS HANDLE NO-UNDO.
...
DO:
  ...
  RUN get-seleccion IN h_bc_origen_tambores_pot_terpeno (OUTPUT elegidos, 
                                                         OUTPUT num).
  ...
  CASE choice:
    WHEN TRUE THEN /* Yes */ DO:
        DO i = 1 TO num:
            ...
            END.
            ELSE  MESSAGE "Hay problemas con la eleccion del origen de lote.Avisar a sistemas"
                            VIEW-AS ALERT-BOX.  
            APPLY "window-close" TO h_bc_origen_tambores_pot_terpeno. 
        END.      
    END.
END case.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_origen_tambores_pot_terpeno.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_origen_tambores_pot_terpeno ).
       RUN set-position IN h_bc_origen_tambores_pot_terpeno ( 2.67 , 3.00 ) NO-ERROR.
       RUN set-size IN h_bc_origen_tambores_pot_terpeno ( 16.33 , 155.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_origen_tambores_pot_terpeno ,
...
do:
    if lotes_aceite.id_articulo = 571 then
        articulo = 571.
    else
        do:
            if lotes_aceite.id_articulo = 581 then
                articulo = 581.
            else
                articulo = 511.
        end.
