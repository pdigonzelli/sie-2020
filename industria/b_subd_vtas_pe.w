...
subd_vtas.nro_comp subd_vtas.proforma subd_vtas.nro_proforma ~
subd_vtas.vencimiento 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH r_subd_ventas_embarque WHERE r_subd_ventas_embarque.anio_permiso = permisos_embarque.anio ~
  ...
  AND r_subd_ventas_embarque.nro_embarque = permisos_embarque.id_permiso_embarque NO-LOCK, ~
    ...
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_subd_ventas_embarque WHERE r_subd_ventas_embarque.anio_permiso = permisos_embarque.anio ~
  AND r_subd_ventas_embarque.id_aduana = permisos_embarque.id_aduana ~
  AND r_subd_ventas_embarque.nro_embarque = permisos_embarque.id_permiso_embarque NO-LOCK, ~
      EACH subd_vtas OF r_subd_ventas_embarque NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      subd_vtas.id_punto_venta COLUMN-LABEL "PtoVta" FORMAT ">>>9":U
            WIDTH 8.2 COLUMN-FONT 0 LABEL-FONT 0
      subd_vtas.nro_comp COLUMN-LABEL "NroComp" FORMAT ">>>>>>>9":U
            WIDTH 15.2 COLUMN-FONT 0 LABEL-FONT 0
      subd_vtas.proforma COLUMN-LABEL "Prof" FORMAT "si/no":U WIDTH 5.2
      subd_vtas.nro_proforma COLUMN-LABEL "N.Prof" FORMAT ">>>>":U
            WIDTH 9.2
      subd_vtas.vencimiento COLUMN-LABEL "FechaVenc." FORMAT "99/99/99":U
            WIDTH 13 COLUMN-FONT 0 LABEL-FONT 0
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 63 BY 7.62
         BGCOLOR 15 FGCOLOR 2 FONT 0 ROW-HEIGHT-CHARS .67.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ventas.r_subd_ventas_embarque WHERE general.permisos_embarque <external> ...,ventas.subd_vtas OF ventas.r_subd_ventas_embarque"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "ventas.r_subd_ventas_embarque.anio_permiso = general.permisos_embarque.anio
...
"subd_vtas.id_punto_venta" "PtoVta" ? "integer" ? ? 0 ? ? 0 no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > ventas.subd_vtas.nro_comp
"subd_vtas.nro_comp" "NroComp" ? "integer" ? ? 0 ? ? 0 no ? no no "15.2" yes no no "U" "" ""
     _FldNameList[3]   > ventas.subd_vtas.proforma
"subd_vtas.proforma" "Prof" "si/no" "logical" ? ? ? ? ? ? no ? no no "5.2" yes no no "U" "" ""
     _FldNameList[4]   > ventas.subd_vtas.nro_proforma
"subd_vtas.nro_proforma" "N.Prof" ">>>>" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[5]   > ventas.subd_vtas.vencimiento
"subd_vtas.vencimiento" "FechaVenc." "99/99/99" "date" ? ? 0 ? ? 0 no ? no no "13" yes no no "U" "" ""
