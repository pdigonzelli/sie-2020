...
DEFINE VARIABLE h_b_movimientos_industria AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_movimientos1 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.43 , 17.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.43 , 51.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.43 , 74.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_movimientos_industria.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_movimientos_industria ).
       RUN set-position IN h_b_movimientos_industria ( 4.33 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 84.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_movimientos1.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_movimientos1 ).
       RUN set-position IN h_v_movimientos1 ( 12.14 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.57 , 109.00 ) */

       /* Links to csmartbrowser h_b_movimientos_industria. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_movimientos_industria ).

       /* Links to SmartViewer h_v_movimientos1. */
       RUN add-link IN adm-broker-hdl ( h_b_movimientos_industria , 'Record':U , h_v_movimientos1 ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_movimientos1 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_movimientos_industria ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_movimientos1 ,
             h_b_movimientos_industria , 'AFTER':U ).
    END. /* Page 0 */
