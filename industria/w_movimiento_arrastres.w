...
DEFINE VARIABLE FILL-IN-4 AS CHARACTER FORMAT "X(256)":U INITIAL "TAMBORES DE ARRASTRE" 
     VIEW-AS FILL-IN 
     SIZE 152.6 BY .95
...
DEFINE FRAME F-Main
     ...
     BUTTON-1 AT ROW 20.76 COL 90
     BUTTON-2 AT ROW 20.76 COL 122
...
ON END-ERROR OF W-Win /* ARRASTRE */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ARRASTRE */
DO:
...
DO:
  ...
  if available lotes_jugo then
    do:
        find arrastre_lote of lotes_jugo no-lock no-error.
...
run dispatch in h_b_tambores_arrastre ('open-query':U).      
...
DO:
  ...
  run tercera_tabla in h_b_tambores_arrastre (output r).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-position IN h_b_tambores_arrastre ( 11.29 , 1.80 ) NO-ERROR.
       /* Size in UIB:  ( 7.14 , 153.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 18.62 , 126.00 ) NO-ERROR.
