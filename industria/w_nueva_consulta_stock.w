...
&Scoped-Define ENABLED-OBJECTS RECT-2 sucursal BUTTON-1 
&Scoped-Define DISPLAYED-OBJECTS sucursal 
...
DEFINE VARIABLE h_b_nueva_consulta_stock AS HANDLE NO-UNDO.
...
DEFINE VARIABLE sucursal AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "Planta (95 Famailla - 96 Lavalle)" 
     VIEW-AS FILL-IN 
     SIZE 8 BY 1 NO-UNDO.
...
DEFINE FRAME F-Main
     sucursal AT ROW 1.71 COL 55 COLON-ALIGNED
     BUTTON-1 AT ROW 1.71 COL 72
     RECT-2 AT Y 0 X 655
...
DO:
  run cargar_datos in h_b_nueva_consulta_stock.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 133.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_nueva_consulta_stock.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_nueva_consulta_stock ).
       RUN set-position IN h_b_nueva_consulta_stock ( 3.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 128.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             sucursal:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_nueva_consulta_stock ,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dame_sucursal W-Win 
PROCEDURE dame_sucursal :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
p_suc = integer(sucursal:screen-value in frame F-Main).
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-2 sucursal BUTTON-1 
      WITH FRAME F-Main IN WINDOW W-Win.
