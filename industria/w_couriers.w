...
DEFINE VARIABLE h_b_couriers AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_couriers AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Couriers */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Couriers */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_couriers.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_couriers ).
       RUN set-position IN h_b_couriers ( 3.86 , 7.00 ) NO-ERROR.
       RUN set-size IN h_b_couriers ( 6.71 , 66.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_couriers.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_couriers ).
       RUN set-position IN h_v_couriers ( 11.24 , 16.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 47.00 ) */

       /* Links to csmartbrowser h_b_couriers. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_couriers ).

       /* Links to SmartViewer h_v_couriers. */
       RUN add-link IN adm-broker-hdl ( h_b_couriers , 'Record':U , h_v_couriers ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_couriers ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_couriers ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_couriers ,
             h_b_couriers , 'AFTER':U ).
    END. /* Page 0 */
