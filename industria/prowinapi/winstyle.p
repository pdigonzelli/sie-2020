...
PROCEDURE AddHelpButton :
  ...
  run FlashWindow in hpApi (hNonClient,0, output ReturnValue).
...
PROCEDURE AddPaletteStyle :
  ...
  run ShowWindow in hpApi(hNonClient, 0, output ReturnValue). /* 0 = SW_HIDE */
...
PROCEDURE NoSizingMenu :
  ...
  run DeleteMenu in hpApi (hSysmenu, {&SC_SIZE}    , {&MF_BYCOMMAND}, output ReturnValue).
  run DeleteMenu in hpApi (hSysmenu, {&SC_MINIMIZE}, {&MF_BYCOMMAND}, output ReturnValue).
  run DeleteMenu in hpApi (hSysmenu, {&SC_MAXIMIZE}, {&MF_BYCOMMAND}, output ReturnValue).
END PROCEDURE.
...
PROCEDURE FitFrame :
  ...
  run GetClientRect in hpApi(hNonClient, 
