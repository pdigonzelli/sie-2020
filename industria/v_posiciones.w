...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES posiciones
&Scoped-define FIRST-EXTERNAL-TABLE posiciones


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR posiciones.
/* Standard List Definitions                                            */
&Scoped-Define ENABLED-FIELDS posiciones.id_empresa posiciones.id_sucursal ~
posiciones.id_locacion posiciones.id_posicion posiciones.descripcion 
...
&Scoped-define ENABLED-TABLES posiciones
&Scoped-define FIRST-ENABLED-TABLE posiciones
&Scoped-Define DISPLAYED-FIELDS posiciones.id_empresa ~
posiciones.id_sucursal posiciones.id_locacion posiciones.id_posicion ~
posiciones.descripcion 
&Scoped-Define DISPLAYED-OBJECTS fi-empresas-razon_social ~
fi-sucursales-nombre fi-locaciones-descripcion 

/* Custom List Definitions                                              */
/* ADM-CREATE-FIELDS,ADM-ASSIGN-FIELDS,List-3,List-4,List-5,List-6      */
&Scoped-define ADM-CREATE-FIELDS posiciones.id_empresa ~
posiciones.id_sucursal posiciones.id_locacion posiciones.id_posicion 
...
<FOREIGN-KEYS>
id_sucursal||y|general.posiciones.id_sucursal
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
DEFINE FRAME F-Main
     posiciones.id_empresa AT ROW 1 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 4.8 BY 1
     ...
     posiciones.id_sucursal AT ROW 2 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
     posiciones.id_locacion AT ROW 3 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
     fi-locaciones-descripcion AT ROW 3 COL 31 COLON-ALIGNED NO-LABEL
     posiciones.id_posicion AT ROW 4 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
     posiciones.descripcion AT ROW 5 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 32 BY 1
...
/* Settings for THIS-PROCEDURE
   Type: SmartViewer
   External Tables: general.posiciones
...
/* SETTINGS FOR FILL-IN fi-sucursales-nombre IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN posiciones.id_empresa IN FRAME F-Main
   1                                                                    */
/* SETTINGS FOR FILL-IN posiciones.id_locacion IN FRAME F-Main
   1                                                                    */
/* SETTINGS FOR FILL-IN posiciones.id_posicion IN FRAME F-Main
   1                                                                    */
/* SETTINGS FOR FILL-IN posiciones.id_sucursal IN FRAME F-Main
   1                                                                    */
...
general.posiciones.id_sucursal;wc_sucursales.w;sucursales.nombre;;
general.posiciones.id_empresa;wc_empreas.w;empresas.razon_social;;
general.posiciones.id_locacion;wc_locaciones.w;locaciones.descripcion;;
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME posiciones.descripcion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.descripcion V-table-Win
ON LEAVE OF posiciones.descripcion IN FRAME F-Main /* Descripción */
DO:
...
&Scoped-define SELF-NAME fi-locaciones-descripcion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-locaciones-descripcion V-table-Win
ON LEAVE OF fi-locaciones-descripcion IN FRAME F-Main
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME posiciones.id_empresa
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_empresa V-table-Win
ON GO OF posiciones.id_empresa IN FRAME F-Main /* Empresa */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_empresa V-table-Win
ON LEAVE OF posiciones.id_empresa IN FRAME F-Main /* Empresa */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_empresa V-table-Win
ON MOUSE-SELECT-DBLCLICK OF posiciones.id_empresa IN FRAME F-Main /* Empresa */
do: 
define var r as rowid no-undo.
run wc_empreas.w(output r).
find empresas where rowid(empresas) = r no-lock no-error.
if available empresas then 
general.posiciones.id_empresa:screen-value = string(empresas.id_empresa).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_empresa V-table-Win
ON U1 OF posiciones.id_empresa IN FRAME F-Main /* Empresa */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&Scoped-define SELF-NAME posiciones.id_locacion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_locacion V-table-Win
ON GO OF posiciones.id_locacion IN FRAME F-Main /* Locacion */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_locacion V-table-Win
ON LEAVE OF posiciones.id_locacion IN FRAME F-Main /* Locacion */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_locacion V-table-Win
ON MOUSE-SELECT-DBLCLICK OF posiciones.id_locacion IN FRAME F-Main /* Locacion */
do: 
define var r as rowid no-undo.
run wc_locaciones.w(output r).
find locaciones where rowid(locaciones) = r no-lock no-error.
if available locaciones then 
general.posiciones.id_locacion:screen-value = string(locaciones.id_locacion).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_locacion V-table-Win
ON U1 OF posiciones.id_locacion IN FRAME F-Main /* Locacion */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&Scoped-define SELF-NAME posiciones.id_posicion
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_posicion V-table-Win
ON LEAVE OF posiciones.id_posicion IN FRAME F-Main /* Posicion */
DO:
{custom/support/validacion.i}
END.
...
&Scoped-define SELF-NAME posiciones.id_sucursal
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_sucursal V-table-Win
ON GO OF posiciones.id_sucursal IN FRAME F-Main /* Sucursal */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_sucursal V-table-Win
ON LEAVE OF posiciones.id_sucursal IN FRAME F-Main /* Sucursal */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_sucursal V-table-Win
ON MOUSE-SELECT-DBLCLICK OF posiciones.id_sucursal IN FRAME F-Main /* Sucursal */
do: 
define var r as rowid no-undo.
run wc_sucursales.w(output r).
find sucursales where rowid(sucursales) = r no-lock no-error.
if available sucursales then 
general.posiciones.id_sucursal:screen-value = string(sucursales.id_sucursal).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL posiciones.id_sucursal V-table-Win
ON U1 OF posiciones.id_sucursal IN FRAME F-Main /* Sucursal */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "posiciones"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "posiciones"}
...
find first sucursales where sucursales.id_sucursal = integer(posiciones.id_sucursal:screen-value in frame F-Main)  no-lock no-error .
if available sucursales then 
...
find first empresas where empresas.id_empresa = integer(posiciones.id_empresa:screen-value in frame F-Main)  no-lock no-error .
if available empresas then 
...
find first locaciones where locaciones.id_locacion = integer(posiciones.id_locacion:screen-value in frame F-Main)  no-lock no-error .
if available locaciones then 
...
define var lista_relacion as character no-undo initial "id_sucursal,id_empresa,id_locacion".
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "posiciones" "id_sucursal"}
