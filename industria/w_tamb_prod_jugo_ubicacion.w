...
DEFINE VARIABLE h_b_prod_jugo_ubicacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_seleccion_locaciones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tamb_prod_jugo_ubicacion AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "PRODUCCIONES DE JUGO" 
     VIEW-AS FILL-IN 
     SIZE 135 BY .95
     BGCOLOR 2 FGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE FILL-IN-4 AS CHARACTER FORMAT "X(256)":U INITIAL "TAMBORES DE PRODUCCIONES DE JUGO" 
     VIEW-AS FILL-IN 
     SIZE 135 BY .95
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 1.24 COL 3 NO-LABEL
     FILL-IN-4 AT ROW 9.57 COL 3 NO-LABEL
     FILL-IN-5 AT ROW 17.91 COL 3 NO-LABEL
     BUTTON-1 AT ROW 19.1 COL 92
     BUTTON-2 AT ROW 21 COL 92
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Tambores de Produccion de Jugo"
         HEIGHT             = 23.95
         WIDTH              = 139.2
         MAX-HEIGHT         = 23.95
         MAX-WIDTH          = 140.8
...
ON END-ERROR OF W-Win /* Tambores de Produccion de Jugo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tambores de Produccion de Jugo */
DO:
...
DO:
  ...
  run get-rowid1 in h_b_prod_jugo_ubicacion (output r).
  find produccion_jugo where rowid(produccion_jugo) = r no-lock no-error.
  if available produccion_jugo then
    do:
        for each tambores_industria of produccion_jugo.
            {i_mov_manuales.i}
        end.      
    end.                                              

run dispatch in h_b_tamb_prod_jugo_ubicacion ('open-query':U).
...
DO:
  ...
  run get-rowid1 in h_b_tamb_prod_jugo_ubicacion (output r).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_prod_jugo_ubicacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_prod_jugo_ubicacion ).
       RUN set-position IN h_b_prod_jugo_ubicacion ( 2.43 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b_prod_jugo_ubicacion ( 6.71 , 118.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tamb_prod_jugo_ubicacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tamb_prod_jugo_ubicacion ).
       RUN set-position IN h_b_tamb_prod_jugo_ubicacion ( 10.76 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b_tamb_prod_jugo_ubicacion ( 6.71 , 137.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b_seleccion_locaciones ( 19.10 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 22.91 , 101.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_tamb_prod_jugo_ubicacion. */
       RUN add-link IN adm-broker-hdl ( h_b_prod_jugo_ubicacion , 'Record':U , h_b_tamb_prod_jugo_ubicacion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_prod_jugo_ubicacion ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tamb_prod_jugo_ubicacion ,
