...
gastos_venta.descripcion r_gastos_items_contrato.importe ~
r_gastos_items_contrato.id_moneda tipo_moneda.descripcion ~
r_gastos_items_contrato.id_tipo_unidad_venta tipo_unidad_venta.abreviatura 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ~
r_gastos_items_contrato.id_gasto r_gastos_items_contrato.importe ~
r_gastos_items_contrato.id_moneda ~
r_gastos_items_contrato.id_tipo_unidad_venta 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}id_gasto ~{&FP2}id_gasto ~{&FP3}~
 ~{&FP1}importe ~{&FP2}importe ~{&FP3}~
 ~{&FP1}id_moneda ~{&FP2}id_moneda ~{&FP3}~
 ~{&FP1}id_tipo_unidad_venta ~{&FP2}id_tipo_unidad_venta ~{&FP3}
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_gastos_items_contrato WHERE r_gastos_items_contrato.id_contrato = items_contratos.id_contrato ~
  AND r_gastos_items_contrato.id_tipo_contrato = items_contratos.id_tipo_contrato ~
  AND r_gastos_items_contrato.anio = items_contratos.anio ~
  AND r_gastos_items_contrato.item = items_contratos.item NO-LOCK, ~
      EACH gastos_venta OF r_gastos_items_contrato NO-LOCK, ~
      EACH tipo_moneda OF r_gastos_items_contrato OUTER-JOIN NO-LOCK, ~
      EACH tipo_unidad_venta OF r_gastos_items_contrato OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table r_gastos_items_contrato ~
gastos_venta tipo_moneda tipo_unidad_venta
...
general.r_gastos_items_contrato.id_moneda;wc_tipo_moneda.w;moneda.descripcion
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      r_gastos_items_contrato.id_moneda FORMAT ">>,>>9"
      tipo_moneda.descripcion FORMAT "x(15)"
      r_gastos_items_contrato.id_tipo_unidad_venta FORMAT ">,>>9"
      tipo_unidad_venta.abreviatura COLUMN-LABEL "Unidad"
  ENABLE
      r_gastos_items_contrato.id_gasto
      r_gastos_items_contrato.importe
      r_gastos_items_contrato.id_moneda
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 140 BY 4.76
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_gastos_items_contrato WHERE general.items_contratos ...,industria.gastos_venta OF general.r_gastos_items_contrato,comercial.tipo_moneda OF general.r_gastos_items_contrato,industria.tipo_unidad_venta OF general.r_gastos_items_contrato"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",, OUTER, OUTER"
     _JoinCode[1]      = "general.r_gastos_items_contrato.id_contrato = general.items_contratos.id_contrato
  AND general.r_gastos_items_contrato.id_tipo_contrato = general.items_contratos.id_tipo_contrato
  AND general.r_gastos_items_contrato.anio = general.items_contratos.anio
  AND general.r_gastos_items_contrato.item = general.items_contratos.item"
     _FldNameList[1]   > general.r_gastos_items_contrato.id_gasto
"r_gastos_items_contrato.id_gasto" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[2]   = general.gastos_venta.descripcion
     _FldNameList[3]   > general.r_gastos_items_contrato.importe
"r_gastos_items_contrato.importe" ? ? "decimal" ? ? ? ? ? ? yes ?
     _FldNameList[4]   > general.r_gastos_items_contrato.id_moneda
"r_gastos_items_contrato.id_moneda" ? ">>,>>9" "integer" ? ? ? ? ? ? yes ?
     _FldNameList[5]   > comercial.tipo_moneda.descripcion
"tipo_moneda.descripcion" ? "x(15)" "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   > general.r_gastos_items_contrato.id_tipo_unidad_venta
"r_gastos_items_contrato.id_tipo_unidad_venta" ? ">,>>9" "integer" ? ? ? ? ? ? yes ?
     _FldNameList[7]   > general.tipo_unidad_venta.abreviatura
"tipo_unidad_venta.abreviatura" "Unidad" ? "character" ? ? ? ? ? ? no ?
...
ON LEAVE OF r_gastos_items_contrato.id_gasto IN BROWSE br_table /* CodGas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL r_gastos_items_contrato.id_gasto br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF r_gastos_items_contrato.id_gasto IN BROWSE br_table /* CodGas */
DO:
...
ON LEAVE OF r_gastos_items_contrato.id_moneda IN BROWSE br_table /* Moneda */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL r_gastos_items_contrato.id_moneda br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF r_gastos_items_contrato.id_moneda IN BROWSE br_table /* Moneda */
DO:
    define var r as rowid.
...
r_gastos_items_contrato.id_moneda:label-bgcolor in browse {&BROWSE-NAME} = 7.
r_gastos_items_contrato.id_moneda:label-fgcolor in browse {&BROWSE-NAME} = 15.
r_gastos_items_contrato.id_tipo_unidad_venta:label-bgcolor in browse {&BROWSE-NAME} = 7.
r_gastos_items_contrato.id_tipo_unidad_venta:label-fgcolor in browse {&BROWSE-NAME} = 15.
...
run pasa_rowid_cabecera in h_con (output r).
...
if available items_contratos then
    ...
    message "Hubo un error en la grabación" view-as alert-box.
    


message "Estoy por recalcular en post-create". 

   
/* run recalculo_precio_venta. */
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE recalculo_precio_venta B-table-Win 
PROCEDURE recalculo_precio_venta :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
run get-container (output h_con).
run recalculo_precio_de_venta in h_con.
