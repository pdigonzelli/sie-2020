...
&Scoped-Define ENABLED-OBJECTS RECT-8 RECT-2 
...
DEFINE VARIABLE h_b_gastos_fletes AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_gastos_fletes AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Gastos Fletes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Gastos Fletes */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 25.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 59.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 82.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_gastos_fletes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_gastos_fletes ).
       RUN set-position IN h_b_gastos_fletes ( 3.86 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 125.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_gastos_fletes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_gastos_fletes ).
       RUN set-position IN h_v_gastos_fletes ( 11.71 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.00 , 113.00 ) */

       /* Links to csmartbrowser h_b_gastos_fletes. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_gastos_fletes ).

       /* Links to SmartViewer h_v_gastos_fletes. */
       RUN add-link IN adm-broker-hdl ( h_b_gastos_fletes , 'Record':U , h_v_gastos_fletes ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_gastos_fletes ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_gastos_fletes ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_gastos_fletes ,
             h_b_gastos_fletes , 'AFTER':U ).
    END. /* Page 0 */
