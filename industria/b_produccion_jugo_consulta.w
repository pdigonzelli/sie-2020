...
define var tipo as integer.

define temp-table tt-produccion
    field id_empresa as integer
    field id_sucursal as integer
    field id_produccion as integer
    field id_articulo as integer
    field descripcion as char format "x(12)"
    field cantidad as integer
    field cantidad_restante as integer
    field fecha as date
    field bx_20_20 as decimal
    field bx_correg as decimal
    field acidez_w_w as decimal
    field acidez_w_v as decimal
    field sodio as decimal
    field pulpa as decimal
    field unidad_medida as integer
    field id_envase as integer
    field descripcion_envase as char format "x(12)".
...
&Scoped-define INTERNAL-TABLES tt-produccion

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table /* tt-produccion.id_empresa */ tt-produccion.id_sucursal tt-produccion.id_produccion tt-produccion.id_articulo tt-produccion.descripcion /* tt-produccion.cantidad */ tt-produccion.cantidad_restante tt-produccion.unidad_medida tt-produccion.id_envase tt-produccion.descripcion_envase tt-produccion.fecha tt-produccion.bx_20_20 tt-produccion.bx_correg tt-produccion.acidez_w_w tt-produccion.acidez_w_v tt-produccion.sodio tt-produccion.pulpa   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH tt-produccion NO-LOCK     ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tt-produccion
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tt-produccion
...
<FOREIGN-KEYS>
id_envase||y|general.produccion_jugo.id_envase
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      /*      tt-produccion.id_empresa          label "Emp." */
      tt-produccion.id_sucursal         label "Suc."
      tt-produccion.id_produccion       label "Id. Prod"
      tt-produccion.id_articulo         label "Art."
      tt-produccion.descripcion         label "Descripción"
/*      tt-produccion.cantidad          label "Cantidad"  */
      tt-produccion.cantidad_restante   label "Restante"
      tt-produccion.unidad_medida       label "U.M."
      tt-produccion.id_envase           label "Envase"
      tt-produccion.descripcion_envase  label "Descripcion"
      tt-produccion.fecha               label "Fecha"
      tt-produccion.bx_20_20            label "Bx 20/20"
      tt-produccion.bx_correg           label "Bx Corr"
      tt-produccion.acidez_w_w          label "Acidez w/w"
      tt-produccion.acidez_w_v          label "Acidez w/v"
      tt-produccion.sodio               label "Sodio"
      tt-produccion.pulpa               label "Pulpa"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 154 BY 13.52
         BGCOLOR 15 FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt-produccion NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
tt-produccion.cantidad_restante:COLUMN-BGCOLOR in browse {&BROWSE-NAME} = 17.
/*
/******** EMPIEZA MENU ************/
...
label 'id_produccion( produccion_jugo ) '
  ...
  menu-item produccion_jugo-id_articulo
label 'id_articulo( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.id_articulo').
                  run set-attribute-list('titulo=Artículo').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.id_articulo
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
label 'Acidez_w_v( produccion_jugo ) '
  ...
  menu-item produccion_jugo-unidad_medida
label 'unidad_medida( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.unidad_medida').
                  run set-attribute-list('titulo=Unidad Medida').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.unidad_medida
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item produccion_jugo-id_envase
label 'id_envase( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.id_envase').
                  run set-attribute-list('titulo=Envase').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.id_envase
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item produccion_jugo-cantidad
label 'cantidad( produccion_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=produccion_jugo.cantidad').
                  run set-attribute-list('titulo=Cantidad').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by produccion_jugo.cantidad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  ...
  WHEN 'produccion_jugo.id_articulo' then  
...
p1 = tt-produccion.id_empresa.
p2 = tt-produccion.id_sucursal.
p3 = tt-produccion.id_produccion.
p4 = tt-produccion.cantidad_restante.
...
define var descr1 as char format "x(12)".
define var c as integer.

FOR EACH general.produccion_jugo NO-LOCK.
       c = 0.
      FOR EACH general.productos_terminados OF general.produccion_jugo  NO-LOCK.
        descr = productos_terminados.abreviatura.
      end.
      FOR EACH envases_prod OF general.produccion_jugo  NO-LOCK.
        descr1 = envases_prod.abreviatura.
      end.
      FOR EACH general.envasado_lote WHERE 
                general.envasado_lote.id_empresa_origen = general.produccion_jugo.id_empresa
          AND general.envasado_lote.id_sucursal_origen = general.produccion_jugo.id_sucursal
          AND general.envasado_lote.id_origen = general.produccion_jugo.id_produccion
          AND general.envasado_lote.id_tipo_origen = 1 NO-LOCK:
          
          c = c + envasado_lote.cantidad.
     end.
     
     /*message "cantidad " c "prod_jugo.cantidad " produccion_jugo.cantidad view-as alert-box.*/
     
     if c < produccion_jugo.litros then
        do:
           create tt-produccion.
           tt-produccion.id_produccion = produccion_jugo.id_produccion.
           tt-produccion.id_articulo = produccion_jugo.id_articulo.
           tt-produccion.descripcion = descr.
           tt-produccion.cantidad = produccion_jugo.litros.
           tt-produccion.cantidad_restante = produccion_jugo.litros - c.
           tt-produccion.id_empresa = produccion_jugo.id_empresa.         
           tt-produccion.id_sucursal = produccion_jugo.id_sucursal.        
           tt-produccion.fecha = produccion_jugo.fecha.             
           tt-produccion.bx_20_20 = produccion_jugo.bx_20_20.           
           tt-produccion.bx_correg = produccion_jugo.bx_correg.       
           tt-produccion.acidez_w_w = produccion_jugo.acidez_w_w.         
           tt-produccion.acidez_w_v = produccion_jugo.acidez_w_v.       
           tt-produccion.sodio = produccion_jugo.sodio.             
           tt-produccion.pulpa = produccion_jugo.pulpa.            
           tt-produccion.unidad_medida = produccion_jugo.unidad_medida.    
           tt-produccion.id_envase = produccion_jugo.id_envase.
           tt-produccion.descripcion_envase = descr1.          
           /*message "adentro del loop de generacion de temp-table" view-as alert-box.*/
        end.
end. 
...
define tipo as integer.

run get-container (output hcontainer).

run dame-rowid in hcontainer (output tipo).
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_envase" "produccion_jugo" "id_envase"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tt-produccion"}
