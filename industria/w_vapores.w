...
DEFINE VARIABLE h_b_vapores AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_vapores AS HANDLE NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Vapores */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_vapores.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_vapores ).
       RUN set-position IN h_b_vapores ( 3.86 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 75.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_vapores.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_vapores ).
       RUN set-position IN h_v_vapores ( 11.71 , 18.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 47.00 ) */

       /* Links to csmartbrowser h_b_vapores. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_vapores ).

       /* Links to SmartViewer h_v_vapores. */
       RUN add-link IN adm-broker-hdl ( h_b_vapores , 'Record':U , h_v_vapores ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_vapores ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_vapores ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_vapores ,
             h_b_vapores , 'AFTER':U ).
    END. /* Page 0 */
                    ...
                    v_filtro = "".
                          ...
                          RUN  aderb\_prntrb2(
                           ...
                           "",                              /* RB-OUTPUT-FILE */
                            ...
                            0,                              /* RB-END-PAGE - zero */
                           no,                              /* RB-TEST-PATTERN */
                           "Reporte de Barcos",         /* RB-WINDOW-TITLE */
