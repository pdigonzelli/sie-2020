...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-4 fecha_desde fecha_hasta ~
BUTTON-3 articulo calidad nombre_articulo nombre_calidad 
...
DEFINE FRAME F-Main
     ...
     "Reporte de Produccion de Lotes:" VIEW-AS TEXT
...
ON END-ERROR OF W-Win /* Reporte de Produccion */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Reporte de Produccion */
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-2 RECT-4 fecha_desde fecha_hasta BUTTON-3 articulo calidad 
         nombre_articulo nombre_calidad 
      WITH FRAME F-Main IN WINDOW W-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE reporte_produccion W-Win 
PROCEDURE reporte_produccion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
if fecha_desde:screen-value in frame F-Main <> "" and 
   fecha_hasta:screen-value in frame F-Main = "" then
    do:
        ...
        v_filtro = "tambores_industria.fecha = date('" + string(v_fecha_desde) + "')".
    end.
...
if fecha_desde:screen-value in frame F-Main <> "" and 
   fecha_hasta:screen-value in frame F-Main <> "" then
    do:
        v_fecha_desde = date(fecha_desde:screen-value in frame F-Main).
        v_fecha_hasta = date(fecha_hasta:screen-value in frame F-Main).
        v_filtro = "tambores_industria.fecha >= date('" + string(v_fecha_desde) + "') and tambores_industria.fecha <= date('" + string(v_fecha_hasta) + "')".
    end.
...
if v_calidad > 0 then
    v_filtro = v_filtro + " and tambores_industria.id_calidad = " + string(v_calidad).    
         ...
         "parte_produccion",                    /* RB-REPORT-NAME */
         "",                             /* RB-DB-CONNECTION */
