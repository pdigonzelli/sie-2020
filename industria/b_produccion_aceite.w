...
&Scoped-define INTERNAL-TABLES Produccion_aceite
...
Produccion_aceite.id_produccion_aceite Produccion_aceite.fecha ~
Produccion_aceite.kilos_aceite 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH Produccion_aceite WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table Produccion_aceite
&Scoped-define FIRST-TABLE-IN-QUERY-br_table Produccion_aceite
...
<FOREIGN-KEYS>
id_sucursal||y|general.Produccion_aceite.id_sucursal
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      Produccion_aceite.id_produccion_aceite COLUMN-LABEL "Producción de aceite" FORMAT ">>>,>>9"
      Produccion_aceite.fecha COLUMN-LABEL "Fecha"
      Produccion_aceite.kilos_aceite COLUMN-LABEL "Kilos de aceite"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.Produccion_aceite"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.Produccion_aceite.id_produccion_aceite
"id_produccion_aceite" "Producción de aceite" ">>>,>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > general.Produccion_aceite.fecha
"fecha" "Fecha" ? "date" ? ? ? ? ? ? no ?
     _FldNameList[3]   > general.Produccion_aceite.kilos_aceite
"kilos_aceite" "Kilos de aceite" ? "integer" ? ? ? ? ? ? no ?
...
define sub-menu Ordena 
  menu-item Produccion_aceite-id_produccion_aceite
label 'id_produccion_aceite( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.id_produccion_aceite').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.id_produccion_aceite
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item Produccion_aceite-fecha
label 'fecha( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.fecha').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.fecha
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item Produccion_aceite-kilos_aceite
label 'kilos_aceite( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.kilos_aceite').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.kilos_aceite
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item Produccion_aceite-cantidad_tambores
label 'cantidad_tambores( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.cantidad_tambores').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.cantidad_tambores
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item Produccion_aceite-numeracion_desde
label 'numeracion_desde( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.numeracion_desde').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.numeracion_desde
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item Produccion_aceite-numeracion_hasta
label 'numeracion_hasta( Produccion_aceite ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=Produccion_aceite.numeracion_hasta').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by Produccion_aceite.numeracion_hasta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by Produccion_aceite.id_produccion_aceite
...
case p:
  WHEN 'Produccion_aceite.id_produccion_aceite' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_produccion_aceite = integer(valor)  no-lock no-error .
  WHEN 'Produccion_aceite.fecha' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  fecha = date(valor)  no-lock no-error .
  WHEN 'Produccion_aceite.kilos_aceite' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  kilos_aceite = integer(valor)  no-lock no-error .
  WHEN 'Produccion_aceite.cantidad_tambores' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  cantidad_tambores = integer(valor)  no-lock no-error .
  WHEN 'Produccion_aceite.numeracion_desde' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  numeracion_desde = integer(valor)  no-lock no-error .
  WHEN 'Produccion_aceite.numeracion_hasta' then  
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "Produccion_aceite" "id_sucursal"}
