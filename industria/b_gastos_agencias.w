...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES agencias
&Scoped-define FIRST-EXTERNAL-TABLE agencias


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR agencias.
/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES gastos_agencias gastos_venta

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table gastos_agencias.id_gasto ~
gastos_venta.descripcion gastos_agencias.importe 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table gastos_agencias.id_gasto ~
gastos_agencias.importe 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}id_gasto ~{&FP2}id_gasto ~{&FP3}~
 ~{&FP1}importe ~{&FP2}importe ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table gastos_agencias
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table gastos_agencias
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH gastos_agencias WHERE gastos_agencias.id_agencia = agencias.id_agencia NO-LOCK, ~
      EACH gastos_venta OF gastos_agencias NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table gastos_agencias gastos_venta
&Scoped-define FIRST-TABLE-IN-QUERY-br_table gastos_agencias
...
<FOREIGN-KEYS>
id_gasto||y|general.gastos_agencias.id_gasto
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
general.gastos_agencias.id_gasto;wc_gastos_venta.w;gastos_venta.descripcion
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 78 BY 6.71
...
/* Settings for THIS-PROCEDURE
   Type: csmartbrowser
   External Tables: general.agencias
   Allow: Basic,Browse
   Frames: 1
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.gastos_agencias WHERE general.agencias <external> ...,industria.gastos_venta OF general.gastos_agencias"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "general.gastos_agencias.id_agencia = general.agencias.id_agencia"
     _FldNameList[1]   > general.gastos_agencias.id_gasto
"gastos_agencias.id_gasto" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[2]   = general.gastos_venta.descripcion
     _FldNameList[3]   > general.gastos_agencias.importe
"gastos_agencias.importe" ? ? "decimal" ? ? ? ? ? ? yes ?
...
&Scoped-define SELF-NAME gastos_agencias.id_gasto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gastos_agencias.id_gasto br_table _BROWSE-COLUMN B-table-Win
ON LEAVE OF gastos_agencias.id_gasto IN BROWSE br_table /* CodGas */
DO:
  /*
  find gastos_venta where gastos_venta.id_gasto = integer(gastos_agencias.id_gasto:screen-value in browse {&BROWSE-NAME}) no-lock no-error.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gastos_agencias.id_gasto br_table _BROWSE-COLUMN B-table-Win
ON MOUSE-SELECT-DBLCLICK OF gastos_agencias.id_gasto IN BROWSE br_table /* CodGas */
DO:
  /*
  ...
  if available gastos_venta then
  do:
    gastos_agencias.id_gasto:screen-value in browse {&BROWSE-NAME} = string(gastos_venta.id_gasto).
    gastos_venta.descripcion:screen-value in browse {&BROWSE-NAME} = gastos_venta.descripcion.
...
gastos_agencias.id_gasto:label-bgcolor in browse {&BROWSE-NAME} = 7.
gastos_agencias.id_gasto:label-fgcolor in browse {&BROWSE-NAME} = 15.
on MOUSE-SELECT-DBLCLICK,F10 of gastos_agencias.id_gasto in browse {&BROWSE-NAME} do:
   define var r as rowid no-undo.
...
define sub-menu Ordena 
  menu-item gastos_agencias-id_gasto
label 'id_gasto( gastos_agencias ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=gastos_agencias.id_gasto').
                  run set-attribute-list('titulo=Codigo Gasto').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by gastos_agencias.id_gasto
...
label 'descripcion( gastos_venta ) '
  ...
  menu-item gastos_agencias-importe
label 'importe( gastos_agencias ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=gastos_agencias.importe').
                  run set-attribute-list('titulo=Importe').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by gastos_agencias.importe
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by gastos_agencias.id_gasto
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
define var p_agencia as rowid.
define var h_con as handle.

run get-container (output h_con).
run dame-agencia in h_con (output p_agencia).

find agencias where rowid(agencias) = p_agencia no-lock no-error.
if available agencias then
    assign gastos_agencias.id_agencia = agencias.id_agencia.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "agencias"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "agencias"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_gasto" "gastos_agencias" "id_gasto"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "gastos_agencias"}
