...
productos_terminados envases_prod contratos items_contratos clientes
...
envases_prod.abreviatura productos_terminados.abreviatura ~
calidades.abreviatura contratos.orden_fabricacion contratos.id_contrato ~
clientes.nombre items_contratos.semana_entrega ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tambores_industria WHERE ~{&KEY-PHRASE} ~
 ...
 AND (industria.tambores_industria.id_tipotambor = 3 ~
 OR tambores_industria.id_tipotambor = 6) NO-LOCK, ~
      ...
      EACH contratos WHERE contratos.id_contrato = tambores_industria.id_contrato_of ~
  AND contratos.id_tipo_contrato = tambores_industria.id_tipocontrato_of ~
  AND contratos.anio = tambores_industria.anio_of OUTER-JOIN NO-LOCK, ~
      EACH items_contratos OF contratos OUTER-JOIN NO-LOCK, ~
      EACH clientes OF contratos OUTER-JOIN NO-LOCK ~
    BY tambores_industria.id_tipotambor ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      envases_prod.abreviatura COLUMN-LABEL "Envase"
      ...
      clientes.nombre COLUMN-LABEL "Cliente" COLUMN-FONT 8
      items_contratos.semana_entrega COLUMN-LABEL "Sem" FORMAT ">9"
      items_contratos.semana_entrega_hasta COLUMN-LABEL "ana" FORMAT ">9"
      items_contratos.anio_semana_entrega COLUMN-LABEL ""
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 152 BY 22.14
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.tambores_industria,general.calidades OF general.tambores_industria,general.productos_terminados OF general.tambores_industria,general.envases_prod OF general.tambores_industria,industria.contratos WHERE general.tambores_industria ...,general.items_contratos OF general.contratos,general.clientes OF general.contratos"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ",,,, OUTER, OUTER, OUTER"
     _OrdList          = "general.tambores_industria.id_tipotambor|yes,industria.tambores_industria.id_lote|yes,industria.tambores_industria.id_articulo|yes,industria.tambores_industria.id_calidad|yes"
     _Where[1]         = "general.tambores_industria.id_tambor = 1
 AND general.tambores_industria.id_sucursal_ubicacion = 95
 AND general.tambores_industria.id_locacion_ubicacion = 4
 AND (industria.tambores_industria.id_tipotambor = 3
 OR general.tambores_industria.id_tipotambor = 6)"
     _JoinCode[5]      = "general.contratos.id_contrato = general.tambores_industria.id_contrato_of
  AND general.contratos.id_tipo_contrato = general.tambores_industria.id_tipocontrato_of
  AND general.contratos.anio = general.tambores_industria.anio_of"
     _FldNameList[1]   > general.tambores_industria.id_lote
"tambores_industria.id_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > general.envases_prod.abreviatura
"envases_prod.abreviatura" "Envase" ? "character" ? ? ? ? ? ? no ?
...
"clientes.nombre" "Cliente" ? "character" ? ? 8 ? ? ? no ?
     _FldNameList[8]   > general.items_contratos.semana_entrega
"items_contratos.semana_entrega" "Sem" ">9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[9]   > general.items_contratos.semana_entrega_hasta
"items_contratos.semana_entrega_hasta" "ana" ">9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[10]   > general.items_contratos.anio_semana_entrega
"items_contratos.anio_semana_entrega" "" ? "integer" ? ? ? ? ? ? no ?
