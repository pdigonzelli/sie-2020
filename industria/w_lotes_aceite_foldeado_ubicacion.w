...
DEFINE VARIABLE h_b_lotes_aceite_foldeado_ubicac AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_seleccion_locaciones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_lotes_aceite_ubicac AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-2 
     LABEL "Movimiento de un solo tambor" 
     SIZE 30 BY 1.43.
...
DEFINE VARIABLE FILL-IN-4 AS CHARACTER FORMAT "X(256)":U INITIAL "TAMBORES DE LOTES DE JUGO" 
     VIEW-AS FILL-IN 
     SIZE 140 BY .95
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 1.24 COL 4 NO-LABEL
     FILL-IN-4 AT ROW 9.33 COL 4 NO-LABEL
     FILL-IN-5 AT ROW 17.67 COL 4 NO-LABEL
     BUTTON-1 AT ROW 18.86 COL 92
     BUTTON-2 AT ROW 20.76 COL 92
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Ubicacion de Tambores de Lotes de Aceite"
         HEIGHT             = 23.48
         WIDTH              = 144.6
         MAX-HEIGHT         = 23.48
         MAX-WIDTH          = 144.6
...
ON END-ERROR OF W-Win /* Ubicacion de Tambores de Lotes de Aceite */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Ubicacion de Tambores de Lotes de Aceite */
DO:
...
DO:
  ...
  run get-rowid1 in h_b_lotes_aceite_foldeado_ubicac (output r).
...
run dispatch in h_b_tambores_lotes_aceite_ubicac ('open-query':U).
message "Se termino el movimiento" view-as alert-box.  
END.
...
ON CHOOSE OF BUTTON-2 IN FRAME F-Main /* Movimiento de un solo tambor */
DO:
  define var r as rowid.
  define var r2 as rowid.
  define var v_emp like locaciones.id_empresa.
  ...
  run get-rowid1 in h_b_tambores_lotes_aceite_ubicac (output r).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_aceite_foldeado_ubicacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_aceite_foldeado_ubicac ).
       RUN set-position IN h_b_lotes_aceite_foldeado_ubicac ( 2.43 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b_lotes_aceite_foldeado_ubicac ( 6.71 , 127.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_lotes_aceite_ubicacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_lotes_aceite_ubicac ).
       RUN set-position IN h_b_tambores_lotes_aceite_ubicac ( 10.52 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b_tambores_lotes_aceite_ubicac ( 6.71 , 141.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b_seleccion_locaciones ( 18.86 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b_seleccion_locaciones ( 5.48 , 83.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 18.86 , 124.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_tambores_lotes_aceite_ubicac. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_aceite_foldeado_ubicac , 'Record':U , h_b_tambores_lotes_aceite_ubicac ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_aceite_foldeado_ubicac ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_lotes_aceite_ubicac ,
