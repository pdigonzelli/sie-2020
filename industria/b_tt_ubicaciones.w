...
define temp-table tt_tambores
    ...
    field id_sucursal as integer format ">99"
    field tipo as char format "x(15)"
    field id_origen as integer format ">>>99"
    field id_etiqueta as integer format ">>>9999"
    field id_articulo as integer format ">99"
...
&Scoped-define FIELDS-IN-QUERY-br_table tt_tambores.id_empresa tt_tambores.id_sucursal tt_tambores.tipo tt_tambores.id_origen tt_tambores.id_etiqueta tt_tambores.id_articulo tt_tambores.descripcion   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH tt_tambores WHERE ~{&KEY-PHRASE} NO-LOCK     ~{&SORTBY-PHRASE}.
...
<FOREIGN-KEYS>
id_sucursal||y|general.ubicaciones.id_sucursal
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _FREEFORM
  QUERY br_table NO-LOCK DISPLAY
      tt_tambores.id_empresa
      tt_tambores.id_sucursal
      tt_tambores.tipo 
      tt_tambores.id_origen
      tt_tambores.id_etiqueta
      tt_tambores.id_articulo
      tt_tambores.descripcion
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 135 BY 12.76
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt_tambores WHERE ~{&KEY-PHRASE} NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
define sub-menu Ordena 
  menu-item ubicaciones-id_sucursal
label 'id_sucursal( ubicaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=ubicaciones.id_sucursal').
                  run set-attribute-list('titulo=Sucursal').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by ubicaciones.id_sucursal
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item ubicaciones-id_locacion
label 'id_locacion( ubicaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=ubicaciones.id_locacion').
                  run set-attribute-list('titulo=Locacion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by ubicaciones.id_locacion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item ubicaciones-id_posicion
label 'id_posicion( ubicaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=ubicaciones.id_posicion').
                  run set-attribute-list('titulo=Posicion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by ubicaciones.id_posicion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item ubicaciones-id_etiqueta
label 'id_etiqueta( ubicaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=ubicaciones.id_etiqueta').
                  run set-attribute-list('titulo=Etiqueta').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by ubicaciones.id_etiqueta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by ubicaciones.id_sucursal
...
run get-datos in hcontainer (output p1, output p2, output p3).

for each tambores_industria where tambores_industria.id_empresa_ubicacion = p1 and
                                  tambores_industria.id_sucursal_ubicacion = p2 and
                                  tambores_industria.id_locacion_ubicacion = p3.

        create tt_tambores.
        assign tt_tambores.id_empresa = tambores_industria.id_empresa
               tt_tambores.id_sucursal = tambores_industria.id_sucursal
               tt_tambores.tipo  = string(tambores_industria.id_tipotambor)
               tt_tambores.id_origen = tambores_industria.id_lote
               tt_tambores.id_etiqueta = tambores_industria.id_etiqueta
               tt_tambores.id_articulo = tambores_industria.id_articulo
               tt_tambores.descripcion = "Jugo".
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_sucursal" "ubicaciones" "id_sucursal"}
