...
define var copia as logical no-undo initial false.
DEFINE VAR mi-alta AS LOGICAL NO-UNDO INITIAL FALSE.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 30.80 ) NO-ERROR.
       ...
       RUN set-size IN h_folder ( 13.57 , 143.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_v_items_contratos_principal ( 12.19 , 6.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_items_contratos_principal ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN set-position IN h_cus-updsav-2 ( 14.10 , 10.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_cus-updsav-2 , 'TableIO':U , h_b_r_gastos_items_contratos_aux ).
    ...
    WHEN 3 THEN DO:
       ...
       RUN set-position IN h_b_consulta_item_contrato_item_ ( 12.67 , 8.00 ) NO-ERROR.
       RUN set-size IN h_b_consulta_item_contrato_item_ ( 6.71 , 119.00 ) NO-ERROR.
...
FIND FIRST contratos where contratos.id_tipo_contrato = p_id_tipo_contrato
...
IF AVAILABLE contratos THEN DO:
    ...
    IF p_id_tipo_contrato <= 4 THEN DO:
...
assign items_contratos.c_usuario = userid("userdb")
...
define input parameter r as rowid. /* rowid de la tabla updateada */
define input parameter h as handle. /* handle del viewer */
DEFINE BUFFER bb_items_contrato FOR items_contrato.
DEFINE VAR v_tambores_oe AS INTEGER.

RUN confirma-deposito IN h_v_items_contratos_principal.

IF NOT copia THEN DO:
    FIND FIRST bb_items_contrato WHERE ROWID(bb_items_contrato) = r NO-LOCK NO-ERROR.
    IF AVAILABLE bb_items_contrato THEN DO:
        FIND FIRST items_orden_entrega OF bb_items_contrato
                                    WHERE items_orden_entrega.pendiente = FALSE
                                    NO-LOCK NO-ERROR.
        IF AVAILABLE items_orden_entrega THEN DO:
            MESSAGE "No puede modificar este contrato porque ya hay una OE cerrada asociada." 
                    view-as alert-box.
            RETURN "ADM-ERROR".
        END.
    END.              
