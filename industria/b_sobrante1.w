...
&Scoped-define FIELDS-IN-QUERY-br_table sobrante.cantidad ~
sobrante.tipo_almacenamiento sobrante.fecha_elaboracion ~
sobrante.identificacion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table sobrante.cantidad ~
sobrante.tipo_almacenamiento sobrante.fecha_elaboracion ~
sobrante.identificacion 
...
&Scoped-define TABLES-IN-QUERY-br_table sobrante
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      sobrante.cantidad COLUMN-LABEL "Cantidad(Kilos)" FORMAT "->>,>>>,>>9.99"
      sobrante.tipo_almacenamiento COLUMN-LABEL "Almacenamiento" FORMAT "tanque/tambor"
      sobrante.fecha_elaboracion
      sobrante.identificacion COLUMN-LABEL "Identificación"
  ENABLE
      sobrante.cantidad
      sobrante.tipo_almacenamiento
      sobrante.fecha_elaboracion
      sobrante.identificacion
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 104 BY 5.05
         BGCOLOR 15 FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.sobrante OF general.lotes_jugo"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.sobrante.cantidad
"sobrante.cantidad" "Cantidad(Kilos)" "->>,>>>,>>9.99" "decimal" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > general.sobrante.tipo_almacenamiento
"sobrante.tipo_almacenamiento" "Almacenamiento" "tanque/tambor" "logical" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > general.sobrante.fecha_elaboracion
"sobrante.fecha_elaboracion" ? ? "date" ? ? ? ? ? ? yes ?
     _FldNameList[4]   > general.sobrante.identificacion
"sobrante.identificacion" "Identificación" ? "character" ? ? ? ? ? ? yes ?
...
define sub-menu Ordena 
  menu-item sobrante-cantidad
label 'cantidad( sobrante ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sobrante.cantidad').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by sobrante.cantidad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item sobrante-tipo_almacenamiento
label 'tipo_almacenamiento( sobrante ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sobrante.tipo_almacenamiento').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by sobrante.tipo_almacenamiento
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item sobrante-fecha_elaboracion
label 'fecha_elaboracion( sobrante ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sobrante.fecha_elaboracion').
                  run set-attribute-list('titulo=Fecha Elaboración').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by sobrante.fecha_elaboracion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item sobrante-identificacion
label 'identificacion( sobrante ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sobrante.identificacion').
                  run set-attribute-list('titulo=Dato').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by sobrante.identificacion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by sobrante.cantidad
...
for each lotes_jugo where rowid(lotes_jugo) = r.
    ...
    for each sob where sob.id_empresa = lotes_jugo.id_empresa and 
                       sob.id_sucursal = lotes_jugo.id_sucursal and
                       sob.id_lote = lotes_jugo.id_lote by id_sobrante descending.
...
case p:
  WHEN 'sobrante.cantidad' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  cantidad = decimal(valor)  no-lock no-error .
  WHEN 'sobrante.tipo_almacenamiento' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  tipo_almacenamiento no-lock no-error .
  WHEN 'sobrante.fecha_elaboracion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  fecha_elaboracion = date(valor)  no-lock no-error .
  WHEN 'sobrante.identificacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  identificacion begins valor  no-lock no-error .
end.
END PROCEDURE.
