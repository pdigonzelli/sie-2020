...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES subd_vtas
&Scoped-define FIRST-EXTERNAL-TABLE subd_vtas


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR subd_vtas.
...
&Scoped-define FIELDS-IN-QUERY-br_table aux_items_venta.desde_lote ~
aux_items_venta.hasta_lote items_venta.cantidad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH aux_items_venta OF subd_vtas WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH items_venta OF subd_vtas NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH aux_items_venta OF subd_vtas WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH items_venta OF subd_vtas NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table aux_items_venta items_venta
&Scoped-define FIRST-TABLE-IN-QUERY-br_table aux_items_venta
&Scoped-define SECOND-TABLE-IN-QUERY-br_table items_venta
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      aux_items_venta.desde_lote COLUMN-LABEL "DLote" FORMAT ">>>9":U
            WIDTH 7.2 COLUMN-FONT 0
      aux_items_venta.hasta_lote COLUMN-LABEL "HLote" FORMAT ">>>9":U
            WIDTH 7.2
      items_venta.cantidad FORMAT ">>>>>9":U WIDTH 12.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 40 BY 3.57
         BGCOLOR 15 FGCOLOR 4 FONT 0 ROW-HEIGHT-CHARS .57.
...
/* Settings for THIS-PROCEDURE
   Type: csmartbrowser
   External Tables: ventas.subd_vtas
   Allow: Basic,Browse
   Frames: 1
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ventas.aux_items_venta OF ventas.subd_vtas,ventas.items_venta OF ventas.subd_vtas"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > ventas.aux_items_venta.desde_lote
"aux_items_venta.desde_lote" "DLote" ">>>9" "integer" ? ? 0 ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[2]   > ventas.aux_items_venta.hasta_lote
"aux_items_venta.hasta_lote" "HLote" ">>>9" "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[3]   > ventas.items_venta.cantidad
"items_venta.cantidad" ? ">>>>>9" "decimal" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "subd_vtas"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "subd_vtas"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "aux_items_venta"}
