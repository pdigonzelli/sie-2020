...
&Scoped-define INTERNAL-TABLES muestras items_muestras ~
r_muestras_protocolos protocolos tambores_industria
...
muestras.id_destinatario protocolos.id_protocolo protocolos.anio ~
protocolos.cantidad_tambores tambores_industria.id_lote ~
tambores_industria.Anio 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH muestras WHERE ~{&KEY-PHRASE} ~
      AND if v_dest <> 0 then muestras.id_destinatario = v_dest else true NO-LOCK, ~
      EACH items_muestras OF muestras NO-LOCK, ~
      EACH r_muestras_protocolos OF items_muestras OUTER-JOIN NO-LOCK, ~
      EACH protocolos OF r_muestras_protocolos OUTER-JOIN NO-LOCK, ~
  ...
  AND tambores_industria.nromov = protocolos.nromov ~
      AND tambores_industria.id_tambor = 1 OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&Scoped-define THIRD-TABLE-IN-QUERY-br_table r_muestras_protocolos
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table protocolos
...
&Scoped-Define ENABLED-OBJECTS destinatario BUTTON-1 br_table 
&Scoped-Define DISPLAYED-OBJECTS destinatario 
...
DEFINE VARIABLE destinatario AS INTEGER FORMAT ">>9":U INITIAL 0 
     LABEL "Destinatario" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      protocolos.anio FORMAT ">>>9":U WIDTH 6.6
...
DEFINE FRAME F-Main
     destinatario AT ROW 1 COL 12 COLON-ALIGNED
     BUTTON-1 AT ROW 1 COL 35
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.muestras,general.items_muestras OF general.muestras,industria.r_muestras_protocolos OF general.items_muestras,industria.protocolos OF general.r_muestras_protocolos,industria.tambores_industria WHERE general.protocolos ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",, OUTER, OUTER, FIRST OUTER"
     _Where[1]         = "if v_dest <> 0 then general.muestras.id_destinatario = v_dest else true"
  ...
  AND general.tambores_industria.nromov = general.protocolos.nromov"
     _Where[5]         = "general.tambores_industria.id_tambor = 1"
     ...
     _FldNameList[6]   = general.protocolos.id_protocolo
     _FldNameList[7]   > general.protocolos.anio
"protocolos.anio" ? ? "integer" ? ? ? ? ? ? no ? no no "6.6" yes no no "U" "" ""
     _FldNameList[8]   = general.protocolos.cantidad_tambores
     _FldNameList[9]   = general.tambores_industria.id_lote
...
DO:
  v_dest = INTEGER(destinatario:SCREEN-VALUE IN FRAME F-Main).
