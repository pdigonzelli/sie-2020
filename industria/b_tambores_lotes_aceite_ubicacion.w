...
&Scoped-define INTERNAL-TABLES tambores_industria locaciones sucursales ~
...
tambores_industria.id_etiqueta tambores_industria.kilos_tambor ~
tambores_industria.tara tambores_industria.id_articulo ~
productos_terminados.abreviatura tambores_industria.id_calidad ~
calidades.abreviatura 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tambores_industria OF lotes_aceite WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH locaciones WHERE locaciones.id_empresa = tambores_industria.id_empresa_ubicacion ~
  AND locaciones.id_sucursal = tambores_industria.id_sucursal_ubicacion ~
  AND locaciones.id_locacion = tambores_industria.id_locacion_ubicacion OUTER-JOIN NO-LOCK, ~
      EACH sucursales WHERE sucursales.id_sucursal = tambores_industria.id_sucursal_ubicacion OUTER-JOIN NO-LOCK, ~
      EACH productos_terminados OF tambores_industria NO-LOCK, ~
      EACH calidades OF tambores_industria NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tambores_industria locaciones ~
sucursales productos_terminados calidades
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tambores_industria
&Scoped-define SECOND-TABLE-IN-QUERY-br_table locaciones
&Scoped-define THIRD-TABLE-IN-QUERY-br_table sucursales
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table productos_terminados
&Scoped-define FIFTH-TABLE-IN-QUERY-br_table calidades
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_envase,nro,nromov,id_sucursal,id_etiqueta,id_tipotambor"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      tambores_industria.id_sucursal_ubicacion COLUMN-LABEL "Suc" FORMAT ">>9":U
      sucursales.abreviatura FORMAT "X(10)":U
      tambores_industria.id_locacion_ubicacion COLUMN-LABEL "Loc" FORMAT ">>9":U
      locaciones.descripcion FORMAT "x(20)":U
      tambores_industria.id_etiqueta FORMAT ">>>,>>>,>>9":U
      tambores_industria.kilos_tambor FORMAT ">>>,>>9.999":U
      tambores_industria.tara FORMAT "->>,>>9.99":U
      tambores_industria.id_articulo FORMAT ">>>,>>9":U
      productos_terminados.abreviatura FORMAT "X(12)":U
      tambores_industria.id_calidad FORMAT ">>,>>9":U
      calidades.abreviatura FORMAT "X(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.tambores_industria OF general.lotes_aceite,industria.locaciones WHERE general.tambores_industria ...,comercial.sucursales WHERE general.tambores_industria ...,general.productos_terminados OF general.tambores_industria,general.calidades OF general.tambores_industria"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", OUTER, OUTER,,"
     _JoinCode[2]      = "general.locaciones.id_empresa = general.tambores_industria.id_empresa_ubicacion
  AND general.locaciones.id_sucursal = general.tambores_industria.id_sucursal_ubicacion
  AND general.locaciones.id_locacion = general.tambores_industria.id_locacion_ubicacion"
     _JoinCode[3]      = "comercial.sucursales.id_sucursal = general.tambores_industria.id_sucursal_ubicacion"
...
"tambores_industria.id_sucursal_ubicacion" "Suc" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = comercial.sucursales.abreviatura
     _FldNameList[4]   > general.tambores_industria.id_locacion_ubicacion
"tambores_industria.id_locacion_ubicacion" "Loc" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.locaciones.descripcion
"locaciones.descripcion" ? "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   = general.tambores_industria.id_etiqueta
     _FldNameList[7]   = general.tambores_industria.kilos_tambor
     _FldNameList[8]   = general.tambores_industria.tara
     _FldNameList[9]   = general.tambores_industria.id_articulo
     _FldNameList[10]   = general.productos_terminados.abreviatura
     _FldNameList[11]   = general.tambores_industria.id_calidad
     _FldNameList[12]   = general.calidades.abreviatura
...
label 'descripcion( locaciones ) '
  ...
  menu-item tambores_industria-id_etiqueta
label 'id_etiqueta( tambores_industria ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=tambores_industria.id_etiqueta').
                  run set-attribute-list('titulo=Etiqueta').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by tambores_industria.id_etiqueta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by tambores_industria.id_tambor
...
case p:
  ...
  WHEN 'tambores_industria.id_sucursal_ubicacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_sucursal_ubicacion = integer(valor)  no-lock no-error .
  WHEN 'tambores_industria.id_locacion_ubicacion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_locacion_ubicacion = integer(valor)  no-lock no-error .
  WHEN 'tambores_industria.id_etiqueta' then  
