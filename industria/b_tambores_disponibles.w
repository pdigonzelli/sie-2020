...
&Scoped-define FIELDS-IN-QUERY-br_table tt_tambores.id_lote tt_tambores.anio tt_tambores.id_tambor tt_tambores.id_articulo tt_tambores.id_calidad tt_tambores.kilos_tambor tt_tambores.elegido   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table   
&Scoped-define SELF-NAME br_table
&Scoped-define OPEN-QUERY-br_table RUN carga-datos. OPEN QUERY {&SELF-NAME} FOR EACH tt_tambores WHERE ~{&KEY-PHRASE} NO-LOCK     ~{&SORTBY-PHRASE}.
...
DEFINE VAR v_id_tipotambor AS INTEGER.
DEFINE VAR v_id_articulo AS INTEGER.
DEFINE VAR v_id_lote AS INTEGER.
...
RUN dame-parametros IN hcon (OUTPUT v_id_tipotambor,
                             OUTPUT v_id_articulo,
                             OUTPUT v_id_lote,
                             OUTPUT v_anio,
                             OUTPUT v_sucursal).

FIND FIRST tipostambor WHERE tipostambor.id_tipotambor = v_id_tipotambor NO-LOCK NO-ERROR.
IF AVAILABLE tipostambor THEN DO:
    IF tipostambor.tabla = "tambores_industria" THEN DO:
    
        FOR EACH tambores_industria WHERE tambores_industria.id_tipotambor          = v_id_tipotambor
                                      AND tambores_industria.id_articulo            = v_id_articulo
                                      AND tambores_industria.id_lote                = 0
                                      AND tambores_industria.anio                   = v_anio
    ...
    END.
    ELSE DO:
        RUN p_busca_tabla_cabecera.p (INPUT v_id_tipotambor,
                                      INPUT v_id_articulo,
                                      INPUT v_id_lote,
                                      INPUT v_anio,
                                      INPUT v_sucursal,
                                      OUTPUT p_id_empresa,
                                      OUTPUT p_id_sucursal,
                                      OUTPUT p_id_tipotambor,
                                      OUTPUT p_nromov) tipostambor.tabla.
    
        FOR EACH tambores_industria WHERE tambores_industria.id_empresa             = p_id_empresa
                                      AND tambores_industria.id_sucursal            = p_id_sucursal
                                      AND tambores_industria.id_tipotambor          = p_id_tipotambor
                                      AND tambores_industria.nromov                 = p_nromov
                                      AND tambores_industria.id_sucursal_ubicacion  = v_sucursal
                                      AND tambores_industria.id_locacion_ubicacion  = 4
                                      NO-LOCK
                                      BY tambores_industria.id_tambor.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dame-tabla B-table-Win 
PROCEDURE dame-tabla :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/*
RUN carga-datos.
MESSAGE "Se calcularon los datos" VIEW-AS ALERT-BOX.
*/
