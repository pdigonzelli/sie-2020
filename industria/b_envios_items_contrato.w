...
&Scoped-define FIELDS-IN-QUERY-br_table envios_items.semana_entrega ~
envios_items.Cantidad envios_items.total_tambores ~
envios_items.comision_broker 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   = general.envios_items.semana_entrega
     _FldNameList[2]   = general.envios_items.Cantidad
     _FldNameList[3]   = general.envios_items.total_tambores
     _FldNameList[4]   = general.envios_items.comision_broker
...
label 'total_tambores( envios_items ) '
  ...
  menu-item envios_items-comision_broker
label 'comision_broker( envios_items ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=envios_items.comision_broker').
                  run set-attribute-list('titulo=Comisión').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by envios_items.comision_broker
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by envios_items.semana_entrega
...
case p:
  ...
  WHEN 'envios_items.comision_broker' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  comision_broker = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
