...
DEFINE VAR v_cantidad_tambores AS INTEGER.
...
if available lotes_aceite then
 do:
    ...
    RUN w_cantidad_tambores.w (OUTPUT v_cantidad_tambores).
    
    TOTAL = v_cantidad_tambores.
    ...
    */
    ...
    IF desde = 0 AND hasta = 0 THEN DO:
        FOR EACH tambores_industria WHERE tambores_industria.id_empresa     = lotes_aceite.id_empresa
            ...
            /*IF tambores_industria.id_articulo = 58 THEN kilos = 200.
            IF tambores_industria.id_articulo = 51 THEN kilos = 180. */
            
            IF tambores_industria.id_tambor = 1 THEN DO:
                ...
                IF tambores_industria.codigo_lote <> "" THEN 
                  ASSIGN cLot = tambores_industria.codigo_lote
                         cLot = STRING(integer(ENTRY(3, cLot, ".")), "999") + "." + ENTRY(4, cLot, ".").
                ELSE
                  cLot = STRING(lotes_aceite.id_lote,">>99")  + "/" + SUBSTRING(STRING(tambores_industria.anio),3,2).
                ...
                if available sucursales then do:
                    ...
                    if available productos_terminados THEN DO: 
                       if tambores_industria.id_sucursal = 96 then do: 
                ...
                put control "^FO510,710^A0R,50,50^FD" cLot "^FS".
            
                put control "^FO410,32^A0R,50,60^FD" "DRUM N." "^FS".
                put control "^FO410,710^A0R,50,50^FD" "00/" + string(total,">>9") "^FS".
            
                put control "^FO310,32^A0R,50,60^FD" "GROSS WEIGHT" "^FS".
                put control "^FO310,710^A0R,50,50^FD" string((round(tambores_industria.tara,2) + tambores_industria.kilos_tambor),"99999.99") "^FS".
                ...
                put control "^FO120,710^A0R,50,50^FD" string(tambores_industria.kilos_tambor,"99999.99") "^FS".
      ...
      ELSE do:
          ...
          do ii = desde to hasta:
            FIND FIRST tambores_industria of lotes_aceite where tambores_industria.id_tambor = ii
                                          no-lock no-error.
          
            if available tambores_industria THEN do:  
              /*by facundo 08/08/2006 */
              DEFINE VARIABLE cNro AS CHARACTER  NO-UNDO.
              DEFINE VARIABLE cAno AS CHARACTER  NO-UNDO.
              IF tambores_industria.codigo_lote <> "" THEN 
                ASSIGN cLot = tambores_industria.codigo_lote
                       cNro = ENTRY(1, ENTRY(4, cLot, "."), "/")
                       cAno = ENTRY(2, ENTRY(4, cLot, "."), "/") 
                       cLot = ENTRY(3, cLot, ".") + "." + STRING(INTEGER(cNro), "999")  + "/" + cAno
                       .
              ELSE
                cLot = STRING(lotes_aceite.id_lote,">>99")  + "/" + SUBSTRING(STRING(tambores_industria.anio),3,2).
