...
lotes_jugo.id_lote productos_terminados.descripcion lotes_jugo.id_calidad ~
lotes_jugo.Pulpa lotes_jugo.Fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_jugo WHERE ~{&KEY-PHRASE} ~
      AND if (v_desde = ? or v_hasta = ?) then lotes_jugo.Fecha >= today - 30 else (lotes_jugo.fecha >= v_desde and lotes_jugo.fecha <= v_hasta) ~
 AND lotes_jugo.id_tipotambor = 3 NO-LOCK, ~
      EACH productos_terminados OF lotes_jugo NO-LOCK, ~
      EACH sucursales OF lotes_jugo NO-LOCK ~
    BY lotes_jugo.Fecha DESCENDING.
...
&Scoped-Define ENABLED-OBJECTS desde hasta BUTTON-34 br_table 
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_calidad,id_sucursal"':U).
...
DEFINE FRAME F-Main
     desde AT ROW 1 COL 10 COLON-ALIGNED
     hasta AT ROW 1 COL 34 COLON-ALIGNED
     BUTTON-34 AT ROW 1 COL 61
...
/* BROWSE-TAB br_table BUTTON-34 F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_jugo,general.productos_terminados OF general.lotes_jugo,comercial.sucursales OF general.lotes_jugo"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ",,"
     _OrdList          = "general.lotes_jugo.Fecha|no"
     _Where[1]         = "if (v_desde = ? or v_hasta = ?) then general.lotes_jugo.Fecha >= today - 30 else (lotes_jugo.fecha >= v_desde and lotes_jugo.fecha <= v_hasta)
 AND general.lotes_jugo.id_tipotambor = 3"
     _FldNameList[1]   = comercial.sucursales.nombre
     _FldNameList[2]   > general.lotes_jugo.id_lote
"lotes_jugo.id_lote" "Lote" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = general.productos_terminados.descripcion
     _FldNameList[4]   > general.lotes_jugo.id_calidad
"lotes_jugo.id_calidad" "Calidad" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = general.lotes_jugo.Pulpa
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE habilita-boton B-table-Win 
PROCEDURE habilita-boton :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
if lotes_jugo.id_articulo = 53 then run habilitar_boton in hcontainer. else run deshabilitar_boton in hcontainer.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
message "ATENCIadasdasdasdasdasdON" lotes_jugo.id_articulo view-as alert-box. 
  /* Code placed here will execute PRIOR to standard behavior. */
{custom/support/crow-available.i}
 
MESSAGE "nosequepasaaaaaaaaaaaaaaa". 
 
message "ATENCION" lotes_jugo.id_articulo view-as alert-box. 
 
  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'row-available':U ) .

message "ATENCION2" lotes_jugo.id_articulo view-as alert-box. 
