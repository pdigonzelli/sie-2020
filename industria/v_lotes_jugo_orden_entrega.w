...
&Scoped-define FIELD-PAIRS~
 ~{&FP1}id_orden_entrega ~{&FP2}id_orden_entrega ~{&FP3}
...
DEFINE FRAME F-Main
     lotes_jugo.id_orden_entrega AT ROW 1 COL 6 COLON-ALIGNED FORMAT ">>>>>9"
          VIEW-AS FILL-IN 
          SIZE 11.8 BY 1
     tambores AT ROW 1 COL 38 COLON-ALIGNED
     tambores_asignados AT ROW 2.43 COL 20 COLON-ALIGNED
     tambores_free AT ROW 2.43 COL 56 COLON-ALIGNED
...
run wc_orden_entrega.w(output r).
find orden_entrega where rowid(orden_entrega) = r no-lock no-error.
if available orden_entrega then 
    do:
        general.lotes_jugo.id_orden_entrega:screen-value = string(orden_entrega.id_orden_entrega).
        /* run descriptivos1. */
    end.
end.
...
if lotes_jugo.control_calidad then
    do:
        if lotes_jugo.microbiologia then
         do:
