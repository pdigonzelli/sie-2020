...
DEFINE VAR nroloteremito AS INTEGER.
DEFINE VAR nroanio AS INTEGER.
...
&Scoped-define INTERNAL-TABLES items_factura productos_terminados ~
envases_prod
...
&Scoped-define FIELDS-IN-QUERY-br_table items_factura.id_articulo ~
productos_terminados.abreviatura items_factura.id_envase ~
items_factura.id_calidad ~
SUBSTRING (items_factura.nro_lote,1,4) @ nroloteremito ~
items_factura.desde_lote ~
2000 + integer(substring(items_factura.nro_lote,6,2)) @ nroanio ~
items_factura.hasta_lote items_factura.cantidad items_factura.peso ~
items_factura.c_usuario items_factura.c_fecha items_factura.c_hora 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH items_factura OF remitos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF items_factura NO-LOCK, ~
      EACH envases_prod OF items_factura NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table items_factura productos_terminados ~
envases_prod
&Scoped-define FIRST-TABLE-IN-QUERY-br_table items_factura
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      items_factura.id_articulo COLUMN-LABEL "Art" FORMAT ">>9":U
            WIDTH 5.2
      productos_terminados.abreviatura COLUMN-LABEL "Producto" FORMAT "X(12)":U
      items_factura.id_envase FORMAT ">>9":U WIDTH 9.2
      items_factura.id_calidad COLUMN-LABEL "Cal" FORMAT ">>9":U
      SUBSTRING (items_factura.nro_lote,1,4) @ nroloteremito COLUMN-LABEL "Lote" FORMAT "9999":U
            WIDTH 7
      items_factura.desde_lote FORMAT ">>>9":U WIDTH 8.2
      2000 + integer(substring(items_factura.nro_lote,6,2)) @ nroanio COLUMN-LABEL "Año" FORMAT "9999":U
            WIDTH 8.2
      items_factura.hasta_lote FORMAT ">>>9":U WIDTH 7.2
      items_factura.cantidad FORMAT ">>>9":U WIDTH 11.2
      items_factura.peso FORMAT ">>>>9.99":U WIDTH 10.4
      items_factura.c_usuario FORMAT "x(12)":U
      items_factura.c_fecha FORMAT "99/99/99":U
      items_factura.c_hora FORMAT "x(8)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 148 BY 5.24
         BGCOLOR 15 FONT 0 ROW-HEIGHT-CHARS .67.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.items_factura OF general.remitos,general.productos_terminados OF general.items_factura,general.envases_prod OF general.items_factura"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.items_factura.id_articulo
"items_factura.id_articulo" "Art" ">>9" "integer" ? ? ? ? ? ? no ? no no "5.2" yes no no "U" "" ""
     _FldNameList[2]   > general.productos_terminados.abreviatura
"productos_terminados.abreviatura" "Producto" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.items_factura.id_envase
"items_factura.id_envase" ? ">>9" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[4]   > general.items_factura.id_calidad
"items_factura.id_calidad" "Cal" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > "_<CALC>"
"SUBSTRING (items_factura.nro_lote,1,4) @ nroloteremito" "Lote" "9999" ? ? ? ? ? ? ? no ? no no "7" yes no no "U" "" ""
     _FldNameList[6]   > general.items_factura.desde_lote
"items_factura.desde_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[7]   > "_<CALC>"
"2000 + integer(substring(items_factura.nro_lote,6,2)) @ nroanio" "Año" "9999" ? ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[8]   > general.items_factura.hasta_lote
"items_factura.hasta_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[9]   > general.items_factura.cantidad
"items_factura.cantidad" ? ">>>9" "decimal" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[10]   > general.items_factura.peso
"items_factura.peso" ? ">>>>9.99" "decimal" ? ? ? ? ? ? no ? no no "10.4" yes no no "U" "" ""
     _FldNameList[11]   = general.items_factura.c_usuario
     _FldNameList[12]   = general.items_factura.c_fecha
     _FldNameList[13]   = general.items_factura.c_hora
