...
rowObject.id_produccion rowObject.anio rowObject.Tambores ~
rowObject.Articulo rowObject.Fecha rowObject.litros rowObject.pesticida ~
rowObject.nromov rowObject.codigo_prod 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      id_produccion FORMAT ">>>,>>9":U
      anio FORMAT ">>>9":U
      Tambores FORMAT ">>>9":U
      Articulo FORMAT "x(50)":U
      Fecha FORMAT "99/99/9999":U
      litros FORMAT "->,>>>,>>9":U
      pesticida FORMAT "conTratamiento/sinTratamiento":U
      nromov FORMAT ">>>,>>>,>>9":U
      codigo_prod FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"Articulo" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     ...
     _FldNameList[7]   > _<SDO>.rowObject.litros
"litros" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[8]   > _<SDO>.rowObject.pesticida
"pesticida" ? ? "logical" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[9]   > _<SDO>.rowObject.nromov
"nromov" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[10]   = _<SDO>.rowObject.codigo_prod
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND hCont:FILE-NAME MATCHES "*wProduccionJugo*" THEN DO:
    RUN displayTambores IN hCont.
  END. 
END.
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND hCont:FILE-NAME MATCHES "*wProduccionJugo*" THEN DO:
    ...
    ASSIGN rowObject.id_sucursal:BGCOLOR IN BROWSE br_table   = ibgcolor
           rowObject.id_produccion:BGCOLOR IN BROWSE br_table = ibgcolor
           ...
           rowObject.pesticida:BGCOLOR IN BROWSE br_table     = ibgcolor
           rowObject.nromov:BGCOLOR IN BROWSE br_table        = ibgcolor
           rowObject.fecha:BGCOLOR IN BROWSE br_table         = ibgcolor
           rowObject.codigo_prod:BGCOLOR IN BROWSE br_table   = ibgcolor
           .
  END. 
