...
tambores_industria.kilos_tambor sucursales.abreviatura ~
tambores_industria.id_contrato_of tambores_industria.id_tipocontrato_of ~
tambores_industria.anio_of 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH tambores_industria OF lotes_jugo WHERE ~{&KEY-PHRASE} NO-LOCK, ~
...
id_articulo||y|general.tambores_industria.id_articulo
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_envase,id_orden_entrega,id_articulo,id_etiqueta,id_tipotambor"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      sucursales.abreviatura COLUMN-LABEL "Sucursal" FORMAT "X(10)":U
            WIDTH 12.2
      tambores_industria.id_contrato_of FORMAT "X(12)":U
      tambores_industria.id_tipocontrato_of COLUMN-LABEL "TipoOF" FORMAT ">>9":U
      tambores_industria.anio_of FORMAT ">>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 67 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.tambores_industria OF general.lotes_jugo,industria.locaciones WHERE general.tambores_industria ...,comercial.sucursales WHERE general.tambores_industria ..."
...
"tambores_industria.id_tambor" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[2]   > general.tambores_industria.kilos_tambor
"tambores_industria.kilos_tambor" ? ">>>>9.99" "decimal" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[3]   > comercial.sucursales.abreviatura
"sucursales.abreviatura" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[4]   = general.tambores_industria.id_contrato_of
     _FldNameList[5]   > general.tambores_industria.id_tipocontrato_of
"tambores_industria.id_tipocontrato_of" "TipoOF" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > general.tambores_industria.anio_of
"tambores_industria.anio_of" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_articulo" "tambores_industria" "id_articulo"}
