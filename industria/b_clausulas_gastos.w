...
&Scoped-define INTERNAL-TABLES r_clausulas_gastos gastos_venta clausulas

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table r_clausulas_gastos.id_clausula ~
clausulas.descripcion r_clausulas_gastos.id_gasto_venta ~
gastos_venta.abreviatura r_clausulas_gastos.programa 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_clausulas_gastos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH gastos_venta WHERE gastos_venta.id_gasto = r_clausulas_gastos.id_gasto_venta NO-LOCK, ~
      EACH clausulas OF r_clausulas_gastos NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table r_clausulas_gastos gastos_venta ~
clausulas
&Scoped-define FIRST-TABLE-IN-QUERY-br_table r_clausulas_gastos
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      r_clausulas_gastos.id_clausula COLUMN-LABEL "Cod." FORMAT ">>9"
      clausulas.descripcion COLUMN-LABEL "Clausula" FORMAT "x(5)"
      r_clausulas_gastos.id_gasto_venta COLUMN-LABEL "Cod." FORMAT ">>9"
      gastos_venta.abreviatura
      r_clausulas_gastos.programa
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_clausulas_gastos,industria.gastos_venta WHERE general.r_clausulas_gastos ...,general.clausulas OF general.r_clausulas_gastos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[2]      = "general.gastos_venta.id_gasto = general.r_clausulas_gastos.id_gasto_venta"
     _FldNameList[1]   > general.r_clausulas_gastos.id_clausula
"general.r_clausulas_gastos.id_clausula" "Cod." ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > general.clausulas.descripcion
"general.clausulas.descripcion" "Clausula" "x(5)" "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > general.r_clausulas_gastos.id_gasto_venta
"general.r_clausulas_gastos.id_gasto_venta" "Cod." ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[4]   = general.gastos_venta.abreviatura
     _FldNameList[5]   = general.r_clausulas_gastos.programa
...
define sub-menu Ordena 
  menu-item r_clausulas_gastos-id_clausula
label 'id_clausula( r_clausulas_gastos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_clausulas_gastos.id_clausula').
                  run set-attribute-list('titulo=Cod. Clausula').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_clausulas_gastos.id_clausula
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item r_clausulas_gastos-id_gasto_venta
label 'id_gasto_venta( r_clausulas_gastos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_clausulas_gastos.id_gasto_venta').
                  run set-attribute-list('titulo=Cod.Gastos').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_clausulas_gastos.id_gasto_venta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item r_clausulas_gastos-programa
label 'programa( r_clausulas_gastos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_clausulas_gastos.programa').
                  run set-attribute-list('titulo=Programa').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_clausulas_gastos.programa
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by r_clausulas_gastos.id_clausula
...
case p:
  WHEN 'r_clausulas_gastos.id_clausula' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_clausula = integer(valor)  no-lock no-error .
  WHEN 'r_clausulas_gastos.id_gasto_venta' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_gasto_venta = integer(valor)  no-lock no-error .
  WHEN 'r_clausulas_gastos.programa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  programa begins valor  no-lock no-error .
end.
END PROCEDURE.
