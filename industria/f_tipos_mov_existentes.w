...
&Scoped-Define ENABLED-OBJECTS RECT-15 RECT-2 RECT-17 
...
DEFINE VARIABLE h_b_tipos_mov_existentes AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_tipos_mov_existentes AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.33 , 38.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.33 , 72.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.33 , 95.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tipos_mov_existentes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tipos_mov_existentes ).
       RUN set-position IN h_b_tipos_mov_existentes ( 3.95 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 124.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_tipos_mov_existentes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_tipos_mov_existentes ).
       RUN set-position IN h_v_tipos_mov_existentes ( 11.86 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.05 , 142.00 ) */

       /* Links to csmartbrowser h_b_tipos_mov_existentes. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tipos_mov_existentes ).

       /* Links to SmartViewer h_v_tipos_mov_existentes. */
       RUN add-link IN adm-broker-hdl ( h_b_tipos_mov_existentes , 'record':U , h_v_tipos_mov_existentes ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_tipos_mov_existentes ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tipos_mov_existentes ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_tipos_mov_existentes ,
             h_b_tipos_mov_existentes , 'AFTER':U ).
    END. /* Page 0 */
