...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.lote rowObject.cantidad ~
rowObject.id_lote_deposito 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table rowObject.cantidad ~
rowObject.id_lote_deposito 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      rowObject.id_lote_deposito FORMAT "X(40)":U
  ENABLE
      rowObject.cantidad
      rowObject.id_lote_deposito HELP "?"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Settings for THIS-PROCEDURE
   ...
   Data Source: "ditemingresoloteubicacion.w"
   Allow: Basic,Browse
   Frames: 1
...
"rowObject.cantidad" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > _<SDO>.rowObject.id_lote_deposito
"rowObject.id_lote_deposito" ? ? "character" ? ? ? ? ? ? yes "?" no no ? yes no no "U" "" ""
...
DO:
  ...
  IF VALID-HANDLE (hColumn) THEN
    RUN CustomCreateSearchField (hColumn:NAME).
END.
