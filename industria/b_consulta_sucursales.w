...
&Scoped-define FIELDS-IN-QUERY-br_table sucursales.id_sucursal ~
sucursales.nombre sucursales.RNE 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table sucursales.id_sucursal ~
sucursales.nombre sucursales.RNE 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}id_sucursal ~{&FP2}id_sucursal ~{&FP3}~
 ~{&FP1}nombre ~{&FP2}nombre ~{&FP3}~
 ~{&FP1}RNE ~{&FP2}RNE ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table sucursales
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table sucursales
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH sucursales WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table sucursales
&Scoped-define FIRST-TABLE-IN-QUERY-br_table sucursales
...
<FOREIGN-KEYS>
id_tipo_sucursal|y|y|comercial.sucursales.id_tipo_sucursal
id_categoria||y|comercial.sucursales.id_categoria
id_sucursal||y|comercial.sucursales.id_sucursal
codigo_dgi||y|comercial.sucursales.codigo_dgi
abreviatura||y|comercial.sucursales.abreviatura
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
RUN set-attribute-list (
    'Keys-Accepted = "id_tipo_sucursal",
     Keys-Supplied = "id_tipo_sucursal,id_categoria,id_sucursal,codigo_dgi,abreviatura"':U).
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 77 BY 11.43
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > comercial.sucursales.id_sucursal
"sucursales.id_sucursal" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > comercial.sucursales.nombre
"sucursales.nombre" ? ? "character" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > comercial.sucursales.RNE
"sucursales.RNE" ? ? "character" ? ? ? ? ? ? yes ?
...
label 'nombre( sucursales ) '
  ...
  menu-item sucursales-RNE
label 'RNE( sucursales ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sucursales.RNE').
                  run set-attribute-list('titulo=RNE').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by sucursales.RNE
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by sucursales.id_sucursal
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'id_tipo_sucursal':U THEN DO:
       &Scope KEY-PHRASE sucursales.id_tipo_sucursal eq INTEGER(key-value)
       {&OPEN-QUERY-{&BROWSE-NAME}}
    END. /* id_tipo_sucursal */
    OTHERWISE DO:
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_tipo_sucursal" "sucursales" "id_tipo_sucursal"}
  {src/adm/template/sndkycas.i "id_categoria" "sucursales" "id_categoria"}
  {src/adm/template/sndkycas.i "id_sucursal" "sucursales" "id_sucursal"}
  {src/adm/template/sndkycas.i "codigo_dgi" "sucursales" "codigo_dgi"}
  {src/adm/template/sndkycas.i "abreviatura" "sucursales" "abreviatura"}
