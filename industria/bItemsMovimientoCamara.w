...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.item rowObject.Origen ~
rowObject.TipoMov rowObject.Camara rowObject.nro_fila rowObject.nro_columna ~
rowObject.cantidad rowObject.destino rowObject.id_origen ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      rowObject.Origen FORMAT "x(20)":U
      rowObject.TipoMov FORMAT "x(20)":U
      rowObject.Camara FORMAT "x(25)":U
      rowObject.nro_fila FORMAT "X(4)":U
      rowObject.nro_columna FORMAT "X(4)":U
      rowObject.cantidad FORMAT ">>>>>9":U
      rowObject.destino FORMAT "X(50)":U
      rowObject.id_origen FORMAT ">>>>>9":U
      rowObject.id_tipo_movimiento FORMAT "9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"rowObject.item" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.Origen
"rowObject.Origen" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[3]   > _<SDO>.rowObject.TipoMov
"rowObject.TipoMov" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[4]   = _<SDO>.rowObject.Camara
     _FldNameList[5]   = _<SDO>.rowObject.nro_fila
     _FldNameList[6]   = _<SDO>.rowObject.nro_columna
     _FldNameList[7]   = _<SDO>.rowObject.cantidad
     _FldNameList[8]   = _<SDO>.rowObject.destino
     _FldNameList[9]   > _<SDO>.rowObject.id_origen
"rowObject.id_origen" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[10]   = _<SDO>.rowObject.id_tipo_movimiento
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND hCont:FILE-NAME MATCHES "*wqOrigenTambores.w" OR hCont:FILE-NAME MATCHES "*wMovimientosCamara.w" THEN DO:
    CASE rowObject.id_tipo_movimiento:
      WHEN 1 THEN iBgColor = 23.
      WHEN 2 THEN iBgColor = 24.
      WHEN 3 THEN iBgColor = 22.
    END CASE.
    
    ASSIGN rowObject.ITEM:BGCOLOR IN BROWSE br_table                = ibgcolor
           rowObject.id_origen:BGCOLOR IN BROWSE br_table           = ibgcolor
           rowObject.nro_fila:BGCOLOR IN BROWSE br_table            = ibgcolor
           rowObject.nro_columna:BGCOLOR IN BROWSE br_table         = ibgcolor
           rowObject.cantidad:BGCOLOR IN BROWSE br_table            = ibgcolor
           rowObject.destino:BGCOLOR IN BROWSE br_table             = ibgcolor
           rowObject.id_tipo_movimiento:BGCOLOR IN BROWSE br_table  = ibgcolor
           rowObject.origen:BGCOLOR IN BROWSE br_table              = ibgcolor
           rowObject.tipomov:BGCOLOR IN BROWSE br_table             = ibgcolor
           rowObject.camara:BGCOLOR IN BROWSE br_table              = ibgcolor.
  END.  
