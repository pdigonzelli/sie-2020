...
DEFINE INPUT PARAMETER p_aduana AS INTEGER.
DEFINE INPUT PARAMETER p_anio AS INTEGER.
DEFINE INPUT PARAMETER p_permiso_embarque AS CHAR.
...
&Scoped-Define DISPLAYED-OBJECTS saldo 
...
DEFINE VARIABLE h_b_r_cobranzas_pe AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_r_cobranzas_pe AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE VARIABLE saldo AS DECIMAL FORMAT "->>>,>>>,>>9.9999":U INITIAL 0 
     LABEL "Saldo" 
     VIEW-AS FILL-IN 
     SIZE 23 BY 1
...
DEFINE FRAME F-Main
     saldo AT ROW 11.24 COL 55 COLON-ALIGNED
     RECT-2 AT Y 0 X 20
     RECT-4 AT ROW 10.76 COL 2
...
/* SETTINGS FOR FRAME F-Main
                                                                        */
/* SETTINGS FOR FILL-IN saldo IN FRAME F-Main
   NO-ENABLE                                                            */
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 5.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 39.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 62.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_r_cobranzas_pe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_r_cobranzas_pe ).
       RUN set-position IN h_b_r_cobranzas_pe ( 3.86 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_r_cobranzas_pe ( 6.71 , 87.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_r_cobranzas_pe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_r_cobranzas_pe ).
       RUN set-position IN h_v_r_cobranzas_pe ( 13.38 , 8.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.10 , 78.00 ) */

       /* Links to csmartbrowser h_b_r_cobranzas_pe. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_r_cobranzas_pe ).

       /* Links to SmartViewer h_v_r_cobranzas_pe. */
       RUN add-link IN adm-broker-hdl ( h_b_r_cobranzas_pe , 'Record':U , h_v_r_cobranzas_pe ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_r_cobranzas_pe ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-updsav ,
             saldo:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_r_cobranzas_pe ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_r_cobranzas_pe ,
             saldo:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE calculo_saldo W-Win 
PROCEDURE calculo_saldo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR v_cobranzas AS DECIMAL.

FIND permisos_embarque WHERE permisos_embarque.id_aduana            = p_aduana
                         AND permisos_embarque.anio                 = p_anio
                         AND permisos_embarque.id_permiso_embarque  = p_permiso_embarque
                         NO-LOCK NO-ERROR.
...
DO:
    FOR EACH r_cobranzas_pe WHERE r_cobranzas_pe.id_aduana = permisos_embarque.id_aduana
                              ...
                              NO-LOCK.
        v_cobranzas = v_cobranzas + r_cobranzas_pe.importe.
    END.
    v_importe = permisos_embarque.importe - v_cobranzas.
    saldo:SCREEN-VALUE IN FRAME F-Main = STRING(v_importe).
END.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE paso-rowid-pe W-Win 
PROCEDURE paso-rowid-pe :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE OUTPUT PARAMETER p1_aduana AS INTEGER.
DEFINE OUTPUT PARAMETER p1_anio AS INTEGER.
DEFINE OUTPUT PARAMETER p1_permiso_embarque AS CHAR.

p1_aduana = p_aduana.
p1_anio     = p_anio.
p1_permiso_embarque = p_permiso_embarque.
END PROCEDURE.
