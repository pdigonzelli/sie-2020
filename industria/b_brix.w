...
&Scoped-define INTERNAL-TABLES brix

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table brix.brix brix.pe 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table brix.pe 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}pe ~{&FP2}pe ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table brix
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table brix
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH brix WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table brix
&Scoped-define FIRST-TABLE-IN-QUERY-br_table brix
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 33 BY 19.62
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.brix"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = general.brix.brix
     _FldNameList[2]   > general.brix.pe
"pe" ? ? "decimal" ? ? ? ? ? ? yes ?
...
label 'brix( brix ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=brix.brix').
                  run set-attribute-list('titulo=Brix').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by brix.brix
...
label 'pe( brix ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=brix.pe').
                  run set-attribute-list('titulo=Pe').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by brix.pe
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by brix.brix
...
case p:
  WHEN 'brix.brix' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  brix = decimal(valor)  no-lock no-error .
  WHEN 'brix.pe' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  pe = decimal(valor)  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "brix" "brix" "brix"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "brix"}
