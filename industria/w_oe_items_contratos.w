...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-1 FILL-IN-5 FILL-IN-6 
...
DEFINE VARIABLE h_b_consulta_items_contratos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_consulta_oe AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "OE" 
     VIEW-AS FILL-IN 
     SIZE 49 BY .95
     BGCOLOR 3 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE FILL-IN-5 AS CHARACTER FORMAT "X(256)":U INITIAL "PARTES DE OE" 
     VIEW-AS FILL-IN 
     SIZE 102 BY .95
     BGCOLOR 4 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE FILL-IN-6 AS CHARACTER FORMAT "X(256)":U INITIAL "PARTES DE CONTRATOS" 
     VIEW-AS FILL-IN 
     SIZE 117 BY .95
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 3.14 COL 2 NO-LABEL
     FILL-IN-5 AT ROW 3.14 COL 53 NO-LABEL
     FILL-IN-6 AT ROW 11.48 COL 2 NO-LABEL
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.00 , 36.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.00 , 59.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_consulta_oe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_consulta_oe ).
       RUN set-position IN h_b_consulta_oe ( 4.33 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_consulta_oe ( 6.67 , 49.00 ) NO-ERROR.
       ...
       RUN set-position IN h_b_consulta_oe_item_oe ( 4.33 , 53.00 ) NO-ERROR.
       RUN set-size IN h_b_consulta_oe_item_oe ( 6.71 , 102.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_consulta_items_contratos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_consulta_items_contratos ).
       RUN set-position IN h_b_consulta_items_contratos ( 12.67 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_consulta_items_contratos ( 6.71 , 120.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_consulta_oe. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_consulta_oe ).

       /* Links to csmartbrowser h_b_consulta_oe_item_oe. */
       RUN add-link IN adm-broker-hdl ( h_b_consulta_oe , 'Record':U , h_b_consulta_oe_item_oe ).

       /* Links to csmartbrowser h_b_consulta_items_contratos. */
       RUN add-link IN adm-broker-hdl ( h_b_consulta_oe_item_oe , 'record':U , h_b_consulta_items_contratos ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_consulta_oe ,
             FILL-IN-5:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_consulta_oe_item_oe ,
             h_b_consulta_oe , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_consulta_items_contratos ,
             FILL-IN-6:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-1 FILL-IN-5 FILL-IN-6 
