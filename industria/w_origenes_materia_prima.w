...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-27 
...
DEFINE VARIABLE h_b_origenes_materia_prima AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_origenes_materia_prima AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Origenes de Materia Prima"
         HEIGHT             = 16.38
         WIDTH              = 79.6
         MAX-HEIGHT         = 20.19
         MAX-WIDTH          = 126.4
...
ON END-ERROR OF W-Win /* Origenes de Materia Prima */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Origenes de Materia Prima */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 1.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 35.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 58.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_origenes_materia_prima.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_origenes_materia_prima ).
       RUN set-position IN h_b_origenes_materia_prima ( 3.86 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b_origenes_materia_prima ( 6.71 , 68.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_origenes_materia_prima.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_origenes_materia_prima ).
       RUN set-position IN h_v_origenes_materia_prima ( 11.95 , 7.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.10 , 64.00 ) */

       /* Links to csmartbrowser h_b_origenes_materia_prima. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_origenes_materia_prima ).

       /* Links to SmartViewer h_v_origenes_materia_prima. */
       RUN add-link IN adm-broker-hdl ( h_b_origenes_materia_prima , 'Record':U , h_v_origenes_materia_prima ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_origenes_materia_prima ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_origenes_materia_prima ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_origenes_materia_prima ,
             h_b_origenes_materia_prima , 'AFTER':U ).
    END. /* Page 0 */
