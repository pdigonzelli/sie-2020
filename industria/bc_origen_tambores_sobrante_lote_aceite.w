...
&Scoped-define INTERNAL-TABLES tambores_industria sucursales envases_prod ~
...
&Scoped-define QUERY-STRING-br_table FOR EACH tambores_industria WHERE ~{&KEY-PHRASE} ~
 ...
 AND tambores_industria.id_articulo = articulo ~
 AND tambores_industria.id_locacion_ubicacion = 4) OR  ~
(industria.tambores_industria.id_tipotambor = 8 and (if v_articulo = 0 then ( ~
general.tambores_industria.id_articulo = 513 or ~
general.tambores_industria.id_articulo = 514 or tambores_industria.id_articulo = 516 or  ~
general.tambores_industria.id_articulo = 518 or tambores_industria.id_articulo = 519 or  ~
general.tambores_industria.id_articulo = 520 ) else true ) and  ~
general.tambores_industria.id_locacion_ubicacion = 4 and  ~
(if v_sucursal = 0 then true else (industria.tambores_industria.id_sucursal = v_sucursal)) and ~
(if v_articulo = 0 then true else (industria.tambores_industria.id_articulo = v_articulo)) and ~
(if v_lote = 0 then true else (industria.tambores_industria.id_lote = v_lote))) NO-LOCK, ~
    ...
    BY tambores_industria.id_sucursal ~
...
(if v_lote = 0 then true else (industria.tambores_industria.id_lote = v_lote))) NO-LOCK, ~
    ...
    BY tambores_industria.id_sucursal ~
       BY tambores_industria.id_lote ~
        BY tambores_industria.id_tambor.
&Scoped-define TABLES-IN-QUERY-br_table tambores_industria sucursales ~
...
&Scoped-define THIRD-TABLE-IN-QUERY-br_table envases_prod
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table productos_terminados


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS suc art lot BUTTON-13 br_table 
&Scoped-Define DISPLAYED-OBJECTS suc art lot 
...
DEFINE VARIABLE lot AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "NroLote" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.

DEFINE VARIABLE suc AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "Sucursal" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      productos_terminados.descripcion COLUMN-LABEL "Articulo" FORMAT "x(30)":U
      tambores_industria.id_tambor FORMAT ">>>>9":U WIDTH 9.2
      tambores_industria.kilos_tambor FORMAT ">>>>9.99":U WIDTH 11.2
      tambores_industria.Fecha FORMAT "99/99/9999":U
      tambores_industria.id_sucursal_ubicacion FORMAT ">,>>9":U
      tambores_industria.nromov FORMAT ">>>,>>>,>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 148 BY 15.38
         FONT 0.
...
DEFINE FRAME F-Main
     suc AT ROW 1 COL 12 COLON-ALIGNED
     art AT ROW 1 COL 40 COLON-ALIGNED
     lot AT ROW 1 COL 66 COLON-ALIGNED
     BUTTON-13 AT ROW 1 COL 90
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.tambores_industria,comercial.sucursales OF general.tambores_industria,general.envases_prod OF general.tambores_industria,general.productos_terminados OF general.tambores_industria"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ",,,"
     _OrdList          = "general.tambores_industria.id_sucursal|yes,industria.tambores_industria.id_lote|yes,industria.tambores_industria.id_tambor|yes"
     _Where[1]         = "(industria.tambores_industria.id_tipotambor = 8
 AND general.tambores_industria.id_empresa_destino = 0
 ...
 AND general.tambores_industria.id_articulo = articulo
 AND general.tambores_industria.id_locacion_ubicacion = 4) OR 
(industria.tambores_industria.id_tipotambor = 8 and (if v_articulo = 0 then (
general.tambores_industria.id_articulo = 513 or
general.tambores_industria.id_articulo = 514 or general.tambores_industria.id_articulo = 516 or 
general.tambores_industria.id_articulo = 518 or general.tambores_industria.id_articulo = 519 or 
general.tambores_industria.id_articulo = 520 ) else true ) and 
general.tambores_industria.id_locacion_ubicacion = 4 and 
(if v_sucursal = 0 then true else (industria.tambores_industria.id_sucursal = v_sucursal)) and
(if v_articulo = 0 then true else (industria.tambores_industria.id_articulo = v_articulo)) and
(if v_lote = 0 then true else (industria.tambores_industria.id_lote = v_lote)))"
     _FldNameList[1]   > comercial.sucursales.abreviatura
"comercial.sucursales.abreviatura" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.tambores_industria.id_lote
"general.tambores_industria.id_lote" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.tambores_industria.id_articulo
"general.tambores_industria.id_articulo" "Art." ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.descripcion
"general.productos_terminados.descripcion" "Articulo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.tambores_industria.id_tambor
"general.tambores_industria.id_tambor" ? ">>>>9" "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[6]   > general.tambores_industria.kilos_tambor
"general.tambores_industria.kilos_tambor" ? ">>>>9.99" "decimal" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[7]   = general.tambores_industria.Fecha
     _FldNameList[8]   = general.tambores_industria.id_sucursal_ubicacion
     _FldNameList[9]   = general.tambores_industria.nromov
...
DO:
  v_sucursal = integer(suc:screen-value in frame F-Main).
  v_articulo = integer(art:screen-value in frame F-Main).
  v_lote = integer(lot:screen-value in frame F-Main).
...
run get-articulo in hcontainer (output articulo, output fecha, output lote).
/* message articulo fecha empresa sucursal lote. */
