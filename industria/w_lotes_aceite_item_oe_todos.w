...
&Scoped-Define ENABLED-OBJECTS BUTTON-17 RECT-16 RECT-2 
...
DEFINE VARIABLE h_b_lotes_aceite_contratos_todos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cus-misc AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cus-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cus-update-only AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v_lotes_aceite_item_oe AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-17 AT ROW 1.71 COL 2
     RECT-16 AT ROW 11.48 COL 6
     RECT-2 AT Y 0 X 90
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Lotes de Aceite Orden Entrega"
         HEIGHT             = 14.86
         WIDTH              = 90.4
         MAX-HEIGHT         = 20.62
         MAX-WIDTH          = 123
...
ON END-ERROR OF W-Win /* Lotes de Aceite Orden Entrega */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Lotes de Aceite Orden Entrega */
DO:
...
ON CHOOSE OF BUTTON-17 IN FRAME F-Main /* Enviar Mail */
DO:
define var r as rowid.
run get-rowid1 in h_b_lotes_aceite_contratos_todos (output r).
find lotes_aceite where rowid(lotes_aceite) = r no-lock no-error.
if available lotes_aceite then
    do:
        find items_orden_entrega of lotes_aceite no-lock no-error.
        if available items_orden_entrega then
            do:
               run p_correo_aviso_contrato_item_oe.p (input 3, 
                                              input rowid(items_orden_entrega), 
                                              input "Se han relacionado los tambores").
            end.
    end.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'custom/objects/cus-update-only.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Save,
                     AddFunction = Multiple-Records':U ,
             OUTPUT h_cus-update-only ).
       RUN set-position IN h_cus-update-only ( 1.24 , 20.00 ) NO-ERROR.
       RUN set-size IN h_cus-update-only ( 1.91 , 20.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 40.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 63.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_aceite_contratos_todos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_aceite_contratos_todos ).
       RUN set-position IN h_b_lotes_aceite_contratos_todos ( 4.10 , 6.00 ) NO-ERROR.
       RUN set-size IN h_b_lotes_aceite_contratos_todos ( 6.71 , 81.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_lotes_aceite_item_oe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_lotes_aceite_item_oe ).
       RUN set-position IN h_v_lotes_aceite_item_oe ( 11.71 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.43 , 68.00 ) */

       /* Links to csmartbrowser h_b_lotes_aceite_contratos_todos. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_lotes_aceite_contratos_todos ).

       /* Links to SmartViewer h_v_lotes_aceite_item_oe. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_aceite_contratos_todos , 'Record':U , h_v_lotes_aceite_item_oe ).
       RUN add-link IN adm-broker-hdl ( h_cus-update-only , 'TableIO':U , h_v_lotes_aceite_item_oe ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-update-only ,
             BUTTON-17:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-navico ,
             h_cus-update-only , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             h_cus-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_aceite_contratos_todos ,
             BUTTON-17:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_lotes_aceite_item_oe ,
             h_b_lotes_aceite_contratos_todos , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE BUTTON-17 RECT-16 RECT-2 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE paso_lote W-Win 
PROCEDURE paso_lote :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
run get-rowid1 in h_b_lotes_aceite_contratos (output r).
END PROCEDURE.
