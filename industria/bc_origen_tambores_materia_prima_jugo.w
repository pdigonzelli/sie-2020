...
&Scoped-define INTERNAL-TABLES lotes_jugo productos_terminados sucursales ~
calidades tambores_industria origenes_materia_prima
...
lotes_jugo.id_calidad calidades.abreviatura 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_jugo WHERE ~{&KEY-PHRASE} ~
      AND lotes_jugo.Fecha <= fecha ~
 AND lotes_jugo.Fecha > 01/01/2001 NO-LOCK, ~
 ...
 AND tambores_industria.id_sucursal_ubicacion = sucursal NO-LOCK, ~
      EACH origenes_materia_prima WHERE origenes_materia_prima.id_articulo_mp = lotes_jugo.id_articulo ~
...
sucursales calidades tambores_industria origenes_materia_prima
...
&Scoped-define SIXTH-TABLE-IN-QUERY-br_table origenes_materia_prima
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      sucursales.nombre COLUMN-LABEL "Sucursal" FORMAT "x(20)":U
      lotes_jugo.id_lote COLUMN-LABEL "Lote" FORMAT ">>>9":U WIDTH 10.4
      lotes_jugo.id_articulo FORMAT ">>>9":U WIDTH 11.2
      productos_terminados.descripcion COLUMN-LABEL "Producto Terminado" FORMAT "x(30)":U
      lotes_jugo.id_calidad COLUMN-LABEL "CodCal" FORMAT ">>9":U
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_jugo,general.productos_terminados OF general.lotes_jugo,comercial.sucursales OF general.lotes_jugo,general.calidades OF general.lotes_jugo,industria.tambores_industria OF general.lotes_jugo,industria.origenes_materia_prima WHERE general.lotes_jugo ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,,, FIRST,"
     _Where[1]         = "general.lotes_jugo.Fecha <= fecha
 AND general.lotes_jugo.Fecha > 01/01/2001"
     _Where[5]         = "general.tambores_industria.id_locacion_ubicacion = 4
 AND general.tambores_industria.id_sucursal_ubicacion = sucursal"
     _JoinCode[6]      = "general.origenes_materia_prima.id_articulo_mp = general.lotes_jugo.id_articulo"
     _Where[6]         = "general.origenes_materia_prima.id_articulo_lote = articulo"
     _FldNameList[1]   > comercial.sucursales.nombre
"comercial.sucursales.nombre" "Sucursal" "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.lotes_jugo.id_lote
"general.lotes_jugo.id_lote" "Lote" ">>>9" "integer" ? ? ? ? ? ? no ? no no "10.4" yes no no "U" "" ""
     _FldNameList[3]   > general.lotes_jugo.id_articulo
"general.lotes_jugo.id_articulo" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.descripcion
"general.productos_terminados.descripcion" "Producto Terminado" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.lotes_jugo.id_calidad
"general.lotes_jugo.id_calidad" "CodCal" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > general.calidades.abreviatura
"general.calidades.abreviatura" "Calidad" ? "character" ? ? ? ? ? ? no ? no no "25.6" yes no no "U" "" ""
...
run get-articulo in hcontainer (output fecha, output sucursal, OUTPUT articulo).
