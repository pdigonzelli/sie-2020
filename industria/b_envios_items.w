...
&Scoped-define FIELDS-IN-QUERY-br_table envios_items.Cantidad ~
envios_items.semana_entrega 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table envios_items.Cantidad ~
envios_items.semana_entrega 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table~
 ~{&FP1}Cantidad ~{&FP2}Cantidad ~{&FP3}~
 ~{&FP1}semana_entrega ~{&FP2}semana_entrega ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table envios_items
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table envios_items
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH envios_items OF items_orden_fabricacion WHERE ~{&KEY-PHRASE} NO-LOCK ~
    BY envios_items.semana_entrega INDEXED-REPOSITION.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 55 BY 7.62
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "general.envios_items.semana_entrega|yes"
     _FldNameList[1]   > general.envios_items.Cantidad
"envios_items.Cantidad" ? ? "decimal" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > general.envios_items.semana_entrega
"envios_items.semana_entrega" ? ? "integer" ? ? ? ? ? ? yes ?
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
define var v_ultimo as integer.
define buffer  b_envios_items for envios_items.

find last b_envios_items no-lock no-error.
    if not available b_envios_items then
        v_ultimo = 1.
    else
        v_ultimo = b_envios_items.id_envio + 1.

   assign  envios_items.nro = items_orden_fabricacion.nro
            envios_items.anio = items_orden_fabricacion.anio
            envios_items.id_cliente = items_orden_fabricacion.id_cliente
            envios_items.item = items_orden_fabricacion.item
            envios_items.id_articulo = items_orden_fabricacion.id_articulo
            envios_items.id_calidad = items_orden_fabricacion.id_calidad
            envios_items.id_envase = items_orden_fabricacion.id_envase
            envios_items.id_envio = v_ultimo
            envios_items.c_usuario = userid("userdb")
            envios_items.c_hora = string(time,"HH:MM:SS").     
...
------------------------------------------------------------------------------*/
  case nombre:
    ...
    when "semana_entrega" then
        if integer(valor) > 52 then 
        do:
            mensaje = "No se puede ingresar una semana mayor de 52".
            return false.
         end.
