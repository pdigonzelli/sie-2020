...
&Scoped-define INTERNAL-TABLES r_productos_calidad calidades ~
...
&Scoped-define FIELDS-IN-QUERY-br_table r_productos_calidad.id_articulo ~
productos_terminados.descripcion r_productos_calidad.id_calidad ~
calidades.descripcion r_productos_calidad.rnpa 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH r_productos_calidad WHERE ~{&KEY-PHRASE} ~
      AND r_productos_calidad.id_articulo = v_articulo NO-LOCK, ~
      EACH calidades OF r_productos_calidad NO-LOCK, ~
      EACH productos_terminados OF r_productos_calidad NO-LOCK ~
    BY r_productos_calidad.id_calidad.
&Scoped-define TABLES-IN-QUERY-br_table r_productos_calidad calidades ~
productos_terminados
&Scoped-define FIRST-TABLE-IN-QUERY-br_table r_productos_calidad
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      r_productos_calidad.id_articulo COLUMN-LABEL "Art" FORMAT ">>9":U
      productos_terminados.descripcion FORMAT "x(30)":U WIDTH 30.8
      r_productos_calidad.id_calidad FORMAT ">>9":U WIDTH 11.2
      calidades.descripcion FORMAT "X(20)":U WIDTH 21
      r_productos_calidad.rnpa FORMAT "X(8)":U WIDTH 13.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 93 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.r_productos_calidad,general.calidades OF general.r_productos_calidad,general.productos_terminados OF general.r_productos_calidad"
     _Options          = "NO-LOCK KEY-PHRASE"
     _TblOptList       = ",,"
     _OrdList          = "general.r_productos_calidad.id_calidad|yes"
     _Where[1]         = "general.r_productos_calidad.id_articulo = v_articulo"
     _FldNameList[1]   > general.r_productos_calidad.id_articulo
"r_productos_calidad.id_articulo" "Art" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > general.productos_terminados.descripcion
"productos_terminados.descripcion" ? ? "character" ? ? ? ? ? ? no ? no no "30.8" yes no no "U" "" ""
     _FldNameList[3]   > general.r_productos_calidad.id_calidad
"r_productos_calidad.id_calidad" ? ? "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[4]   > general.calidades.descripcion
"calidades.descripcion" ? "X(20)" "character" ? ? ? ? ? ? no ? no no "21" yes no no "U" "" ""
     _FldNameList[5]   > general.r_productos_calidad.rnpa
"r_productos_calidad.rnpa" ? ? "character" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item r_productos_calidad-id_articulo
label 'id_articulo( r_productos_calidad ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_productos_calidad.id_articulo').
                  run set-attribute-list('titulo=Código').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_productos_calidad.id_articulo
...
label 'descripcion( envases_prod ) '
  ...
  menu-item r_productos_calidad-id_calidad
label 'id_calidad( r_productos_calidad ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_productos_calidad.id_calidad').
                  run set-attribute-list('titulo=Calidad').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_productos_calidad.id_calidad
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
label 'descripcion( calidades ) '
  ...
  menu-item r_productos_calidad-rnpa
label 'rnpa( r_productos_calidad ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=r_productos_calidad.rnpa').
                  run set-attribute-list('titulo=RNPA').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by r_productos_calidad.rnpa
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by r_productos_calidad.id_articulo
...
case p:
  WHEN 'r_productos_calidad.id_articulo' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_articulo = integer(valor)  no-lock no-error .
  WHEN 'r_productos_calidad.id_calidad' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_calidad = integer(valor)  no-lock no-error .
  WHEN 'r_productos_calidad.rnpa' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  rnpa begins valor  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  RUN dame-articulo IN h_con (OUTPUT v_articulo).
/*  MESSAGE "El articulo que trae es: " v_articulo VIEW-AS ALERT-BOX. */
