...
&Scoped-define INTERNAL-TABLES locaciones sucursales

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table locaciones.id_sucursal ~
sucursales.abreviatura locaciones.id_locacion locaciones.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH locaciones WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH sucursales OF locaciones NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table locaciones sucursales
&Scoped-define FIRST-TABLE-IN-QUERY-br_table locaciones
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      locaciones.id_sucursal FORMAT ">>9"
      sucursales.abreviatura
      locaciones.id_locacion FORMAT ">>9"
      locaciones.descripcion FORMAT "x(20)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 83 BY 5.48
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.locaciones,comercial.sucursales OF general.locaciones"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.locaciones.id_sucursal
"general.locaciones.id_sucursal" ? ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   = comercial.sucursales.abreviatura
     _FldNameList[3]   > general.locaciones.id_locacion
"general.locaciones.id_locacion" ? ">>9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[4]   > general.locaciones.descripcion
"general.locaciones.descripcion" ? "x(20)" "character" ? ? ? ? ? ? no ?
...
define sub-menu Ordena 
  menu-item locaciones-id_sucursal
label 'id_sucursal( locaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=locaciones.id_sucursal').
                  run set-attribute-list('titulo=Sucursal').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by locaciones.id_sucursal
...
label 'abreviatura( sucursales ) '
  ...
  menu-item locaciones-id_locacion
label 'id_locacion( locaciones ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=locaciones.id_locacion').
                  run set-attribute-list('titulo=Locacion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by locaciones.id_locacion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
