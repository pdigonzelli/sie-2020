...
DEFINE TEMP-TABLE tt_prod_terceros
      FIELD id_sucursal AS INTEGER COLUMN-LABEL "CodSuc" FORMAT ">>9"
      FIELD sucursal AS CHAR COLUMN-LABEL "Sucursal" FORMAT "x(30)"
      FIELD id_lote AS INTEGER COLUMN-LABEL "Lote" FORMAT ">>>>9"
      FIELD anio    AS INTEGER COLUMN-LABEL "Anio" FORMAT ">>>9"
      FIELD fecha AS DATE COLUMN-LABEL "Fecha"
      FIELD id_proveedor AS INTEGER COLUMN-LABEL "Cod." FORMAT ">>>>9"
      FIELD proveedor AS CHAR COLUMN-LABEL "Proveedor" FORMAT "x(30)"
      FIELD id_articulo AS INTEGER COLUMN-LABEL "Cod." FORMAT ">>>>9"
      FIELD articulo AS CHAR COLUMN-LABEL "Articulo" FORMAT "x(30)"
      FIELD cantidad AS INTEGER COLUMN-LABEL "Cantidad" FORMAT ">>>9"
      FIELD kilos AS DECIMAL COLUMN-LABEL "Kilos Netos" FORMAT ">>>>>9,999".
    RCODE-INFORMATION.
...
&Scoped-Define ENABLED-OBJECTS desde hasta BUTTON-34 RECT-2 
...
DEFINE FRAME F-Main
     desde AT ROW 2.43 COL 9 COLON-ALIGNED
     hasta AT ROW 2.43 COL 34 COLON-ALIGNED
     BUTTON-34 AT ROW 4.33 COL 36
     RECT-2 AT Y 0 X 285
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Exportacion Prod de Terceros"
         HEIGHT             = 6.29
         WIDTH              = 80.4
         MAX-HEIGHT         = 19.14
         MAX-WIDTH          = 107
...
ON END-ERROR OF W-Win /* Exportacion Prod de Terceros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Exportacion Prod de Terceros */
DO:
...
ON CHOOSE OF BUTTON-34 IN FRAME F-Main /* Aceptar */
DO:
...
FOR EACH tt_prod_terceros.
    DELETE tt_prod_terceros.
END.

FOR EACH productos_terceros WHERE productos_terceros.fecha >= v_desde
                              AND productos_terceros.fecha <= v_hasta
                            NO-LOCK.
    FOR EACH tambores_industria OF productos_terceros NO-LOCK.
        v_cantidad = v_cantidad + 1.
        v_kilos     = v_kilos + tambores_industria.kilos_tambor.
    END.
    
    FIND FIRST proveedores OF productos_terceros NO-LOCK NO-ERROR.
    FIND FIRST sucursales OF productos_terceros NO-LOCK NO-ERROR.
    FIND FIRST productos_terminados OF productos_terceros NO-LOCK NO-ERROR.
    CREATE tt_prod_terceros.
    ASSIGN tt_prod_terceros.id_sucursal     = productos_terceros.id_sucursal
           tt_prod_terceros.sucursal        = IF AVAILABLE sucursales THEN sucursales.abreviatura ELSE "NONE"
           tt_prod_terceros.id_lote         = productos_terceros.id_lote
           tt_prod_terceros.anio            = productos_terceros.anio
           tt_prod_terceros.fecha           = productos_terceros.fecha
           tt_prod_terceros.id_proveedor    = productos_terceros.id_proveedor
           tt_prod_terceros.proveedor       = IF AVAILABLE proveedores THEN proveedores.nombre ELSE "NONE"
           tt_prod_terceros.id_articulo     = productos_terceros.id_articulo
           tt_prod_terceros.articulo        = IF AVAILABLE productos_terminados THEN productos_terminados.descripcion ELSE "NONE"
           tt_prod_terceros.cantidad        = v_cantidad
           tt_prod_terceros.kilos           = v_kilos.

    v_cantidad = 0.
    v_kilos = 0.
END.

RUN generateExcel.p (INPUT TABLE tt_prod_terceros,
                     INPUT " Productos de Terceros",
                     INPUT " Desde: " + STRING(v_desde) + " hasta: " + STRING(v_hasta),
                     INPUT 7,
                     INPUT 8,
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             desde:HANDLE IN FRAME F-Main , 'BEFORE':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE desde hasta BUTTON-34 RECT-2 
      WITH FRAME F-Main IN WINDOW W-Win.
