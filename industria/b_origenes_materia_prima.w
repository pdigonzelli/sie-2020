...
&Scoped-define INTERNAL-TABLES origenes_materia_prima
...
origenes_materia_prima.id_tipo_origen ~
origenes_materia_prima.id_articulo_lote ~
origenes_materia_prima.id_articulo_mp 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH origenes_materia_prima WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table origenes_materia_prima
&Scoped-define FIRST-TABLE-IN-QUERY-br_table origenes_materia_prima
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      origenes_materia_prima.id_tipo_origen COLUMN-LABEL "Cod.TO" FORMAT ">>>9":U
            WIDTH 12.2
      origenes_materia_prima.id_articulo_lote COLUMN-LABEL "Art. LoteFinal" FORMAT ">>>9":U
            WIDTH 20.2
      origenes_materia_prima.id_articulo_mp COLUMN-LABEL "Art.MP" FORMAT ">>>9":U
            WIDTH 14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.origenes_materia_prima"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _OrdList          = "general.origenes_materia_prima.id_tipo_origen|yes,industria.origenes_materia_prima.id_articulo_lote|yes,industria.origenes_materia_prima.id_articulo_mp|yes"
     _JoinCode[1]      = "general.tipos_origenes_materia_prima.id_tipo_origen = general.origenes_materia_prima.id_tipo_origen"
     _FldNameList[1]   > general.origenes_materia_prima.id_tipo_origen
"id_tipo_origen" "Cod.TO" ? "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[2]   > general.origenes_materia_prima.id_articulo_lote
"id_articulo_lote" "Art. LoteFinal" ? "integer" ? ? ? ? ? ? no ? no no "20.2" yes no no "U" "" ""
     _FldNameList[3]   > general.origenes_materia_prima.id_articulo_mp
"id_articulo_mp" "Art.MP" ? "integer" ? ? ? ? ? ? no ? no no "14" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item origenes_materia_prima-id_tipo_origen
label 'id_tipo_origen( origenes_materia_prima ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=origenes_materia_prima.id_tipo_origen').
                  run set-attribute-list('titulo=CodTOrigen').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by origenes_materia_prima.id_tipo_origen
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item origenes_materia_prima-id_articulo_lote
label 'id_articulo_lote( origenes_materia_prima ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=origenes_materia_prima.id_articulo_lote').
                  run set-attribute-list('titulo=Art.Lote').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by origenes_materia_prima.id_articulo_lote
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item origenes_materia_prima-id_articulo_mp
label 'id_articulo_mp( origenes_materia_prima ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=origenes_materia_prima.id_articulo_mp').
                  run set-attribute-list('titulo=Art.MP').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by origenes_materia_prima.id_articulo_mp
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by origenes_materia_prima.id_tipo_origen
...
case p:
  WHEN 'origenes_materia_prima.id_tipo_origen' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_tipo_origen = integer(valor)  no-lock no-error .
  WHEN 'origenes_materia_prima.id_articulo_lote' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_articulo_lote = integer(valor)  no-lock no-error .
  WHEN 'origenes_materia_prima.id_articulo_mp' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_articulo_mp = integer(valor)  no-lock no-error .
end.
END PROCEDURE.
