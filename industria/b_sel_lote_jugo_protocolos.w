...
&Scoped-define FIELDS-IN-QUERY-br_table lotes_jugo.id_lote lotes_jugo.Fecha ~
lotes_jugo.id_articulo productos_terminados.descripcion ~
lotes_jugo.id_calidad calidades.descripcion 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH lotes_jugo WHERE ~{&KEY-PHRASE} ~
      AND lotes_jugo.id_articulo = v_articulo NO-LOCK, ~
      ...
      WHERE tambores_industria.id_locacion_ubicacion = 4 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-define FOURTH-TABLE-IN-QUERY-br_table tambores_industria
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      lotes_jugo.id_lote COLUMN-LABEL "Lote" FORMAT ">>>9":U WIDTH 6.2
      lotes_jugo.Fecha FORMAT "99/99/9999":U
      lotes_jugo.id_articulo COLUMN-LABEL "Cod" FORMAT ">>9":U
      productos_terminados.descripcion COLUMN-LABEL "Producto" FORMAT "x(30)":U
      lotes_jugo.id_calidad COLUMN-LABEL "Cod" FORMAT ">>9":U WIDTH 7.8
      calidades.descripcion COLUMN-LABEL "Calidad" FORMAT "X(30)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_jugo,general.productos_terminados OF general.lotes_jugo,general.calidades OF general.lotes_jugo,industria.tambores_industria OF general.lotes_jugo"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,, FIRST"
     _OrdList          = "general.lotes_jugo.Fecha|no,industria.lotes_jugo.id_lote|no"
     _Where[1]         = "general.lotes_jugo.id_articulo = v_articulo"
     _Where[4]         = "general.tambores_industria.id_locacion_ubicacion = 4"
     _FldNameList[1]   > general.lotes_jugo.id_lote
"lotes_jugo.id_lote" "Lote" ">>>9" "integer" ? ? ? ? ? ? no ? no no "6.2" yes no no "U" "" ""
     _FldNameList[2]   = general.lotes_jugo.Fecha
     _FldNameList[3]   > general.lotes_jugo.id_articulo
"lotes_jugo.id_articulo" "Cod" ">>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.descripcion
"productos_terminados.descripcion" "Producto" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > general.lotes_jugo.id_calidad
"lotes_jugo.id_calidad" "Cod" ">>9" "integer" ? ? ? ? ? ? no ? no no "7.8" yes no no "U" "" ""
     _FldNameList[6]   > general.calidades.descripcion
"calidades.descripcion" "Calidad" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
case p:
  ...
  WHEN 'lotes_jugo.Fecha' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  lotes_jugo.Fecha = date(valor)  no-lock no-error .
...
RUN dame-articulo IN hcon (OUTPUT v_articulo).
