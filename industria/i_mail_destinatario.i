...
IF contactos_muestras.email <> "" THEN DO:
    ...
    IF items_muestras.fecha_enviado_bue = ? THEN DO:
        RUN mes_largo.p (INPUT MONTH(general.items_muestras.fecha_enviado_tuc),
                         INPUT "ing",
                         OUTPUT v_mes).
    
        FIND FIRST couriers WHERE items_muestras.id_courier_tuc = couriers.id_courier NO-LOCK NO-ERROR.
        v_body = "On " + v_mes + "/" + STRING(DAY(general.items_muestras.fecha_enviado_tuc)) + 
    ...
    ELSE DO:
        ...
        v_body = "This sample was dispached on " + STRING(general.items_muestras.fecha_enviado_bue) + 
