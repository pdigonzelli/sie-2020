...
DEFINE VARIABLE h_b_inventarios AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_inventarios AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Inventarios */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Inventarios */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 22.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 56.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 79.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_inventarios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_inventarios ).
       RUN set-position IN h_b_inventarios ( 3.67 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 120.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_inventarios.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_inventarios ).
       RUN set-position IN h_v_inventarios ( 11.19 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.00 , 100.20 ) */

       /* Links to csmartbrowser h_b_inventarios. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_inventarios ).

       /* Links to SmartViewer h_v_inventarios. */
       RUN add-link IN adm-broker-hdl ( h_b_inventarios , 'Record':U , h_v_inventarios ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_inventarios ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_inventarios ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_inventarios ,
             h_b_inventarios , 'AFTER':U ).
    END. /* Page 0 */
