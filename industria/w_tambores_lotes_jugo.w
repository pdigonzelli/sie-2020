...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-18 RECT-19 
...
DEFINE VARIABLE h_b_lotes_jugo_tambores AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_lotes_jugo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     RECT-2 AT Y 0 X 235
     RECT-18 AT ROW 3.86 COL 7
     RECT-19 AT ROW 12.71 COL 7
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Tambores de Lotes de Jugo"
         HEIGHT             = 21.05
         WIDTH              = 140.2
         MAX-HEIGHT         = 23.62
         MAX-WIDTH          = 140.2
...
ON END-ERROR OF W-Win /* Tambores de Lotes de Jugo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tambores de Lotes de Jugo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 49.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 72.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_jugo_tambores.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_jugo_tambores ).
       RUN set-position IN h_b_lotes_jugo_tambores ( 4.43 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 130.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_lotes_jugo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_lotes_jugo ).
       RUN set-position IN h_b_tambores_lotes_jugo ( 13.19 , 25.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 79.00 ) */

       /* Links to csmartbrowser h_b_lotes_jugo_tambores. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_lotes_jugo_tambores ).

       /* Links to csmartbrowser h_b_tambores_lotes_jugo. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_jugo_tambores , 'Record':U , h_b_tambores_lotes_jugo ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             h_cus-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_jugo_tambores ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_lotes_jugo ,
             h_b_lotes_jugo_tambores , 'AFTER':U ).
    END. /* Page 0 */
