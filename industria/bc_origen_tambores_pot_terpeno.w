...
productos_terminados.descripcion tambores_industria.id_lote ~
tambores_industria.Anio tambores_industria.id_tambor ~
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tambores_industria WHERE ~{&KEY-PHRASE} ~
      AND tambores_industria.id_tipotambor = 7 ~
 ...
 AND if tambores_industria.id_articulo <> 59 then tambores_industria.Fecha_cierre <= fecha else true ~
 AND tambores_industria.id_locacion_ubicacion = 4 ~
 AND tambores_industria.id_sucursal_ubicacion <> 85 ~
 AND if v_sucursal = 0 then true else tambores_industria.id_sucursal = v_sucursal ~
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else tambores_industria.id_lote >= v_tambor_desde) ~
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else tambores_industria.id_lote <= v_tambor_hasta) NO-LOCK, ~
...
&Scoped-Define ENABLED-OBJECTS sucursal tambor_desde tambor_hasta BUTTON-13 ~
br_table 
&Scoped-Define DISPLAYED-OBJECTS sucursal tambor_desde tambor_hasta 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      sucursales.abreviatura COLUMN-LABEL "Sucursal" FORMAT "X(10)":U
            WIDTH 13.2
      productos_terminados.descripcion COLUMN-LABEL "Producto" FORMAT "x(20)":U
            WIDTH 25.2
      tambores_industria.id_lote FORMAT ">>>9":U WIDTH 12.2
      tambores_industria.Anio FORMAT ">>>9":U WIDTH 7.2
      tambores_industria.id_tambor FORMAT ">>>9":U WIDTH 12.2
      tambores_industria.kilos_tambor FORMAT ">>>>9.99":U WIDTH 15.2
      tambores_industria.Fecha FORMAT "99/99/99":U WIDTH 13.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     sucursal AT ROW 1 COL 17 COLON-ALIGNED
     tambor_desde AT ROW 1 COL 49 COLON-ALIGNED
     tambor_hasta AT ROW 1 COL 71 COLON-ALIGNED
     BUTTON-13 AT ROW 1 COL 95
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ",,,"
     _Where[1]         = "general.tambores_industria.id_tipotambor = 7
 ...
 AND if tambores_industria.id_articulo <> 59 then general.tambores_industria.Fecha_cierre <= fecha else true
 AND general.tambores_industria.id_locacion_ubicacion = 4
 AND general.tambores_industria.id_sucursal_ubicacion <> 85
 AND if v_sucursal = 0 then true else general.tambores_industria.id_sucursal = v_sucursal
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else general.tambores_industria.id_lote >= v_tambor_desde)
AND (if v_tambor_desde = 0 or v_tambor_hasta = 0 then true else general.tambores_industria.id_lote <= v_tambor_hasta)"
     _FldNameList[1]   > comercial.sucursales.abreviatura
"sucursales.abreviatura" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
     _FldNameList[2]   > general.productos_terminados.descripcion
"productos_terminados.descripcion" "Producto" "x(20)" "character" ? ? ? ? ? ? no ? no no "25.2" yes no no "U" "" ""
     _FldNameList[3]   > general.tambores_industria.id_lote
"tambores_industria.id_lote" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[4]   > general.tambores_industria.Anio
"tambores_industria.Anio" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[5]   > general.tambores_industria.id_tambor
"tambores_industria.id_tambor" ? ">>>9" "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[6]   > general.tambores_industria.kilos_tambor
"tambores_industria.kilos_tambor" ? ">>>>9.99" "decimal" ? ? ? ? ? ? no ? no no "15.2" yes no no "U" "" ""
     _FldNameList[7]   > general.tambores_industria.Fecha
"tambores_industria.Fecha" ? "99/99/99" "date" ? ? ? ? ? ? no ? no no "13.2" yes no no "U" "" ""
...
run get-articulos in hcontainer (output articulo, output fecha).
