...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 20.52 COL 113
     BUTTON-2 AT ROW 20.52 COL 131
     RECT-3 AT ROW 1.95 COL 3
     RECT-4 AT ROW 20.05 COL 111
...
DO:
  ...
  CASE choice:
    WHEN TRUE THEN /* Yes */ DO:
        DO i = 1 TO num:
            FIND tambores_industria WHERE ROWID(tambores_industria) = TO-ROWID(ENTRY(i,elegidos)) no-error.
            IF AVAILABLE tambores_industria THEN DO:
                ...
                END.
            END.
            ELSE MESSAGE "Hay un problema con la eleccion de origen de lote" VIEW-AS ALERT-BOX.
        END.
    END.
END CASE.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_origen_tambores_sobrante_lote_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_origen_tambores_sobrante_lo ).
       RUN set-position IN h_bc_origen_tambores_sobrante_lo ( 2.43 , 6.00 ) NO-ERROR.
       RUN set-size IN h_bc_origen_tambores_sobrante_lo ( 16.57 , 148.00 ) NO-ERROR.
...
do:
    articulo = integer(string(lotes_aceite.id_articulo) + "8") .
    fecha = lotes_aceite.fecha.
    lote = lotes_aceite.id_lote. 
end.
