...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.semana rowObject.Origen ~
rowObject.Cantidad rowObject.id_puerto_origen 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      rowObject.Origen FORMAT "x(50)":U WIDTH 24.6
      rowObject.Cantidad FORMAT ">>>>>>9":U
      rowObject.id_puerto_origen FORMAT ">>>>>9":U WIDTH 18.4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Settings for THIS-PROCEDURE
   ...
   Data Source: "dRegistroExportacion.w"
   Allow: Basic,Browse
   Frames: 1
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.semana
"rowObject.semana" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.Origen
"rowObject.Origen" ? ? "character" ? ? ? ? ? ? no ? no no "24.6" yes no no "U" "" ""
     _FldNameList[3]   > _<SDO>.rowObject.Cantidad
"rowObject.Cantidad" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[4]   > _<SDO>.rowObject.id_puerto_origen
"rowObject.id_puerto_origen" ? ? "integer" ? ? ? ? ? ? no "?" no no "18.4" yes no no "U" "" ""
...
DO:
  ...
  hCont = DYNAMIC-FUNCTION('linkHandles', 'CONTAINER-SOURCE').
  hData = DYNAMIC-FUNCTION('getDataSource').
  IF VALID-HANDLE(hCont) THEN
    RUN drawGraph IN hCont (DYNAMIC-FUNCTION('columnValue' IN hData, 'semana'), 
                            DYNAMIC-FUNCTION('columnValue' IN hData, 'id_puerto_origen')).
