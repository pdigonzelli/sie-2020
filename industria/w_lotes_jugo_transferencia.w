...
&Scoped-Define ENABLED-OBJECTS BUTTON-14 desde_tambor hasta_tambor BUTTON-1 ~
RECT-2 
&Scoped-Define DISPLAYED-OBJECTS desde_tambor hasta_tambor 
...
DEFINE VARIABLE h_b_lotes_jugo_transferencia AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_lotes_jugo_transfer AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cus-misc AS HANDLE NO-UNDO.
DEFINE VARIABLE h_cus-navico AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     LABEL "Transferencia Lote Completo" 
     SIZE 31 BY 1.14.
...
DEFINE FRAME F-Main
     BUTTON-14 AT ROW 20.29 COL 76
     desde_tambor AT ROW 20.52 COL 35 COLON-ALIGNED
     hasta_tambor AT ROW 20.52 COL 58 COLON-ALIGNED
     BUTTON-1 AT ROW 22.19 COL 110
     RECT-2 AT Y 0 X 170
...
ON END-ERROR OF W-Win /* Transferencias */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Transferencias */
DO:
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Transferencia Lote Completo */
DO:
  define var r as rowid.
  define var v_articulo as integer.

  run get-rowid1 in h_b_lotes_jugo_transferencia (output r).
  ...
  IF AVAILABLE lotes_jugo THEN DO:
    ...
    FOR EACH tambores_industria OF lotes_jugo
        ...
        MESSAGE "Estoy por cambiar el articulo " lotes_jugo.id_articulo " a " v_articulo view-as alert-box.
  ...
  RUN dispatch in h_b_lotes_jugo_transferencia ('open-query':U).
END.
...
DO:
  ...
  FOR EACH lotes_jugo WHERE ROWID(lotes_jugo) = r.
    ...
    DO i = INTEGER(desde_tambor:SCREEN-VALUE IN FRAME F-Main) TO INTEGER(hasta_tambor:SCREEN-VALUE IN FRAME F-Main).
        ...
        IF AVAILABLE tambores_industria THEN DO:
            IF tambores_industria.id_orden_entrega <> 0 THEN DO:
                MESSAGE "Este tambor esta asociado a la OE " tambores_industria.id_orden_entrega " .Esta seguro que lo quiere transferir?"
                                        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL TITLE "Alerta" UPDATE choice AS LOGICAL.
                IF choice THEN DO:
                   {i_lotes_jugo_transferencia.i}
                END.
                ELSE DO:
                    MESSAGE "El tambor NO fue transferido" VIEW-AS ALERT-BOX.
                END.
            END.
            ELSE DO:
                {i_lotes_jugo_transferencia.i}
 ...
 RUN dispatch in h_b_tambores_lotes_jugo_transfer ('open-query':U).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_jugo_transferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_jugo_transferencia ).
       RUN set-position IN h_b_lotes_jugo_transferencia ( 3.62 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_lotes_jugo_transferencia ( 9.05 , 142.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_lotes_jugo_transferencia.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_lotes_jugo_transfer ).
       RUN set-position IN h_b_tambores_lotes_jugo_transfer ( 13.14 , 15.00 ) NO-ERROR.
       RUN set-size IN h_b_tambores_lotes_jugo_transfer ( 6.71 , 110.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_lotes_jugo_transferencia. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_lotes_jugo_transferencia ).

       /* Links to csmartbrowser h_b_tambores_lotes_jugo_transfer. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_jugo_transferencia , 'Record':U , h_b_tambores_lotes_jugo_transfer ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-navico ,
             BUTTON-14:HANDLE IN FRAME F-Main , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-misc ,
             h_cus-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_jugo_transferencia ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_lotes_jugo_transfer ,
             h_b_lotes_jugo_transferencia , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY desde_tambor hasta_tambor 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE BUTTON-14 desde_tambor hasta_tambor BUTTON-1 RECT-2 
      WITH FRAME F-Main IN WINDOW W-Win.
