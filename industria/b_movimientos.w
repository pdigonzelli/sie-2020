...
&Scoped-define INTERNAL-TABLES movimientos sucursales locaciones
...
locaciones.descripcion movimientos.id_etiqueta movimientos.fecha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define FIELD-PAIRS-IN-QUERY-br_table
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH movimientos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH sucursales OF movimientos NO-LOCK, ~
      EACH locaciones OF movimientos NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table movimientos sucursales locaciones
&Scoped-define FIRST-TABLE-IN-QUERY-br_table movimientos
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.movimientos,comercial.sucursales OF general.movimientos,industria.locaciones OF general.movimientos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   = comercial.sucursales.nombre
     _FldNameList[2]   = general.locaciones.descripcion
     _FldNameList[3]   = general.movimientos.id_etiqueta
     _FldNameList[4]   = general.movimientos.fecha
...
label 'descripcion( locaciones ) '
  ...
  menu-item movimientos-id_etiqueta
label 'id_etiqueta( movimientos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=movimientos.id_etiqueta').
                  run set-attribute-list('titulo=Etiqueta').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by movimientos.id_etiqueta
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item movimientos-fecha
label 'fecha( movimientos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=movimientos.fecha').
                  run set-attribute-list('titulo=Fecha').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by movimientos.fecha
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by sucursales.nombre
