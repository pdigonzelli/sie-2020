...
&Scoped-define FIELDS-IN-QUERY-br_table lotes_jugo.id_lote lotes_jugo.anio ~
lotes_jugo.Fecha productos_terminados.abreviatura_ingles ~
calidades.Abreviatura_ingles 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH lotes_jugo OF protocolos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF lotes_jugo NO-LOCK, ~
      EACH calidades OF lotes_jugo NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH lotes_jugo OF protocolos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH productos_terminados OF lotes_jugo NO-LOCK, ~
      EACH calidades OF lotes_jugo NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
<FOREIGN-KEYS>
id_orden_entrega||y|general.lotes_jugo.id_orden_entrega
id_articulo||y|general.lotes_jugo.id_articulo
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_orden_entrega,id_articulo,id_tipotambor,id_tipolimon"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      lotes_jugo.id_lote COLUMN-LABEL "Batch" FORMAT ">>>9":U WIDTH 8.2
      lotes_jugo.anio COLUMN-LABEL "Year" FORMAT ">>>9":U
      lotes_jugo.Fecha COLUMN-LABEL "Date" FORMAT "99/99/9999":U
      productos_terminados.abreviatura_ingles COLUMN-LABEL "Merchandise" FORMAT "X(12)":U
            WIDTH 17.2
      calidades.Abreviatura_ingles COLUMN-LABEL "Quality" FORMAT "X(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 73 BY 4.52
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "general.lotes_jugo OF general.protocolos,general.productos_terminados OF general.lotes_jugo,general.calidades OF general.lotes_jugo"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > general.lotes_jugo.id_lote
"lotes_jugo.id_lote" "Batch" ">>>9" "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > general.lotes_jugo.anio
"lotes_jugo.anio" "Year" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.lotes_jugo.Fecha
"lotes_jugo.Fecha" "Date" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.productos_terminados.abreviatura_ingles
"productos_terminados.abreviatura_ingles" "Merchandise" ? "character" ? ? ? ? ? ? no ? no no "17.2" yes no no "U" "" ""
     _FldNameList[5]   > general.calidades.Abreviatura_ingles
"calidades.Abreviatura_ingles" "Quality" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
label 'id_lote( lotes_jugo ) '
  ...
  menu-item lotes_jugo-anio
label 'anio( lotes_jugo ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=lotes_jugo.anio').
                  run set-attribute-list('titulo=Año Lote').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by lotes_jugo.anio
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_orden_entrega" "lotes_jugo" "id_orden_entrega"}
  {src/adm/template/sndkycas.i "id_articulo" "lotes_jugo" "id_articulo"}
