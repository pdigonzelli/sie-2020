...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.id_carga rowObject.anio ~
rowObject.Articulo rowObject.Tanque rowObject.litros rowObject.Fecha ~
rowObject.volumen_final rowObject.Sucursal rowObject.nromov ~
rowObject.SolSolubles rowObject.Coef rowObject.Kilos rowObject.Kilos400 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      rowObject.id_carga FORMAT ">>>>>9":U
      rowObject.anio FORMAT ">>>9":U
      rowObject.Articulo FORMAT "x(20)":U
      rowObject.Tanque FORMAT "x(10)":U
      rowObject.litros FORMAT "->,>>>,>>9":U
      rowObject.Fecha FORMAT "99/99/9999":U
      rowObject.volumen_final FORMAT ">>,>>9.99":U
      rowObject.Sucursal FORMAT "x(15)":U
      rowObject.nromov FORMAT ">>>,>>>,>>9":U
      rowObject.SolSolubles FORMAT "->>,>>>,>>9.99":U
      rowObject.Coef FORMAT ">,>>9.99":U
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.id_carga
"rowObject.id_carga" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   = _<SDO>.rowObject.anio
     _FldNameList[3]   = _<SDO>.rowObject.Articulo
     _FldNameList[4]   = _<SDO>.rowObject.Tanque
     _FldNameList[5]   = _<SDO>.rowObject.litros
     _FldNameList[6]   = _<SDO>.rowObject.Fecha
     _FldNameList[7]   = _<SDO>.rowObject.volumen_final
     _FldNameList[8]   > _<SDO>.rowObject.Sucursal
"rowObject.Sucursal" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[9]   = _<SDO>.rowObject.nromov
     _FldNameList[10]   = _<SDO>.rowObject.SolSolubles
     _FldNameList[11]   = _<SDO>.rowObject.Coef
     _FldNameList[12]   = _<SDO>.rowObject.Kilos
     _FldNameList[13]   = _<SDO>.rowObject.Kilos400
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND (hCont:FILE-NAME MATCHES "*wProcesos.w") THEN DO:
    /*RUN fillPieGraph IN hCont.*/
    RUN fillTankGraph IN hCont.
  END.
...
DO:
  ...
  /*para frame fCargas*/
  IF VALID-HANDLE(hCont) AND (hCont:FILE-NAME MATCHES "*wProcesos.w") THEN DO:
    iBgColor = IF rowObject.rowNum MODULO 2 = 0 THEN 25 /*11 light blue*/ ELSE ? /*default*/.
    
    ASSIGN rowObject.sucursal:BGCOLOR IN BROWSE br_table      = ibgcolor
           rowObject.id_carga:BGCOLOR IN BROWSE br_table      = ibgcolor
           ...
           rowObject.volumen_final:BGCOLOR IN BROWSE br_table = ibgcolor
