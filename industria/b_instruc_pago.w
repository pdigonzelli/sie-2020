...
&Scoped-define INTERNAL-TABLES instruc_pago

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table instruc_pago.id_instruc_pago ~
instruc_pago.nombre instruc_pago.account 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH instruc_pago WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table instruc_pago
&Scoped-define FIRST-TABLE-IN-QUERY-br_table instruc_pago
...
<FOREIGN-KEYS>
id_instruc_pago||y|ventas.instruc_pago.id_instruc_pago
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "id_instruc_pago"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      instruc_pago.id_instruc_pago COLUMN-LABEL "Nro.Pay.Ins." FORMAT ">>>>9":U
            WIDTH 17.2
      instruc_pago.nombre FORMAT "X(60)":U
      instruc_pago.account FORMAT "X(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ventas.instruc_pago"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _FldNameList[1]   > ventas.instruc_pago.id_instruc_pago
"id_instruc_pago" "Nro.Pay.Ins." ">>>>9" "integer" ? ? ? ? ? ? no ? no no "17.2" yes no no "U" "" ""
     _FldNameList[2]   = ventas.instruc_pago.nombre
     _FldNameList[3]   = ventas.instruc_pago.account
...
define sub-menu Ordena 
  menu-item instruc_pago-id_instruc_pago
label 'id_instruc_pago( instruc_pago ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=instruc_pago.id_instruc_pago').
                  run set-attribute-list('titulo=No. Payment Instruction').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by instruc_pago.id_instruc_pago
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item instruc_pago-nombre
label 'nombre( instruc_pago ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=instruc_pago.nombre').
                  run set-attribute-list('titulo=Bank').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by instruc_pago.nombre
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item instruc_pago-account
label 'account( instruc_pago ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=instruc_pago.account').
                  run set-attribute-list('titulo=Account').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by instruc_pago.account
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
&SCOPE SORTBY-PHRASE by instruc_pago.id_instruc_pago
...
case p:
  WHEN 'instruc_pago.id_instruc_pago' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_instruc_pago = integer(valor)  no-lock no-error .
  WHEN 'instruc_pago.nombre' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  nombre begins valor  no-lock no-error .
  WHEN 'instruc_pago.account' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  account begins valor  no-lock no-error .
end.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "id_instruc_pago" "instruc_pago" "id_instruc_pago"}
