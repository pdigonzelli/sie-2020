...
&Scoped-Define ENABLED-OBJECTS RECT-16 RECT-15 RECT-2 BUTTON-1 
...
DEFINE VARIABLE h_b_tambores_produccion_aceite AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_tambores_produccion_aceite AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-1 AT ROW 1.57 COL 101
     RECT-16 AT ROW 10.71 COL 3
...
run get-rowid1 in h_b_tambores_produccion_aceite (output r).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.33 , 18.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.33 , 52.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.33 , 75.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_produccion_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_produccion_aceite ).
       RUN set-position IN h_b_tambores_produccion_aceite ( 3.76 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 137.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_tambores_produccion_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_tambores_produccion_aceite ).
       RUN set-position IN h_v_tambores_produccion_aceite ( 11.10 , 31.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.14 , 73.00 ) */

       /* Links to csmartbrowser h_b_tambores_produccion_aceite. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tambores_produccion_aceite ).

       /* Links to SmartViewer h_v_tambores_produccion_aceite. */
       RUN add-link IN adm-broker-hdl ( h_b_tambores_produccion_aceite , 'record':U , h_v_tambores_produccion_aceite ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_tambores_produccion_aceite ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_produccion_aceite ,
             BUTTON-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_tambores_produccion_aceite ,
             h_b_tambores_produccion_aceite , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-16 RECT-15 RECT-2 BUTTON-1 
      WITH FRAME F-Main.
