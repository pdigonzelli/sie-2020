...
define var del_produccion_aceite as integer.
/* Local Variable Definitions ---                                       */
...
/**********EMPIEZA-TEMP-TABLES*********/
&SCOPED-DEFINE TABLA-CABECERA produccion_aceite
&SCOPED-DEFINE TABLA-DETALLE tambores_aceite
...
&Scoped-Define ENABLED-OBJECTS RECT-4 RECT-16 etiquetas 
...
DEFINE VARIABLE h_b_produccion_aceite AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_aceite AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_produccion_aceite AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON etiquetas 
     LABEL "Button 2" 
     SIZE 21 BY 1.43.
...
/*produccion_aceite,tambores_aceite
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME etiquetas
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL etiquetas F-Frame-Win
ON CHOOSE OF etiquetas IN FRAME F-Main /* Button 2 */
DO:
    define var r as rowid.
    run get-rowid1 in h_b_tambores_aceite (output r).
    find tambores_aceite where rowid(tambores_aceite) = r no-lock no-error.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_produccion_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_produccion_aceite ).
       RUN set-position IN h_b_produccion_aceite ( 4.05 , 36.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 72.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'cabecera|detalle' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_cfolder ).
       RUN set-position IN h_cfolder ( 11.00 , 4.00 ) NO-ERROR.
       RUN set-size IN h_cfolder ( 9.52 , 136.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_produccion_aceite. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_produccion_aceite ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_produccion_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_produccion_aceite ).
       RUN set-position IN h_v_produccion_aceite ( 13.00 , 26.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.95 , 95.00 ) */

       /* Links to SmartViewer h_v_produccion_aceite. */
       RUN add-link IN adm-broker-hdl ( h_b_produccion_aceite , 'Record':U , h_v_produccion_aceite ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_produccion_aceite ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_aceite ).
       RUN set-position IN h_b_tambores_aceite ( 13.00 , 31.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 83.00 ) */

       /* Links to csmartbrowser h_b_tambores_aceite. */
       RUN add-link IN adm-broker-hdl ( h_b_produccion_aceite , 'Record':U , h_b_tambores_aceite ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav-2 , 'TableIO':U , h_b_tambores_aceite ).
...
------------------------------------------------------------------------------*/
  ENABLE RECT-4 RECT-16 etiquetas 
      WITH FRAME F-Main.
...
run get-rowid1 in h_b_produccion_aceite (output r).
for each produccion_aceite where rowid(produccion_aceite) = r.
    v1 = produccion_aceite.id_empresa.
    v2 = produccion_aceite.id_sucursal.
    v3 = produccion_aceite.id_produccion_aceite.
end.
END PROCEDURE.
...
v_filtro = "produccion_aceite.id_empresa = " + string(v1) + 
           " and produccion_aceite.id_sucursal = " + string(v2) +
           " and produccion_aceite.id_produccion_aceite = " + string(v3).
...
RB-MEMO-FILE = SESSION:TEMP-DIRECTORY + STRING(RANDOM(1,1000000)) + '.TXT'.
      ...
      RUN  aderb\_printrb(
       ...
       "produccion_aceite",                    /* RB-REPORT-NAME */
       "",                             /* RB-DB-CONNECTION */
       ...
       "",                              /* RB-OUTPUT-FILE */
        ...
        0,                              /* RB-END-PAGE - zero */
       no,                              /* RB-TEST-PATTERN */
       "Producción de Aceite",         /* RB-WINDOW-TITLE */
...
------------------------------------------------------------------------------*/
 /* Code placed here will execute PRIOR to standard behavior. */
  ...
  do:
        ...
        run genero-detalle.
        etiquetas:visible in frame {&FRAME-NAME} = true.
        run dispatch in hdetalle ('open-query').
  end.
  run get-attribute ('current-page').
  if return-value = "1" and valid-handle(hcabecera) then
  do:
    etiquetas:visible in frame {&FRAME-NAME} = false.
    if cambio-detalle() then    
        run proceso-cabecera-detalle.        
  end.
  run get-attribute ('current-page').
...
message "Esta seguro que desea borrar los tambores de aceite?" view-as alert-box
        QUESTION BUTTONS YES-NO-CANCEL TITLE "Atención!" UPDATE choice AS LOGICAL.

if choice then
do:
for each tambores_aceite where tambores_aceite.id_empresa = del_empresa and
                               tambores_aceite.id_sucursal = del_sucursal and
                               tambores_aceite.id_produccion_aceite = del_produccion_aceite.

    delete tambores_aceite.
...
run get-rowid-cabecera (output r).
for each produccion_aceite where rowid(produccion_aceite) = r.
    del_empresa = produccion_aceite.id_empresa.
    del_sucursal = produccion_aceite.id_sucursal.
    del_produccion_aceite = produccion_aceite.id_produccion_aceite.
end.
END PROCEDURE.
