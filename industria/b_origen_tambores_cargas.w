...
&Scoped-define FIELDS-IN-QUERY-br_table tt_origen_jugo.descripcion_sucursal tt_origen_jugo.id_lote tt_origen_jugo.id_tambor tt_origen_jugo.descripcion_envase tt_origen_jugo.id_etiqueta tt_origen_jugo.fecha   
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 135 BY 5.62
...
RUN get-rowid-cargas in hcontainer (output r).

find cargas where rowid(cargas) = r  no-lock no-error.
if available cargas then
do:   
    for each tambores_industria where tambores_industria.id_empresa_destino = cargas.id_empresa and
                                      tambores_industria.id_sucursal_destino = cargas.id_sucursal and
                                      tambores_industria.id_tipotambor_destino = cargas.id_tipotambor and
                                      tambores_industria.nromov_destino = cargas.nromov no-lock.
