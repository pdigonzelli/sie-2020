...
&Scoped-Define ENABLED-FIELDS lotes_aceite.control_calidad ~
lotes_aceite.quimico_control_calidad 
&Scoped-define ENABLED-TABLES lotes_aceite
&Scoped-define FIRST-ENABLED-TABLE lotes_aceite
&Scoped-Define DISPLAYED-FIELDS lotes_aceite.control_calidad ~
lotes_aceite.quimico_control_calidad 
...
DEFINE FRAME F-Main
     lotes_aceite.control_calidad AT ROW 1 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16.6 BY 1
     fi-quimicos-nombre AT ROW 1.95 COL 24 COLON-ALIGNED NO-LABEL
     lotes_aceite.quimico_control_calidad AT ROW 2 COL 14 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9 BY 1
...
general.lotes_aceite.quimico_control_calidad;wc_quimicos.w;quimicos.nombre;id_quimico;
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME lotes_aceite.control_calidad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.control_calidad V-table-Win
ON LEAVE OF lotes_aceite.control_calidad IN FRAME F-Main /* Estado Lote */
DO:
...
&Scoped-define SELF-NAME lotes_aceite.quimico_control_calidad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.quimico_control_calidad V-table-Win
ON GO OF lotes_aceite.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.quimico_control_calidad V-table-Win
ON LEAVE OF lotes_aceite.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.quimico_control_calidad V-table-Win
ON MOUSE-SELECT-DBLCLICK OF lotes_aceite.quimico_control_calidad IN FRAME F-Main /* Quimico */
do: 
define var r as rowid no-undo.
run wc_quimicos.w(output r).
find quimicos where rowid(quimicos) = r no-lock no-error.
if available quimicos then 
general.lotes_aceite.quimico_control_calidad:screen-value = string(quimicos.id_quimico).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_aceite.quimico_control_calidad V-table-Win
ON U1 OF lotes_aceite.quimico_control_calidad IN FRAME F-Main /* Quimico */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
if lotes_aceite.control_calidad:screen-value in frame F-Main = "Aprobado" and 
   integer(lotes_aceite.quimico_control_calidad:screen-value in frame F-Main) <> 0 then
   do:
      assign lotes_aceite.estado_lote = 5.
      /*
      if lotes_aceite.id_articulo > 900 then
            do:
                v_articulo = lotes_aceite.id_articulo - 900.
                assign lotes_aceite.id_articulo = v_articulo.
...
if lotes_aceite.control_calidad:screen-value in frame F-Main = "Desaprobado" and 
   integer(lotes_aceite.quimico_control_calidad:screen-value in frame F-Main) <> 0 then
   do:
        assign lotes_aceite.estado_lote = 4.
        ...
        if choice then
            do:
                ...
                assign lotes_aceite.estado_lote = 4
                       lotes_aceite.id_articulo = v_articulo.
...
find first quimicos where quimicos.id_quimico = integer(lotes_aceite.quimico_control_calidad:screen-value in frame F-Main)  no-lock no-error .
if available quimicos then 
fi-quimicos-nombre:screen-value in frame F-Main = quimicos.nombre.
else
