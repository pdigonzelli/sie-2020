...
DEFINE FRAME F-Main
     ...
     lotes_jugo.Fecha_finalizacion AT ROW 6.43 COL 124 COLON-ALIGNED
          LABEL "Fecha de finalización" FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
...
ON LEAVE OF lotes_jugo.Calibracion IN FRAME F-Main /* Calibración */
DO:
{custom/support/validacion.i}
END.
...
ON LEAVE OF lotes_jugo.Fecha_finalizacion IN FRAME F-Main /* Fecha de finalización */
DO:
{custom/support/validacion.i}
END.
...
ON LEAVE OF lotes_jugo.Hora_finalizacion IN FRAME F-Main /* Hora de finalización */
DO:
{custom/support/validacion.i}
END.
...
ON LEAVE OF lotes_jugo.Hora_fin_envase IN FRAME F-Main /* Finalización de envase */
DO:
{custom/support/validacion.i}
END.
...
ON GO OF lotes_jugo.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.id_articulo V-table-Win
ON LEAVE OF lotes_jugo.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.id_articulo V-table-Win
ON MOUSE-SELECT-DBLCLICK OF lotes_jugo.id_articulo IN FRAME F-Main /* Artículo */
do: 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.id_articulo V-table-Win
ON U1 OF lotes_jugo.id_articulo IN FRAME F-Main /* Artículo */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
if (date(lotes_jugo.fecha_finalizacion:screen-value in frame {&FRAME-NAME}) + 2) < date(lotes_jugo.fecha_finalizacion_envase:screen-value in frame {&FRAME-NAME}) then 
    do:
            message "Esta seguro que la fecha de finalización de envase fue hecha dos dias posteriores a la fecha del armado del lote?"
                     view-as alert-box.
            return no-apply.
    end.
      ...
      do: 
        ...
        find productos_terminados where productos_terminados.id_articulo = integer(valor) and productos_terminados.id_tipo_articulo = 2 no-lock no-error.
         if not available productos_terminados then 
            do:
                mensaje = "Debe ingresar un Producto terminado válido!".
                return false.
            end.
        end.
...
/*    when "id_articulo" then
    ...
    when "fecha_finalizacion" then
      do:
        ...
        do:
            mensaje = "La Fecha de Finalización no puede ser anterior a la Fecha de comienzo".
            return false.
         end.
    ...
    when "fecha_finalizacion_envase" then
        ...
        do:
            mensaje = "La Fecha de Finalización no puede ser anterior a la Fecha de comienzo de envase".
            return false.
         end.
   ...
   when "hora_fin_envase" then
    do:
        if integer(substring(valor,1,2)) > 24 or integer(substring(valor,4,2)) > 60 then 
            do:
                mensaje = "Debe ingresar una hora válida".
                    return false.
            end.
      /*  else
            do:
                ...
                do:
                    mensaje = "Debe ingresar una hora de finalización posterior a la hora de inicio".
                    return false.
                end.
            end. */
    end. 
