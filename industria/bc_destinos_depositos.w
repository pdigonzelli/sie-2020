...
&Scoped-define INTERNAL-TABLES destinos sucursales

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table destinos.id_destino ~
destinos.descripcion destinos.abreviatura sucursales.id_sucursal ~
sucursales.nombre 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH destinos WHERE ~{&KEY-PHRASE} ~
      AND destinos.id_sucursal <> 91 NO-LOCK, ~
      EACH sucursales OF destinos NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table destinos sucursales
&Scoped-define FIRST-TABLE-IN-QUERY-br_table destinos
...
<FOREIGN-KEYS>
id_destino||y|comercial.destinos.id_destino
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      destinos.id_destino COLUMN-LABEL "Cod" FORMAT "->>9":U WIDTH 8.2
      destinos.descripcion COLUMN-LABEL "Destinos" FORMAT "x(30)":U
      destinos.abreviatura COLUMN-LABEL "Destinos" FORMAT "x(10)":U
            WIDTH 12.4
      sucursales.id_sucursal COLUMN-LABEL "CodSuc" FORMAT ">>9":U
            WIDTH 9.2
      sucursales.nombre COLUMN-LABEL "Sucursal" FORMAT "x(30)":U
            WIDTH 35.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 111 BY 6.71
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "comercial.destinos,comercial.sucursales OF comercial.destinos"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "comercial.destinos.id_sucursal <> 91"
     _FldNameList[1]   > comercial.destinos.id_destino
"destinos.id_destino" "Cod" ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[2]   > comercial.destinos.descripcion
"destinos.descripcion" "Destinos" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > comercial.destinos.abreviatura
"destinos.abreviatura" "Destinos" ? "character" ? ? ? ? ? ? no ? no no "12.4" yes no no "U" "" ""
     _FldNameList[4]   > comercial.sucursales.id_sucursal
"sucursales.id_sucursal" "CodSuc" ? "integer" ? ? ? ? ? ? no ? no no "9.2" yes no no "U" "" ""
     _FldNameList[5]   > comercial.sucursales.nombre
"sucursales.nombre" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no "35.2" yes no no "U" "" ""
...
define sub-menu Ordena 
  menu-item destinos-id_destino
label 'id_destino( destinos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=destinos.id_destino').
                  run set-attribute-list('titulo=Destino').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by destinos.id_destino
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
  menu-item destinos-descripcion
label 'descripcion( destinos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=destinos.descripcion').
                  run set-attribute-list('titulo=Descripcion').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by destinos.descripcion
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
label 'abreviatura( destinos ) '
  ...
  menu-item sucursales-id_sucursal
label 'id_sucursal( sucursales ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=sucursales.id_sucursal').
                  run set-attribute-list('titulo=Sucursal').
                  run set-attribute-list('busca= no').
                  &SCOPE SORTBY-PHRASE  by sucursales.id_sucursal
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  WHEN 'destinos.id_destino' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  id_destino = integer(valor)  no-lock no-error .
  WHEN 'destinos.descripcion' then  
       find first {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} where  descripcion begins valor  no-lock no-error .
  WHEN 'destinos.abreviatura' then  
