...
destinos.descripcion destinos.id_sucursal sucursales.nombre 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH destinos WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH sucursales OF destinos OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      destinos.id_destino COLUMN-LABEL "CodDes" FORMAT "->>9":U
            WIDTH 11.2
      destinos.descripcion COLUMN-LABEL "Destino" FORMAT "x(30)":U
      destinos.id_sucursal COLUMN-LABEL "CodSuc" FORMAT ">>9":U
            WIDTH 11.4
      sucursales.nombre COLUMN-LABEL "Sucursal" FORMAT "x(30)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 100 BY 6.71
         FONT 0.
...
"destinos.id_destino" "CodDes" ? "integer" ? ? ? ? ? ? no ? no no "11.2" yes no no "U" "" ""
     _FldNameList[2]   > comercial.destinos.descripcion
"destinos.descripcion" "Destino" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > comercial.destinos.id_sucursal
"destinos.id_sucursal" "CodSuc" ? "integer" ? ? ? ? ? ? no ? no no "11.4" yes no no "U" "" ""
     _FldNameList[4]   > comercial.sucursales.nombre
"sucursales.nombre" "Sucursal" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
label 'descripcion( destinos ) '
  ...
  menu-item destinos-id_sucursal
label 'id_sucursal( destinos ) '
          triggers: 
              on choose 
                       do:
                  run set-attribute-list('orden=destinos.id_sucursal').
                  run set-attribute-list('titulo=Sucursal').
                  run set-attribute-list('busca= yes').
                  &SCOPE SORTBY-PHRASE  by destinos.id_sucursal
                  {&OPEN-QUERY-{&BROWSE-NAME}} 
              end.
          end.
...
case p:
  ...
  WHEN 'destinos.id_sucursal' then  
