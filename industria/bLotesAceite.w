...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.id_orden ~
rowObject.id_lote rowObject.id_articulo rowObject.Articulo ~
rowObject.Tambores rowObject.Kilos rowObject.fecha rowObject.Envase ~
rowObject.Peso_neto rowObject.tanque rowObject.SucLote rowObject.Estado ~
rowObject.Quimico rowObject.nromov rowObject.c_usuario 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      id_orden FORMAT ">>9":U
      id_lote FORMAT "->,>>>,>>9":U
      id_articulo FORMAT "->,>>>,>>9":U
      Articulo FORMAT "x(25)":U
      Tambores FORMAT ">>>9":U
      Kilos FORMAT ">>>,>>>,>>9.99":U
      fecha FORMAT "99/99/99":U
      Envase FORMAT "x(25)":U
      Peso_neto FORMAT "->>,>>9.999":U
      tanque FORMAT ">9":U
      SucLote FORMAT "x(15)":U
      Estado FORMAT "x(15)":U
      Quimico FORMAT "x(15)":U
      nromov FORMAT ">>>,>>>,>>9":U
      c_usuario FORMAT "x(12)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"id_orden" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   = _<SDO>.rowObject.id_lote
     _FldNameList[3]   = _<SDO>.rowObject.id_articulo
     _FldNameList[4]   > _<SDO>.rowObject.Articulo
"Articulo" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[5]   = _<SDO>.rowObject.Tambores
     _FldNameList[6]   = _<SDO>.rowObject.Kilos
     _FldNameList[7]   = _<SDO>.rowObject.fecha
     _FldNameList[8]   = _<SDO>.rowObject.Envase
     _FldNameList[9]   = _<SDO>.rowObject.Peso_neto
     _FldNameList[10]   = _<SDO>.rowObject.tanque
     _FldNameList[11]   = _<SDO>.rowObject.SucLote
     _FldNameList[12]   = _<SDO>.rowObject.Estado
     _FldNameList[13]   = _<SDO>.rowObject.Quimico
     _FldNameList[14]   > _<SDO>.rowObject.nromov
"nromov" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[15]   = _<SDO>.rowObject.c_usuario
...
DO:
  ...
  IF rowObject.id_articulo = 51 THEN
    ibgcolor = 14.
  ELSE
    ibgcolor = ?.

  ASSIGN rowObject.id_orden:BGCOLOR IN BROWSE br_table    = ibgcolor
         ...
         rowObject.peso_neto:BGCOLOR IN BROWSE br_table   = ibgcolor 
         rowObject.tanque:BGCOLOR IN BROWSE br_table      = ibgcolor
         rowObject.suclote:BGCOLOR IN BROWSE br_table     = ibgcolor
         ...
         rowObject.tambores:BGCOLOR IN BROWSE br_table    = ibgcolor
...
DO:
  ...
  IF VALID-HANDLE(hCont) AND hCont:FILE-NAME MATCHES "*wCtrlAceite.w*" THEN DO:

    IF rowObject.estado:SCREEN-VALUE <> "Aprobado" THEN
