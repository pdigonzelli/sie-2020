...
DEFINE VARIABLE h_bc_contratos_ff AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bc_distribuciones_contratos AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bc_items_contratos_ff AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     IMAGE-UP FILE "c:/dlc83b/gui/adeicon\check":U
     LABEL "Button 1" 
     SIZE 23 BY 1.43.

DEFINE BUTTON BUTTON-2 
     IMAGE-UP FILE "c:/dlc83b/gui/adeicon\cross":U
     LABEL "Button 2" 
     SIZE 23 BY 1.43.
...
DEFINE FRAME F-Main
     FILL-IN-1 AT ROW 1.24 COL 2 COLON-ALIGNED NO-LABEL
     FILL-IN-2 AT ROW 9.1 COL 2 NO-LABEL
     BUTTON-1 AT ROW 21.95 COL 97
     BUTTON-2 AT ROW 21.95 COL 125
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "CONSULTA DE ORDENES DE FABRICACION"
         HEIGHT             = 23.29
         WIDTH              = 152.2
         MAX-HEIGHT         = 23.29
         MAX-WIDTH          = 152.2
...
ON END-ERROR OF W-Win /* CONSULTA DE ORDENES DE FABRICACION */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* CONSULTA DE ORDENES DE FABRICACION */
DO:
...
DO:
  ...
  DEFINE VAR rr2 AS ROWID.

  RUN get-rowid1 IN h_bc_items_contratos_ff (OUTPUT rresult).
  r = rresult.  
  RUN get-rowid1 IN h_bc_distribuciones_contratos (OUTPUT rr2).
  r1 = rr2.
  APPLY "WINDOW-CLOSE":U TO W-Win.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_contratos_ff.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_contratos_ff ).
       RUN set-position IN h_bc_contratos_ff ( 2.43 , 4.00 ) NO-ERROR.
       RUN set-size IN h_bc_contratos_ff ( 6.43 , 139.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_items_contratos_ff.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_items_contratos_ff ).
       RUN set-position IN h_bc_items_contratos_ff ( 10.29 , 4.00 ) NO-ERROR.
       RUN set-size IN h_bc_items_contratos_ff ( 6.71 , 128.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/bc_distribuciones_contratos.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bc_distribuciones_contratos ).
       RUN set-position IN h_bc_distribuciones_contratos ( 17.43 , 4.00 ) NO-ERROR.
       RUN set-size IN h_bc_distribuciones_contratos ( 6.71 , 54.00 ) NO-ERROR.

       /* Links to csmartbrowser h_bc_items_contratos_ff. */
       RUN add-link IN adm-broker-hdl ( h_bc_contratos_ff , 'Record':U , h_bc_items_contratos_ff ).

       /* Links to csmartbrowser h_bc_distribuciones_contratos. */
       RUN add-link IN adm-broker-hdl ( h_bc_items_contratos_ff , 'Record':U , h_bc_distribuciones_contratos ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_contratos_ff ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_items_contratos_ff ,
             FILL-IN-2:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_bc_distribuciones_contratos ,
             h_bc_items_contratos_ff , 'AFTER':U ).
    END. /* Page 0 */
