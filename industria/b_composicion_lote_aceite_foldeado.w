...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      composicion_lote_aceite.cantidad_tambores COLUMN-LABEL "Cant.Tambores" FORMAT ">>>9":U
...
"composicion_lote_aceite.cantidad_tambores" "Cant.Tambores" ">>>9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > general.composicion_lote_aceite.Numeracion_desde
"composicion_lote_aceite.Numeracion_desde" "NumDesde" ">>>9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > general.composicion_lote_aceite.Numeracion_hasta
"composicion_lote_aceite.Numeracion_hasta" "NumHasta" ">>>9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
...
IF AVAILABLE lotes_aceite THEN DO:
    ...
    /*******************************************************************************/
    /******************CREO LOS REGISTROS PARA LA TABLA TAMBORES_INDUSTRIA***************/
    ...
    FIND FIRST b_tam WHERE b_tam.id_empresa     = lotes_aceite.id_empresa
                       AND b_tam.id_sucursal    = lotes_aceite.id_sucursal
                       AND b_tam.id_articulo    = lotes_aceite.id_articulo
                       AND b_tam.anio           = lotes_aceite.anio
                       AND b_tam.id_lote        = lotes_aceite.id_lote
                       AND b_tam.id_tambor      >= desde
                       AND b_tam.id_tambor      <= hasta
                       NO-LOCK NO-ERROR.
    IF AVAILABLE b_tam THEN DO:
        MESSAGE "El tambor "        b_tam.id_tambor
                " con el articulo " b_tam.id_articulo
                " de la sucursal "  b_tam.id_sucursal
                " del año "         b_tam.anio
                " YA EXISTE!. Señor " userid("userdb") 
                " pregunte si desea crear este tambor de todas maneras"
                VIEW-AS ALERT-BOX.
       RETURN "ADM-ERROR".
    END.
    ELSE DO:
        DO i = desde TO hasta:
            ...
            ASSIGN tambores_industria.id_empresa         = lotes_aceite.id_empresa
                   ...
                   tambores_industria.anio               = lotes_aceite.anio
        ...
        find first b_tam where b_tam.id_empresa       = lotes_aceite.id_empresa
                           AND b_tam.id_sucursal      = lotes_aceite.id_sucursal
                           and b_tam.id_tipotambor    = lotes_aceite.id_tipotambor
                           and b_tam.nromov           = lotes_aceite.nromov
                           AND b_tam.id_tambor        = 0
                           no-error.
...
IF AVAILABLE lotes_aceite THEN DO:
    ...
    FIND FIRST composicion_lote_aceite WHERE ROWID(composicion_lote_aceite) = r NO-LOCK NO-ERROR.
