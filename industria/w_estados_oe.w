...
DEFINE VARIABLE h_b_estados_oe AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_estados_oe AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Estados Orden Entrega */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Estados Orden Entrega */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 2.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 36.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 59.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_estados_oe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_estados_oe ).
       RUN set-position IN h_b_estados_oe ( 3.86 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 80.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_estados_oe.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_estados_oe ).
       RUN set-position IN h_v_estados_oe ( 11.24 , 17.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 47.00 ) */

       /* Links to csmartbrowser h_b_estados_oe. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_estados_oe ).

       /* Links to SmartViewer h_v_estados_oe. */
       RUN add-link IN adm-broker-hdl ( h_b_estados_oe , 'Record':U , h_v_estados_oe ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_estados_oe ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_estados_oe ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_estados_oe ,
             h_b_estados_oe , 'AFTER':U ).
    END. /* Page 0 */
