...
DEFINE VARIABLE h_b_lotes_aceite_ubicacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_seleccion_locaciones AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b_tambores_sobrante_aceite AS HANDLE NO-UNDO.
...
DEFINE VARIABLE FILL-IN-1 AS CHARACTER FORMAT "X(256)":U INITIAL "LOTES DE ACEITE" 
     VIEW-AS FILL-IN 
     SIZE 131.8 BY .95
...
DEFINE FRAME F-Main
     ...
     BUTTON-2 AT ROW 19.81 COL 90
     BUTTON-1 AT ROW 21.24 COL 90
...
DO:
  ...
  run get-rowid1 in h_b_lotes_aceite_ubicacion (output r).
  find lotes_aceite where rowid(lotes_aceite) = r no-lock no-error.
  if available lotes_aceite then
    do:
        find sobrante_lotes_aceite of lotes_aceite no-lock no-error.
...
run dispatch in h_b_tambores_sobrante_aceite ('open-query':U).  
...
DO:
  ...
  run get-rowid1 in h_b_lotes_aceite_ubicacion (output r).
  run tercera_tabla in h_b_tambores_sobrante_aceite (output r3).
  
  find lotes_aceite where rowid(lotes_aceite) = r no-lock no-error.
  if available lotes_aceite then
    do:
        FIND FIRST sobrante_lotes_aceite of lotes_aceite no-lock no-error.
        if available sobrante_lotes_aceite then
            do:
                find tambores_industria where rowid(tambores_industria) = r3.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_lotes_aceite_ubicacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_lotes_aceite_ubicacion ).
       RUN set-position IN h_b_lotes_aceite_ubicacion ( 2.43 , 8.00 ) NO-ERROR.
       RUN set-size IN h_b_lotes_aceite_ubicacion ( 6.71 , 127.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tambores_sobrante_aceite.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tambores_sobrante_aceite ).
       RUN set-position IN h_b_tambores_sobrante_aceite ( 10.76 , 7.00 ) NO-ERROR.
       RUN set-size IN h_b_tambores_sobrante_aceite ( 6.95 , 124.00 ) NO-ERROR.
       ...
       RUN set-position IN h_cus-updsav-3 ( 17.91 , 89.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 23.14 , 111.00 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       /* Links to csmartbrowser h_b_tambores_sobrante_aceite. */
       RUN add-link IN adm-broker-hdl ( h_b_lotes_aceite_ubicacion , 'Record':U , h_b_tambores_sobrante_aceite ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav-3 , 'TableIO':U , h_b_tambores_sobrante_aceite ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_lotes_aceite_ubicacion ,
             FILL-IN-1:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tambores_sobrante_aceite ,
             FILL-IN-4:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_cus-updsav-3 ,
             FILL-IN-5:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_seleccion_locaciones ,
             h_cus-updsav-3 , 'AFTER':U ).
