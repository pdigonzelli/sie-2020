...
rowObject.id_proceso rowObject.Anio rowObject.descripcion rowObject.fecha ~
rowObject.nromov rowObject.fecha_fin 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      rowObject.Sucursal FORMAT "x(25)":U
      rowObject.id_proceso FORMAT ">,>>>,>>9":U
      ...
      rowObject.fecha_fin FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"rowObject.Sucursal" ? ? "character" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.id_proceso
"rowObject.id_proceso" ? ? "integer" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" ""
     _FldNameList[3]   = _<SDO>.rowObject.Anio
     _FldNameList[4]   = _<SDO>.rowObject.descripcion
     _FldNameList[5]   = _<SDO>.rowObject.fecha
     _FldNameList[6]   = _<SDO>.rowObject.nromov
     _FldNameList[7]   = _<SDO>.rowObject.fecha_fin
...
DO:
  ...
  /*para pantalla de procesos*/
  IF VALID-HANDLE(hCont) AND 
     (hCont:FILE-NAME MATCHES "*wProcesos.w" OR
      hCont:FILE-NAME MATCHES "*wAjustesCierre.w" ) THEN DO:

    iBgColor = IF rowObject.rowNum MODULO 2 = 0 THEN 19 /*11 light blue*/ ELSE ? /*default*/.
    
    IF rowObject.fecha_fin <> ? THEN DO:
      iBgColor = 22.
    END.
    
    
    ASSIGN rowObject.sucursal:BGCOLOR IN BROWSE br_table     = ibgcolor
           rowObject.id_proceso:BGCOLOR IN BROWSE br_table   = ibgcolor
           ...
           rowObject.fecha_fin:BGCOLOR IN BROWSE br_table    = ibgcolor.
