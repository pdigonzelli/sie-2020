...
Define output parameter p_semana_desde as integer.
Define output parameter p_anio_desde as integer.
Define output parameter p_semana_hasta as integer.
Define output parameter p_anio_hasta as integer.
DEFINE OUTPUT PARAMETER p_id_articulo AS INTEGER.
...
&Scoped-Define ENABLED-OBJECTS semana_desde anio_desde semana_hasta ~
anio_hasta articulo BUTTON-1 salir nombre_articulo 
&Scoped-Define DISPLAYED-OBJECTS semana_desde anio_desde semana_hasta ~
anio_hasta articulo nombre_articulo 
...
DEFINE BUTTON salir 
     ...
     LABEL "&Salir" 
     SIZE 10 BY .95 TOOLTIP "Abandonar Tarea".

DEFINE VARIABLE anio_desde AS INTEGER FORMAT "9999":U INITIAL 0 
     LABEL "Año" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1 NO-UNDO.

DEFINE VARIABLE anio_hasta AS INTEGER FORMAT "9999":U INITIAL 0 
     LABEL "Año" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1 NO-UNDO.
...
DEFINE VARIABLE semana_desde AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "Desde semana" 
     VIEW-AS FILL-IN 
     SIZE 5 BY 1 NO-UNDO.

DEFINE VARIABLE semana_hasta AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "hasta" 
     VIEW-AS FILL-IN 
     SIZE 5 BY 1 NO-UNDO.
...
DEFINE FRAME F-Main
     semana_desde AT ROW 1.48 COL 23 COLON-ALIGNED
     anio_desde AT ROW 1.48 COL 35 COLON-ALIGNED
     semana_hasta AT ROW 2.67 COL 23 COLON-ALIGNED
     anio_hasta AT ROW 2.67 COL 35 COLON-ALIGNED
     articulo AT ROW 4.1 COL 9 COLON-ALIGNED
     BUTTON-1 AT ROW 6.48 COL 44
     salir AT ROW 6.48 COL 55
...
ON END-ERROR OF W-Win /* Eleccion Semana */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Eleccion Semana */
DO:
...
DO:
  p_semana_desde    = integer(semana_desde:screen-value in frame F-Main).
  p_anio_desde      = integer(anio_desde:screen-value in frame F-Main).
  p_semana_hasta    = integer(semana_hasta:screen-value in frame F-Main).
  p_anio_hasta      = integer(anio_hasta:screen-value in frame F-Main).
  p_id_articulo     = integer(articulo:screen-value in frame F-Main).

  APPLY "END-ERROR":U TO frame F-Main.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY semana_desde anio_desde semana_hasta anio_hasta articulo 
          nombre_articulo 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE semana_desde anio_desde semana_hasta anio_hasta articulo BUTTON-1 
         salir nombre_articulo 
      WITH FRAME F-Main IN WINDOW W-Win.
...
------------------------------------------------------------------------------*/
  ...
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
    ...
    anio_desde:screen-value in frame F-Main = string(year(today)).
    anio_hasta:screen-value in frame F-Main = string(year(today)).
