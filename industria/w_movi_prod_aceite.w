...
&Scoped-Define ENABLED-OBJECTS sucursal BUTTON-1 articulo BUTTON-13 ~
cantidad_tambores 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-4 sucursal articulo ~
cantidad_tambores 
...
DEFINE BUTTON BUTTON-1 
     LABEL "Movimiento de tambores a REPROCESO" 
     SIZE 43 BY 1.43.

DEFINE BUTTON BUTTON-13 
     LABEL "Movimiento de tambores a STOCK" 
     SIZE 43 BY 1.43.
...
DEFINE VARIABLE FILL-IN-4 AS CHARACTER FORMAT "X(256)":U INITIAL "TAMBORES DE PRODUCCIONES DE ACEITE" 
     VIEW-AS FILL-IN 
     SIZE 106 BY .95
     BGCOLOR 6 FGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE sucursal AS LOGICAL FORMAT "95/96":U INITIAL NO 
     LABEL "Sucursal" 
     VIEW-AS FILL-IN 
     SIZE 5 BY 1 NO-UNDO.
...
DEFINE FRAME F-Main
     FILL-IN-4 AT ROW 1.24 COL 3 NO-LABEL
     sucursal AT ROW 2.91 COL 10 COLON-ALIGNED
     BUTTON-1 AT ROW 2.91 COL 36
     articulo AT ROW 3.86 COL 10 COLON-ALIGNED
     BUTTON-13 AT ROW 4.57 COL 36
     cantidad_tambores AT ROW 4.81 COL 10 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Tambores de Produccion de Aceite"
         HEIGHT             = 5.91
         WIDTH              = 110.6
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 155.4
...
ON END-ERROR OF W-Win /* Tambores de Produccion de Aceite */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tambores de Produccion de Aceite */
DO:
...
ON CHOOSE OF BUTTON-1 IN FRAME F-Main /* Movimiento de tambores a REPROCESO */
DO:
  define var tambores_movidos as integer.
  ...
  tambores_movidos = integer(cantidad_tambores:screen-value in frame F-Main).
  i = 1.
  
  MESSAGE "Esta seguro que desea mandar a reprocesar tambores de la sucursal " v_sucursal "?" 
            VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL TITLE "" UPDATE choice AS LOGICAL.
    CASE choice:
    WHEN TRUE THEN /* Yes */
        DO:
            
            do while i <= tambores_movidos:
              find first tambores_industria where tambores_industria.id_tipotambor = 2
                                              and tambores_industria.id_articulo = v_articulo
                                              and tambores_industria.id_sucursal_ubicacion = v_sucursal
                                              and tambores_industria.id_locacion_ubicacion = 4 no-error.
            ...
            i = i - 1.
            message "Se actualizaron " i " tambores." view-as alert-box.
        END.
    END CASE. 
END.
...
ON CHOOSE OF BUTTON-13 IN FRAME F-Main /* Movimiento de tambores a STOCK */
DO:
  define var tambores_movidos as integer.
  define var i as integer.
  define var v_sucursal as integer.
  ...
  MESSAGE "Esta seguro que desea mover los tambores hacia la sucursal " v_sucursal "?" 
            VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL TITLE "" UPDATE choice AS LOGICAL.
    CASE choice:
    WHEN TRUE THEN /* Yes */
        DO:
            do while i <= tambores_movidos:
              find first tambores_industria where id_tipotambor = 2
                                              and id_articulo = v_articulo
                                              and id_locacion_ubicacion = 10 no-error.
              if available tambores_industria then
                do:
                    assign tambores_industria.id_empresa_ubicacion = 1
                           tambores_industria.id_sucursal_ubicacion = v_sucursal
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 2.67 , 89.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-4 sucursal articulo cantidad_tambores 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE sucursal BUTTON-1 articulo BUTTON-13 cantidad_tambores 
      WITH FRAME F-Main IN WINDOW W-Win.
