...
&Scoped-define FIELDS-IN-QUERY-br_table rowObject.nro_comp ~
rowObject.fecha_comp rowObject.id_proveedor rowObject.nombre 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      rowObject.nro_comp FORMAT ">>>>>>>9":U WIDTH 12.2
      rowObject.fecha_comp FORMAT "99/99/99":U
      rowObject.id_proveedor FORMAT ">>>,>>9":U WIDTH 6.8
      rowObject.nombre FORMAT "x(30)":U WIDTH 29.6
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 68 BY 6.67 EXPANDABLE.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.nro_comp
"rowObject.nro_comp" ? ? "integer" ? ? ? ? ? ? no ? no no "12.2" yes no no "U" "" ""
     _FldNameList[2]   = _<SDO>.rowObject.fecha_comp
     _FldNameList[3]   > _<SDO>.rowObject.id_proveedor
"rowObject.id_proveedor" ? ? "integer" ? ? ? ? ? ? no ? no no "6.8" yes no no "U" "" ""
     _FldNameList[4]   > _<SDO>.rowObject.nombre
"rowObject.nombre" ? ? "character" ? ? ? ? ? ? no ? no no "29.6" yes no no "U" "" ""
