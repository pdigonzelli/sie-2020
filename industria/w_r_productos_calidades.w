...
DEFINE VARIABLE h_b_r_productos_calidad AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_r_productos_calidades AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relacion entre articulos y calidades"
         HEIGHT             = 19.43
         WIDTH              = 146
         MAX-HEIGHT         = 19.43
         MAX-WIDTH          = 152.6
...
ON END-ERROR OF W-Win /* Relacion entre articulos y calidades */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Relacion entre articulos y calidades */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 32.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 66.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 89.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_r_productos_calidad.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_r_productos_calidad ).
       RUN set-position IN h_b_r_productos_calidad ( 3.76 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 143.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_r_productos_calidades.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_r_productos_calidades ).
       RUN set-position IN h_v_r_productos_calidades ( 11.29 , 37.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 64.00 ) */

       /* Links to csmartbrowser h_b_r_productos_calidad. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_r_productos_calidad ).

       /* Links to SmartViewer h_v_r_productos_calidades. */
       RUN add-link IN adm-broker-hdl ( h_b_r_productos_calidad , 'Record':U , h_v_r_productos_calidades ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_r_productos_calidades ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_r_productos_calidad ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_r_productos_calidades ,
             h_b_r_productos_calidad , 'AFTER':U ).
    END. /* Page 0 */
