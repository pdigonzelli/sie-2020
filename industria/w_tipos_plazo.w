...
&Scoped-Define ENABLED-OBJECTS BUTTON-17 RECT-2 RECT-3 
...
DEFINE VARIABLE h_b_tipos_plazo AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_tipos_plazo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-17 AT ROW 12.19 COL 90
     RECT-2 AT Y 0 X 75
     RECT-3 AT ROW 10.52 COL 3
...
ON END-ERROR OF W-Win /* Tipos de Plazo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Tipos de Plazo */
DO:
...
ON CHOOSE OF BUTTON-17 IN FRAME F-Main /* Excell */
DO:
...
/*-- CONFIGURACION INICIAL --*/
  ...
  */
  ...
  chWorkSheet:Range("B6"):Value = "COD".
  chWorkSheet:Range("B6"):BorderAround(1,2,1,1).
  chWorkSheet:Range("C6"):Value = "DESCRIPCION".
  chWorkSheet:Range("C6"):BorderAround(1,2,1,1).
  chWorkSheet:Range("D6"):Value = "DESCRIPCION INGLES".
  ...
  /*------------------------------------ PROCESO PRINCIPAL ---------------------------------------*/
  ifila = 7.
  FOR EACH tipos_plazo BY tipos_plazo.id_tipo_plazo.
    
    cfila  = string(ifila).
    cRange = "B" + cfila.
    chWorkSheet:Range(crange):value = tipos_plazo.id_tipo_plazo.
    cRange = "C" + cfila.
    chWorkSheet:Range(crange):value = tipos_plazo.descripcion.
    cRange = "D" + cfila.
    chWorkSheet:Range(crange):value = tipos_plazo.descripcion_ingles.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 16.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 50.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 73.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_tipos_plazo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_tipos_plazo ).
       RUN set-position IN h_b_tipos_plazo ( 3.62 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b_tipos_plazo ( 6.71 , 100.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_tipos_plazo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_tipos_plazo ).
       RUN set-position IN h_v_tipos_plazo ( 11.00 , 30.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.91 , 52.00 ) */

       /* Links to csmartbrowser h_b_tipos_plazo. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_tipos_plazo ).

       /* Links to SmartViewer h_v_tipos_plazo. */
       RUN add-link IN adm-broker-hdl ( h_b_tipos_plazo , 'Record':U , h_v_tipos_plazo ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_tipos_plazo ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_tipos_plazo ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_tipos_plazo ,
             h_b_tipos_plazo , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE BUTTON-17 RECT-2 RECT-3 
