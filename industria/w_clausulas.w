...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-11 
...
DEFINE VARIABLE h_b_clausulas AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_clausulas AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Clausulas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Clausulas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_clausulas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_clausulas ).
       RUN set-position IN h_b_clausulas ( 3.86 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 76.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_clausulas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_clausulas ).
       RUN set-position IN h_v_clausulas ( 11.71 , 15.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 47.00 ) */

       /* Links to csmartbrowser h_b_clausulas. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_clausulas ).

       /* Links to SmartViewer h_v_clausulas. */
       RUN add-link IN adm-broker-hdl ( h_b_clausulas , 'Record':U , h_v_clausulas ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_clausulas ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_clausulas ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_clausulas ,
             h_b_clausulas , 'AFTER':U ).
    END. /* Page 0 */
