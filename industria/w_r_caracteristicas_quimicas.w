...
DEFINE VARIABLE h_b_r_caracteristicas_articulo AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v_r_caracteristicas_quimicas AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relacion Tipo Protocolo, Articulo, Caracteristica"
         HEIGHT             = 20.14
         WIDTH              = 152
         MAX-HEIGHT         = 22.24
         MAX-WIDTH          = 153.2
...
ON END-ERROR OF W-Win /* Relacion Tipo Protocolo, Articulo, Caracteristica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Relacion Tipo Protocolo, Articulo, Caracteristica */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-updsav ).
       RUN set-position IN h_cus-updsav ( 1.24 , 14.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_cus-navico ).
       RUN set-position IN h_cus-navico ( 1.24 , 48.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_cus-misc ).
       RUN set-position IN h_cus-misc ( 1.24 , 71.80 ) NO-ERROR.
       RUN set-size IN h_cus-misc ( 1.91 , 21.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/b_r_caracteristicas_articulo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b_r_caracteristicas_articulo ).
       RUN set-position IN h_b_r_caracteristicas_articulo ( 3.86 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b_r_caracteristicas_articulo ( 10.95 , 151.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  '../industria/v_r_caracteristicas_quimicas.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v_r_caracteristicas_quimicas ).
       RUN set-position IN h_v_r_caracteristicas_quimicas ( 15.76 , 40.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.91 , 68.00 ) */

       /* Links to csmartbrowser h_b_r_caracteristicas_articulo. */
       RUN add-link IN adm-broker-hdl ( h_cus-navico , 'Navigation':U , h_b_r_caracteristicas_articulo ).

       /* Links to SmartViewer h_v_r_caracteristicas_quimicas. */
       RUN add-link IN adm-broker-hdl ( h_b_r_caracteristicas_articulo , 'Record':U , h_v_r_caracteristicas_quimicas ).
       RUN add-link IN adm-broker-hdl ( h_cus-updsav , 'TableIO':U , h_v_r_caracteristicas_quimicas ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_b_r_caracteristicas_articulo ,
             h_cus-misc , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v_r_caracteristicas_quimicas ,
             h_b_r_caracteristicas_articulo , 'AFTER':U ).
    END. /* Page 0 */
