...
&Scoped-Define ENABLED-FIELDS lotes_jugo.estado_lote 
&Scoped-define FIELD-PAIRS~
 ~{&FP1}estado_lote ~{&FP2}estado_lote ~{&FP3}
&Scoped-define ENABLED-TABLES lotes_jugo
&Scoped-define FIRST-ENABLED-TABLE lotes_jugo
&Scoped-Define DISPLAYED-FIELDS lotes_jugo.estado_lote 
&Scoped-Define DISPLAYED-OBJECTS fi-estados_lotes-descripcion 
...
DEFINE VARIABLE fi-estados_lotes-descripcion AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 38 BY .95
     FGCOLOR 1  NO-UNDO.
...
DEFINE FRAME F-Main
     lotes_jugo.estado_lote AT ROW 1 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 6.2 BY 1
     fi-estados_lotes-descripcion AT ROW 1 COL 25 COLON-ALIGNED NO-LABEL
...
/* SETTINGS FOR FILL-IN fi-estados_lotes-descripcion IN FRAME F-Main
   NO-ENABLE                                                            */
...
general.lotes_jugo.estado_lote;wc_estado_lote.w;estados_lotes.descripcion;id_estado_lote;
*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME lotes_jugo.estado_lote
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.estado_lote V-table-Win
ON GO OF lotes_jugo.estado_lote IN FRAME F-Main /* Estado del Lote */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.estado_lote V-table-Win
ON LEAVE OF lotes_jugo.estado_lote IN FRAME F-Main /* Estado del Lote */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.estado_lote V-table-Win
ON MOUSE-SELECT-DBLCLICK OF lotes_jugo.estado_lote IN FRAME F-Main /* Estado del Lote */
do: 
define var r as rowid no-undo.
run wc_estado_lote.w(output r).
find estados_lotes where rowid(estados_lotes) = r no-lock no-error.
if available estados_lotes then 
general.lotes_jugo.estado_lote:screen-value = string(estados_lotes.id_estado_lote).
apply 'U1' to self.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL lotes_jugo.estado_lote V-table-Win
ON U1 OF lotes_jugo.estado_lote IN FRAME F-Main /* Estado del Lote */
DO:
{custom/support/validacion.i}
     run descriptivos.
END.
...
find first estados_lotes where estados_lotes.id_estado_lote = integer(lotes_jugo.estado_lote:screen-value in frame F-Main)  no-lock no-error .
if available estados_lotes then 
fi-estados_lotes-descripcion:screen-value in frame F-Main = string(estados_lotes.descripcion).
else
fi-estados_lotes-descripcion:screen-value in frame F-Main = ''.
...
define var lista_relacion as character no-undo initial "estado_lote".
