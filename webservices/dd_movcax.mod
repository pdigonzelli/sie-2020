...
if x_movsto = 72 or x_movsto = 77 then
    assign items_stock.id_tipo_cosecha = 0.
*/
...
assign
    ...
    items_stock.cantidad1  = items_stock.peso / 20.


/******* Graba Partidas por Remitos *******/
find aux_partidas where
    aux_partidas.nro_partida_unica = g_partida no-error.
  
if available aux_partidas then do:
    assign aux_partidas.saldo = aux_partidas.saldo - x_cantidad.
    
    x_partida     = aux_partidas.nro_partida.
    x_partida_s   = aux_partidas.nro_partida_serial.
       
    assign
         items_stock.nro_partida_general  = aux_partidas.nro_partida_unica
         items_stock.nro_comprobante      = aux_partidas.nro_comprobante.
end.

release aux_partidas.
...
/******* Llama a los programas para imprimir etiquetas *******/
run dd_eticam.p (input x_partida, input x_partida_s).
/*
run dd_etibin.p (input x_partida, input x_partida_s).
*/
