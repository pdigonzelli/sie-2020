...
DEFINE TEMP-TABLE POSICIONES 
    FIELD PDOCVENTAS AS CHARACTER FORMAT 'X(10)'
...
cTipoEsquinero = STIPOESQUINERO.                 

RESPUESTA =  'OK'.


IF LENGTH(SCONTRAMARCA) > 4 THEN
DO:
    RESPUESTA = "Contramarca para la orden " + STRING(iOrden)  + " Tiene mas de 4 caracteres. No se procesara".
    RETURN RESPUESTA.
END.
...
DO:
  RESPUESTA = "Cliente o esquinero no definido para la orden. No se procesara".
  RETURN RESPUESTA.
END.
...
FOR EACH POSICIONES NO-LOCK:
        ...
        DO:
            RESPUESTA = "La orden " + STRING(iOrden) + " del cliente SAP " + string(iCliente) + " - " + Clientes_ventas.razon_social + CHR(10) +
                    "tiene pedido un esquinero no autorizado. No se procesara".
            flag-esquinero = TRUE.
          LEAVE.
        END.
END.

IF flag-esquinero = TRUE THEN RETURN RESPUESTA.
MESSAGE 'ESQUINERO' .
...
MESSAGE 'VARIOS' SVAPOR ILUG IPUERTO CDESTINO ICLIENTE.
...
ASSIGN pedidos_packing.union_europea    = IF SMERCADO  = 'UE' THEN TRUE ELSE FALSE 
       ...
       /* pedidos_packing.oe               = INTEGER() */ /* VER CON MARIANO */
...
/*                   pedidos_packing.pedido_traslado_sap = hTable:value(i,26) */ /* VER CON MARIANO */
...
FOR EACH POSICIONES:
  i = i + 1.
  iItem = i.
/*                iItem = integer(hTable1:VALUE(k,3)) + integer(hTable1:VALUE(k,4)). */ /* VER CON MARIANO */
  ...
  DO:
      items_pedidos_packing.cant_pallets      = INTEGER(POSICIONES.PCANTIDAD).
      NEXT.
  END.
...
/*                  cMarca         = SMARCA. */ /* VER CON MARIANO */
  iPaletizado    = INTEGER(ENTRY(NUM-ENTRIES(POSICIONES.PMATERIAL, '-'), POSICIONES.PMATERIAL , '-')).


  ctipoPalet     = STIPOPALLET.
  cTipoEsquinero = STIPOESQUINERO.
  ccategoria     = SCATEGORIA.
/*                  iTratamiento   = INTEGER(). */ /* VER CON MARIANO */ 


  MESSAGE 'VARIOS1' CESPECIE CTIPOESQUINERO CCATEGORIA CTIPOPALET CMARCA CENVASE CCATEGORIA ICALIDAD ITRATAMIENTO CESPECIE.
  ...
  MESSAGE 'VARIOS2' CESPECIE CENVASE CMARCA CTIPOPALET CTIPOESQUINERO CCATEGORIA ICALIDAD ITRATAMIENTO.
  FIND FIRST tipo_esquineros WHERE tipo_esquineros.id_tipo_esquinero_sap = cTipoEsquinero NO-LOCK NO-ERROR.
  ...
  FIND FIRST calidades WHERE calidades.id_calidad_sap = STRING(iCalidad) NO-LOCK NO-ERROR.
  MESSAGE 'VARIOS4' ITRATAMIENTO.
  FIND FIRST caracteristicas WHERE caracteristicas.id_caract_sap = STRING(iTratamiento) NO-LOCK NO-ERROR.

  MESSAGE 'VARIOS5' CESPECIE CENVASE CMARCA CTIPOPALET CTIPOESQUINERO CCATEGORIA ICALIDAD ITRATAMIENTO.
...
/*                      items_pedidos_packing.posicion_oe       = htable:VALUE(i,6)  */ /* VER CON MARIANO */
