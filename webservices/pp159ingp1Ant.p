...
DO:
  ...
  IF AVAILABLE pallets  THEN DO:
    ...
    MESSAGE 'PALLET ' PALLETS.ID_PALLET.  
    ...
    IF AVAILABLE TTCAJAS THEN /*** MODIFICADO PABLO ****/
    DO: 
        FOR EACH items_pallets OF pallets NO-LOCK.
            ...
            IF NOT AVAILABLE TTCAJAS THEN
                FOR LAST TTCAJAS BY OMTS.
                    XOMTS = TTCAJAS.OMTS.
                END.
    
            IF XOMTS = '' THEN
                RUN ULTIMAMTS(OUTPUT XOMTS).
            ...
            ASSIGN 
                   ...
                   TTPOSICIONPALLET.orden_mts = '000010000097' /*XOMTS*/ 
    ...
    DO:
       FOR EACH items_pallets OF pallets NO-LOCK.    
        MESSAGE 'ITEMS PALLETS1 ' items_pallets.codigo_trazabilidad items_pallets.calibre.
        FOR EACH TTCAJAS WHERE
            ...
            ASSIGN 
               ...
               TTPOSICIONPALLET.ITEM = items_pallets.ITEM
               TTPOSICIONPALLET.ID_PACKING = items_pallets.ID_PACKING
               TTPOSICIONPALLET.posicion = TTCAJAS.POSICION
    ...
    MESSAGE 'RES' TTPALLET.PALLET_ID OPALLET RES.
...
/*        
    DO CONTADOR = 2 TO NUM-ENTRIES(RES,'|'):
        ...
        FOR EACH  TTCAJAS1 WHERE
            ...
            TTCAJAS1.CODIGO_TRAZABILIDAD = TTPOSICIONPALLET.trazabilidad AND
            TTCAJAS1.CALIBRE = TTPOSICIONPALLET.Ccalibre AND
            TTCAJAS1.letra_color = TTPOSICIONPALLET.Ccolor.      
...
PROCEDURE ULTIMAMTS.
    ...
    END.
    /* 
     FOR LAST CAJAS OF PALLETS NO-LOCK , FIRST volcado_packing OF CAJAS NO-LOCK,
            EACH items_stock WHERE  items_stock.nro_partida = volcado_packing.nro_partida AND
                                items_stock.id_tipo_movsto = 70 AND
                                items_stock.nro_partida_serial = volcado_packing.nro_partida_serial BY
                                cajas.id_caja. 
        ...
        VOMTS = ENTRY(3,ENTRY(2,ENTRY(1,items_stock.documento_sap,';'),'|'),'&') NO-ERROR.
        IF ERROR-STATUS:ERROR THEN VOMTS = items_stock.orden_entrega_sap.
        MESSAGE 'ENCONTRO VOMTS' .
    END.
    */
    MESSAGE 'SALIO ULTIMAMTS'.
END.
