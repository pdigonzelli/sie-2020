...
DEFINE BUFFER AUX_IT FOR items_stock.


DEFINE VARIABLE ARCHIVO AS CHARACTER NO-UNDO.
DEFINE VARIABLE VTEXTO  AS CHARACTER NO-UNDO.
DEFINE VARIABLE ICOS    AS INTEGER NO-UNDO.
DEFINE VARIABLE XCOD    AS CHARACTER NO-UNDO.

DEFINE VARIABLE VPARTIDAENTREGA AS INTEGER NO-UNDO.
DEFINE VARIABLE VPARTIDASERIALENTREGA AS INTEGER NO-UNDO.
DEFINE VARIABLE VFRUTAPROCESADA AS LOGICAL INITIAL FALSE NO-UNDO.



DEFINE BUFFER BTI FOR balanza_tickets.
...
FOR EACH POSICIONES.
    ICOS =  INTEGER(SUBSTRING(POSICIONES.PSMATERIAL,4,1)).
    IF ICOS > 1 THEN
    DO:
        VTEXTO = ' TIPO DE COSECHA DISTINTO DE TIJERA O PROCESADO NO SE PERMITE ' + POSICIONES.PSMATERIAL.
        WLOG(ARCHIVO,VTEXTO).
        
        ORESPUESTA = 'NOK'.
        OMENSAJERESPUESTA = ' TIPO DE COSECHA DISTINTO DE TIJERA O PROCESADO  NO SE PERMITE ' + POSICIONES.PSMATERIAL.  
        RETURN.        
    END.
    
    FIND FIRST lotes_plantacion WHERE lotes_plantacion.codigo_trazabilidad = POSICIONES.PSTRAZABILIDAD AND LOOKUP('SM', lotes_plantacion.empresas) > 0 NO-LOCK NO-ERROR.
    FIND FIRST lote        OF lotes_plantacion  NO-LOCK NO-ERROR.
    FIND FIRST origenes    OF lote NO-LOCK NO-ERROR.       


    IF NOT AVAILABLE LOTES_PLANTACION OR NOT AVAILABLE LOTE OR NOT AVAILABLE origenes THEN
    DO:
        VTEXTO = 'INCONSISTENCIA DE TRAZABILIDADES '  + POSICIONES.PSTRAZABILIDAD.
        WLOG(ARCHIVO,VTEXTO).
        
        ORESPUESTA = 'NOK'.
        OMENSAJERESPUESTA = 'INCONSISTENCIA DE TRAZABILIDADES '  + POSICIONES.PSTRAZABILIDAD.  
        RETURN.
    END.

    FIND mercados WHERE mercados.id_mercado_sap = posiciones.PSMERCADO NO-LOCK NO-ERROR.

    IF NOT AVAILABLE MERCADOS  THEN
    DO:
        VTEXTO = 'INCONSISTENCIA DE MERCADOS '  + POSICIONES.PSTRAZABILIDAD.
        WLOG(ARCHIVO,VTEXTO).
        
        ORESPUESTA = 'NOK'.
        OMENSAJERESPUESTA = 'INCONSISTENCIA DE MERCADOS '  + POSICIONES.PSTRAZABILIDAD.  
        RETURN.
    END.

    IF POSICIONES.PSNROREMITO = ? OR POSICIONES.PSNROREMITO = '' THEN
    DO:
        IF INTEGER(SUBSTRING(POSICIONES.PSMATERIAL,1,3)) <> 303 THEN
        DO:
            VTEXTO = 'POSICIONES S/REMITOS '  + POSICIONES.PSTRAZABILIDAD.
            WLOG(ARCHIVO,VTEXTO).
            
            ORESPUESTA = 'NOK'.
            OMENSAJERESPUESTA = 'POSICIONES S/REMITOS '  + POSICIONES.PSTRAZABILIDAD.  
            RETURN.
        END.
    END.
...
DEFINE VARIABLE CANTPES1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE SUMAPES AS DECIMAL NO-UNDO.
DEFINE VARIABLE VPESONETO1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE VPESODESCARTE1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE VPESOENTRADA1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE VPESOSALIDA1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE DIF1 AS DECIMAL NO-UNDO.

FOR EACH POSICIONES.
    IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , PSCANTPES ) <> 0 THEN
        cantpes1 = DECIMAL(PSCANTPES).
    ELSE
    DO:
        IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
            cantpes1 = DECIMAL (REPLACE(PSCANTPES , ',' , '.')  ).
        ELSE
            cantpes1 = DECIMAL (REPLACE(PSCANTPES , '.' , ',') ).
    END.
    cantpes1 = TRUNCATE(cantpes1,0).
    PSCANTPES = STRING(CANTPES1).
    SUMAPES = SUMAPES + DECIMAL(PSCANTPES).
END.
 
IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , SPESONETO ) <> 0 THEN
    vpesoneto1 = TRUNCATE(DECIMAL(SPESONETO),0).
ELSE
DO:
    IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
        vpesoneto1 = TRUNCATE(DECIMAL (REPLACE(SPESONETO , ',' , '.')  ),0).
    ELSE
        vpesoneto1 = DECIMAL (REPLACE(SPESONETO , '.' , ',') ).
END.

IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , SPESODESCARTE ) <> 0 THEN
    vpesodescarte1 = DECIMAL(SPESODESCARTE).
ELSE
DO:
    IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
        vpesodescarte1 = DECIMAL (REPLACE(SPESODESCARTE , ',' , '.')  ).
    ELSE
        vpesodescarte1 = DECIMAL (REPLACE(SPESODESCARTE , '.' , ',') ).
END.

IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , SPESOENTRADA ) <> 0 THEN
    vpesoentrada1 = DECIMAL(SPESOENTRADA).
ELSE
DO:
    IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
        vpesoentrada1 = DECIMAL (REPLACE(SPESOENTRADA , ',' , '.')  ).
    ELSE
        vpesoentrada1 = DECIMAL (REPLACE(SPESOENTRADA , '.' , ',') ).
END.

IF INDEX(SESSION:NUMERIC-DECIMAL-POINT , SPESOSALIDA ) <> 0 THEN
    vpesosalida1 = DECIMAL(SPESOSALIDA).
ELSE
DO:
    IF SESSION:NUMERIC-DECIMAL-POINT = '.' THEN
        vpesosalida1 = DECIMAL (REPLACE(SPESOSALIDA , ',' , '.')  ).
    ELSE
        vpesosalida1 = DECIMAL (REPLACE(SPESOSALIDA , '.' , ',') ).
END.

VPESOENTRADA1 = TRUNCATE(VPESOENTRADA1,0).
VPESOSALIDA1 = TRUNCATE(VPESOSALIDA1,0).
VPESONETO1 = TRUNCATE(VPESONETO1,0).
VPESODESCARTE1 = TRUNCATE(VPESODESCARTE1,0).


DIF1 = SUMAPES - VPESONETO1.



VPESONETO1 = VPESONETO1 + DIF1.
VPESOENTRADA1 = VPESOENTRADA1 + DIF1.



SPESONETO = STRING(VPESONETO1).
SPESOSALIDA = STRING(VPESOSALIDA1).
SPESOENTRADA = STRING(VPESOENTRADA1).
SPESODESCARTE = STRING(VPESODESCARTE1).
...
VTEXTO = 'ENCONTRANDO DATOS'.
MESSAGE VTEXTO  .

WLOG(ARCHIVO,VTEXTO).
/*
DEFINE VAR NROS AS INTEGER.
DEFINE BUFFER TTX FOR TTING.
FOR EACH POSICIONES NO-LOCK,FIRST items_stock WHERE items_stock.ID_SUCURSAL = 110 AND items_stock.ID_TIPO_MOVSTO = 78 AND 
               items_stock.codigo_trazabilidad = POSICIONES.PSTRAZABILIDAD AND 
               items_stock.orden_entrega_sap =  posiciones.PSNROLOTE AND
               items_stock.orden_entrega_procesado = SNROPESADA  NO-LOCK.

    

    MESSAGE POSICIONES.PSTRAZABILIDAD POSICIONES.PSNROLOTE SNROPESADA.
    VTEXTO = POSICIONES.PSTRAZABILIDAD + "|" + POSICIONES.PSNROLOTE + "|" + SNROPESADA + "|" + STRING(items_stock.nro_partida).
    WLOG(ARCHIVO,VTEXTO).

    NROS = 0.
    FOR LAST TTING WHERE TTING.NROP = items_stock.nro_partida NO-LOCK BY TTING.NROPS.
        NROS = TTING.NROPS + 1.
    END.
    IF NROS = 0 THEN 
    DO:
        NROS = 1.
        FOR  LAST  AUX_IT WHERE AUX_IT.NRO_PARTIDA = items_stock.NRO_PARTIDA BY items_stock.NRO_PARTIDA_SERIAL.
            NROS = AUX_IT.NRO_PARTIDA_SERIAL + 1.
        END.
    END.        
    CREATE TTING.
    ASSIGN  TTING.PES       = POSICIONES.PSNROPESADA
            TTING.POSPES    = POSICIONES.PSNROPOSICIONPESADA
            TTING.NROP      = items_stock.nro_partida
            TTING.NROPS     = NROS
            TTING.NRO_COMP  = items_stock.NRO_COMP.

    VTEXTO = "TTING-PARTIDA: " + STRING(TTING.NROP) + "|" + STRING(TTING.NROPS).
    WLOG(ARCHIVO,VTEXTO)  .
...
DO ON ERROR UNDO, THROW:
    VTEXTO = "PESADA ENCONTRADA: " + STRING(IPESADA).
    WLOG(ARCHIVO,VTEXTO)  .
...
DO TRANSACTION ON ERROR UNDO, THROW:
    ...
    FOR EACH POSICIONES BY POSICIONES.PSNROLOTE:
        ...
        RUN devuelvevaloresMovflete ( i, vsucursal ,
            ...
            OUTPUT vzona_up,
            OUTPUT vrenspa,
            OUTPUT VFRUTAPROCESADA) NO-ERROR.
        IF ERROR-STATUS:ERROR THEN 
        DO:
            ORESPUESTA = 'NOK'.
            OMENSAJERESPUESTA = RETURN-VALUE. 
            UNDO, THROW NEW Progress.Lang.AppError(RETURN-VALUE , 550).
        END.

        VTEXTO = "SALE POSICION " + STRING(I).
        WLOG(ARCHIVO,VTEXTO)  .
        ...
        IF  PSFECHAREMITO <> '' AND PSFECHAREMITO <> ? THEN
        ...
        IF INTEGER(SUBSTRING(POSICIONES.PSMATERIAL,1,3)) = 303 THEN
            ASSIGN  VFECHAREMITO = TODAY
                    VNROREMITO = '999999999999'.
        ELSE
            ASSIGN
                vfecharemito    = DATE( INTEGER(SUBSTRING(PSFECHAREMITO,4,2)) , INTEGER(SUBSTRING(PSFECHAREMITO,1,2)) , INTEGER(SUBSTRING(PSFECHAREMITO,7,4)) )
        ...
        VPARTIDAENTREGA = 0.
        VPARTIDASERIALENTREGA = 0.
      

        FOR EACH MOVSUCU WHERE movsucu.id_sucursal = 110 AND movsucu.ID_TIPO_MOVSTO = 78 AND ORDEN_ENTREGA_PROCESADO = SNROPESADA, FIRST ITEMS_STOCK OF MOVSUCU 
            WHERE items_stock.orden_entrega_sap = posiciones.PSNROLOTE NO-LOCK.
            
                FOR LAST AUX_IT WHERE AUX_IT.NRO_PARTIDA = items_stock.nro_partida BY AUX_IT.NRO_PARTIDA_SERIAL.
                    VPARTIDASERIALENTREGA = AUX_IT.nro_partida_serial + 1.
                END.           
                
                VPARTIDAENTREGA = items_stock.NRO_PARTIDA.
 /*               VPARTIDASERIALENTREGA = items_stock.nro_partida_serial + 1. */
                VNROREMITO = items_stock.nro_COMP.
                vcantenvasesentrada = items_stock.cantidad / 20.
                VCOLOR = items_stock.ID_COLOR.
        END.        
        
        IF VPARTIDAENTREGA = 0          THEN 
            VPARTIDAENTREGA = NEXT-VALUE(nro_partida, produccion).
        IF VPARTIDASERIALENTREGA = 0    THEN 
            VPARTIDASERIALENTREGA = 1.

        VTEXTO = "VPARTIDA" + "|" + STRING(VPARTIDAENTREGA) + "|" + STRING(VPARTIDASERIALENTREGA).
        WLOG(ARCHIVO,VTEXTO)  .
...
/*    CATCH ErrB AS Progress.Lang.ProError .
        OUTPUT TO F:\\TEMP\WS2.TXT APPEND.
...
PROCEDURE ACTUALIZADOCUMENTOMATERIALPESADA.
    ...
    FOR EACH POSICIONES BY POSICIONES.PSNROLOTE.
        I = I + 1.
        FIND FIRST balanza_tickets OF bp WHERE balanza_tickets.nro_ticket = I NO-ERROR.
        IF AVAILABLE balanza_tickets THEN
            ASSIGN 
                balanza_tickets.orden_compra_sap     = POSICIONES.PSNRODOCUMENTOMATERIAL
...
PROCEDURE SAPTOBALANZAPESADA.
    ...
    DO TRANSACTION ON ERROR UNDO, THROW.
        ...
        VHORAENTRADA = STRING(TIME,'HH:MM').     
...
PROCEDURE procesarBalanzaTicket :
    ...
    DO ON ERROR UNDO , THROW:
        ...
        DO:
            ...
            WLOG(ARCHIVO,'SALESALDOSPARTIDAS').
...
PROCEDURE devuelvevaloresmovflete :
    ...
    IF vtipocosecha = 4 AND NOT VFRUTAPROCESADA THEN 
    DO:
    ...
    WLOG(ARCHIVO, 'VSUCURSAL' + STRING(VSUCURSAL)).
    WLOG(ARCHIVO, 'VSUCURSALETIQUETA' + STRING(VSUCURSALETIQUETA)).

    MESSAGE VSUCURSALETIQUETA VFINCA.
           
    VTEXTO = "DEVUELVE VALORES #2".
    WLOG(ARCHIVO,VTEXTO)  .
    ...
    WLOG(ARCHIVO, 'TIPOCOSECHA-' + CTIPOCOSECHA).
    ...
    WLOG(ARCHIVO, 'VTIPOCOSECHA-' + STRING(VTIPOCOSECHA)).


    
    VTEXTO = "DEVUELVE VALORES #3".
    WLOG(ARCHIVO,VTEXTO)  .
    ...
    /* cantpes = truncate(cantpes,0). */
    VTEXTO = "DEVUELVE VALORES #5".
    WLOG(ARCHIVO,VTEXTO)  .
...
/*        vcantenvasesentrada = IF TRIM(STRING( PSUNIDAD )) = 'KG' THEN (IF AVAILABLE r_envases_prod THEN cantpes / r_envases_prod.Kilos ELSE 0) ELSE INTEGER(PSCANTPES). */
    ...
    VTEXTO = "DEVUELVE VALORES #61 " + CCODIGOTRAZABILIDAD.
    WLOG(ARCHIVO,VTEXTO)  .

    FIND FIRST      lotes_plantacion WHERE lotes_plantacion.codigo_trazabilidad = ccodigotrazabilidad NO-LOCK.
    /* IF NOT AVAILABLE lotes_plantacion THEN RETURN ERROR 'Error de lotes de plantacion'. */

    VTEXTO = "DEVUELVE VALORES #612 " + STRING(AVAILABLE(LOTES_PLANTACION)).

    FIND FIRST   lote        OF lotes_plantacion  NO-LOCK NO-ERROR.
    IF AVAILABLE lote  THEN ASSIGN 
            vlotesenasa = lote.id_lote_senasa
            vlote       = lote.id_lote. 
    ELSE ASSIGN vlotesenasa = 0 
                vlote       = 0.

    VTEXTO = "DEVUELVE VALORES #62" + string(vlote).
    WLOG(ARCHIVO,VTEXTO)  .
    ...
    */
    ...
    IF vtipocosecha = 4 AND VFINCA THEN 
    DO:
    ...
    */
    ...
    VTEXTO = "DEVUELVE VALORES #8".
    WLOG(ARCHIVO,VTEXTO)  .
    
    vcertunion  = lote.certificado.
    vcertchina  = lote.cert_china.

    VTEXTO = "DEVUELVE VALORES #81-" + vcertunion.
    WLOG(ARCHIVO,VTEXTO)  .
    ...
    VTEXTO = "DEVUELVE VALORES #82-" + vcertunion.
    WLOG(ARCHIVO,VTEXTO)  .
    ...
    VTEXTO = "DEVUELVE VALORES #83-" + vcertunion.
    WLOG(ARCHIVO,VTEXTO)  .
...
/*     
        ...
        IF AVAILABLE balanza_tickets THEN
        DO: */
        FOR  FIRST BTI NO-LOCK WHERE BTI.id_sucursal = partidas.id_suc_trabajo AND
            BTI.nro_partida = partidas.nro_partida BY BTI.nro_partida_serial.        
            WLOG(ARCHIVO,'PARTIDAS').
            MESSAGE 'PARTIDAS'.            
            BUFFER-COPY BTI TO partidas.
        END.
    END.   
    MESSAGE 'SALE DE CREAR PARTIDAS'.
    WLOG(ARCHIVO,'SALE DE CREAR PARTIDAS').
/*  
CATCH ERRX AS Progress.Lang.Error :
    UNDO, THROW ERRX.
END CATCH. */
...
PROCEDURE crearitemsstockdebalanza :
    ...
    MESSAGE X_BALANZA X_PESADA X_NRO_TICKET.
    ...
    DO:
        ...
        DO:
            ...
            WLOG(ARCHIVO, ('CREAITEMSTOCK' + "|" + STRING(MOVSUCU.ID_TIPO_MOVSTO ) + "|" + STRING(MOVSUCU.ID_SUCURSAL ) + "|" + STRING(MOVSUCU.ID_SUC_ENVIO ) ) ).
            ...
            ASSIGN
                ...
                items_stock.id_color             = balanza_tickets.id_color
            ...
            CASE balanza_tickets.id_tipo_cosecha:
                ...
                DO:
                    ...
                    items_stock.orden_entrega_sap = SUBSTRING(balanza_tickets.cod_barra_sap,12).
                    WLOG(ARCHIVO, 'FRUTAPROCESADA-3-' + SUBSTRING(balanza_tickets.cod_barra_sap,12)).
                END.
                OTHERWISE
                items_stock.estado_fruta = FALSE.
            END CASE.
            WLOG(ARCHIVO, 'EMPIEZABINES ' + STRING(balanza_tickets.NRO_REMITO)).
            /******* Archivo de Bines *******/
            /***
            FIND FIRST bines USE-INDEX proveedor_remito WHERE
                ...
                balanza_tickets.nro_remito <> '' and
                balanza_tickets.nro_remito <> ? NO-LOCK NO-ERROR.
            IF AVAILABLE bines THEN 
            DO:
                WLOG(ARCHIVO, 'EMPIEZABINES1').
            ...
            */
            ...
            DO:
                ...
                WLOG(ARCHIVO, 'ESTIMACIONCOSECHA').
                RUN dd_sdoest.p
...
PROCEDURE actsaldospartidadebalanza :
    ...
    DO:
        ...
        DO:
            ...
            ASSIGN 
                ...
                saldos_partidas.nombrePerfil   = 'SM'.
    ...
    DO:
        ...
        ASSIGN
            ...
            saldos_packing.c_usuario            = 'mm050'
            saldos_packing.nombrePerfil         = 'SM'.

        RELEASE saldos_packing.
    END.
    
    WLOG(ARCHIVO, 'BALANZA TICKETS-PARTIDA-' + STRING(balanza_tickets.NRO_PARTIDA) + "-" + STRING(balanza_tickets.NRO_PARTIDA_SERIAL) ).
    WLOG(ARCHIVO, 'BALANZA TICKETS-SUC ETIQUETA-' +  STRING(balanza_tickets.id_sucursal_etiqueta)).
