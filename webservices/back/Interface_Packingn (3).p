...
FUNCTION CAMARA  RETURNS INTEGER (ICAMARA AS INTEGER , ISUCURSAL AS INTEGER).
    ...
    IF ISucursal <> 106 THEN RETURN 1202.
    /*       IF ICAMARA < 5 THEN
                RETURN (1100 + ICAMARA).
           ELSE
                RETURN 1106.*/
...
DEFINE VARIABLE pNumDoc       AS CHARACTER NO-UNDO FORMAT 'x(20)'.
...
DO:     
    ...
    OPERACION = 'python ' +  dir + '\pp141ingn.py ' + cparametros.
...
/*        PIMATERIALSAP = SUBSTRING(balanza_tickets.cod_barra_sap,1,11) */
    ...
    VESPECIE = BALANZA_TICKETS.ESPECIESAP.
    VVARIEDAD =  balanza_tickets.VARIEDADSAP.

    IF VESPECIE = '' THEN
    DO:
        IF AVAILABLE productos_terminados THEN
            VESPECIE = productos_terminados.id_articulo_sap.
    END.

    IF VVARIEDAD = '' THEN
    DO:
        IF AVAILABLE variedades THEN
            VVARIEDAD = variedades.id_variedad_sap.
    END.

    CPARAMETROS =   PISUCURSAL + ',' + STRING(items_stock.NRO_PARTIDA) + ',' + TRIM(PIMATERIALSAP) + ',' + /*PILOTESAP*/ '' + ',' + STRING(ITEMS_STOCK.PESO) + ',KG,' + STRING(PIALMACEN) 
        + ',' + CPLANTAEMPAQUE     
        + ',' + /*STRING(productos_terminados.id_articulo_sap)*/ balanza_tickets.ESPECIESAP + ',' + /*STRING(variedades.id_variedad_sap)*/ balanza_tickets.VARIEDADSAP + ',' + items_stock.codigo_trazabilidad  
        + ',' +  /*origenes.id_origen_sap*/ balanza_tickets.FINCASAP + ',' + balanza_tickets.loteagricolaSap + ',' + XUNIDPROD + ',' + STRING(colores.id_color_sap) + ',' + DFECHA + ',' + STRING(ICANTIDAD)
...
RUN OPERACION.P (INPUT operacion , INPUT CPARAMETROS, OUTPUT  RES).    
