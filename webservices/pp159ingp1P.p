...
ARCHIVO = SESSION:TEMP-DIRECTORY + STRING(ISUCTRABAJO) + STRING(IPALLET).
...
DO:
  ...
  IF AVAILABLE pallets  THEN DO:
    ...
    WLOG(ARCHIVO, 'PALLET ' + STRING(PALLETS.ID_PALLET)).  
 ...
 /*   RUN PREPARA-CAJAS. */
    ...
    FOR EACH items_pallets OF pallets NO-LOCK.
        WLOG(ARCHIVO,('ITEM_PALLET ' + ' ' + items_pallets.codigo_trazabilidad + ' ' + STRING(items_pallets.bultos))).
        ...
        IF XOMTS = '' THEN
            RETURN ERROR 'NO ENCUENTRA ORDEN MTS'.
    ...
    WLOG(ARCHIVO, ('RES ' + STRING(TTPALLET.PALLET_ID) + ' ' + STRING(OPALLET) + ' ' + RES)).
    DO CONTADOR = 2 TO NUM-ENTRIES(RES,'|'):
        ...
        WLOG(ARCHIVO, ('LOTE ' + STRING(ILOTE) + ' ' + STRING(ICANT))).
    ...
    FOR EACH items_pallets OF PALLETS NO-LOCK.
        ...
        DO:
            ...
            FOR LAST TTCAJAS WHERE 
                ...
                WLOG(ARCHIVO,('ULTIMA ' + STRING(TTCAJAS.BULTOS))).
            END.            
            
        END.
    END.
    WLOG(ARCHIVO, 'SALIO CAJAS').
END PROCEDURE.
