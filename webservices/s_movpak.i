/****************************************************************************/
/*  NOMBRE PROGRAMA......:   s_movpak.i                                     */
/****************************************************************************/
/*  FECHA CREACION ......:   25/06/03                                       */
...
define button b_borrar label "&Borrar"
    size 11 by 1.19.
/*
define button b_imprim label "&Imprimir"
    size 11 by 1.19.
*/
/********** Frame de botones del browse **********/
define frame botones
    b_add b_borrar with three-d . 
...
display {{&campos_display1}} 
    with 8 down size 132 by 8 SEPARATORS font 0.
...
/*************RELACIONES******************/
on "F3" of frame trabajo or "RETURN" of frame trabajo
    or "TAB" of frame trabajo anywhere do:
    {{&relaciones}}.
end.
...
on "F9" OF  b1 or "INSERT" of b1 or "CHOOSE" of b_add
    ...
    do on endkey undo, leave :
        ...
        create {&archivo}.
        {{&creacion}}
        if "{{&campos_create}}" <> "" Then
            update {{&campos_create}} with frame trabajo.
        else 
            update {{&campos_update}} with frame trabajo.
    ...
    do on error undo, retry:
        do transaction on endkey undo _sale, leave _sale on error undo _sale, leave _sale:
            ...
            if available {&archivo} then do:
                {{&borrado}}
                if x_borrar then delete {&archivo}.
                {{&borrado_posterior}}
                run abre_query.
            end.
        end.
...
ENABLE b_add b_borrar WITH FRAME botones.
