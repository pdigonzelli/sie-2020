...
try:
    ...
    #cparametro = "SPL,424964,50111010101,0000000847,28744,KG,1101"
...
#    client = Client(url='http://smazapppiprd.sanmiguel.local:50000/dir/wsdl?p=sa/6ac60074b3f13c27aaa7f82a85174b9c',
    ...
    result = client.service.SI_OS_PP141ING('PRD300',momento1,momento2,)
#    ctypes.windll.user32.MessageBoxA(0, str(result) , "python1", 1)
    documento = result.momento1.documentoMaterial

    if documento is None:
        retval = 'ERROR:' + result.momento1.mensaje.encode('utf8') + '(PRD)'
    elif not documento.isdigit():
        retval = 'ERROR: documento no numerico' + '(PRD)'
    else:
...
#        f.close()
    ...
    retval = 'ERROR:' + str(e2.message).encode('utf8') + '(PRD)'

except urllib2.URLError as e1:
    retval = 'ERROR:' + str(e1.reason).encode('utf8') + '(PRD)'

except Exception as e:
    retval = 'ERROR:' + str(e.message).encode('utf8') +  '(PRD)'
finally:
    exit(retval)
