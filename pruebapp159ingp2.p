...
/* ***************************  Definitions  ************************** */
  DEFINE VAR iSucTrabajo AS INTEGER NO-UNDO INITIAL 98.
  DEFINE VAR  iPallet AS INTEGER NO-UNDO INITIAL 18000035.
  DEFINE VAR  CENTORNO AS CHARACTER NO-UNDO INITIAL 'PRD'.
...
DO:
 ...
 /*   RUN PREPARA-CAJAS. */
    ...
    FOR EACH items_pallets OF pallets NO-LOCK.
        WLOG(ARCHIVOL,('EMPIEZA ITEM_PALLET ' + ' ' + items_pallets.codigo_trazabilidad + ' ' + STRING(items_pallets.bultos))).
        ...
        FOR  LAST items_stock WHERE  items_stock.nro_partida = volcado_packing.nro_partida AND
            ...
            /* XOMTS = ENTRY(3,ENTRY(2,ENTRY(1,items_stock.documento_sap,';'),'|'),'&').*/
            XOMTS = items_stock.orden_entrega_sap.
        END.                                
/*        XOMTS = ENTRY(3,ENTRY(2,ENTRY(1,items_stock.documento_sap,';'),'|'),'&') NO-ERROR. */                                
/*        IF ERROR-STATUS:ERROR THEN
            XOMTS = ''. */
          
/*        MESSAGE items_stock.nro_partida items_stock.nro_partida_serial. */
        ...
        ASSIGN 
               ...
               TTPOSICIONPALLET.orden_mts = XOMTS.
/*               TTPOSICIONPALLET.orden_mts = /* '000300000129'*/ ENTRY(3,ENTRY(2,ENTRY(1,items_stock.documento_sap,';'),'|'),'&') */
        ...
        WLOG(ARCHIVOL,('MTS: ' + ' '  + XOMTS)).
        WLOG(ARCHIVOL,('TERMINA ITEM_PALLET ' + ' ' + items_pallets.codigo_trazabilidad + ' ' + STRING(items_pallets.bultos))).
    END.
    cStatus = 'OK'.

    WLOG(ARCHIVOL,'CONECTANDO APP SERVER').   
    ret = hAppSrv:CONNECT("-H 192.168.1.115 -S 5162 -AppService pallets") NO-ERROR.
    IF NOT ret OR ERROR-STATUS:ERROR THEN
    DO:
      WLOG(ARCHIVOL,'FALLO AL CONECTAR EL APP SERVER ' + RETURN-VALUE).   
      RETURN ERROR "Fallo al conectar AppServer".
    END.
    WLOG(ARCHIVOL,'CONECTADO APP SERVER').   

    DEFINE VAR TEMPDIR AS CHARACTER.
    RUN GETTEMPDIRECTORY.P ON hAppSrv TRANSACTION DISTINCT (OUTPUT TEMPDIR). 
    FIND FIRST TTPALLET.
    IF CENTORNO = 'PRD' THEN
    DO:
        ARCHIVO  = TEMPDIR + 'P' + STRING(TTPALLET.pallet_id) + '.XML'.
        ARCHIVO1 = TEMPDIR + 'P' + STRING(TTPALLET.pallet_id) + '.RES'.
/*        ret = hAppSrv:CONNECT("-H 192.168.1.115 -S 5162 -AppService sap1"). */
    END.
    ELSE
    DO:
        ARCHIVO  = TEMPDIR + 'PQ' + STRING(TTPALLET.pallet_id) + '.XML'.
        ARCHIVO1 = TEMPDIR + 'PQ' + STRING(TTPALLET.pallet_id) + '.RES'.
    END.       
    
    WLOG(ARCHIVOL,'EXPORTANDO XML').   
    RUN EXPORTAXML.P ON hAppSrv TRANSACTION DISTINCT ( ARCHIVO , INPUT TABLE TTPALLET , INPUT TABLE TTPOSICIONPALLET).
    ...
    RUN OPERACION.P ON hAppSrv TRANSACTION DISTINCT  (INPUT operacion , INPUT CPARAMETROS, OUTPUT  RES) NO-ERROR.
    IF ERROR-STATUS:ERROR THEN
    DO:
        WLOG(ARCHIVOL, (' ERROR OPERACION ' + RETURN-VALUE)).
        RETURN ERROR ('ERROR DE OPERACION ' + RETURN-VALUE).   
    END.
    WLOG(ARCHIVOL, (' RESPUESTA: ' + RES)).
    ...
    IF ENTRY(1,ENTRY(1,RES,'|'),':') = 'ERROR' THEN 
    DO:
        WLOG(ARCHIVOL, (' ERROR OPERACION ' + ENTRY(2,ENTRY(1,RES,'|'),':'))).        
        RETURN ERROR ('ERROR OPERACION ' + ENTRY(2,ENTRY(1,RES,'|'),':')).
    END.
